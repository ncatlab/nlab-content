...
If $H \in \mathcal{A}$ is a [[finite number|finite]] [[sum]] of [[decomposable differential form|decomposables]]
...
So assume now that there is $1 \leq \ell \lek k$ such that (eq:InductiveDecompositionOfAlpha) holds for all $\ell \lt \ell_0$. Then
...
By vanishing of $H$-cohomology, this implies that (eq:InductiveDecompositionOfAlpha).
