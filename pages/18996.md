...
But this should imply that the full $(H = H_{fib} + H_{(1)} +  + H_{(ferm)})$-cohomology vanishes on this subalgebra:

because if we decompose the underlying $H$-chain complex as the [[double complex]] with $d_1 \coloneqq H_{fib}$ and $d_2 \coloneqq H_{(1)} + H_{(ferm)}$, then the corresponding [[spectral sequence of a double complex]] has vanishing second page when restricted to $CE\left( T_{exc,-6}\mathbb{R}^{10,1\vert\mathbf{32}} \right)^{Spin(10,1)}_{B simp}$ (eq:BSimplePartOfDFAlgebra).
