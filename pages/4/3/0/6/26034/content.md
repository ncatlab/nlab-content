...
by so called Brzeziński-Militaru construction or scalar extension bialgebroid in [BrzezinskiMilitaru2002](#BrzezinskiMilitaru).

## Definition

If $(H,\Delta,\epsilon)$ (where $\Delta(h) = \sum h_{(1)}\otimes h_{(2)}$ is a bialgebra and $A$ a left-right Yetter-Drinfeld $H$-module algebra with Hopf action $\triangleright:H\otimes A\to A$ and a coaction $\rho: A\to A\otimes H$, then the smash product $A\sharp H$ inherits the left $A$-bialgebroid structure $(A\sharp H,\alpha,\beta,\Delta^{A\sharp H},\epsilon^{A\sharp H})$ where source $\alpha(a) = a\sharp 1$, target $\beta$ is $\rho$ followed by the identification of vector spaces $A\sharp H\cong A\otimes H$ and the coproduct is obtained by extending $\Delta$ along inclusion $H\to A\sharp H$, that is,
$$
...
antipode $S$ of $H$ is
invertible then it gives Bohm-Slachanyi symmetric Hopf algebroid. In fact, if the antipode $S$ is not invertible
then the data of a Yetter-Drinfeld module algebra has to be replaced by a compatible pair of a left-right Yetter-Drinfeld module algebra and a right-left Yetter-Drinfeld module algebra yielding a symmetric Hopf algebroid [Stojic2023](#Stojic2023).
...
* [[G. Böhm]], _Hopf algebroids_, in chapter 6 of Handbook of algebra, arXiv:[math.RA/0805.3806](https://arxiv.org/abs/0805.3806)
...
An infinite-dimensional case of a Heisenberg double as a completed Hopf algebroid is exposed in
