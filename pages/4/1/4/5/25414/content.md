...
*Embeddings* of manifold $n$-diagrams are [[manifold diagram#framed_map_euclidean_case|framed]] [[stratified space|stratified maps]] whose underlying maps are [[subspace|embeddings]] of spaces (this is different from the notion of [[stratified space|substratifications]]). The next figure illustrates mappings in this class (in dimension $n = 2$).

<center>
<figure>
  <img src="https://ncatlab.org/nlab/files/mdiag_inclusions_and_coarsenings.svg" alt="" style="width:90%; max-width: 600px; height:auto;">
<figcaption>Figure 1: Embeddings of manifold diagrams (for which 'substratifications' and 'coarsenings' form a factorization system).</figcaption>
</figure>
</center>
...
<center>
<figure>
  <img src="https://ncatlab.org/nlab/files/mdiag_faces_and_degeneracies_v3.svg" alt="" style="width:90%; max-width: 600px; height:auto;">
<figcaption>Figure 2: Geometric duals of embeddings are cellular maps (for which 'face' and 'degeneracy' maps form a factorization system)</figcaption>
</figure>
</center>
...
where $T' \xrightarrow{\mathsf{norm}} T$ is a normalizing map and $T' \hookrightarrow S$ is map of labeled trusses whose underlying map is a [[regular]] inclusion. Such spans can be composed by pullback.
...
Geometric dualization unveils that quotients of manifold diagrams correspond to subdivisions of framed cell complexes. The next figure illustrates this for our above examples.

<center>
<figure>
  <img src="https://ncatlab.org/nlab/files/mdiag_subdivisions.svg" alt="" style="width:90%; max-width: 600px; height:auto;">
...
whose underlying map of trusses is a [[n-truss|singular]] quotient map.
...
When defining higher categories parametrized by manifold $n$-diagrams, we would want higher categories to be compatible both with embeddings ('morphisms attachment') and quotients ('morphism composition') in the following sense. 
...
{#mdiag_infty}
\begin{rmk} \label{mdiag_infty} (The $n = \infty$ case). So far we worked with $n$-diagrams for fixed finite $n \in \mathbb{N}$ which work well as parametrizing objects for $n$-categories. However, we can also address the case $n = \infty$ as follows. Given a manifold $n$-diagram $f$, note that $f \times \mathbb{R}$ is a manifold $(n+1)$-diagram. This defines a chain of inclusions of (ordinary resp. double) categories, the colimit $\mathsf{MDiag}$ of which is a category of manifold diagram that may be used to parametrize $(\infty,\infty)$-categories.
\end{rmk}
...
* presheafs on (unlabeled) [[n-truss|atomic trusses]]
...
* While in some instances (such as the 'free' case) definitions of manifold-diagrammatic $n$-categories have been given, the theory of manifold diagrammatic higher categories overall remains under-developed.
