...
A __colax monoidal functor__, (also called __lax comonoidal functor__ or simply __comonoidal functor__), is a monoidal functor from the [[opposite categories]] $C^{op}$ to $D^{op}$.  Equivalently, it is a functor from $C$ to $D$ equipped with coherent morphisms $\epsilon$ and $\mu_{x,y}$ that go in the opposite direction from those in a lax monoidal functor.
