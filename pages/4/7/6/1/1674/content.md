...
(Recall the GCH: it says that for infinite sets $Y, Z$, if $|Y| \leq |Z| \leq |P(Y)|$, then $|Y| = |Z|$ or $|Z| = |P(Y)|$, letting $|Y|$ denote the cardinality as usual and $|Y| \leq |Z|$ means there is a [[monomorphism]] $Y \to Z$.) 
...
whose ZF-proof uses a diagonal argument that we give in a moment. Granting the claim, let us continue. We have 
...
where $\pi_1$ denotes first projection. By Cantor's argument, no element $p \in P$ gets mapped by $f$ to $C = \{x \in P: x \notin f(x)\}$. It follows that under the bijection $A + P \cong 2^P \times 2^P$, those elements of $A + P$ that map to elements of the fiber $\pi_1^{-1}(\{C\})$ can only belong to the summand $A$. Thus it is a subset of $A$ that maps bijectively onto $\pi_1^{-1}(\{C\}) \cong 2^P$, and this proves $|2^P| \leq |A|$. 
=-- 
