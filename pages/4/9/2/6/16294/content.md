...
* The covariant [[(∞,1)-Grothendieck construction]] sends any $F : C \to (\infty,1)Cat$ to the cocartesian fibration $oplaxcolim(F)$ with structure map to $C \simeq \oplaxcolim(1)$ induced by $F \to 1$. Dually, the contravariant version sends $F : C^{op} \to (\infty,1)Cat$ to $laxcolim(F)$.
