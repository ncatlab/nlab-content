...
according to Def. \ref{AdjointFunctorsInTermsOfNaturalBijectionOfHomSets}, with [[adjunction units]] $\eta_c$ and [[adjunction counits]] $\epsilon_d$ according to Def. \ref{AdjunctionUnitFromHomIsomorphism}.
...
\begin{tikzcd}
  \mathcal{D}\big(L_{2}(-),(-)\big) 
    \ar[r, "\sim"] 
    \ar[d, "{\mathcal{D}\big(\lambda_{(-)},(-)\big)}"']  
    & 
    \mathcal{C}\big((-),R_{2}(-)\big) 
    \ar[d, "{\mathcal{C}\big((-),\rho_{(-)}\big)}"] 
    \\
    \mathcal{D}\big(L_{1}(-),(-)\big) 
    \ar[r, "\sim"] 
    & 
    \mathcal{C}\big((-),R_{1}(-)\big)
    \mathrlap{\,,}
\end{tikzcd}

where the horizontal arrows denote the given hom-isomorphisms (eq:HomIsomorphismForAdjointFunctors).
\end{definition}
