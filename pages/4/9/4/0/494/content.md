...
More precisely:
\begin{proposition}
Let $(\mathcal{C},\mathcal{D},F,U,\eta,\epsilon)$ be an adjunction ie $F \dashv U$ are adjoint functors where $L:\mathcal{C} \rightarrow \mathcal{D}$, $R:\mathcal{D} \rightarrow \mathcal{C}$, $\eta_{A}:A \rightarrow U(F(A))$ is the unit and $\epsilon_{B}:F(U(B)) \rightarrow B$ is the counit. Then:

* $U \circ F$ is a monad on $\mathcal{C}$, with unit $\eta$ and multiplication $U(\epsilon_{F(A)}):U(F(U(F(A)))) \rightarrow U(F(A))$.

* $F \circ U$ is a comonad on $\mathcal{D}$, with counit $\espilon$ and comultiplication $F(\eta_{U(B)}):F(U(B)) \rightarrow F(U(F(B))$.

\end{proposition}
\begin{proof}

We verify only that we obtain a monad.

* We know that this diagram commutes:
\begin{tikzcd}
F(A) \arrow[r, "F(\eta_{A})"] \arrow[rd, "="'] & F(U(F(A)) \arrow[d, "\epsilon_{F(A)}"] \\
                                               & F(A)                                  
\end{tikzcd}
