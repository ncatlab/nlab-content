...
Any extensive category admits a [[Grothendieck topology]] whose [[cover|covering families]] are (generated by) the families of inclusions into a finite coproduct.  We call this the **extensive coverage**.  In general, we call a [[site]] __superextensive__ if its underlying category is extensive, its covering families are generated by finite families, and its coverage includes the extensive one. (Here, the prefix 'super-' is opposite to 'sub-' as in '[[subset]]'.) In this case, one can replace any finite covering family $(j_i: U_i \to B)_i$ with the single map $[j_i]_i: \coprod_i U_i \to B$.  The original coverage is then generated by these single maps together with the extensive coverage.

Likewise, an $\infty$-extensive category has an $\infty$-extensive coverage, and we have the notion of __super-$\infty$-extensive__ site, all obtained by replacing "finite" by "small" in the previous paragraph. Well known examples of super-$\infty$-extensive sites include [[Top]] and [[Diff]] with their usual topologies, generated by the open covers.
