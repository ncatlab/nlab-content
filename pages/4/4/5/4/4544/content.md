...
Let $(X,\mathcal{O}_X)$ be a [[ringed space]]. Then a sheaf $\mathcal{F}$ of $\mathcal{O}_X$-modules is of finite type if for every point $x\in X$ there is a neighborhood $U$ such that the sheaf $\mathcal{F}|_U$ is generated by finitely many sections. This local version coincides with the global version for coherent sheaves over Noetherian schemes (by gluing).
