...
Let $(X,\mathcal{O}_X)$ be a [[ringed space]]. Then a sheaf $\mathcal{F}$ of $\mathcal{O}_X$-modules is of finite type if for every point $x\in X$ there is an open nighborhood $U\ni x$ such that the sheaf $\mathcal{F}|_U$ is generated by finitely many sections. That means that there is a finite sequence $s_1,\ldots,s_n$ of elements in $\mathcal{F}(U)$ such that for each open $V\subset U$ any section of $\mathcal{F}(V)$ is of the form $\sum_i f_i s_i$ where $f_i\in\mathcal{O}(V)$. In other words, there is an epimorphism of sheaves $(\mathcal{O}_X|U)^n\to\mathcal{F}_U$ (this amounts to stalkwise surjection). This local version coincides with the global version for coherent sheaves over Noetherian schemes (by gluing).

See also a different notion of _[[morphism of finite type]]_ in [[algebraic geometry]].  
