
+-- {: .rightHandSide}
+-- {: .toc .clickDown tabindex="0"}
###Context###
#### Monoidal categories
+--{: .hide}
[[!include monoidal categories - contents]]
=--
=--
=--

This page is to provide links related to the text

* Geraldine Brady, [[Todd Trimble]],

  _A string diagram calculus for predicate logic and C. S. Peir
e's System Beta_

  ([[BradyTrimbleString.pdf:file]])

on [[string diagram]] calculus for [[indexed monoidal categories]] with [[poset]] fibers (monoidal [[hyperdoctrines]]).

Based on this a string diagram calculus for general [[indexed monoidal categories]] is given in 

* {#PontoShulman12} [[Kate Ponto]], [[Michael Shulman]], _Duality and traces for indexed monoidal categories_, Theory and Applications of Categories, Vol. 26, 2012, No. 23, pp 582-659 ([arXiv:1211.1555](http://arxiv.org/abs/1211.1555))


category: reference