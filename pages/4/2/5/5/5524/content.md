...
A morphism of 0-truncated objects in $SynthDiff\infty Grpd$, hence in the [[Cahiers topos]], is a [[formally étale morphism]] with respect to the [[infinitesimal cohesion]] $Smooth \infty Grpd \hookrightarrow SynthDiff\infty Grpd$ precisely if it is so in the sense of ([Kock, p. 82](#Kock)).

In particular, a [[smooth function]] $f : X \to Y$ in [[SmoothMfd]] $\hookrightarrow$ [[Smooth∞Grpd]] between [[smooth manifold]]s is a [[formally étale morphism]] with respect to the [[infinitesimal cohesion]] $Smooth \infty Grpd \hookrightarrow SynthDiff\infty Grpd$ precisely if it is a [[local diffeomorphism]] in the traditional sense.
