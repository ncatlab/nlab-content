...
* $\mathcal{B}$ is closed under finite unions: if $B_1 \ldots, B_n \in \mathcal{B}$, then $\bigcup_{1 \leq \i \keq n} B_i \in \mathcal{B}$. 
...
* If $X$ is any metric space, there is a bornology where a set is bounded if it is contained in some open ball. Any uniformly continuous map is bounded with respect to this uniform choice of bornology. 
