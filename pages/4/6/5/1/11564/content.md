...
1. $\flat_{dR} \Sigma (\Omega \otimes C)^{\bullet \geq n} \simeq (\Omega \otimes C^{\bullet \leq n-1})^{\bullet \geq n}$

1. $\flat ((\Omega \otimes C)^{\bullet \geq n}) \simeq C^{\bullet \geq n} $

1. $\Pi((\Omega \otimes C)^{\bullet \geq n})\simeq C^\bullet$
...
For $C \in Ch_\bullet$ and $n \in \mathbb{N}$, let $C_{conn,-n}$ be the [[homotopy fiber product]] in

$$
  \array{
    C_{conn,-n} &\longrightarrow& \Omega^{\bullet \geq n}(C\otimes \mathbb{R})
    \\
    \downarrow && \downarrow
    \\
    C 
     &\longrightarrow&
    C \otimes \mathbb{R} & \simeq \Pi(\Omega^{\bullet \geq n}(C\otimes \mathbb{R}))
  }
  \,,
$$
...
1. $C \in Ch_\bullet$ a [[chain complex]];

1. $c \colon E \longrightarrow C$ a [[homomorphism]] of [[spectra]] from $E$ to the image of $C$ under the [[stable Dold-Kan correspondence]];

1. $n \in \mathbb{N}$

write $Diff^{-n}(E,C,c)$ for the [[homotopy fiber product]] in

$$
  \array{
    Diff^{-n}(E,C,c) &\longrightarrow& (\Omega \otimes C)^{\bullet \geq -n}
    \\
    \downarrow && \downarrow
    \\
...
Specifically when $C \simeq E \wedge DK(\mathbb{R})$ is a model for the rationalization (realification) of $E$ then $E \to C$ is a model for the [[Chern character]] and def. \ref{HopkinsSingerPullback} is essentially the definition of ([[Quadratic Functions in Geometry, Topology,and M-Theory|Hopkins-Singer 05]]).


Notice that in this case the above pullback does NOT in general quite coincide with the right square in the differential cohomology hexagon.

([Bunke-Nikolaus-V&#246;lkl 13, lemma 4.7](#BunkeNikolausVoelkl13))

but the discrepancy is only in the truncation of the cycle data and there is a canonical truncation map
