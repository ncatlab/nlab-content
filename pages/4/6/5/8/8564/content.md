...
Many important rings may fail to be principal ideal domains in [[intuitionistic logic|constructive mathematics]]; a better notion in this case is _[[Bézout domain]]_, in which only the finitely generated ideals are required to be principal.

For instance, the ring of integers is a principal ideal domain if and only if the [[excluded middle|law of excluded middle]] holds: One direction is obvious, by the usual proof. For the converse, let $\varphi$ be an arbitrary proposition. Consider the ideal $\{ x \in \mathbb{Z} | (x = 0) \vee \varphi \}$. By assumption, it is generated by some number $n$. Since the integers are [[decidable equality|discrete]], it holds that $n = 0$ or $n \neq 0$. In the first case $\neg\varphi$ holds, in the second $\varphi$.
