...
On the other hand, if we modify the concept of principal ideal domain, we can recover a concept that is identical to the usual one in [[classical mathematics]] but also includes $\mathbb{Z}$.  For instance, we could demand that every [[decidable subset|decidable]] ideal is principal, or (potentially more strongly) that any ideal generated by a decidable subset is principal.  Classically, all ideals are like this, and this condition is sufficient to allow the usual proofs for $\mathbb{Z}$.  Furthermore, the content is nontrivial, since many ideals trivially have this property (including all finitely generated ideals, because the integers have [[decidable equality]]).
