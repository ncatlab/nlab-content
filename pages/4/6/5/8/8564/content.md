...
In [[classical mathematics]], a _pid_ can also be characterized as an [[integral domain]] which possesses a [[Dedekind-Hasse norm]], making very clear the fact that it is a generalization of an [[Euclidean domain]]. However, that the two notions coincide implies [[excluded middle]] (see [below](#InConstructiveMathematics)) so this characterisation of a principal ideal domain is not available in [[constructive mathematics]]. 
...
In [[constructive mathematics]], many important rings may fail to be principal ideal domains in the na&#239;ve sense:
...
As a result, it is common to modify the concept of principal ideal domain to recover a concept that is identical to the usual one in [[classical mathematics]] but also includes [[discrete fields]] and $\mathbb{Z}$. There are many such possible definitions, such as
...
Unique factorization domains, atomic domains, and Noetherian domains all satisfy the [[ascending chain condition on principal ideals]]. Since [[Bézout domains]] are [[GCD domains]], the [[ascending chain condition on principal ideals]] implies that the [[Bézout domain]] is a [[unique factorization domain]]. Thus, the first four definitions are the same. 
