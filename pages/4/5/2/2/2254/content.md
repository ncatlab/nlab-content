...
in $Adic R CAlg$ satisfying an associativity condition.

This is a **commutative formal group law** if it is an abelian group object.
...
1. (unitality) $\mu(x,0) = x$ and $\mu(0,x) = x$;

1. (associativity) $\mu(x,\mu(y,z)) = \mu(\mu(x,y),z)$.
...
The existence of inverses is no extra condition: there exists a unique 

$$
  \iotat(x) = \underset{i \geq 1}{\sum} \iota(x)_i x^i
$$
