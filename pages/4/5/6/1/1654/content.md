...
so that a cocone for $F$ is an object of an [[comma category]] 
...
Assume for the moment that the receiving category $C$ has all coproducts and coequalizers. Then we can write down a formula for the colimit of $F: J \to C$: as a coequalizer of a pair 
...
and the unsuitability of this notion for general enriched categories could be thought of as a case of putting all one's eggs in 1 basket. 
...
Let $J$ be a small category. Given a weight $W: J^{op} \to Set$ and a functor $F: J \to C$, the **weighted colimit** or tensor product is an object $W \cdot F$ of $C$ together with an isomorphism 
...
The notion of weighted colimit carries over in straightforward fashion to categories enriched in a complete, cocomplete, closed symmetric monoidal category $V$. In that case, if $J$ is a small $V$-category (one whose object class is small), and if $F: J \to C$ and $W: J^{op} \to V$ are $V$-functors, a colimit of $F$ with respect to the weight $W$ is an object $W \cdot F$ of $C$ together with an $V$-natural isomorphism 
