...
Let $\mathcal{C}$ be a [[category of fibrant objects]]. Let $\mathcal{D}$ be a [[category with weak equivalences]]. Let $F : \mathcal{C} \to \mathcal{D}$ be a functor with the property that, for every arrow $f$ of $\mathcal{C}$ which is a trivial fibration, we have that $F(f)$ is a weak equivalence. 
