...
The Transcendental Syntax starts from the strong claim that [[formal logic]] makes a lot of rather arbitrary choices such as the so-called *rules of logic* (see [[deductive systems]]) and that it should be possible to study the mechanisms of reasoning without these assumptions (to which Girard refers to as "prejudices" of logic).
...
This is related to the distinction between existentialism and essentialism in philosophy. The **logical certainty** corresponds to the adequacy between usine and usage: we would like the usine to be finite and sufficient to ensure what we consider a right use of epistates. According to Girard [Girard 16](#Girard16), the separation between constat and performance comes from the notion of undecidability (as in the halting problem) because the potential of programs, for instance, cannot always be reduced to their result (which is not necessarily defined). The separation between usine and usage comes from Gödel's [[incompleteness theorem]] which intuitively means that the possible uses of a logical object may go beyond what we expect (think of Gödel's formulas which are rare and not so problematic in practice).
