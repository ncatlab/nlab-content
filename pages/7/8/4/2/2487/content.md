...
1. For $k = \mathbb{C}$: every invertible complex matrix is diagonalizable by a sequence of elementary matrix operations. Each of these is clearly path-connected to the identity. Finally the subspace of invertible [[diagonal matrices]] is the [[product topological space]] $\underset{ \{1, \cdots, n\} }{\prod} (\mathbb{C} \setminus \{0\})$ and hence connected (by [this prop.](ProductSpaceOfConnectedSpacesIsConnected), since each factor space is).
