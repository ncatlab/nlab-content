...
+-- {: .proof} 
###### Proof 
(That the theory is finitely satisfiable.) This follows if we show that any finitely generated commutative ring $R$ has a prime ideal. In fact we will do more and show that any provably [[countable set|countable]] commutative ring $R$ with unit admits a prime ideal. For this we use WKL (weak K&#246;nig's lemma), which follows from (UF) by the argument presented [here](/nlab/show/compactness+theorem#wkl). 
