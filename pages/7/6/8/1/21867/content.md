...
Although their normal form is not yet implemented, DisCoPy also implements the syntax for diagrams in:
...
* as diagrams with [[Frobenius algebras]] on every object,
* as cospans of labeled hypergraphs, following [Bonchi et al. (2022)](#BonchiEtAl22).
...
Concretely, a DisCoPy functor is defined by `F = Functor(ob, ar)` where `ob` and `ar` are mappings from generating object to types and from box to diagram respectively. These mappings can be either given as Python dictionaries if they have finite domains or as Python functions otherwise. Once its image for each generating object and each box is given, `F` can be applied to arbitrary diagrams. An optional argument `cod` can be used to define functors with arbitrary categories as codomain. The domain of the functor is given implicitly by the domain of the `ob` and `ar` maps.
By default DisCoPy functors are endofunctors from the free category to itself, i.e. they send diagrams to diagrams. 

Currently, DisCoPy implements [[strong monoidal functors]] into:

* the category of [[Python]] functions,

* the category of [[matrices]] in arbitrary [[rig|semirings]], including [[finite dimensional vector spaces]] and finite [[relation|relations]], with either the [[direct sum]] or the [[tensor product]] as monoidal product,
...
* [[formal grammar]] where derivations are diagrams in a [[monoidal category]] and in particular [[context-free grammar]] where derivations are syntax trees,
* [[categorial grammar]] where derivations are diagrams in a [[closed monoidal category]],
* [[pregroup grammar]] and [[dependency grammar]] where derivations are diagrams in a [[rigid monoidal category]].
...
* {#BonchiEtAl22} Filippo Bonchi, Fabio Gadducci, Aleks Kissinger, Pawel Sobocinski, Fabio Zanasi, _String Diagram Rewrite Theory I: Rewriting with Frobenius Structure._ [DOI:10.1145/3502719](https://doi.org/10.1145/3502719) (2022).



* {#DisCoPyDoc} _DisCoPy's documentation._ [discopy.readthedocs.io](https://docs.discopy.org/)
