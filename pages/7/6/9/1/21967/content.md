...
For a [[monomorphism]] $i_T \colon T \hookrightarrow B$, the _corestriction_ $f|^T \colon A \to T$ of $f$ onto $T$ is, if it exists, the unique morphism $f|^T \colon A \to T$ such that there is a decomposition $f = i_T \circ f|^T$. 
...
Alternatively, one may consider a notion of corestriction which is the [[formal dual]] to the notion of a restriction: if $p^U \colon B\to U$ is an [[epimorphism]], then the co-restriction of a morphism $f \colon A\to B$, in this dual sense, is the [[composition|postcomposition]] $p^U \circ f \colon \colon A\to U$ of $f$ by $p^U$, which surely always exists. 
