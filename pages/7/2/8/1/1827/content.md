...
Let an element of $F_{\mathbf{N},\mathbf{N}}$ be a _pre[[sequence]]_ of natural numbers, and fix a [[bijection]] $\phi\colon \mathbf{N} \to \mathbf{Q}$ between $\mathbf{N}$ and the set $\mathbf{Q}$ of [[rational number]]s.  We call such a presequence $r$ _regular_ if, whenever $i,j,x,y$ are natural numbers such that $r(i,x)$ and $r(j,y)$, we have
