...
* If $X$ is any [[topological space|space]] and $Y$ is compact, then the [[projection map]] $p: X \times Y \to X$ is a [[closed map]], i.e., the [[direct image]] which is the [[left adjoint]] $\exists_p: P (X \times Y) \to P(X)$ to the [[inverse image]] map $p^\ast: P(X) \to P(X \times Y)$ takes [[closed sets]] in $X \times Y$ to closed sets in $X$. 
