...
Other basic examples of rings are the [[cyclic groups]] $\mathbb{Z}_n$ under their mod-$n$ operations inherited from the integers ([[integers modulo n]]); the [[polynomial rings]], etc.
...
* For each  $n$, this induces a ring structure on the [[cyclic group]] $\mathbb{Z}_n$, given by operations in the [[integers modulo n]].
...
{#TTFormalizations} Formalization in [[univalent foundations of mathematics]] ([[homotopy type theory]] with the [[univalence axiom]]):
