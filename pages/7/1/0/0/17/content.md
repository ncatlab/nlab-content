...
* Page titles should contain only ASCII characters.
  ...
  * Reason: It\'s possible to put non-ASCII characters in a name, e.g. [[∞-category]], but not ones generated by iTeX or &&#8230;; character entities, so this can be difficult, depending on your browser. To keep things easy, therefore, use only ASCII characters in links.
  * PS: If you would like to be a rebel and create a page using symbols in the title anyway, one way to do so is to copy the symbol from somewhere within a page and paste it into the double brackets, e.g. <nowiki>[[∞-groupoid]]</nowiki> produces [[∞-groupoid]]
