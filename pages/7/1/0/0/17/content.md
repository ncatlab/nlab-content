...
These are not set in stone, but we\'re following them for now. Most [[Toby Bartels]] goes around and corrects any violations. But this results in unnecessary kruft (in :category: redirect), so you should try to follow these if possible.

* Page titles should contain only ASCII characters.
  * Examples: `\[\[omega-category\]\]` instead of `\[\[$\omega$-category\]\]`.
  * Tricks: To produce '$\omega$-[[omega-category]]', try `\$\\omega\$-\[\[omega-category\]\]`. Unfortunately, [[omega-category|$\omega$-category]] does not work.
