...
The left adjoint $L : X \to Y$ is [[fully faithful]] (i.e. a [[coreflection]]) if and only if the unit $\eta : id_X \to R \circ L$ is a [[natural isomorphism]]. Dually, the right adjoint $R : Y \to X$ is fully faithful (i.e. a [[reflective subcategory|reflection]]) if and only if the counit $\epsilon : L \circ R \to id_Y$ is a natural isomorphism. (See [this Prop.](adjoint+functor#FullyFaithfulAndInvertibleAdjoints) at *[[adjoint functor]]*.)
