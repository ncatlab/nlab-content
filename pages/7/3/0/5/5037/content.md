...
* The dual of $F$ is $G$, so $G\phi = \neg F\neg $. This means that we read $G\phi$ as \at no future time is $\phi$ not true', i.e., $\phi$ is always **going** to be true. ($G$ for 'going'.)
...
+--{: . un_example}
######Examples######
*  If for all $\phi$ we have $P\phi \to GP\phi$, then _whatever has happened will always hav happened', which seems reasonable so this might be a suitable 'general truth' we might want a temporal logic to contain.
