...
The standard example is the sequence of consecutive doubles starting with $\mathbb{R}$ (with trivial involution): the [[real number]]s $\mathbb{R}$, the [[complex number]]s $\mathbb{C}$, the [[quaternion]]s $\mathbb{H}$, the [[octonion]]s $\mathbb{O}=\mathbb{C}\mathrm{a}$, the [[sedenion]]s $\mathbb{C}\mathrm{a}^2$, etc. 
