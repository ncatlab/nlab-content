...
## Idea
The _theory of decidable objects_ $\mathbb{D}$ is the logical [[theory]] whose models in a [[coherent category]] are precisely the [[decidable objects]].

##Definition##
The _theory of decidable objects_ $\mathbb{D}$ is the theory over the signature with one sort and one binary relation $#$ besides equality with axioms $(x#x)\vdash_x\perp$ and $\top\vdash_{x,y} ((x#y)\vee(x=y))$.
