...
The [[octonionic Hopf fibration]] is [[equivariant function|equivariant]] with respect to a Spin(9) [[action]], the one on $S^8 = S(\mathbb{R}^9)$ induced from the canonical action of $Spin(9)$ on $\mathbb{R}^9$, and on $S^{15} = S(\mathbb{R}^{16})$ induced from the canonical inclusion $Spin(9) \hookrightarrow Spin(16)$.
...
The group $Spin(9)$ has a 16-dimensional faithful irreducible complex representation called its Dirac [[spinor representation]].   ([Bryant 20](#Bryant20)) constructs this using [[octonions]] as follows.   
