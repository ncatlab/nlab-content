...
_Toby_: Yes, despite the historical justification that Johnstone gives in the Elephant, I\'d stick with (what I understand to be) the usual terminology: 'cartesian' means finite limits.  Then 'locally cartesian' means each slice is cartesian, hence pullbacks.  Now the term for all finite limits is 'cartesian locally cartesian' (and topologists, at least, do say things like 'connected locally connected'), but that mouthful just tells us that the time has come to say 'left exact' instead.
=--
