...
The **exterior algebra** $\Lambda V$ of a vector space is the [[free object|free]] graded-commutative algebra over $V$, where the elements of $V$ are taken to be of degree 1.  This construction generalizes to group representations, chain complexes, vector bundles, coherent sheaves, and indeed objects in any [[symmetric monoidal category|symmetric monoidal]]  [[linear category|linear categories]] with enough [[colimit|colimits]], where the tensor product distributes over those colimits.  We begin with the construction for vector spaces and then sketch how to generalize it.

#Definition#

Suppose $V$ is a vector space over a field not of characteristic 2.  Then the **exterior algebra** $\Lambda V$ is generated by the elements of $V$ using these operations:
...
Over a field of characteristic 2 we need to be more careful in defining the exterior algebra: we must replace the relations 
...
for all $v, w \in V$.  These are equivalent if we can divide by 2.  
...
For $V$ a [[super vector space]], the exterior algebra $\Lambda V$ is often called the **Grassmann algebra** over $V$.    This is the $\Lambda V$ or $\wedge^\bullet V$ is the [[free object|free]] [[supercommutative algebra|graded commutative]] [[superalgebra]] on $V$.
