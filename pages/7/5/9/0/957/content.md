...
A **direction** on a set $S$ is a [[preorder]] on $S$ in which any finite subset has an [[upper bound]].  A **directed set** is a [[set]] equipped with a direction.  A **directed poset** is a directed set whose preorder is a [[partial order]] (so _directed poset_ might be a better term than _directed set_). Note that many authors by directed set mean directed poset.
