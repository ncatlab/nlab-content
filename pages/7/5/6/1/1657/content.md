...
*  A measure is __$\sigma$-finite__ if every measurable set is the union of countably many sets with finite measure.
...
*  When $\infty$ is allowed as a value of $\mu$, then the requirement in (3) that the sum converges should be interpreted in this light; that is, the sum may diverge to infinity.  (Therefore, for a positive measure, the convergence criterion is vacuous in [[classical mathematics]].)
...
Another possibility is to generalize the [[source]] of $\mu$; instead of using a $\sigma$-algebra on $X$, we could use a $\sigma$-ring or even a $\delta$-ring.  These versions are mostly more about changing the definition of a [[measurable space]], so refer there for details of the definitions; however, we note that (3), when $\Sigma$ is a $\delta$-ring, should state that the left-hand side exists (that is, the union is measurable) if the right-hand side converges.  Generalizing $\Sigma$ in this way is complementary to generalizing the target above; in particular, it may allow one to avoid dealing with $\infty$.  For example, while the Lebesgue measure is only a positive measure on a $\sigma$-algebra, it is a *finite* positive measure on the $\delta$-ring of bounded measurable sets.  Indeed, every signed measure gives rise to a finite measure on its $\delta$-ring of finitely measurable sets (as defined below); conversely, every $\sigma$-finite measure can be recovered from this by imposing (3) in all cases.
...
Then the various notions of measures and measure spaces above also make sense on a $\sigma$-complete Boolean algebra, but where the [[countable set|countable]] [[union]] and [[intersection]] of [[measurable subsets]] in a traditional measurable space are replaced with the countable [[join]] and [[meet]] of measurables in the $\sigma$-complete Boolean algebra. For example, a probability measure on a $\sigma$-complete Boolean algebra $\Sigma$ is a function $\mu:\Sigma \to [0, 1]$ such that 
...
In general, a _measurable set_ is any set $S$ such that $(S,T)$ is a complemented pair for some set $T$; the term 'measurable set' in the classical theory should be interpreted as either 'measurable set' or 'complemented pair' in the constructive theory, depending on context.  Usually both interpretations will actually work, but often only the first set of the pair will matter, thanks to the axiom above.
