...
There exists [[1-site|1-sites]] $C$ such that the [[(n,1)-topos|(1,1)-topos]] of sheaves of sets on $C$ has [[point of a topos|enough points in the 1-topos sense]], but such that the corresponding [[n-localic (∞,1)-topos|1-localic (∞,1)-topos]] does not have enough points in the sense of def. \ref{PointOfAndInfinityTopos}. An example is given by the site of open subsets of the topological space $\prod_{\mathbb{N}} \{x,z,y\}$ where the topology on $\{x,z,y\}$ is generated by the two open subsets $\{x,y\}$ and $\{x,z\}$. See [[Higher Topos Theory|HTT, Remark 6.5.4.7]].
