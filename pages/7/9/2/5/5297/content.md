...
The bicategory $Map K$ is the locally full subbicategory of $K$ determined by the maps.
...
More generally, if $C$ is a category with finite [[limits]], then there is a bicategory $Span C$ of [[spans]] in $C$.  The bicategory $Map Span C$ is equivalent to $C$.

## Properties

If $K$ is a [[cartesian bicategory]], then $Map K$ has finite 2-products.

If every map in $K$ is [[comonadic]] and $Map K$ has a terminal object, then $Map K$ is equivalent to a 1-category.
