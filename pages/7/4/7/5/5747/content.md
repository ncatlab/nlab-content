...
A topology $j_w$ corresponds to a [[dense subtopos]] $Sh_{j_w}(Set^{\Delta_+^{op}})$ precisely when $w_0=0$  since otherwise every $j_w$-sheaf contains exactly one vertex hence $0\in Set^{\Delta_+^{op}}$ wouldn't be a $j_w$-sheaf.
...
We denote the corresponding topologies by $j^0_k$ where $k$ indicates the position where the 0 occurs e.g. $j^0_0=j_{\neg\neg}$. The endofunctor of the $j^0_k$-skeletal [[comonad]] on $Set^{\Delta_+^{op}}$ is then given by $X\mapsto \coprod_{X[k]}[k]$ (in particular, the $j^0_k$-skeleta are those semi-simplicial sets $X$ with $X\simeq \coprod_{X[k]}[k]$ ). This basically replaces $X$ by its "set" of $k$-semi-simplices in the disconnected form of copies of the standard $k$-semi-simplex and discards everything else.
...
These are precisely the $j_{U_m}^o$-skeleta of $j_{U_m}^o:=j_{0^m111\dots}$ whereas the $j_{U_m}^o$-sheaves are presheaves that have unique filling $n$-semi-simplices for $n\geq m$. Hence, a $j_{U_m}^o$-skeletal presheaf $X$ is an $j_{U_{m+1}}^o$-sheaf and in general not a $j_w$-sheaf for any $j_w$ with $w_{m+1}=1$ since $X[m+1]=\empty\,$. Accordingly, $\hat{j}_{U_m}^o=j_{U_{m+1}}^o\,$: the "Hegelian [[Aufhebung]] of $\empty$" starting with $j_{max}=j_{U_{-1}}^o$ passes stepwise $j_{U_{m}}^o\to j_{U_{m+1}}^o$ exactly through the open subtoposes except the "subtopos at $\infty$" $j_{U_\infty}^o=j_min$ for which $\hat{j}_{U_\infty}^o=j_{U_\infty}^o$. This contrasts somewhat with the situation in [[SSet]] where $\hat{j}_{0^m111\dots}=j_{0^{2m-1}111\dots}$ for $m\geq 2$ (cf. [[Aufhebung]] for more on this case).

Before determining the topologies $j_{U_m}^c$ for the [[closed subtopos|closed subtoposes]], let's have a look at the lattice operations: the [[join]] $j_w\vee j_z$ of two topologies is given by the topology $j_{w\vee z}$ with $(w\vee z)_i=w_i$ in case $w_i=z_i$ or else $(w\vee z)_i=1$. As usual, the sheaf topos corresponding to $j_w\vee j_z$ is given by the intersection of the two sheaf toposes. The [[meet]] $j_w\wedge j_z$ is given by $j_{w\wedge z}$ with $(w\wedge z)_i=w_i$ in case $w_i=z_i$ or else $(w\wedge z)_i=0$. Hence the [[complement]] of a topology $j_w$ is given by $j_{\overline{w}}$ with $\overline{w}_i\neq w_i$ for all $i$ e.g. for an open topology $j_{U_m}^o =j_{0^n111\dots}$ the closed complement $j_{U_m}^c$ is given by $j_{1^n000\dots}$.

The open and closed topologies are far from being the only complemented topologies - in fact, the letter flipping operation $w\to\overline{w}$ provides a _complement_ $j_\overline{w}$ for _every_ topology $j_w$. Hence, the lattice of topologies is not only as usually a [[Heyting algebra]] but a [[Boolean algebra]].
