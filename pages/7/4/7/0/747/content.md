...
Write $S(\{U_i\}) \to j(U)$ for the [[sieve]] generated by a covering family, regarded as a [[subfunctor]] of the [[representable functor]] $j(U)$, which we both regard as simplicially discrete objects in $sPSh(C)$.
...
Homming this into a fibrant object in $sPSh(C)_{inj,cov}$ therefore produces a [[transfinite composition|transfinite composite]] of weak equivalences in $sSet_{Quillen}$ between Kan complexs, which is itself a weak equivalence. Therefore $S(\{U_i\}, \{V_{j,k}\}) \to \tilde S$ is a weak equivalence in $sPSh(C)_{inj,cov}$.
