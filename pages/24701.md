...
The following section is adapted from [[Egbert Rijke]]'s [[Introduction to Homotopy Type Theory]]:
...
$$\frac{\Gamma \vdash a:A}{\Gamma \vdash B \coloneqq A \; \mathrm{type}}$$
...
$$\frac{\mathcal{D}}{B \; \mathrm{type}} \qquad \frac{\mathcal{D}}{\mathrm{defequiv}(A, B): \sum_{f:A \to B} \prod_{b:B} \left(\sum_{a:A} f(a) =_B b \right) \times \prod_{p:\sum_{a:A} f(a) =_B b} \prod_{q:\sum_{a:A} f(a) =_B b} p =_{\sum_{a:A} f(a) =_B b} q}$$

In essence, we define an element $b$ to be equal to $a$, and a type $B$ to be equivalent to $A$, in the same way that in [[structural set theory]] one would define element $b$ to be equal to $A$ and set $B$ to be in [[bijection]] with $A$. 
