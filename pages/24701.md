...
However, it is not true that every internally univalent universe is externally univalent. While it could be proven, from the closure of the universe under identity types, function types, pi types, product types, and sigma types, that there is an equivalence 
...
and the composite of $\mathrm{idtoequiv}(A, B)$ and $\mathrm{canonical}_\simeq(A, B)$ is an equivalence, it is not true that there is an identification $p(A, B):\mathrm{canonical}_\simeq(A, B) \circ \mathrm{idtoequiv}(A, B) =_{(A =_\mathcal{U} B) \to (\mathcal{T}_\mathcal{U}(A) \simeq \mathcal{T}_\mathcal{U}(B))} \mathrm{trans}^{\mathcal{T}_\mathcal{U}}(A, B)$ for all encodings $A:\mathcal{U}$ and $B:\mathcal{U}$. 
