...
In set theory, the fiber of a function $f$ with domain $A$ and codomain $B$ at an element $b$ in $B$ is the indexed union of all elements $a:A$ such that $f(a) = b$. This definition could also be translated into objective type theory: the fiber of a function $f:A \to B$ at the element $b:B$ is the type 
$$\sum_{a:A} f(a) =_B b$$
