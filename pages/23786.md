...
We define the predicate $\mathcal{A}$ on a model of Nelson arithmetic $N$ to be that $\mathcal{A}(x)$ if for all $x \in N$ such that $\mathcal{C}(x)$, and for all $y \in N$, $\mathcal{C}(y + x)$. Nelson referred to elements where $\mathcal{A}(x)$ is true as "additionable numbers"; however, these are better interpreted as "additionable counting numbers", as Nelson is really talking about the natural numbers. 

In the same vein, we could define the predicate $\mathcal{M}$ on a model of Nelson arithmetic $N$ to be that $\mathcal{M}(x)$ if for all $x \in N$ such that $\mathcal{C}(x)$, and for all $y \in N$, $\mathcal{C}(y \cdot x)$. Nelson referred to elements where $\mathcal{M}(x)$ is true as "multiplicable numbers"; however, similarly to "additionable counting numbers" these are better interpreted as "multiplicable counting numbers", as Nelson is really talking about the natural numbers. 

Now, suppose that we define the [[binary function]] $(-)\uparrow(-):N \times N \to N$ called "exponentiation" such that
