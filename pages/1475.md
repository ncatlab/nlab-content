...
Given a [[preorder]]ed set $(S, \leq)$, an element $x$ of $S$ is _[[maximal element|maximal]]_ if $y \leq x$ whenever $x \leq y$.  A _[[chain]]_ in $S$ is a subset $A$ of $S$ such that $\leq$ is a [[total order]] on $A$.  An _[[upper bound]]_ of a subset $A$ of $S$ is an element $x$ of $S$ such that $y \leq x$ whenever $y \in A$.  In these terms, we have:
