...
3. More precisely, $f$-inductive sets are well-ordered by inclusion. For if $A$ and $B$ are distinct (well-ordered and) $f$-inductive sets, then there is a smallest element contained in one but not the other, say $x \in A$ but $\neg(x \in B)$. But then $\{y \in A: y \lt x\}$ is a down-closed subset of $B$, i.e., is an initial segment of $B$. If it were a _proper_ subset of $B$, then for the least $z \in B$ in its complement we would have 
