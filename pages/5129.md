...
* Consider $p$ and $q$ as the [[linear orders]] $\mathbf{p} = \{ 0 \lt \dots \lt p \}$ and $\mathbf{q} = \{ 0 \lt \dots \lt q \}$. Then a $(p,q)$-shuffle is a way of extending the [[partial order]] on the [[coproduct]] $\mathbf{p} + \mathbf{q}$ to a linear order, or equivalently, a [[surjective]] [[monotone function]]
