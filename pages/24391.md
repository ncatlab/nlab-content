...
\begin{example}
 ...
 \end{tikzpicture}
 \end{centre}
 commutes.  In $\mathbf{SCvx}$ the only isomorphism $\mathbb{N} \rightarrow \mathbb{N}$ is the identity map, and hence each equivalence classes of morphisms $\mathbb{N} \rightarrow \mathbb{N}$ is a singleton set.  The isomorphisms $\Delta_{\mathbb{N}}\rightarrow \Delta_{\mathbb{N}}$ are of the form $\Delta_{\phi}: \Delta_{\mathbb{N}} \rightarrow \Delta_{\mathbb{N}}$ where $\phi$ is any permutation on $\Nat$, mapping $\delta_i \mapsto \delta_{\phi(i)}$.
 
 Up to homotopy equivalence, the only countably affine maps $m:A \rightarrow \mathbb{N}$ are of the form $m(\underline{u})=m(\underline{0})=i$  and $m(\underline{1})=i+k$ for all $i,k \in \bN$.  (One can take an isomorphism on $A$ interchanging  $0$ and $1$, to obtain another element in each of the equivalence classes.)  Due to the condition 
that $r \underline{0} + (1-r) \underline{1} = \underline{u}$ for all $r \in (0,1)$ the only countably affine maps $m: A \rightarrow \Delta_{\mathbb{N}}$ are constant maps. 
 ...
 Endowing $A$ with the smallest $\sigma$-algebra such that the morphisms into $\bN$, endowed with the discrete $\sigma$-algebra, are all measurable functions yields, up to an isomorphism, the measurable space $\mS{A} = (A, \{ \{\underline{u}, \underline{0}\}, \{\underline{1}\}, A, \emptyset\}$ which is not a separated measurable space, but it is isomorphic to the separated measurable space $(\twos, \powerset{\twos})$.
Moreover there exists a countably affine  map $\epsilon_A: \T(\Sigma A) \rightarrow A$ given by
\begin{equation} 
\begin{array}{ccl}
\T\big((\mathbf{2}, \powerset{\mathbf{2}})\big) & \longrightarrow & A \\
(1-r) \delta_0 + r \delta_1 & \mapsto & \left\{ \begin{array}{ll} \underline{u} & for all r \in (0,1) \\
\underline{0} & if and only if r=0 \\
\underline{1} & if and only if r=1 \end{array} \right.
\end{array}.
\end{equation}
Thus, by probing $A$ with maps to the two objects $\mathbb{N}$ and $\Delta_{\mathbb{N}}$,  we have recovered the original space $[0,1] \cong \mathcal{P}( (\twos, \powerset{\twos}))$ we started with prior to constructing $A$ as a coequalizer of two points  $\frac{1}{3},\frac{2}{3}: \one \rightarrow [0,1]$.
\end{example} 
