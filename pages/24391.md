...
Warning: This is not quite an accurate picture because it is necessary to replace the monoid category $\Delta_{\mathbb{N}}$ with the full subcategory of $\SCvx$ consisting of two objects, $\mathbb{N}$ and $\Delta_{\mathbb{N}}$. $\mathbb{N}$ is required because the category of standard measurable spaces has the codense subcategory consisting of the discrete space $\mathbb{N}$ with monotonic maps.
\end{example} 
