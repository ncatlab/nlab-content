...
To view this set as a countably generated measurable space  take the generating set as follows. Let
\begin{centre}
\begin{tikzpicture}
...
We note there is also a contravariant functor $\mathcal{O}: \mathbf{Set}^{\Delta_{\mathbb{N}}} \rightarrow\mathbf{Set}^{\Delta_{\mathbb{N}}^{op}}$ given by $\mathcal{O}(\widehat{X}) = Nat( \widehat{X}, \Delta_{\mathbb{N}}(\star,\bullet))$
