...
A [[function]] $f:X\to Y$ between topological spaces is a __continuous map__ (or is said to be *continuous*) if for every open subset $U\subset Y$, the [[preimage]] $f^{-1}(U)$ is open in $X$.
...
### Types of Continuous Map for Topological Spaces ###
...
1. The image of an open set need not be open; a continuous map for which this is true is said to be an __[[open map]]__.  (Technically, an open map is one with just this property.)
1. The image of an closed set need not be closed; a continuous map for which this is true is said to be an __[[closed map]]__.
1. A continuous map of topological spaces which is invertible as a function of sets, is a __[[homeomorphism]]__ if the [[inverse function]] is a continuous map as well.
