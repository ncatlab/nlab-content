...
However, there is a more subtle sense in which $\mathbb{C}$ is algebraically closed even without WCC; essentially, this allows us to approximate the [[subset]] of $\mathbb{C}$ whose elements are the two solutions of (eq:eqn) (using two-element subsets of the field of, say, [[Gaussian number]]s) even if we can\'t approximate any one solution (using individual, say, Gaussian numbers).  See _[The fundamental theorem of algebra: a constructive development without choice](http://math.fau.edu/Richman/HTML/DOCS.HTM)_ by [[Fred Richman]] for details.
