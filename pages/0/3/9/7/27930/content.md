
+-- {: .rightHandSide}
+-- {: .toc .clickDown tabindex="0"}
###Context###
#### Higher algebra
+--{: .hide}
[[!include higher algebra - contents]]
=--
#### Categorical algebra
+-- {: .hide}
[[!include categorical algebra -- contents]]
=--
=--
=--


#Contents#
* table of contents
{:toc}

## Idea

An **operadic category** is a structure for describing various category-like structures (**generalised operads**) with different kinds of composition operations, including [[monoids]], ([[symmetric operad|symmetric]] and non-symmetric) [[operads]], $n$-operads, hyperoperads, and charades.

Every generalised operad has a notion of algebra, with which can be captured notions like ([[wheeled graph|wheeled]]) [[properads]], [[PROPs]], [[cyclic operads]], and (twisted) [[modular operads]].

Operadic categories are thus a framework for [[generalised multicategories]], albeit of a different style than approaches based on monads.

## Related pages

- [[generalised multicategory]]
- [[operad]]

## References

* [[Michael Batanin]] and [[Martin Markl]], _Operadic categories and duoidal Deligne's conjecture_, Advances in Mathematics 285 (2015): 1630-1687.

* [[Steve Lack]], _Operadic structures and their skew monoidal categories of collections_, Higher Structures 2.1 (2018): 1-29.

* [[Richard Garner]], [[Joachim Kock]], and [[Mark Weber]], _Operadic categories and décalage_, Advances in Mathematics 377 (2021): 107440.

* [[Michael Batanin]] and [[Martin Markl]], _Operadic categories as a natural environment for Koszul duality_, Compositionality 5 (2023).

[[!redirects operadic categories]]
