...
+-- {: .proof} 
###### Proof 
First we show that Sylow subgroups exist. We start by observing that if some group $H$ has a $p$-Sylow subgroup $P$, then so does any subgroup $G$ of $H$. To prove this, first note that if we let $G$ act on $H/P$ by left translation, then the stabilizer of any element $h P$ is $G \cap h P h^{-1}$, a $p$-group since $h P h^{-1}$ is. Then note that since $H/P$ has cardinality prime to $p$, so must one of its [[connected object|connected components]] $G/Stab(a_x)$ in its $G$-set decomposition 
