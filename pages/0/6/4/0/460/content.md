...
Sobriety is a separation property that is stronger than $T_0$, but incomparable with $T_1$.  With classical logic, every Hausdorff ($T_2$) space is sober, but this can fail [[constructivism|constructively]].

Any nontrivial indiscrete space is not sober, since it is not $T_0$. More interestingly, the space $R^2$ with the [[Zariski topology]] is $T_1$ but not sober, since every subvariety is an irreducible closed set which is not the closure of a point.  Its _soberification_ is, unsurprisingly, the [[scheme]] $Spec(R[x,y])$, which contains "generic points" whose closures are the subvarieties.  On the other hand, the [[Alexandrov topology]] on any [[poset]] is sober but not $T_1$.
