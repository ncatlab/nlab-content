...
* _[Tensor products and Super vector spaces](#TensorProductsAndMonoidalCategories)_
...
### Monoids in monoidal categories and Supercommutative superalgebra
...
$\{\theta_i\}_{i = 1}^n$ in odd degree is the [[quotient]] of the [[tensor algebra]] $T^\bullet \mathbb{R}^n$,
...
$$
  i \;\colon\; Vect_k \hookrightarrow sVect_k
$$
...
1. every super-algebra homomorphism of the form $B \to i(A_{ordinary})$ must send every odd element of $B$ to 0, again because homomorphisms have to respect the $\mathbb{Z}/2$-grading, and since homomorphisms of course also preserve products, this means that the entire ideal generated by $B_{odd}$ must be sent to zero, hence the homomorphism must factor through the [[projection]] $B \to B/B_{odd}$, which gives a natural bijection
...
+-- {: .num_defn #SuperZGradedVectorSpaces}
###### Definition

Write

$$
  ( Vect^{\mathbb{Z}}, \otimes_k, \tau^{super} )
$$

for the [[symmetric monoidal category]] whose underlying [[monoidal category]] is that
of $\mathbb{Z}$-[[graded vector spaces]] (example \ref{GradedVectorSpacesAsAMonoidaCategory})
(for $\mathbb{Z} \coloneqq (\mathbb{Z}, +)$ the additive group of [[integers]])
equipped with the [[symmetric monoidal category|symmetric]] [[braided monoidal category|braiding]]
$\tau^{super}$
which is that of the underlying [[super vector space]] (def. \ref{CategoryOfSuperVectorSpaces}) under the
mod-2 reduction of grading $\mathbb{Z} \stackrel{mod\, 2}{\longrightarrow}\mathbb{Z}/2$.

This means that for $V,W \in Vect^{\mathbb{Z}}$ two $\mathbb{Z}$-graded vector spaces then

$$
  \tau^{super}_{V,W}
    \;\colon\;
  V \otimes_k W \longrightarrow W \otimes_k V
$$

is the [[linear map]] given on representatives  elements $v \in V_{deg(v)} \hookrightarrow V$
and $w \in W_{deg(w)} \hookrightarrow W$ of homogeneous degree by

$$
  (v,w) \,\mapsto\, (-1)^{deg(v)deg(w)} (w,v)
  \,.
$$
...
+-- {: .num_example #SupercommutativeSuperalgebraZGraded}
###### Example

The category of [[commutative monoids]] (def. \ref{MonoidsInMonoidalCategory}) in the category
of $\mathbb{Z}$-graded super vector spaces in the sense of def. \ref{SuperZGradedVectorSpaces}


$$
  CMon( Vect^{\mathbb{Z}}, \otimes_k , \tau^{super} )
    \hookrightarrow
  Alg_{\mathbb{R}}
$$

is equivalently the [[full subcategory]] of [[associative algebras]] over $\mathbb{R}$ on those algebras which are
...
$$
  \partial_{V \otimes W} ( v, w)
    \;\coloneqq\;
  (\partial_V v, w)
  +
  (-1)^{deg(v)} (v, \partial_W w)
  \,.
$$
...
$$
  dgCAlg_k \coloneqq CMon(Ch_\bullet(k))
  \,.
$$

Under passing to [[chain homology]], a differential graded-commutative algebra goes to a
$\mathbb{Z}$-graded commutative algebra in the sense of example \ref{SupercommutativeSuperalgebraZGraded}:
...
The following relation between $\mathbb{Z}$-grading and $\mathbb{Z}/2$-grading is elementary but important.
It was highlighted in [Rezk 09, section 2](periodic+ring+spectrum#Rezk09):

+-- {: .num_prop #kbetaModules}
###### Proposition
...
$$
  k[\beta, \beta^{-1}]
    \in
  CMon( Vect^{\mathbb{Z}}_k, \otimes_k, \tau^{super} )
$$

for the $\mathbb{Z}$-graded-commutative algebra (example \ref{SupercommutativeSuperalgebraZGraded})
which is [[free construction|freely generated]] from a single invertible generator
$\beta$ in degree 2:
...
Then there are [[strong monoidal functors]]
...
which exhibit an [[equivalence of categories]]
between its [[categories of modules]] (def. \ref{ModulesInMonoidalCategory}) and the
category of [[super vector spaces]] (def. \ref{CategoryOfSuperVectorSpaces}).

Here the functor $(-)|_{0,1}$ sends a $\mathbb{Z}$-graded vector space $V = \underset{n \in \mathbb{Z}}{\oplus} V_n$
to the $\mathbb{Z}/2$-graded vector space $V_0 \oplus V_1$.
...
and $\beta^{-1}$ establish [[linear map|linear]] [[isomorphisms]] of the form

$$
  N_n
    \underoverset
      {\underset{\beta }{\longrightarrow}}
      {\overset{\beta^{-1}}{\longleftarrow}}
      {\simeq}
  N_{n+2}
$$
...
This implies that, conversely, plain supercommutative superalgebras arise as $\mathbb{Z}$-graded supercommutative
...
for the [[free construction|free]] $\mathbb{Z}$-graded supercommutative superalgebra
on a single invertible generator $\beta$ of even degree from prop. \ref{kbetaModules}.
...
By prop. \ref{kbetaModules} and prop. \ref{AlgebrasOverAAreMonoidsUnderA}.
...
[[periodic ring spectrum|2-periodic]] [[E-infinity ring]], such as [[MP]], [[KU]] or the spectrum underlying
any [[elliptic cohomology theory]] or [[Morava E-theory]]. Then its $\mathbb{Z}$-graded-commutative ring
of [[stable homotopy groups]] according to example \ref{HomotopyCommutativeRingSpectrum} is of the form
$$
...
between the categories of [[supercommutative superalgebras]] (def. \ref{SupercommutativeSuperalgebra})
...
It is immediate to check the [[natural bijection]] between [[hom sets]] that characterizes the [[adjunction]]:

For $A \in sCAlg^{\mathbb{Z}}_{\mathbb{R}}$ and $B \in sCAlg_{\mathbb{R}}$ then a morphism

$$
  \phi \;\colon\; A \longrightarrow B_{per}
$$

in $sCAlg^{\mathbb{Z}}_{\mathbb{R}}$

is a [[homomorphism]] of the underlying $\mathbb{R}$-algebras subject to the constraint that for all $n \in \mathbb{Z}$
then
...
$$
  ...
  \text{and}
  \;\;\;\;\;\;
  \phi(\oplus_{2n+1} A_{2n+1})
    \subset
  B_{odd}
$$

which in turn is equivalently the constraint on an algebra homomorphism to constitute a morphism of the form
...
Monoidal categories were introduced independently by [[Jean Bénabou]] and [[Saunders MacLane]] under the name 'categories with multiplication'. The current name is due to [[Samuel Eilenberg]].
...
* {#MacLane63} [[Saunders Mac Lane]], _Natural Associativity and Commutativity_ , Rice University Studies **49** (1963) pp.28-46.
