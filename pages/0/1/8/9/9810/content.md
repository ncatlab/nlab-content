...
A [[functor]] from a [[category]] to a [[site]] induces a [[Grothendieck topology|topology]] on the source category.  The **comparison lemma** says that, under certain conditions, such a functor induces an equivalence between the [[category of sheaves|categories of sheaves]] on the sites.

## Induced topology

Let $u : B \to C$ be a functor with $C$ a site.  Recall that the topology induced on $B$ by $u$ is defined to be the finest one such that $u$ is a [[continuous functor]], i.e. the map $G \mapsto G \circ u$ takes sheaves on $C$ to sheaves on $B$.
...
Beilinson proves the following generalisation of the classical comparison lemma.
