...
_Mathieu says:_ I agree that, for functors, there is no reason to say "fully faithful" rather than "full and faithful".  But for arrows in a 2-category (like in the new version of the entry on [[subcategory|subcategories]]), there are reasons.  I cite myself (from my thesis): &#171;Remark: we say _fully faithful_ and not _full and faithful_, because the condition that, for all $X:\C$, $C(X,f)$ be full is not equivalent in $\mathrm{Gpd}$ to $f$ being full. Moreover, in $\mathrm{Gpd}$, this condition implies faithfulness.  We will define (Definition 197) a notion of full arrow in a $\mathrm{Gpd}$-category which, in $\mathrm{Gpd}$ and $\mathrm{2-GpS}$ [symmetric 2-groups], gives back the ordinary full functors.&#187;  Note that this works only for some good groupoid enriched categories, not for $\mathrm{Cat}$, for example.