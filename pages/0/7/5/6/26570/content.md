...
The set of semidecidable propositions is the [[subobject classifier|classifier]] of semidecidable subsets on the [[intuitionistic arithmetical hierarchy]]; hence it can be denoted by $\Psi_1$ from the intuitionistic arithmetical hierarchy, since $\Psi_1$ represents semidecidable subsets on the hierarchy. The set of semidecidable propositions is defined as a [[subset]] of the [[set of propositions]] $\Omega$ containing all the semidecidable propositions:

$$\Psi_1 \coloneqq \{P \in \Omega \vert \exists f \in 2^\mathbb{N}.(P = \top) \iff \exists x \in \mathbb{N}.f(x) = 1\}$$
...
$$\Psi_1 \coloneqq \{P \in \Sigma \vert \exists f \in 2^\mathbb{N}.(P = \top) \iff \exists x \in \mathbb{N}.f(x) = 1\}$$
