...
The category of collection carries a [[monoidal category|monoidal product]] $\circ$ defined as follows. Given collections $f: X \to T(1)$, $g: Y \to T(1)$, the underlying globular set of $X \circ Y$ is given by [[pullback]]
...
A **globular operad** is a [[monoid]] in the [[monoidal category]] $Coll$, def. \ref{MonoidalStructureOnCollections}. 
...
Each globular operad $f: P \to T(1)$, def. \ref{GlobularOperad}, gives rise to a globular [[monad]] $M_P$ on $Set^{Glob^{op}}$. Abstractly, $M_P(X)$ is just the pullback 
...
An [[Batanin ∞-category]] is a globular set with a $K$-algebra structure. 
