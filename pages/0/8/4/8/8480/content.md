...
(In more down-to-earth terms, let $a$ be the infimum of $A = \{x: f(x) \leq x\}$. Then for every $x \in A$, we have $f(t) \leq f(x) \leq x$, hence $f(t) \leq t$. We therefore also have $f(f(t)) \leq f(t)$, so that $f(t) \in A$. But then it follows that $t \leq f(t)$, whence $t = f(t)$. Clearly this $t$ is a least fixed point of $f$.) 
