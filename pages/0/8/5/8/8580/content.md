...
The fundamental [[physics]] of the [[observable universe|observed world]], notably the [[standard model of particle physics]], is governed by what is called _[[quantum theory]]_. Two major axiomatizations of [[quantum theory]] are known, namely _[[FQFT]]_ (axiomatizing the "[[Schrödinger picture]]") and _[[AQFT]]_ (axiomatizing the "[[Heisenberg picture]]"). But all fundamental [[quantum field theories]] observed in nature or conjectured to underly nature either directly arise by a process called _[[quantization]]_ from structures in [[differential geometry]], or are induced via a mechanism called the [[holographic principle]] by such they do. This differential geometric data notably involves smooth [[functionals]] on smooth [[moduli spaces]] of differential geometric structures such as [[fiber bundles]] and [[connection on a bundle|connections]]: called _[[action functionals]]_. Similar differential geometric structures are involved in the _[[geometric quantization]]_ of such an action functional to an actual [[quantum field theory]], hence something axiomatized by either [[FQFT]] or [[AQFT]].

Here we discuss both generally abstracty as well as in concrete detail central aspects of the full story of [[quantum theory|quantum]] [[physics]]:
...
> Given a gauge field $\nabla$ as above, there is an underlying [[Yang-Mills instanton|instanton]] sector, $UnderlBund(\nabla)$, in the model $[X,\mathbf{B}\left(U\left(1\right)\times U\left(2\right)\times U\left(3\right)\right]$.
...
|--|--|--|--|--|
|  | [[general abstract]] | [[general concrete]] | [[concrete particular]] | |
| There is an equivalence $e$ between $x$ and $x'$. | $\array{\vdash \; e \colon (x \simeq x') \\ or \\ \vdash \; e \colon (x \rightsquigarrow x') }$ | [[element]] of  [[mapping cocone]] |  | _[Identity types](#IdentityTypes)_ |
...
|--|--|--|--|--|
|  | [[general abstract]] | [[general concrete]] | [[concrete particular]] | |
| Given a cohesive homotopy type $X$, there is the the _decohered_ homotopy type $\sharp X$ in which all separate points are collected to one cohesive blob. | $x\colon X \;\vdash\; \sharp X \colon Type$ | |  |  |
| Given a cohesive homotopy type, there is the map that forgets the cohesion. | $x \colon X \;\vdash\;  DeCoh_X \colon X \to \sharp X$ | | | |
...
| Given $A$, there is a map from flat $A$-connections to the underlying $A$-bundles  | $A \colon \sharp Type \;\vdash\; UnderlBund_A \colon \flat A \to A $  | | | |
...
$$
  ...
  \;\vdash\;
  UnderlBund_{\mathbf{B}G} \colon \flat \mathbf{B}G \to \mathbf{B}G
$$
...
$$
  ...
  X 
    \stackrel{\nabla}{\to} 
  \flat\mathbf{B}G
    \stackrel{UnderlBund_{\mathbf{B}G}}{\to}
  \mathbf{B}G
$$
...
$$
  \mathbf{B}G \colon Type
  \;\vdash \;
  UnderlBund \colon \flat \mathbf{B}G \to \mathbf{B}G
$$
...
$$
  \array{
     \flat_{dR} \mathbf{B}G &\stackrel{UnderlConn}{\to}& \flat \mathbf{B}G
     \\
     \downarrow &pb& \downarrow^{\mathrlap{UnderlBund}}
     \\
     * &\to& \mathbf{B}G
  }
  \,.
