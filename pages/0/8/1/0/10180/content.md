...
A related point of view is that $FinSet^{op}$ is the [[free construction|free]] [[cartesian monoidal category]] generated by an object (or by the terminal category), and its [[free cocompletion]] $[FinSet, Set]$ is the free [[cartesian monoidal category|cartesian]] [[monoidally cocomplete category]] generated by an object. Thus $[FinSet, Set]$ plays the role of cartesian analogue to $[\mathbb{P}^{op}, Set]$, the free [[symmetric monoidal category|symmetric]] [[monoidally cocomplete category]] on an object. And in the same way that $[\mathbb{P}^{op}, Set]$ is a [[monoidal topos]] whose monoids are symmetric or permutative [[operads]], so $[FinSet, Set]$ is seen as a monoidal topos whose monoids are cartesian operads, aka [[Lawvere theories]]. Some material on this can be found [here](http://ncatlab.org/toddtrimble/published/Towards+a+doctrine+of+operads). 
