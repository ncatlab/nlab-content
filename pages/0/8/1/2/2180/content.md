...
__Arrow : Function :: Thing 1 : Relation__

: Let's call Thing 1 a _relational arrow_.

__Category : Arrow :: Thing 2 : Relational Arrow__

: Let's call Thing 2 a _relational category_.

__Metagraph : Category :: Thing 3 : Relational Category__

: Let's call Thing 3 a _polygraph_.

So it seems that a __polygraph__ ought to consist of _objects_ $a, b, c, \ldots$, _relational arrows_ $p, q, r, \ldots$, and a number of operations $\mathop{dom}_j$ that assign to each relational arrow $p$ an object $\mathop{dom}_j p$ for each $j$ in the arity (arrowty?) of $p$.
