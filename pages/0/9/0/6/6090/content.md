...
[[coupling constant]] and [[source field]], respective.
   ...
   1. The S-matrix is the [[exponential]] of the time-ordered products: For $L \in \mathcal{F}_{loc}$
      ...
      $$
        \begin{aligned}
          ...
          T(\underset{n\, \text{arguments}}{\underbrace{ (g L + j A) \cdots (g L + j A) }})
        \end{aligned}
      $$
...
The explicit role $\mathcal{F}_{loc}$ is made explicit in terms of axioms for the [[time-ordered products]] (def. \ref{TimeOrderedProduct}),
...
where the last sum does exist in $\mathcal{W}[ [ g/\hbar, j/\hbar] ]$ because $D(L)$ has no contribution in zeroth order, so that
...
which is the [[derivative]] of the generating function at vanishing [[source field]]:

$$
  \widehat{A}(h)
    \;\coloneqq\;
  - i \hbar \frac{d}{d j} V_{g_{sw} L_{int}}( j h A)\vert_{j = 0}
  \,.
$$
