...
where $L_{int}, \{J_n\}_n \in \Omega_{poly}^{d,0}(E)$ are polynomial local densities, to be regarded
...
([[bump functions]]) on spacetime $\Sigma$, to be regarded as [[adiabatic switching]] functions.
...
A _scattering matrix for fields in $E$ perturbing the free fields encoded by $\mathcal{W}$_, is a [[function]]
...
1. (perturbation)
   ...
   $$
     T \;\colon\; \mathcal{F}_{loc}^{\otimes^k} \longrightarrow \mathcal{W}[ [ g/\hbar ] ]
   $$
   ...
   1. The S-matrix is the [[exponential]] of the time-ordered products: For $L \in \mathcal{F}_{loc}$
      ...
      $$
        \begin{aligned}
          S( L )
          & =
          T\left( \exp_{\otimes}\left( \tfrac{-g }{i \hbar} L \right) \right)
          \\
          & \coloneqq
          \underoverset{k = 0}{\infty}{\sum} \tfrac{1}{k!}
          \left(\tfrac{-g}{i \hbar}\right)^k
          T(\underset{n\, \text{args}}{\underbrace{ L \cdots L}})
        \end{aligned}
      $$
...
for the case of fixed $L_{int}, \{J_n\}_{n} \in \Omega^{d,0}_{poly}(E)$, with
$L = g_{sw}L_{int} + \underset{n}{\sum} j_{sw,n} J_n$ with $g_{sw}, \{j_{sw,n}\}_n \in C^\infty_c(X)$
(in view of lemma \ref{CausalLocalityOfThePerturbativeSMatrix} below).
For general elements of $\mathcal{F}_{loc}$ this is, in terms of axioms for the [[time-ordered products]],
due to ([Brunetti-Fredenhagen 99, section 3](#BrunettiFredenhagen99), [D&#252;tschFredenhagen 04, appendix E](#DuetschFredenhagen04),
[Hollands-Wald 04,  around (20)](#HollandsWald04));
for review see ([Rejzner 16, around def. 6.7](#Rejzner16)).
...
1. (formal invertibility) $T_0 = 1$

1. (perturbation) $T_1(L) = :L:$
...
1. (time-ordering) If $supp(L_1) \cup \cdots \cup supp(L_r) \;\geq\; supp(L_{r+1}) \cup \cdots \cup supp(L_k)$ then
   ...
   $$
     T(L_1 \cdots L_k) = T(L_1 \cdots L_r) T(L_{r+1} \cdots L_k )
     \,.
   $$
...
To see this ([Epstein-Glaser 73, around (73)](#EpsteinGlaser73)) one expresses the induced generating [[retarded product]] (def. \ref{RetardedProductFromPerturbativeSMatrix}) in terms of the time-ordered product:

$$
  R(\mathbf{Y}, \mathbf{X})
   \;=\;
  \underset{\mathbf{I} \subset \mathbf{Y}}{\sum} (-1)^{\vert \mathbf{I}\vert}
    \overline{T}(\mathbf{I}) T(  (\mathbf{Y} \setminus \mathbf{I}) \cup \mathbf{X} )
  \,.
$$

This implies that the [[support of a distribution|support]] of $R$ is inside the subset for which 
$\mathbf{Y}$ is in the causal past of $\mathbf{X}$. By multi-linearity of the $R(-,-)$ this in turn implies that
...
By lemma \ref{CausalLocalityOfThePerturbativeSMatrix} this is equivalent to causal additivity of the S-matrix.
...
with is the [[derivative]] of the generating function:

$$
  \widehat{A}  \;\colon\; j_{sw} \mapsto - i \hbar \frac{d}{d \epsilon} V_{g_{sw} L_{int}}(\epsilon A)\vert_{\epsilon = 0}
  \,.
$$
...
$$
  R \;\colon\; \mathcal{F}_{loc}^{\otimes^k} \otimes (\mathcal{F}_{loc})^{\otimes^k} \longrightarrow \mathcal{W}[ [ g ] ] [ [ \hbar ] ]
$$
...
$$
  V_{L}(J)
  =
  \underset{k,k \in \mathbb{N}}{\sum} \frac{1}{k! l!}  
  R( \underset{k \,\text{args}}{\underbrace{L \cdots L } }, \underset{l \; \text{args}}{\underbrace{ J \cdots J }} )
  \,.
$$
...
$$
  r \;\colon\; \mathcal{F}_{loc}^{\otimes^k} \otimes \mathcal{F}_{loc} \longrightarrow \mathcal{W}[ [ g ] ] [ [ \hbar ] ]
$$
...
$$
  \widehat{A}(j_{sw})
  =
  \underset{k \in \mathbb{N}}{\sum} \frac{1}{k!}  r( \underset{k \,\text{args}}{\underbrace{g_{sw}L_{int} \cdots g_{sw}L_{int}}}, j_{sw} A )
  \,.
$$
...
1. The following conditions are equivalent for all $L, J_1, J_2 \in \mathcal{F}_{loc}$:
   ...
   1. $V_L(J_1 + J_2) = V_L(J_1) V_L(J_2)$

   1. $V_{L + J_1}(J_2) = V_L(J_2)$
...
$$
  -i \hbar \frac{d}{d \epsilon} V_{L_{int}}(\epsilon J)\vert_{\epsilon = 0}
    \coloneqq
  ( - i \hbar \frac{d}{d \epsilon} V_{g_{sw}L_{int}} (\epsilon J)\vert_{\epsilon = 0} )_{g_{sw} \in Cutoffs(\mathcal{O})}
$$
...
$$
  \begin{aligned}
    \left[
      -i\hbar \frac{d}{d \epsilon_1} V_{g_{sw}L_{int}}(\epsilon_1 J_1)\vert_{\epsilon_1 = 0}
      \;,\;
      -i\hbar \frac{d}{d \epsilon_2} V_{g_{sw}L_{int}}(\epsilon_2 J_2)\vert_{\epsilon_2 = 0}
    \right]
    &
    =
     - \hbar^2
     ...
     \underbrace{
    \left[
       V_{g_{sw}L_{int}}(\epsilon_1 J_1)
      \;,\;
       V_{g_{sw}L_{int}}(\epsilon_2 J_2)
