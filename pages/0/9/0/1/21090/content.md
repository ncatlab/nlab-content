...
Intuitively, a Markov category is a category where morphisms behave like "random functions", for example, [[Markov kernels]] form such a category (hence the name). Canonical examples are [[Kleisli categories]] of [[probability monads]]. 
...
In fact, all Markov categories fully faithfully monoidally embed in Markov categories of this form, see Theorem \ref{theorem:embed-in-representable}. 
...
We now characterize those that do, as representable Markov categories (Theorem \ref{theorem:representable}). We also note that every Markov category fully embeds in a representable one (Theorem \ref{theorem:embed-in-representable}).
...
\begin{theorem}\label{theorem:representable}
...
\begin{theorem}\label{theorem:embed-in-representable}
Every Markov category embeds in a representable Markov category.
\end{theorem}
\begin{proof}
Let $\hat C_\mathrm{det}$ be the category of [[presheaves]] on $C_\mathrm{det}$, i.e. functors $[C_\mathrm{det}^{op}\to Set]$ and natural transformations between them. This has products, and the [[Yoneda embedding]] 

$$ \mathbf{y}:C_\mathrm{det}\to \hat C_\mathrm{det}$$

(with $\mathbf{y}(X)=C_\mathrm{det}(-,X)$) is full and faithful and always preserves products. 

Recall that every presheaf is a colimit of representables ("[[co-Yoneda lemma]]"). We define a monad $P$ on $\hat C_\mathrm{det}$ by asking that it preserves colimits and that on representables $\mathbf{y}(Y)$, 

$$ P(\mathbf{y}(Y))(X) = C(X,Y). $$ 

This can be extended to a commutative, affine monad on  $\hat C_\mathrm{det}$ by using the monoidal and composition structure of $C$. 

Moreover, by the Yoneda lemma, 

$$ C(X,Y) \cong \hat C_\mathrm{det}(\mathbf{y}(X)\to P(\mathbf{y}(Y))) $$
...
At a similar time, Carsten Führmann had worked with [[premonoidal categories]], investigating copy-discard structure, and characterizing when they amount to Kleisli categories ([Fuhrmann99](#fuhrmann99), [Fuhrmann02](#fuhrmann02)). See also [[thunk-force category]]. 
...
Around a similar time, [[Anders Kock]] investigated the ideas of [[commutative monads]] as distributions ([Kock'12](#kock12)). 
...
* {#fuhrmann99} Carsten Fuhrmann, _Direct Models for the Computational Lambda Calculus_. MFPS 1999.

* {#fuhrmann02} Carsten Fuhrmann, _Varieties of Effects_. FoSSaCS 2002: 144-158.
...
* {#kock12} [[Anders Kock]], Commutative monads as theories of distributions. Theory and Applications of Categories, Vol 26, Number 4. 2012. ([article](https://www.emis.de/journals/TAC/volumes/26/4/26-04.pdf))
