...
A notion of [[precategory]] which uses the [[equivalence relation]] of [[symmetric prosets]] rather than the [[identity type]] of [[h-sets]]. 

## Definition

In [[intensional type theory]], a **(2,1)-preorder** or **locally symmetric prosetal prebicategory** $\mathcal{C}$ consists of 

* a [[type]] of [[objects]] $Ob(\mathcal{C})$, 
* for each object $A:Ob(\mathcal{C})$ and $B:Ob(\mathcal{C})$, a [[symmetric proset]] $(Hom(A, B), \sim_{Hom(A, B)})$ of [[morphisms]]
...
is an equivalence of types. In a [[univalent type theory]], every symmetric proset is a set and thus every (2,1)-preorder is a precategory. 
...
The type of all isomorphisms between $A$ and $B$ is represented by $A \cong_{Ob(\mathcal{C})} B$ and is also a symmetric proset. 
...
is an equivalence of symmetric prosets. 
