...
* for each object $A:Ob(\mathcal{C})$ and $B:Ob(\mathcal{C})$, a [[setoid]] $(Hom(A, B), \equiv_{Hom(A, B)})$ of [[morphisms]]
...
$$h \circ_{A, C, D} (g \circ_{A, B, C} f) \equiv_{Hom(A, D)} h \circ_{B, C, D} (g \circ_{A, B, D} f)$$
  * the composition of morphisms satisfies the left and right [[unit laws]]: for each object $A:Ob(\mathcal{C})$ and $B :Ob(\mathcal{C})$ and morphism $f:Hom(A, B)$, $\mathrm{id}_B \circ_{A, B, B} f \equiv_{Hom(A, B)} f$ and $f \circ_{A, A, B} \mathrm{id}_A \equiv_{Hom(A, B)} f$.
...
$$idtoequivrel(A, B, f, g):f =_{Hom(A, B)} g \to f \equiv_{Hom(A, B)} g$$ 
is an equivalence. In a [[univalent type theory]], every setoid is a set and thus every (2,1)-preorder is a precategory. 

An isomorphism in a (2,1)-preorder is a morphism $f:Hom(A, B)$ with a morphism $g:Hom(B, A)$ such that $g \circ f \equiv_{Hom(A, A)} id_A$ and $f \circ g \equiv_{Hom(B, B)} id_B$. The type of all isomorphisms between $A$ and $B$ is represented by $A \cong_{Ob(\mathcal{C})} B$ and is also a setoid. 
