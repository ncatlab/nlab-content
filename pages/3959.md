...
Let $\mathbb{R}$ be the real numbers. A function $f:\mathbb{R} \to \mathbb{R}$ is **smooth** if it comes with a sequence of functions $D^{(-)}(f):\mathbb{N} \to (\mathbb{R} \to \mathbb{R})$ called the **iterated derivative sequence** and a sequence of functions $M^{(-)}(f):\mathbb{N} \to (\mathbb{R}_+ \to \mathbb{R}_+)$ in the positive elements of $\mathbb{R}$ called the **moduli sequence of iterated differentiability**, such that 

* $D^{0}(f) = f$

* for every natural number $n \in \mathbb{N}$, for every positive element $\epsilon \in \mathbb{R}_+$, for every element $h \in \mathbb{R}$ such that $0 \lt | h | \lt M^{n+1}(f)(\epsilon)$, and for every element $x \in \mathbb{R}$, 
$$|D^{n}(f)(x + h) - D^{n}(f)(x) - h D^{n + 1}(f)(x)| \lt M^{n}(f)(\epsilon)$$
