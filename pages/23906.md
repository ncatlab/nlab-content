+-- {: .rightHandSide}
+-- {: .toc .clickDown tabindex="0"}
###Context###
#### Algebra
+--{: .hide}
[[!include higher algebra - contents]]
=--
#### Type theory
+-- {: .hide}
[[!include type theory - contents]]
=--
#### Categorification
+-- {: .hide}
[[!include categorification - contents]]
=--
#### Category theory
+-- {: .hide}
[[!include category theory - contents]]
=--
=--
=--

# Binary operations
* table of contents
{: toc}

## Idea

In type theory, a 1-truncated [[H-monoid]]/[[An-space|$A_3$-space]]. In category theory, a [[magmoidal groupoid with unit]] with an [[associator]] but no higher coherence laws. 

## Definitions

### In type theory

In [[intensional type theory]] with [[function extensionality]], a [[groupoid]] is a [[n-truncated|1-truncated]] [[type]] and a **H-monoidal groupoid** or **$A_3$-spatial groupoid** is a [[groupoid]] that is also an [[H-monoid]]/[[An-space|$A_3$-space]]. 

### In category theory

In [[category theory]], a **H-monoidal groupoid** or **$A_3$-spatial groupoid** is a [[magmoidal groupoid with unit]] $(G, \otimes, I)$ with natural unitary ismorphisms $\alpha_{A, B, C} \colon A \otimes (B \otimes C) \simeq^\dagger (A \otimes B) \otimes C$. 

## See also

* [[H-space]]
* [[An-space]]
* [[groupoid]]
* [[H-spatial groupoid]]
* [[A4-spatial groupoid]]

[[!redirects H-monoidal groupoid]]
[[!redirects H-monoidal groupoids]]
[[!redirects A3-spatial groupoid]]
[[!redirects A3-spatial groupoids]]