...
##Historical idea  (Čech 1930s)##
Given a (compact) space $X$ and a finite open cover, $\alpha$, of $X$, we can
form a [[simplicial set]], $C(X,\alpha)$, whose $n$-simplices are $(n + 1)$-strings
...
so one often talks about Aleksandrov-Čech (co)homology. 
...
theory.  In the first, the 'space' is resolved using 'coverings'
and these, in a sheaf theoretic setting, lead to simplicial objects in $Sh(X)$
that are weakly equivalent to $[0]$; in the second, to evaluate the derived
functor of some functor $F : \mathbf{C} \to \mathbf{A}$, say, on an object $C$,
