A [[coverage]], [[Grothendieck topology]], or [[Grothendieck pretopology]] (all of which are different ways of presenting a [[site]]) is said to be **subcanonical** if all [[representable functor|representable functors]] are [[sheaf|sheaves]].  Of course, a **subcanonical site** is one whose coverage is subcanonical.
...
Note also that if $f:V\to U$ is a single morphism having a [[kernel pair]] $p,q:V\times_U V \;\rightrightarrows\; V$, then the sieve generated by $f$ is effective-epimorphic iff $f$ is the [[coequalizer]] of its kernel pair, and thus iff $f$ is a [[regular epimorphism]].
