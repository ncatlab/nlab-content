...
For $U \in \mathbf{H}_{th}$ a test object (say an object in a [[(∞,1)-site]] of definition, under the [[Yoneda embedding]]) a formall &eacute;tale morphsim $U \to X$ is like an [[open map]]/open embedding. Regarded as an object in $(\mathbf{H}_{th})_{/X}^{fet}$ we may consider the sections over $U$ of the cotangent bundle as defined above, which in $\mathbf{H}_{th}$ are diagrams
