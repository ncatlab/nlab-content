...
Then the global [[BV-differential]] (def. \ref{ComplexBVBRSTGlobal}) restricts from [[polynomial observables]] to a linear map on [[microcausal polynomial observables]]
...
and as such is a [[derivation]] not only for the pointwise product, but also for the product in the [[Wick algebra]]:
...
Now by prop. \ref{RetainsOrShrinksWaveFrontSetDifferentialOperator} this only retains or shrinks the [[wave front set]] of the distributional coefficient, hence it preserve the microcausality condition (def. \ref{MicrocausalObservable}).
