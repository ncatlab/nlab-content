+-- {: .rightHandSide}
+-- {: .toc .clickDown tabindex="0"}
###Context###
#### Topos Theory
+--{: .hide}
[[!include topos theory - contents]]
=--
#### Category Theory
+--{: .hide}
[[!include category theory - contents]]
=--
=--
=--

* **topos theory**

* [[2-topos|2-topos theory]]

* [[(∞,1)-topos|(∞,1)-topos theory]]

* [[higher topos theory]]

***

#Contents#
* table of contents
{:toc}

## Idea

_Topos theory_ is the part of [[category theory]] that studies [[categories]] which are [[topos]]es. This includes in particular [[Grothendieck topos]]es, i.e. [[categories of sheaves]].

There are always two ways to think of topos theory: as being

* about [[logic]]

* about [[higher geometry|geometry]].

## References


### Introductions

A gentle basic introduction is 

* [[John Baez]], [Topos Theory in a Nutshell](http://math.ucr.edu/home/baez/topos.html).

A quick introduction of the basic facts of [[Grothendieck topos]] theory is chapter I, "Background in topos theory" in 

* [[Ieke Moerdijk]], _Classifying Spaces and Classifying Topoi_ Lecture Notes in Mathematics 1616, Springer (1995)


A survey is in

* [[Ross Street]], _A survey of topos theory_ (notes for students, 1978) [pdf](http://www.math.mq.edu.au/~street/ToposSurvey.pdf)











### Textbooks

A standard textbook is

* [[Peter Johnstone]], _Topos theory_, London Math. Soc. Monographs __10__, Acad. Press 1977, xxiii+367 pp.

This later grew into the more detailed

* [[Peter Johnstone]], [[Elephant|Sketches of an elephant: a topos theory compendium]]


Textbooks on category/topos theory with hyperlinked keyword entries on the $n$Lab are

* Kashiwara, Shapira, _[[Categories and Sheaves]]_ 

  category and sheaf theory for geometrical applications (no logic, no general topos theory)

* [[Saunders MacLane]], [[Ieke Moerdijk]], _[[Sheaves in Geometry and Logic]]_

  (category and sheaf theory with an emphasis on logical and topos-theoretical applications)

* [[Peter Johnstone]] _[[Elephant]]_ 

  (general topos theory)

* R. Goldblatt, _Topoi. The categorial analysis of logic_, Studies in Logic and the Foundations of Math. __98__, North-Holland Publ. Co., Amsterdam, 1979, 1984; (Rus. transl. Mir Publ., Moscow 1983).


### Course notes

* [[Ieke Moerdijk]], [[Jaap van Oosten]], _Topos theory_ Master Class notes (2007) ([pdf](http://www.staff.science.uu.nl/~ooste110/syllabi/toposmoeder.pdf))

[[!redirects Sheaf and Topos Theory]]
[[!redirects sheaf theory]]
[[!redirects topos theory]]