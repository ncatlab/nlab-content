...
For a non-affirmative example, in the arithmetic of (located) [[real numbers]], it is not constructively valid to derive $(a = 0) \vee (b = 0)$ from $a b = 0$, and it\'s not even valid to derive $\neg\big(\neg(a = 0) \wedge \neg(b = 0)\big)$ without [[Markov's principle]] (or at least some weak version of it), but it *is* valid to derive $(a \# 0) \rightarrow (b = 0)$ (and conversely), where $\#$ is the usual [[apartness relation]] between real numbers.  (Here, $P^+$ is $a = 0$, $P^-$ is $a \# 0$, $Q^+$ is $b = 0$, and similarly for $Q$ and $b$.)  Of course, it\'s also valid to derive $\neg\big((a \# 0) \wedge (b \# 0)\big)$ (which is actually equivalent).
