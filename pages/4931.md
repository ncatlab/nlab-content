...
+-- {: .remark}
###### Proof
Suppose $T$ is a star-shaped open subset of ${\bf R}^n$ centered at the origin.
Theorem 2.29 in Lee proves that there is a function $f$ on ${\bf R}^n$ such that $f>0$ on $T$ and $f$ vanishes on the complement of $T$.
...
Multiply $V$ by a smooth bump function $0\le b\le1$ such that $b=1$ for $\|x\|>1/2$ and $b=0$ in a neighborhood of 0.
...
Observe that for $1/2<\|x\|<2$ the vector field $V$ equals $x\mapsto x/\|x\|$.
Also, all flow lines of $V$ are radial rays.

Now define the flow map $p\colon{\bf R}^n_{>1/2}\to T_{>1/2}$ as $x\mapsto F(\|x\|-1,x/\|x\|)$ for $\|x\|>1/2$.
(The subscript $>1/2$ removes the closed ball of radius $1/2$.)
The flow map is the composition of two diffeomorphisms: ${\bf R}^n_{>1/2}\to(-1/2,\infty)\times S^n\to T_{>1/2}$,
hence itself is a diffeomorphism.
(The latter map is surjective by elementary properties of flow lines.)

Finally, define the desired diffeomorphism $d\colon{\bf R}^n\to T$ as the gluing of the identity map for $\|x\|<2$ and as $p$ for $\|x\|>1/2$.
...
* St&eacute;phane Gonnord, Nicolas Tosel, _Calcul Différentiel_ , ellipses (1998)
