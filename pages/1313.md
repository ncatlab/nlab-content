...
Equivalently, this condition can be repackaged as $\hom_C(-, X^Y) \cong \hom_C(- \times Y, X)$ (where $\mathrm{ev}$ is the image of the identity arrow on $X^Y$), so that exponential objects are representations of the latter as a [[representable functor]].
