...
Suppose $f: X\to Y$ is a quasi-compact and quasi-separated map of schemes, then the $\Delta$-functor $\mathbf{R}f_*: D_{qc}(X)\to D(Y)$ has a bounded below right $\Delta$-adjoint. In other words, $\mathbf{R}Hom_X(\mathcal{F}, f^\times \mathcal{G})\stackrel{\sim}{\to} \mathbf{R}Hom_Y(\mathbf{R}f_*\mathcal{F}, \mathcal{G})$ is a functorial isomorphism.
