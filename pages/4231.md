...
* For any small category $A$, the homotopical datum (Definition \ref{DefinitionHomotopicalDatum}) given by the $(J, \emptyset)$, where $J$ is the Lawvere cylinder (Definition \ref{LawvereCylinder}), generates a Cisinki model structure on the category of presheaves on $A$. By  ([Cisinski 06, rem. 1.3.15](#Cisinski06)), a set of generating acyclic cofibrations is $A\times \Omega \cup_A B \to B \times \Omega$ where $A \to B$ are generating cofibrations. This is sometimes known as the [[minimal Cisinski model structure]]. See also Theorem \ref{TheoremCharacterisationOfAccessibleLocalisersForPresheafCategories}.
