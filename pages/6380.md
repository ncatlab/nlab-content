...
Accordingly, for each [[Artin algebra]] regarded as an [[infinitesimally thickened point]] $S \in ArtAlg_k^{op}$ the [[cohomology group]] $H_{et}^n(X,\mathbb{G}_m)$ is that of [[equivalence classes]] of $\mathbb{G}_n$-[[principal infinity-bundle|principal n-bundles]] on a formal thickening of $X$.

The defining inclusion $\ast \to S$ of the unique global point induces a restriction map $H^n_{et}(X\otimes S, \mathbb{G}_m)\to H^n_{et}(X, \mathbb{G}_m))$ which restricts an $n$-bundle on the formal thickening to just $X$ itself. The [[kernel]] of this map hence may be thought of as the group of $S$-parameterized infinitesimal [[deformations]] of the trivial $\mathbb{G}_m$-$n$-bundle on $X$. 
...
$$
  \Phi_X^n(S) \coloneqq \mathrm{ker}(H^n_{et}(X\otimes S, \mathbb{G}_m)\to H^n_{et}(X, \mathbb{G}_m))
  \,.
$$ 
