...
* Tabulations: for every object $A \in Ob(C)$ and $B \in Ob(C)$ and morphism $R \in Hom(A,B)$, there is an object $\vert R \vert \in Ob(C)$ and [[map in a dagger 2-poset|maps]] $f \in Hom(\vert R \vert, A)$, $g \in Hom(\vert R \vert, B)$, such that $R = f^\dagger \circ g$ and for every object $E \in Ob(C)$ and maps $h \in Hom(E,\vert R \vert)$ and $k \in Hom(E,\vert R \vert)$, $f \circ h = f \circ k$ and $g \circ h = g \circ k$ imply $h = k$. 
