...
* for any object $A$ in $C$ and global elements $0_A:* \to A$ and $1_A:* \to A$ with an equivalence $s_A:0_A \cong 1_A$, there exists a morphism $f:I \to A$ with a morphism $f^{'}:(0 \cong 1) \to (0_A \cong 1_A)$ such that $f \circ 0 = 0_A$, $f \circ 1 = 1_A$, and $f^{'}(s) = s_A$. 
=--
