...
A *cd-structure* on an [[infinity-category]] $C$ with an [[initial object]] is a class of commutative squares which is stable by isomorphism.  It is called **complete** if every [[morphism]] whose target is an [[initial object]] of $C$ is an isomorphism, and if it is stable by [[base change]] along arbitrary morphisms of $C$.
