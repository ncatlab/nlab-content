...
$$
  \array{
    ...
    \big(
      PSh_\infty(\ast)
    \big)_{_{/\mathbf{B}\mathcal{G}}}
    &
    \underoverset{\sim}{\Gamma}{\longrightarrow}
    &
    PSh_\infty(
...
$$
This is the identification of the slice over $B \mathcal{G}$ with  $\mathcal{G}-$[[infinity-action|$\infty$-actions]]. For a [[model category]]-presentation see at *[Borel model structure -- Relation to the slice over the simplicial classifying space](Borel+model+structure#RelationToSliceOverSimplicialClassifyingSpace)*.
\end{example}
