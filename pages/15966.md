...
* coherent $\mathcal{O}^t$-modules, where $\mathcal{O}^t$ is the sheaf for the $G$-topology given on a rational domain $R$ with Berkovich interior $\circ{R}$ by the subsheaf of $\mathcal{O}_{\circ{R}}$ given by functions $f$ with polynomial growth, i.e., such that for every point $x\in \partial R$, there exists a Berkovich neighborhood of $x$ and a rational function without poles $P$ on the intersection of this neighborhood with $\circ{R}$  such that $|P(x)|=0$ and $Pf\in\mathcal{O}^\dagger_R$.
...
* the stack $X\mapsto \mathcal{D}_{perf}(Ran(X))$ of perfect $\mathcal{D}$-modules on the Ran space of $X$ equipped with the local tensor product $\otimes^*$ (see Francis-Gaitsgory, Chiral Koszul duality) and dual given by the $\mathcal{D}$-dual.
