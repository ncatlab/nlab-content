...
We now proceed to show that for every [[standard Borel space]] $(X, \sigma(\mathbb{F}))$ that $G(X)$ is an object in $\mathbf{Std}_{Cvx}$.  (If you are not familiar with the notation $\sigma(\mathbb{F})$  refer to the (alternative) definition of a [[standard Borel space]].)
