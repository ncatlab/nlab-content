...
$$ 
sending a probability measure $P$ to $P(U)$, where $U$ is a measurable set in $X$.
...
$$
 \mu_X (P)(U) := \int_{q \in P(X)} ev_U(q) (dP).
$$
This makes the [[endofunctor]] $G$ into a [[monad]], and this is the _Giry monad_  on Measurable spaces.
...
This makes the [[endofunctor]] $P$ into a [[monad]], and this is the _Giry monad_  on Polish spaces.  The shortcoming of this restriction to Polish spaces is that discrete spaces have no $P$-algebras, whereas $G$-algebras exist for discrete spaces. (Doberkat has noted, that such a $P$-algebra must be connected.)
