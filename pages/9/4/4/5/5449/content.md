...
* with a submodules $F_i \le C$ for $i$ an object of $I$
* such that $F_i \le F_{j} $ for $i\le j$ in $I$
* such that $ d F_i \le F_i $.

(You may have noticed this isn't the usual notation for functors.  It's traditional.)

The most frequent examples have $I=\mathbb{N}\cup\{\infty\}$, $-\mathbb{N}$, and $\mathbb{Z}\cup\{\infty\}$.
...
In the special case of a discrete totally-ordered filtration, one defines the associated graded complex $G_i(F) = F_{i+1}/F_i$ with differential induced by $d[x] = [d x]$.
