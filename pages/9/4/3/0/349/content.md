...
+-- {: .num_defn #DefinitionGrothendieckTopology}
###### Definition
...
+-- {: .num_defn #DefinitionPosetTopologies}
###### Definition

Given a category $C$ and Grothendieck topologies $J$ and $J'$ on $C$, we say that $J$ *is finer than* $J'$ (or that $J'$ *is coarser than* $J$), and we write $J\supseteq J'$, if $J(c)\supseteq J'(c)$ for each $c$ in $C$. 

=--

There is a coarsest and a finest topology on $C$, the *trivial* and *discrete* topologies: for these, the covering sieves on $c\in C$ equals $\{Ob(C/c)\}$ and all sieves on $c$, respectively.
...
Grothendieck topologies may be and in practice quite often are obtained as closures of collections of morphisms that are not yet closed under the operations above (that are not yet sieves, not yet pullback stable, etc.), see Definition \ref{DefinitionTopologyGeneratedByFamilies}.
...
It is clear that $\bigcap_{\alpha}J_\alpha$ satisfies the axioms of Definition \ref{DefinitionGrothendieckTopology}. The intersection topology is the finest topology on $C$ among those that are coarser than $J_\alpha$ for every $\alpha$. That is, it is the infimum of $\{J_\alpha\}$ in the collection of all topologies in $C$, ordered by inclusion (see Definition \ref{DefinitionPosetTopologies}). The collection $\{J_\alpha\}$ also has a supremum; namely, the intersection topology of all topologies in $C$ that are finer than $J_\alpha$ for every $\alpha$.

+-- {: .num_defn #DefinitionCoveringFamily}
###### Definition

([SGA 4, Exp. II, Définition 1.2](#SGA4), [Kashiwara, Schapira, Definition 16.1.5](#KashiwaraSchapira06).)
Let $C$ be a site, let $c$ be an object of $C$. A family of morphisms $\{f_i:d_i\to c\}$ with common target $c$ is said to *cover* $c$ or to be a *covering family* if the [[sieve#SieveGeneratedByAFamily|sieve it generates]] is a covering sieve.
...
Let $C$ be a category. Suppose for each $c\in C$ we are given a collection of families $\{f_i:d_i\to c\}$ of morphisms into $c$. As defined in [SGA 4, Exp. II, 1.1.6](#SGA4), the topology on $C$ generated by this collection is the coarsest topology on $C$ (Definition \ref{DefinitionPosetTopologies}) for which the families $\{f_i:d_i\to c\}$ are covering families (Definition \ref{DefinitionCoveringFamily}).
...
* {#KashiwaraSchapira06} [[Masaki Kashiwara]], [[Pierre Schapira]], _[[Categories and Sheaves]]_, Springer
