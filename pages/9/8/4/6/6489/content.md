...
Let $\mathcal{A}$ be an [[abelian category]]. A **stability function**, sometimes also called a central charge, is a group [[homomorphism]] $Z: K(\mathcal{A})\to \mathbb{C}$ such that for all non-zero objects, the image of $Z$ lies in the semi-upper half plane $H=\{r exp(i\pi \phi) : r$ > $0, 0$ &lt; $\phi \leq 1\}$. The phase of an object is just the $\phi$ that occurs in the representation from $H$. Alternatively, by plotting $Z(E)$ in the complex plane the phase is the argument (slope) divided by $\pi$. The phase of $E$ will be denoted $\phi(E)$.
