...
8. Every finitely presented left $A$-module is [[projective module|projective]].

9. Every finitely presented right $A$-module is projective. 

10. Every left $A$-module is [[flat module|flat]].

11.  Every right $A$-module is [[flat module|flat]].
...
\begin{proof} The following proof is adapted from Kaplansky [KapFaR]. It uses a result called McCoy's lemma:  
If $a x a - a$ is regular, then so is $a$. (Proof: if $(a x a - a)y(a x a - a) = a$, then rewrite this as $a = a x a - (a x a - a)y(a x a - a)$, and notice the right side is of the form $a [\ast] a$.) 
...
where $U, V, W, X \in M_2(A)$, which was just shown to be von Neumann regular; therefore $M_4(A) \cong M_2(M_2(A))$ is also von Neumann regular. By induction, $M_n(A)$ is von Neumann regular for integers $n = 2^k$. 
