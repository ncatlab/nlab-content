...
When using the tool, one usually intends to formalize a proof in an $n$-category, for some fixed value of $n$. In that case, one would typically define signature cells up to dimension $n+1$, with the highest-dimensional cells being interpreted as \emph{equations} between $n$-dimensional diagrams. One would also typically mark them as invertible, as one usually desires equality to be a symmetric relation. However, note that these are just ordinary cells, with no special properties: equality is implemented by rewriting.
