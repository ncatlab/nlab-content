...
Note that to maintain consistency of indices across both sides of this equation, we do not sum over $i$ in the first term.  If the perturbations are "soft", satisfying $|\epsilon_i| \ll 1$, then the coefficients $C_{ijk}$ form the structure constants of a [[CFT]] operator algebra.  Raising the indices on $G_{ij}$ by $G^{ik}G_{kj} = \delta^i_{\,j}$, we can write
