...
A **progroup** is a [[pro-object]] in the category of [[groups]].  In other words, it is a formal [[cofiltered limit]] of groups.
...
1. $G$ is a cofiltered limit of discrete groups (considered as discrete localic groups)
1. $G$ is a cofiltered limit of discrete groups with surjective transition maps (a *surjective progroup*).
1. The open normal subgroups of $G$ form a neighborhood base at the identity $e\in G$.
=--
+--{: .proof}
###### Proof
This can be found in the paper of Moerdijk cited below.
=--
...
+--{: .proof}
###### Proof
It suffices to show that for surjective progroups $(G_i)$ and $(H_j)$, with prodiscrete localic limits $G$ and $H$, we have
...
$$ LocGrp \to Topos $$
is not an embedding, but it can be shown to be so when restricted to prodiscrete localic groups.  One can also characterize the toposes that are sheaves on a prodiscrete localic group as the [[Galois toposes]].
