...
Let $P$ be a poset. We say that another poset $Q$ has the same **order type** as $P$ iff there exists an order preserving bijection $P\cong Q$. We define an equivalence relation $\sim$ on posets by 

\begin{centre}
$P\sim Q\iff \exists f:P\cong Q(\text{f is an order-preserving bijection})$,
\end{centre}
