...
More generally, given a type $A$ and a type family $x:A \vdash B(x)$, a type $C$ is **purely essentially $(A, B)$-small** if one can construct an element $z:A$ and an equivalence $p:C \simeq B(z)$. 

A [[type]] $C$ is **merely essentially $(A, B)$-small** if [[existential quantifier|there merely exists]] an element $z:A$ such that $C \simeq B(z)$: 
