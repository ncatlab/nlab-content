...
_Toby_:  Yeah, you could do that; even the function from $\{\bot, \top\}$ to itself that maps $\bot \mapsto \top$ and $\top \mapsto \bot$ exists.  But the fact that you ask this makes it more clear to me that your Boolean domain is an *unstructured* $2$-element set, where mine is *structured* (pointed or ordered), so it would be proper to move the second bracketed item to [[boolean field]] or something.  If you like, I\'ll do that and also try to rewrite the other bracketed items in a way that fits better.
...
JA: As for the third item, I personally like that usage, as it connects with item of catechism that says "a random variable is a function on a sample space $X$", and the connection between "square" probability densities $p : X \to \{ 0, 1 \} = \mathbb{B}$ and "curvy" probability densities $q : X \to [0, 1] \subseteq \mathbb{R}$ is one of the things that is motivating this approach.  So it was only expository simplicity that was constraining me here.
