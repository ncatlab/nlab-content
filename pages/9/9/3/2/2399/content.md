...
> **raw material**: notes taken verbatim in some seminar -- needs to be polished and/or reworked. 

we will follow the book

* Manin _Frobenius manifolds, quantum cohomology and moduli spaces_
...
3. use [[coarse moduli space]]s

  $M \in Sch/\mathbb{C}$ with $\Psi_M : F \to h_M$ such that 

a) $F(Spec(\mathbb{C})) \to h_M(Spec \mathbb{C}) = hom(Spec \mathbb{C}, M)$
    
    is a [[bijection]]
 
b) given $M'$ and $\Psi_{M'} : F \to h_{M'}$

    then there exists unique $M \to M'$ such that

    $$
      \array{
         F && \stackrel{\Psi_{M'}}{\to}&& h_{M'}
         \\
         & {}_{\Psi_M}\searrow && \nearrow
         \\
         && h_M
...
**exercise** show that the $j$-line is a [[coarse moduli space]]
...
[A seminar](http://golem.ph.utexas.edu/category/2009/09/a_seminar_on_gromovwitten_theo.html)

Some notes are in folder AG/Various/Gromov-Witten inv

nLab page on [[nlab:Gromov-Witten invariants]]
