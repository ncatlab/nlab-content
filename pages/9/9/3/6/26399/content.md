...
The _convex powerset of distributions monad_ is a monad (or family of monads) that encompasses both non-determinism (via [[power set]]) and probability (via [[probability distributions]]). 
...
is the topological $n$-[[simplex]]. A subset $S\subseteq \Delta(n)$ is _[[convex set|convex]]_ if it is closed under convex combinations, i.e. if $\vec r\in S$ and $\vec s\in S$ then $(a\vec r+(1-a) \vec s)\in S$ for any $a\in[0,1]$. 
...
Convex powersets of distributions have been considered by various authors over the last few decades. A recent survey is given in:

* Matteo Mio and Valeria Vignudelli. Monads and Quantitative Equational Theories
for Nondeterminism and Probability. Concur 2020. [Preprint at HAL](https://hal.science/hal-03028173/document). 