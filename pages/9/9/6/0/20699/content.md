
+-- {: .rightHandSide}
+-- {: .toc .clickDown tabindex="0"}
###Context###
#### Rational homotopy theory
+--{: .hide}
[[!include differential graded objects - contents]]
=--
=--
=--

#Contents#
* table of contents
{:toc}

## Idea

A [[Sullivan model]] for the ([[reduced suspension|reduced]]) [[suspension]] of a ([[pointed topological space|pointed]]) [[topological space]] $X$ is obtained from a [[Sullivan model]] for $X$ itself essentially by shifting up all generators in degree by one and forcing all their wedge products to vanish.

## Details

See e.g. [Félix-Murillo-Tanré 10, Prop. 5.1](#FelixMurilloTanre10)

## Related concepts


* [[suspension object]]

  * [[suspension]], [[reduced suspension]]

    * [[Freudenthal suspension theorem]]

    * [[Spanier-Whitehead category]]

    * [[suspension isomorphism]]

  * [[suspension of a chain complex]]

[[!include Sullivan models -- examples]]



## References

* {#FelixMurilloTanre10} [[Yves Félix]], Aniceto Murillo Daniel Tanré, _Fibrewise stable rational homotopy_, Journal of Topology, Volume 3, Issue 4, 2010, Pages 743–758 ([doi:10.1112/jtopol/jtq023](https://doi.org/10.1112/jtopol/jtq023))


[[!redirects Sullivan models for suspensions]]