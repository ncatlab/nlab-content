...
Let $\mathbb{N}^*$ denote the set of [[list|lists]] of [[natural numbers]]. A [[bar]] is a subset $P \subseteq \mathbb{N}^*$ such that for all sequences $\alpha$ of natural numbers, there exists a natural number $n$ such that the list of all $\alpha(i)$ for all $i \lt n$ is in $P$. A bar $P$ is *inductive* if for all predicates $Q$ such that forall $a$, $P(a) \Rightarrow Q(a)$, and if forall lists $a \in \mathbb{N}^*$ and all natural numbers $n \in \mathbb{N}$, if the concatenation of $a$ with $n$ is in $Q$, then $a$ is in $Q$. Finally, a bar $P$ is *monotone* if for all lists $a, b \in \mathbb{N}^*$, if $a$ is in $P$, then the concatenation of $a$ and $b$ is in $P$. 
