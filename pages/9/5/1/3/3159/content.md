...
(where $\nabla$ is the [[codiagonal]] map); by the [[Yoneda lemma]], the composite map $R \to R$ is of the form $\lambda_r$, where $r \in R$ is the value of the composite at $1 \in R$. Since $\lambda_r$ is surjective, we have $\lambda_r(s) = r s = 1$ for some $s$, so that $r$ is invertible. Hence $\lambda_r$ is invertible, and this implies $\lambda_x$ is monic. Therefore $R$ is a domain. Furthermore, if $f$ and $g$ belong to a basis of $(x)$, then we have 

$$0 \neq f g \in R\cdot f \cap R \cdot g$$ 

whence $f$ and $g$ are not linearly independent, so $f = g$ and $(x)$ as an $R$-module is generated by a single element, i.e., $R$ is a principal ideal domain. 
