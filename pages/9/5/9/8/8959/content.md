...
Moreover, as mentioned above, the meaning explanation (like any particular semantics) validates many other "rules" which no one would consider adding to a type theory, such as
$$ x : Id_N(0,1)  \vdash  5 : N \to N $$
Thus, some discrimination is required, as well as some choices (e.g. whether to add the extensionality rule for identity types).
