...
[[modal logic|Modal logics]] also have an algebraic semantics based on a Boolean algebra, but with additional operators that model the [[modal operators]]. This is as well as the [[geometric models for modal logics|geometric semantics]] using [[frames (modal logic)|frames]].
...
Let $\mathfrak{F} = (W ,R)$ be a [[frame (modal logic)|frame]]. We define on the power set Boolean algebra, $\mathbb{P}(W)$, the operator $m$ by, if $T\subseteq W$, 
...
[[!redirects algebraic model for modal logic]]
[[!redirects algebraic model for modal logics]]
[[!redirects algebraic models for modal logic]]
