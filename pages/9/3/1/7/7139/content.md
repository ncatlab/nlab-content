...
such that $M(i, i) = 1$ for all $i \in I$, else $M(i, j) > 1$. Writing $m_{i j} = M(i, j)$, the associated **Coxeter group** $W_M$ is the group presented as having generators $s_i$, $i \in I$, and relations 

$$(s_i s_j)^{m_{i j}} = 1$$ 

for all $i, j \in I$, whenever $m_{i j} \neq \infty$. In other words, $m_{i j}$ is the order of $s_i s_j$, and these orders determine the group. 
...
Let $V$ be a finite dimensional Euclidean space $\mathbb{R}^n$, and suppose $G$ is a finite group generated by linear isometries of $V$ (preserving the origin and the standard Euclidean metric). Then $G$ is a Coxeter group. Such finite Coxeter groups are also known as **spherical Coxeter groups** (being subgroups of isometry groups of Euclidean spheres). 
...
Irreducible essential finite reflection groups fall into four infinite families $A_n$, $B_n = C_n$, $D_n$, and dihedral groups $I_2(m)$, together with a small number of exceptional groups: 
...
The group $H_3$ is the isometry group of the regular dodecahedron or of the regular icosahedron in $\mathbb{R}^3$, and is abstractly isomorphic to the alternating group on 5 letters, having 60 elements. Its Coxeter diagram is: 

The group $H_4$ is the isometry group of a regular polyhedron in $\mathbb{R}^4$ known as the 120-cell, or of the dual configuration known as the 600-cell. Its Coxeter diagram is: 
