...
If $B$ is a bimonoid in $\mathcal{C}$, then the category $Mod_B$ of $B$-[[modules]] inherits a [[monoidal category|monoidal structure]] such that the forgetful functor $Mod_B \to \mathcal{C}$ is a [[strong monoidal functor]].  For $B$-modules $M$ and $N$, we equip the tensor product $M\otimes N$ in $\mathcal{C}$ with the $B$-action given by
$$
...
If $B$ moreover a Hopf monoid, then $Mod_B$ also inherits a [[closed monoidal category|closed]] monoidal structure if $\mathcal{C}$ has one; see [[Hopf monoid]].
