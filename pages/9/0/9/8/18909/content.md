...
Bishop did not formalize his foundations, and in particular did not specify rules for how "constructions" and "methods" should behave.  However, many people believe that a fairly faithful reading is obtained by viewing "methods" as the [[terms]] in a [[dependent type theory]], whose [[types]] are classifiers for families of "constructions".  (Indeed, making this work was one of [[Per Martin-Lof]]'s original motivations for defining dependent type theory.)  Bishop's *logic* is then the *untruncated* [[propositions as types]] logic of this type theory, with for instance "or" interpreted as the [[sum type]] and "there exists" interpreted as the [[dependent sum type]]. Bishop: "choice is implied by the very meaning of existence".  (Another possible formalization of Bishop's mathematics uses Feferman's [[explicit mathematics]].  Alternatively, one can try to isolate the principles implied by Bishop's definitional set theory to formulate an axiomatic set theory in which Bishop's work can be formalized --- one common proposal is [[CZF]] with the [[presentation axiom]] --- but this is getting further from what Bishop actually did.)
