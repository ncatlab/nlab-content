...
Actually, the weak K&#246;nig lemma is a stronger statement which removes the word "nonempty" from the previous version. But the stronger statement follows a similar line of argument: $s(0)$ is again the root, and given $s(n) \in F(n)$, let $s(n+1)$ be the least $x$ in $E$ such that $F(n+1 \to n)(x) = s(n)$ and such that the set of descendants of $x$ is infinite. (There is an inductive argument here: the root $s(0)$ must have infinitely many descendants since the tree is infinite, and if $s(n)$ has infinitely many descendants, then so does at least one of its children.) 
