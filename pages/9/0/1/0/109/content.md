...
The corresponding action groupoid is
...
The Lie algebra omn the sections restricts over 
...
$$
  \mathrm{CE}(g)
  =
  ( \wedge^\bullet_{C^\infty(X) \langle \theta\rangle}, d_\gg)
  =
  ( C^\infty(X) \oplus C^\infty(X)\otimes \theta, d_\gg)
  \,.
$$
...
$d_\gg$ is the BRST operator 

$\mathrm{CE}(\gg)$ is the BRST complex

Next there is the corresponding Weil algebra,
equivalently 
the algebra of differential forms on our Lie algebroid,
equivalently the algebra of functions on the shifted
...
$$
  d_{\mathrm{W}(\gg)} f = v(f)\theta + d_{\mathrm{dR}}f 
$$
$$
  d_{\mathrm{W}(\gg)} d f = 0
$$
for all $f \in C^\infty(X)$ and
$$
  d_{\mathrm{W}(\gg)} \theta = d\theta
$$
$$
  d_{\mathrm{W}(\gg)} d\theta = 0
$$
...
$$
in $\mathrm{W}(\gg)$ (with $x$ the canonical coordinate function
pulled back from $\mathbb{R}$). This, however, is not
a closed form in $\mathrm{W}(\gg)$, as we have
$$
  d_\gg (dx \wedge \theta) = - dx \wedge d\theta
  \neq 0
  \,.
$$
...
ovcer suitable sub-supermanifolds.

We want to recover from all this machinery the ordinary integral
of a 1-form 
$$
  \exp(S_0) := f dx
$$
on the quotient $\mathbb{R}$. This can be done by
...
$$
  \exp(S_0) \in C^\infty(X) \subset \mathrm{CE}(\gg)
  \,.
$$
