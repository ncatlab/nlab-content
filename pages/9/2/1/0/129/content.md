I'll try to work out some stuff here. If anything becomes interesting in the slightest, I hope to add a new page to the nLab collection. Any editorial angels who wish to help add, correct, comment, etc, please feel free.

My ultimate goal is to follow what others at the nLab are doing and reformulate it in a way I can understand (which usually means that I need to "discretize" everything).

**Warning: Under Construction**

##Discrete Smooth Spaces

I hope to adapt the definition of a Frolicher space (from [Andrew Stacey's paper](http://arxiv.org/abs/0802.2225)) to discrete/abstract space.

***

###Definition 2.2 (Frolicher) 

A Frolicher space is a triple $(X,C,F)$ where $X$ is a set, $C$ is a family of curves in $X$, i.e. a subset of $Map(R,X)$, and $F$ is a
family of functionals on $X$, i.e. a subset of $Map(X,R)$. 


The sets $C$ and $F$ have to satisfy the following compatibility condition: a curve $c: R\to X$ is in $C$ if and only if $fc\in C^\infty(R,R)$ for all functionals $f\in F$, and similarly a functional $f: X\to R$ is in
$F$ if and only if $fc\in C^\infty(R,R)$ for all curves $c: R\to X$.

***

I guess the first place to start is to define what I mean by a discrete space.

***

###Definition (Discrete Space)

+--{.query}
Note: I just found that what I am trying to outline here is very closely related to this paper: [C* Algebras of Infinite Graphs](http://www.ams.org/proc/2000-128-08/S0002-9939-99-05378-2/S0002-9939-99-05378-2.pdf)
=--

+--{.query}
Note: The plot thickens. Reference tracking opened a floodgate. I'm trying to track down [Graphs, groupoids and Cuntz-Krieger algebras](http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.38.93), but in the meantime came across [Fundamental groupoids of k-graphs](http://www.emis.de/journals/NYJM/j/2004/10-12.pdf). A $k$-graph seems to be a lot like a diamond complex.
=--

+--{.query}
Note: This looks like a very interesting and readable thesis: [$C^*$-algebras associated to higher-rank graphs](http://www.newcastle.edu.au/service/library/adt/uploads/approved/adt-NNCU20060327.122628/public/02whole.pdf)
=--

Given a [[directed graph]] $G = (G_0,G_1,s,t)$, a discrete space is the [graded differential algebra](http://en.wikipedia.org/wiki/Differential_graded_algebra) (DGA) 

$$\Omega(G) = \bigoplus_{r\ge 0} \Omega^r(G)$$

generated by a complete set of orthogonal projections

$$\pi_v:G_0\to G_0\in\Omega^0(G)$$

and

$$\pi_e:G_1\to G_1\in\Omega^1(G).$$

Right multiplication is defined by

$$
\pi_e\pi_v = 
\begin{cases}
\pi_e, \text{if}\quad t(e) = v, \\
0, \text{otherwise.}
\end{cases}
$$

while left multiplication is defined by

$$
\pi_v\pi_e = 
\begin{cases}
\pi_e, \text{if}\quad s(e) = v, \\
0, \text{otherwise.}
\end{cases}
$$

In other words, $\pi_e \pi_v = 0$ unless $t(e) = v$ and $\pi_v\pi_e = 0$ unless $s(e) = v$.

####Illustration:####


$$\pi_{i,j}\pi_k = \delta_{j,k} \pi_{i,j},$$

$$\pi_i\pi_{j,k} = \delta_{i,j} \pi_{j,k}.$$

Denoting nodes $e_i\in G_0$ and directed edges $(e_i,e_j)\in G_1$ with $s(e_i,e_j) = e_i$ and $t(e_i,e_j) = e_j$, the projections satisfy

$$\pi_i(e_j) = \delta_{i,j} e_j$$

and

$$\pi_{i,j}(e_k,e_l) = \delta_{i,k} \delta_{j,l} (e_k,e_l).$$

Composition of projections of different degree may be written as

$$\pi_{i,j}\pi_k(e_l,e_m) = \pi_{i,j}(e_l,\pi_k(e_m))$$

and

$$\pi_i \pi_{j,k}(e_l,e_m) = \pi_{j,k}(\pi_i(e_l),e_m).$$


###Generating a Discrete Space

The degree 0 algebra $\Omega^0(G)$ of projections is commutative and satisfies the relations
\[\sum_i \pi_i = 1\]

and

\[\pi_i \pi_j = \delta_{i,j} \pi_i.\]

The differential

$$d: \Omega^0(G)\to \Omega^1(G)$$

applied to the individual projections results in,

\[\sum_i d\pi_i = 0\]

and

\[d(\pi_i \pi_j) = (d \pi_i) \pi_j + \pi_i (d \pi_j) = \delta_{i,j} d\pi_i,\]

which, in particular, provides the graded commutative relation

\[[\pi_i,d\pi_i] = d\pi_i.\]

The commutative relation helps us to order the 0-projections to the left of the 1-projections. For example, define

$$\pi_{i,j} := \pi_i (d\pi_j) \pi_j.$$

Commuting $\pi_j$ to the left results in

$$\begin{aligned}
\pi_{i,j} 
&= \pi_i (d\pi_j) \pi_j \\
&= \pi_i ([\pi_j,d\pi_j] - \pi_j d\pi_j) \\
&= (1-\delta_{i,j}) \pi_i d\pi_j
\end{aligned}$$

Blah blah...

Multiplying by $\pi_i$ on the left and $\pi_j$ on the right, we have

$$\pi_i (d\pi_j) \pi_j = (\delta_{i,j} - 1) \pi_i (d\pi_i) \pi_j$$


####Doodles

In our paper, we began the discussion in the framework of directed graphs. I wonder if we could present it in the context of the nerve of a category? Before I can do that, I probably need to learn what a nerve of a category is.

What if we begin with a discrete graph $G$ and....

***

###Research Papers

[Discrete differential geometry on causal graphs](http://arxiv.org/abs/math-ph/0407005), with
Urs Schreiber (2004).

category: people