...
A [[hypertoric variety]], which is constructed from a linear
    algebraic object called a polarized arrangement, is dual to the
...
The cotangent bundle $T^*(G/B)$ is dual to $T^*(G^\vee/B^\vee)$, where $G$ is a
    reductive algebraic group, $G^\vee$ is the Langlands dual of $G$,
    and $B\subset G$ and $B^\vee\subset G^\vee$ are Borel subgroups.

More generally, the S3-variety $\mathfrak{X}^\lambda_\mu$ associated to a
    pair of compositions $\lambda,\mu$ is 
    dual to the S3 variety $\mathfrak{X}^{\mu}_{\lambda}$. 
    One particularly interesting special case of this is when $\mu=0$,
    in which case we obtain the duality between $T^*SL_n/P_{\lambda}$ and
    the Slodowy slice to $O_{\lambda}$ in the full nilcone.  
