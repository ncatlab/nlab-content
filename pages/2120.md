...
A function $f:\mathbb{R} \to \mathbb{R}$ is continuous if the [[graph of a function|graph]] of $f$ defined by the [[function]] $r:\mathbb{R} \to \mathbb{R}^2$, $r(x) \coloneqq (x, f(x))$, has an [[image]] whose [[shape]] is [[contractible]]

$$\esh \mathrm{im}(r) \simeq \mathbb{1}$$
