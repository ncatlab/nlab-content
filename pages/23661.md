...
be the set of positive rational numbers. Let $(S, \sim)$ be a $\mathbb{Q}_{+}$-[[premetric space]]. A __Cauchy approximation__ is a function $x: \mathbb{Q}_{+} \to S$ such that for all positive rational numbers $\delta$ and $\eta$, $x(\delta) \sim_{\delta + \eta} x(\eta)$. 
