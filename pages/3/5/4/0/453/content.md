...
Any regular category $C$ admits a [[subcanonical site|subcanonical]] [[Grothendieck topology]] whose covering families are generated by single [[regular epimorphisms]].  If $C$ is [[exact category|exact]] or has pullback-stable [[reflexive coequalizers]], then its [[codomain fibration]] is a [[stack]] for this topology (the necessary and sufficient condition is that any pullback of a kernel pair is again a kernel pair).
