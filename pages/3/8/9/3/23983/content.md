...
The [[dependent function application to identifications]] is defined in the usual way, by induction on the [[identity type]] of the index type: given a type $A$, a type family $x:A \vdash B(x)$, and a [[dependent function]] $f:\prod_{x:A} B(x)$, there is a dependent function
