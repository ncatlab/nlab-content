...
$$a:A, b:A, p:\mathrm{Id}_A(a, b), y:B(a), z:B(b) \vdash \mathrm{hId}_{A, B}(a, b, p, y, z)$$

generated by the family of elements called "heterogeneous reflexivity"

$$a:A, y:B(a) \vdash \mathrm{hrefl}_{A, B}(a, y):\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)$$
...
$$\frac{
    ...
    \end{array}
  }{\Gamma \vdash \mathrm{hId}_{A, B}(a, b, p, y, z) \; \mathrm{type}}$$ 
...
$$\frac{
    ...
    \end{array}
  }{\Gamma \vdash \mathrm{hrefl}_{A, B}(a, y):\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)}$$ 
...
$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma, a:A, b:A, p:\mathrm{Id}_A(a, b), y:B(a), z:B(b), q:\mathrm{hId}_{A, B}(a, b, p, y, z) \vdash C(a, b, p, y, z, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y)) \\
      \Gamma \vdash a:A \quad \Gamma \vdash b:A \quad \Gamma \vdash p:\mathrm{Id}_A(a, b) \quad \Gamma \vdash y:B(a) \quad \Gamma \vdash z:B(b) \quad \Gamma \vdash q:\mathrm{hId}_{A, B}(a, b, p, y, z)
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{A, B}}(t, a, b, p, y, z, q):C(a, b, p, y, z, q)}$$

The elimination rule then says that if:

1. for any elements $a:A$ and $b:A$, any identification $p:\mathrm{Id}_A(a,b)$, any elements $y:B(a)$ and $z:B(b)$, and any heterogeneous identification $q:\mathrm{hId}_{A, B}(a, b, p, y, z)$ we have a type $C(a, b, p, y, z, q)$, and
1. we have a specified dependent function 
$$t:\prod_{a:A} \prod_{y:B(a)} C(a,a,\mathrm{refl}_A(x),y,y,\mathrm{hrefl}_{A, B}(a, y))$$

then we can construct a canonically defined term called the *eliminator*
$$\mathrm{ind}_{\mathrm{hId}_{A, B}}(t,a,b,p,y,z,q):C(a,b,p,y,z,q)$$ 
for *any* $a$, $b$, $p$, $y$, $z$, and $q$. 

Then, we have the [[computation rule]] or [[beta-reduction]] rule. This says that for all elements $a:A$ and $y:B(a)$, substituting the dependent function $t:\prod_{a:A} \prod_{y:B(a)} C(a,a,\mathrm{refl}_A(x),y,y,\mathrm{hrefl}_{A, B}(a, y))$ into the eliminator along heterogeneous [[reflexive relation|reflexivity]] for $a$ and $y$ yields an element equal to $t(a, y)$ itself.  Normally "equal" here means [[judgmental equality]] (a.k.a. definitional equality), but it is also possible for it to mean [[propositional equality]] (a.k.a. typal equality), so there are two possible computation rules.
...
$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma, a:A, b:A, p:\mathrm{Id}_A(a, b), y:B(a), z:B(b), q:\mathrm{hId}_{A, B}(a, b, p, y, z) \vdash C(a, b, p, y, z, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y)) \quad \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{A, B}}(t, a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y)) \equiv t(a, y):C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y))}$$
...
$$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y)) \quad \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \beta_{\mathrm{hId}_{A, B}}(t, a, y):\mathrm{Id}_{C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y))}(\mathrm{ind}_{\mathrm{hId}_{A, B}}(t, a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y), t(a, y))}$$
...
$$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \mathrm{up}_{\mathrm{hId}_{A, B}}(t, f, a, b, p):
\begin{array}{c}
\exists!J:\left(\prod_{a:A} \prod_{y:B(a)} C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y))\right) \\ 
\to \left(\prod_{c:A} \prod_{d:A} \prod_{r:\mathrm{Id}_A(c, d)} \prod_{e:B(c)} \prod_{f:B(c)} \prod_{s:\mathrm{hId}_{A, B}(c, d, r, e, f)} C(c, d, r, e, f, s)\right) \\
.\mathrm{Id}_{C(a, a, \mathrm{refl}_A(a), y, y)}(J(t, a, a, \mathrm{refl}_A, y, y, \mathrm{hrefl}_{A, B}(a, y)), t(a, y))
\end{array}
}$$

In natural language this states that given a type $A$, a type family $B(x)$ indexed by elements $x:A$, and given a type family $C(a, b, p, y, z, q)$ indexed by elements $a:A$, $b:A$, $p:\mathrm{Id}_A(a, b)$, $y:B(a)$, $z:B(b)$, and $q:\mathrm{hId}_{A, B}(a, b, p, y, z)$, for all elements $t:\prod_{a:A} \prod_{y:B(a)} C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y))$, $a:A$, and $y:B(a)$, there exists a unique function $J$ with domain 
$$\prod_{a:A} \prod_{y:B(a)} C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y))$$
and codomain 
$$\prod_{c:A} \prod_{d:A} \prod_{r:\mathrm{Id}_A(c, d)} \prod_{e:B(c)} \prod_{f:B(c)} \prod_{s:\mathrm{hId}_{A, B}(c, d, r, e, f)} C(c, d, r, e, f, s)$$
such that $J(t, a, a, \mathrm{refl}_A, y, y, \mathrm{hrefl}_{A, B}(a, y))$ is equal to $t(a, y)$ in the type $C(a, a, \mathrm{refl}_A(a), y, y, \mathrm{hrefl}_{A, B}(a, y))$. 
...
$$f:\mathbb{I} \to A, y:B(f(0)), z:B(f(1)) \vdash \mathrm{hId}_{A, B}(f, y, z)$$

generated by the family of elements called "heterogeneous reflexivity"

$$a:A, y:B(a) \vdash \mathrm{hrefl}_{A, B}(a, y):\mathrm{hId}_{A, B}(\lambda i:\mathbb{I}.a, y, y)$$
...
$$\frac{
    ...
    \end{array}
  }{\Gamma \vdash \mathrm{hId}_{A, B}(f, y, z) \; \mathrm{type}}$$ 
...
$$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \mathrm{hrefl}_{A, B}(a, y):\mathrm{hId}_{A, B}(\lambda i:\mathbb{I}.a, y, y)}$$ 
...
$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma, f:\mathbb{I} \to A, y:B(f(0)), z:B(f(1)), q:\mathrm{hId}_{A, B}(f, y, z) \vdash C(f, y, z, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(\lambda i:\mathbb{I}.a, y, y, \mathrm{hrefl}_{A, B}(a, y)) \\
      \Gamma \vdash f:\mathbb{I} \to A \quad \Gamma \vdash y:B(f(0)) \quad \Gamma \vdash z:B(f(1)) \quad \Gamma \vdash q:\mathrm{hId}_{A, B}(f, y, z)
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{A, B}}(t, f, y, z, q):C(f, y, z, q)}$$

The elimination rule then says that if:

1. for any function $f:\mathbb{I} \to A$, any elements $y:B(f(0))$ and $z:B(f(1))$, and any heterogeneous identification $q:\mathrm{hId}_{A, B}(f, y, z)$ we have a type $C(f, y, z, q)$, and
1. we have a specified dependent function 
$$t:\prod_{a:A} \prod_{y:B(a)} C(\lambda i:\mathbb{I}.a,y,y,\mathrm{hrefl}_{A, B}(a, y))$$

then we can construct a canonically defined term called the *eliminator*
$$\mathrm{ind}_{\mathrm{hId}_{A, B}}(t,f,y,z,q):C(a,b,p,y,z,q)$$ 
for *any* $f$, $y$, $z$, and $q$. 

Then, we have the [[computation rule]] or [[beta-reduction]] rule. This says that for all elements $a:A$ and $y:B(a)$, substituting the dependent function $t:\prod_{a:A} \prod_{y:B(a)} C(\lambda i:\mathbb{I}.a,y,y,\mathrm{hrefl}_{A, B}(a, y))$ into the eliminator along heterogeneous [[reflexive relation|reflexivity]] for $a$ and $y$ yields an element equal to $t(a, y)$ itself.  Normally "equal" here means [[judgmental equality]] (a.k.a. definitional equality), but it is also possible for it to mean [[propositional equality]] (a.k.a. typal equality), so there are two possible computation rules.
...
$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma, f:\mathbb{I} \to A, y:B(f(0)), z:B(f(1)), q:\mathrm{hId}_{A, B}(f, y, z) \vdash C(f, y, z, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(\lambda i:\mathbb{I}.a, y, y, \mathrm{hrefl}_{A, B}(a, y)) \quad \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{A, B}}(t, \lambda i:\mathbb{I}.a, y, y, \mathrm{hrefl}_{A, B}(a, y)) \equiv t(a, y):C(\lambda i:\mathbb{I}.a, y, y, \mathrm{hrefl}_{A, B}(a, y))}$$
...
$$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(\lambda i:\mathbb{I}.a, y, y, \mathrm{hrefl}_{A, B}(a, y)) \quad \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \beta_{\mathrm{hId}_{A, B}}(t, a, y):\mathrm{Id}_{C(\lambda i:\mathbb{I}.a, y, y, \mathrm{hrefl}_{A, B}(a, y))}(\mathrm{ind}_{\mathrm{hId}_{A, B}}(t, \lambda i:\mathbb{I}.a, y, y, \mathrm{hrefl}_{A, B}(a, y), t(a, y))}$$
...
Given [[two-type identity types]] in the type theory, indexed heterogeneous identity types between $y:B(a)$ and $z:B(b)$ across the path $p:a =_A b$ could be defined as the two-type identity type between $y:B(a)$ and $z:B(b)$ across the [[transport]] equivalence $\mathrm{transport}_{A, B}(a, b, p)$

$$\mathrm{hId}_{A, B}(a, b, p, y, z) \coloneqq \mathrm{Id}_{B(a), B(b)}(\mathrm{transport}_{A, B}(a, b, p), y, z)$$
...
$$(-) \bullet (-):\mathrm{hId}_{A, B}(a_1, a_2, p_1, y_1, y_2) \times \mathrm{hId}_{A, B}(a_2, a_3, p_2, y_2, y_3) \to \mathrm{hId}_{A, B}(a_1, a_3, p_1 \bullet p_2, y_1, y_3)$$
...
$$\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)$$
\end{definition}
...
$$\mathrm{hlunitor}:\prod_{q:\mathrm{hId}_{A, B}(a, b, p, y, z)} \mathrm{Id}_{\mathrm{hId}_{A, B}(a, b, p, y, z)}(\mathrm{hrefl}_A(a, y) \bullet q, q)$$

defined by induction on heterogeneous reflexivity for $a:A$ and $y:B(a)$:

$$\mathrm{hlunitor}(\mathrm{hrefl}_A(a, y)) \equiv \mathrm{refl}_{\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)}(\mathrm{hrefl}_A(a, y))$$

in the type 

$$\mathrm{Id}_{\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)}(\mathrm{hrefl}_A(a, y) \bullet \mathrm{hrefl}_A(a, y), \mathrm{hrefl}_A(a, y))$$
\end{definition}
...
$$\mathrm{hrunitor}:\prod_{q:\mathrm{hId}_{A, B}(a, b, p, y, z)} \mathrm{Id}_{\mathrm{hId}_{A, B}(a, b, p, y, z)}(q \bullet \mathrm{hrefl}_A(b, z), q)$$

defined by induction on heterogeneous reflexivity for $b:B$ and $z:B(b)$

$$\mathrm{hrunitor}(\mathrm{hrefl}_A(b, z)) \equiv \mathrm{refl}_{\mathrm{hId}_{A, B}(b, b, \mathrm{refl}_A(b), z, z)}(\mathrm{hrefl}_A(b, z))$$

in the type 

$$\mathrm{Id}_{\mathrm{hId}_{A, B}(b, b, \mathrm{refl}_A(b), z, z)}(\mathrm{hrefl}_A(b, z) \bullet \mathrm{hrefl}_A(b, z), \mathrm{hrefl}_A(b, z))$$
\end{definition}
...
$$\mathrm{hassoc}:\prod_{q_1:\mathrm{hId}_{A, B}(a_1, a_2, p_1, y_1, y_2)} \prod_{q_2:\mathrm{hId}_{A, B}(a_2, a_3, p_2, y_2, y_3)} \prod_{q_3:\mathrm{hId}_{A, B}(a_3, a_4, p_3, y_3, y_4)} \mathrm{Id}_{\mathrm{hId}_{A, B}(a_1, a_4, \mathrm{assoc}(p_1, p_2, p_3), y_1, y_4)}(q_1 \bullet (q_2 \bullet q_3), (q_1 \bullet q_2) \bullet q_3)$$

defined by triple induction on heterogeneous reflexivity for $a:A$ and $y:B(a)$

$$\mathrm{hassoc}(\mathrm{hrefl}_A(a, y),  \mathrm{hrefl}_A(a, y), \mathrm{hrefl}_A(a, y)) \equiv \mathrm{refl}_{\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)}(\mathrm{hrefl}_A(a, y))$$

in the type 

$$\mathrm{Id}_{\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)}(\mathrm{hrefl}_A(a, y) \bullet (\mathrm{hrefl}_A(a, y) \bullet \mathrm{hrefl}_A(a, y)), (\mathrm{hrefl}_A(a, y) \bullet \mathrm{hrefl}_A(a, y)) \bullet \mathrm{hrefl}_A(a, y))$$
\end{definition}
...
$$\mathrm{inv}(a_1, a_2, p_1, y_1, y_2):\mathrm{hId}_{A, B}(a_1, a_2, p_1, y_1, y_2) \to \mathrm{hId}_{A, B}(a_2, a_1, p_1^{-1}, y_2, y_1)$$

defined by induction on heterogeneous reflexivity:

$$a:A, y:B(a) \vdash \mathrm{inv}(a, a, \mathrm{refl}_A(a), y, y)(\mathrm{hrefl}_A(a, y)) \equiv \mathrm{hrefl}_A(a, y):\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y) \to \mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), y, y)$$
\end{definition}
...
$$\mathrm{hId}_{A, B}(a, b, p, y, z) \simeq \mathrm{Id}_{B(b)}(\mathrm{transport}_{A, B}(a, b, p)(y), z)$$

and 

$$\mathrm{hId}_{A, B}(a, b, p, y, z) \simeq \mathrm{Id}_{B(a)}(y, \mathrm{transport}_{A, B}(a, b, p)^{-1}(z))$$
...
* for all elements $z:B(b)$, there exists a unique element $y:B(a)$ such that $\mathrm{hId}_{A, B}(a, b, p, y, z)$; the witness is given by the right projection function of [[transport]] across $p$, $\mathrm{transport}_{A, B}(a, b, p):B(a) \simeq B(b)$

$$\pi_2(\mathrm{transport}_{A, B}(a, b, p)):\prod_{z:B(b)} \exists!y:B(a).\mathrm{hId}_{A, B}(a, b, p, y, z)$$

* for all elements $y:B(a)$, there exists a unique element $z:B(b)$ such that $\mathrm{hId}_{A, B}(a, b, p, y, z)$; the witness is given by the right projection function of the [[inverse]] [[transport]] across $p$, $\mathrm{transport}_{A, B}(a, b, p)^{-1}:B(b) \simeq B(a)$

$$\pi_2(\mathrm{transport}_{A, B}(a, b, p)^{-1}):\prod_{y:B(a)} \exists!z:B(b).\mathrm{hId}_{A, B}(a, b, p, y, z)$$
...
$$\mathrm{apd}_{A, B}(f):\prod_{a:A} \prod_{b:A} \prod_{p:\mathrm{Id}_A(a, b)} \mathrm{hId}_{A, B}(a, b, p, f(a), f(b))$$

inductively defined respectively by:

$$\mathrm{apd}_{A, B}(f, a, a, \mathrm{refl}_A(a)) \equiv \mathrm{hrefl}_{A, B}(a, a, \mathrm{refl}_A(a), f(a), f(a)):\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), f(a), f(a))$$
...
$$\mathrm{ext}_\Pi:\prod_{f:\prod_{x:A} B(x)} \prod_{g:\prod_{x:A} B(x)} \mathrm{Id}_{\prod_{x:A} B(x)}(f, g) \simeq \prod_{a:A} \prod_{b:A} \prod_{p:\mathrm{Id}_A(a, b)} \mathrm{hId}_{A, B}(a, b, p, f(a), g(b))$$
...
$$\mathrm{ext}_\Sigma:\prod_{s:\sum_{x:A} B(x)} \prod_{t:\sum_{x:A} B(x)} \mathrm{Id}_{\sum_{x:A} B(x)}(s, t) \simeq \sum_{p:\mathrm{Id}_A(\pi_1(s), \pi_1(t))} \mathrm{hId}_{A, B}(\pi_1(s), \pi_1(t), p, \pi_2(s), \pi_2(t))$$
...
* a indexed heterogeneous identification $c_\mathcal{l}:\mathrm{hId}_{S^1, C}(\mathcal{b}, \mathcal{b}, \mathcal{l}, c_\mathcal{b}, c_\mathcal{b})$, 
...
* Formation rules
  $$\frac{
    ...
    \end{array}
  }{\Gamma \vdash \mathrm{fhId}_{A, B}(f, a, b, p) \; \mathrm{type}}$$ 
...
* Introduction rules
  $$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash a:A
    \end{array}
  }{\Gamma \vdash \mathrm{hrefl}_{A, B}(f, a):\mathrm{fhId}_{A, B}(f, a, b, p)}$$ 
  
* Elimination rules
  $$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \quad \Gamma \vdash f:\prod_{x:A} B(x) \\
      \Gamma, a:A, b:A, p:\mathrm{Id}_A(a, b), q:\mathrm{fhId}_{A, B}(f, a, b, p) \vdash C(a, b, p, q) \; \mathrm{type} \\
      \Gamma \vdash \vdash t:\prod_{x:A} C(a, a, \mathrm{refl}_A(a), \mathrm{hrefl}_{A, B}(f, a)) \\
      \Gamma \vdash a:A \quad \Gamma \vdash b:A \quad \Gamma \vdash p:\mathrm{Id}_A(a, b) \quad \Gamma \vdash q:\mathrm{hId}_{A, B}(f, a, b, p)
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{fhId}_{A, B}}(t, f, a, b, p, q):C(a, b, p, q)}$$
...
* Judgmental computational rule
  $$\frac{
    \begin{array}{l}
      ...
      \Gamma, a:A, b:A, p:\mathrm{Id}_A(a, b), q:\mathrm{fhId}_{A, B}(f, a, b, p) \vdash C(a, b, p, q) \; \mathrm{type} \\
      \Gamma \vdash \vdash t:\prod_{x:A} C(a, a, \mathrm{refl}_A(a), \mathrm{hrefl}_{A, B}(f, a)) \\
      \Gamma \vdash a:A \quad \Gamma \vdash b:A \quad \Gamma \vdash p:a =_A b \quad \Gamma \vdash q:\mathrm{fhId}_{A, B}(f, a, b, p)
    \end{array}
  }{\Gamma, a:A \vdash \mathrm{ind}_{\mathrm{fhId}_{A, B}}(t, f, a, a, \mathrm{refl}_A(a), \mathrm{hrefl}_{A, B}(f, a)) \equiv t(a):C(a, a, \mathrm{refl}_A(a), \mathrm{hrefl}_{A, B}(f, a))}$$
...
* Propositional computational rule  
  $$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash a:A \quad \Gamma \vdash b:A \quad \Gamma \vdash p:a =_A b \quad \Gamma \vdash q:\mathrm{fhId}_{A, B}(f, a, b, p)
    \end{array}
  }{\Gamma, a:A \vdash \beta_{\mathrm{hId}_{A, B}}(t, f, a):\mathrm{Id}_{C(a, a, \mathrm{refl}_A(a), \mathrm{hrefl}_{A, B}(f, a))}(\mathrm{ind}_{\mathrm{fhId}_{A, B}}(t, f, a, a, \mathrm{refl}_A(a), \mathrm{hrefl}_{A, B}(f, a)), t(a))}$$

Given a dependent function $f:\prod_{x:A} B(x)$, we have 

$$\mathrm{hId}_{A, B}(a, b, p, f(a), f(b)) \simeq \mathrm{fhId}_{A, B}(f, a, b, p)$$

since by induction we have functions 

$$\mathrm{hIdTofhId}(f, a, b, p):\mathrm{hId}_{A, B}(a, b, p, f(a), f(b)) \to \mathrm{fhId}_{A, B}(f, a, b, p)$$

and 

$$\mathrm{fhIdTohId}(f, a, b, p):\mathrm{fhId}_{A, B}(f, a, b, p) \to \mathrm{hId}_{A, B}(a, b, p, f(a), f(b))$$

inductively defined by 

$$\mathrm{hIdTofhId}(f, a, a, \mathrm{refl}_A(a))(\mathrm{hrefl}_{A, B}(a, f(a))) \equiv \mathrm{hrefl}_{A, B}(f, a)$$

and 

$$\mathrm{fhIdTohId}(f, a, a, \mathrm{refl}_A(a))(\mathrm{hrefl}_{A, B}(f, a)) \equiv \mathrm{hrefl}_{A, B}(a, f(a))$$

Thus, we have identifications 

$$H(f, a, b, p):\mathrm{Id}_{\mathrm{fhId}_{A, B}(f, a, b, p)}(\mathrm{hIdTofhId}(f, a, b, p)(\mathrm{fhIdTohId}(f, a, b, p)(q), q)$$

and 

$$K(f, a, b, p):\mathrm{Id}_{\mathrm{hId}_{A, B}(a, b, p, f(a), f(b))}(\mathrm{fhIdTohId}(f, a, b, p)(\mathrm{hIdTofhId}(f, a, b, p)(q)), q)$$

inductively defined by 

$$H(f, a, a, \mathrm{refl}_A(a)) \equiv \mathrm{refl}_{\mathrm{fhId}_{A, B}(f, a, a, \mathrm{refl}_A(a))}(\mathrm{hrefl}_{A, B}(f, a))$$

and

$$K(f, a, a, \mathrm{refl}_A(a)) \equiv \mathrm{refl}_{\mathrm{hId}_{A, B}(f, a, a, \mathrm{refl}_A(a))}(\mathrm{hrefl}_{A, B}(a, f(a))$$

since by the properties of [[judgmental equality]]

$$\mathrm{Id}_{\mathrm{fhId}_{A, B}(f, a, a, \mathrm{refl}_A(a))}(\mathrm{fhIdTofhId}(f, a, a, \mathrm{refl}_A(a))(\mathrm{hIdTofhId}(f, a, a, \mathrm{refl}_A(a))(\mathrm{hrefl}_{A, B}(f, a))), \mathrm{hrefl}_{A, B}(f, a)) \equiv \mathrm{Id}_{\mathrm{fhId}_{A, B}(f, a, a, \mathrm{refl}_A(a))}(\mathrm{hrefl}_{A, B}(f, a)), \mathrm{hrefl}_{A, B}(f, a))$$

and 

$$\mathrm{Id}_{\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), f(a), f(a))}(\mathrm{hIdTofhId}(f, a, a, \mathrm{refl}_A(a))(\mathrm{fhIdTohId}(f, a, a, \mathrm{refl}_A(a))(\mathrm{hrefl}_{A, B}(a, f(a))), \mathrm{hrefl}_{A, B}(a, f(a))) \equiv \mathrm{Id}_{\mathrm{hId}_{A, B}(a, a, \mathrm{refl}_A(a), f(a), f(a))}(\mathrm{hrefl}_{A, B}(a, f(a)), \mathrm{hrefl}_{A, B}(a, f(a)))$$
...
## Indexed heterogeneous identity types of arbitrary arity
  {#OfArbitraryArity}

The usual notion of indexed heterogeneous identity types, binary indexed heterogeneous identity types, represents the notion of when two elements of a type family $(B(x))_{x:A}$ are equal or identified with each other. One can generalize this to the notion of when a family of elements of a type family $(B(x))_{x:A}$ are equal or identified, that is, **indexed heterogeneous identity types of [[arbitrary arity]]**. 

Given a type $A$ and an index type $I$, a family of elements in $A$ of [[arity]] $I$ is represented by a function $\overline{x}:I \to A$, and given such an $\overline{x}:I \to A$, a family of elements in $(B(x))_{x:A}$ of arity $I$ is represented by a [[dependent function]] $\overline{y}:\prod_{i:I} B(\overline{x}(i))$. 

The elements of indexed heterogeneous identity types of arity $I$ are called **heterogeneous identifications** of arity $I$. 

Closely related to indexed heterogeneous identity types of arbitrary arity are the [[indexed heterogeneous bridge types]], which can also come with arbitrary arity. Indexed heterogeneous identity types and indexed heterogeneous bridge types share some commonalities with each other: 

* they are reflexive type families that come with a notion of [[dependent function application to identifications]] (or [[bridges]] for indexed heterogeneous bridge types);

* they are used in the [[induction principle]] of [[higher inductive types]]; 

* they are used to satisfy various notions of [[extensionality]] such as dependent [[function extensionality]];

There are many different ways to define indexed heterogeneous identity types of arity $I$. The simplest definition states that given a [[type family]] $x:A \vdash B(x)$, indexed heterogeneous identity types of arity $I$ are an [[inductive family]] of [[types]] 

$$\overline{x}:I \to A, p:\mathrm{Id}_{I, A}(\overline{x}), \overline{y}:\prod_{i:I} B(\overline{x}(i)) \vdash \mathrm{hId}_{I, A, B}(\overline{x}, p, \overline{y})$$

generated by the family of elements called "heterogeneous reflexivity"

$$x:A, y:B(x) \vdash \mathrm{hrefl}_{I, A, B}(x, y):\mathrm{hId}_{I, A, B}(\lambda t.x, \mathrm{refl}_{I, A}(x), \lambda t.y)$$

where $\mathrm{Id}_{I, A}(\overline{x})$ denotes the [[identity type#OfArbitraryArity|identity type]] of [[arbitrary arity]]. 

When the index type $I$ is the [[booleans type]] $\mathbb{2}$, the function $\overline{x} \equiv \mathrm{rec}_{\mathbb{2}, A}(a, b)$ is given by the [[recursion principle]] of the [[booleans type]], and the function $\overline{y} \equiv \mathrm{ind}_{\mathbb{2}, \lambda b.B(\mathrm{rec}_{\mathbb{2}, A}(b))}(y, z)$ is given by the [[induction principle]] of the [[booleans type]], then the binary type family 

$$\mathrm{hId}_A(a, b, p, x, y) \equiv \mathrm{hId}_{\mathbb{2}, A}(\mathrm{rec}_{\mathbb{2}, A}(x, y), p, \mathrm{ind}_{\mathbb{2}, \lambda b.B(\mathrm{rec}_{\mathbb{2}, A}(b))}(y, z))$$

is precisely the usual indexed heterogeneous identity types as defined above, which indicates the heterogeneoud equality / identification of a family of two elements in a type family. 

Indexed heterogeneous identity types of arity $I$ are needed to define [[dependent function application to identifications]] of arity $I$, which are used in the [[induction principle]] of [[higher inductive types]] with path constructors of arity $I$, such as the [[walking identification]]. 

### Inference rules

The inference rules for forming indexed heterogeneous identity types and terms of arity $I$ are as follows. First the inference rule that defines the indexed heterogeneous identity type of arity $I$ itself, as a [[dependent type]], in some [[context]] $\Gamma$.

Formation rule for indexed heterogeneous identity types of arity $I$:
$$\frac{
    \begin{array}{l}
      \Gamma \vdash I \; \mathrm{type} \quad \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma \vdash \overline{x}:I \to A \quad \Gamma \vdash p:\mathrm{Id}_{I, A}(\overline{x}) \quad \Gamma \vdash \overline{y}:\prod_{i:I} B(\overline{x}(i))
    \end{array}
  }{\Gamma \vdash \mathrm{hId}_{I, A, B}(\overline{x}, p, \overline{y}) \; \mathrm{type}}$$ 

Now the basic "introduction" rule, which says that everything is equal to itself in a canonical way.

Introduction rule for indexed heterogeneous identity types of arity $I$:
$$\frac{
    \begin{array}{l}
      \Gamma \vdash I \; \mathrm{type} \quad \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \mathrm{hrefl}_{I, A, B}(a, y):\mathrm{hId}_{I, A, B}(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y)}$$ 

Next we have the "elimination" rule: 

Elimination rule for indexed heterogeneous identity types of arity $I$:
$$\frac{
    \begin{array}{l}
      \Gamma \vdash I \; \mathrm{type} \quad \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma, \overline{x}:I \to A, p:\mathrm{Id}_A(\overline{x}), \overline{y}:\prod_{i:I} B(\overline{x}(i)), q:\mathrm{hId}_{A, B}(\overline{x}, p, \overline{y}) \vdash C(\overline{x}, p, \overline{y}, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y)) \\
      \Gamma \overline{x}:I \to A \quad \Gamma \vdash p:\mathrm{Id}_A(a, b) \quad \Gamma \vdash \overline{y}:\prod_{i:I} B(\overline{x}(i)) \quad \Gamma \vdash q:\mathrm{hId}_{A, B}(\overline{x}, p, \overline{y})
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{A, B}}(t, \overline{x}, p, \overline{y}, q):C(\overline{x}, p, \overline{y}, q)}$$

The elimination rule then says that if:

1. for any $I$-indexed family of elements $\overline{x}:I \to A$, any identification $p:\mathrm{Id}_{I, A}(\overline{x})$ of arity $I$, any $I$-indexed family of elements $\overline{y}:\prod_{i:I} B(\overline{x}(i))$, and any heterogeneous identification $q:\mathrm{hId}_{A, B}(\overline{x}, p, \overline{y})$ we have a type $C(\overline{x}, p, \overline{y}, q)$, and
1. we have a specified dependent function 
$$t:\prod_{a:A} \prod_{y:B(a)} C(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y))$$

then we can construct a canonically defined term called the *eliminator*
$$\mathrm{ind}_{\mathrm{hId}_{A, B}}(t, \overline{x}, p, \overline{y}, q):C(\overline{x}, p, \overline{y}, q)$$ 
for *any* $\overline{x}$, $p$, $\overline{y}$, and $q$. 

Then, we have the [[computation rule]] or [[beta-reduction]] rule. This says that for all elements $a:A$ and $y:B(a)$, substituting the dependent function $t:\prod_{a:A} \prod_{y:B(a)} C(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y))$ into the eliminator along heterogeneous [[reflexive relation|reflexivity]] for $a$ and $y$ yields an element equal to $t(a, y)$ itself. Normally "equal" here means [[judgmental equality]] (a.k.a. definitional equality), but it is also possible for it to mean [[propositional equality]] (a.k.a. typal equality), so there are two possible computation rules.

Computation rules for indexed heterogeneous identity types of arity $I$:
...
$$\frac{
    \begin{array}{l}
      \Gamma \vdash I \; \mathrm{type} \quad \Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \\
      \Gamma, \overline{x}:I \to A, p:\mathrm{Id}_A(\overline{x}), \overline{y}:\prod_{i:I} B(\overline{x}(i)), q:\mathrm{hId}_{A, B}(\overline{x}, p, \overline{y}) \vdash C(\overline{x}, p, \overline{y}, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y)) \quad \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{I, A, B}}(t, \lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y)) \equiv t(a, y):C(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y))}$$
...
$$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash t:\prod_{a:A} \prod_{y:B(a)} C(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y)) \quad \Gamma \vdash a:A \quad \Gamma \vdash y:B(a)
    \end{array}
  }{\Gamma \vdash \beta_{\mathrm{hId}_{I, A, B}}(t, a, y):\mathrm{Id}_{C(\lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y))}(\mathrm{ind}_{\mathrm{hId}_{I, A, B}}(t, \lambda t.a, \mathrm{refl}_{I, A}(a), \lambda t.y, \mathrm{hrefl}_{I, A, B}(a, y), t(a, y))}$$
