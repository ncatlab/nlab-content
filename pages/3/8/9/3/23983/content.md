...
In [[dependent type theory]], a **heterogeneous identity type** is a version of the [[identity type]] which is able to compare two elements from different types for equality; hence the name "heterogeneous identity type". There are two versions of the heterogeneous identity type:
...
Sometimes, if there is a [[type universe]] $U$ and types $A$ and $B$ are $U$-[[small type|small]], these heterogeneous identity types are presented using an [[identification]] $p:A =_U B$ instead of an equivalence $e:A \simeq B$, a type family

$$p:A =_U B, x:A, y:B \vdash \mathrm{hId}_{A, B}(p, x, y)$$

generated by the family of elements called "heterogeneous reflexivity"

$$x:A \vdash \mathrm{hrefl}_{A}(x):\mathrm{hId}_{A, A}(\mathrm{refl}_U(A), x, x)$$

where $\mathrm{refl}_U(A)$ is [[reflexivity]] for $A$ in the type universe $U$. However the [[univalence axiom]] on the [[type universe]] $U$ states that the types $A =_U B$ and $A \simeq B$ are equivalent to each other, so these two definitions result in equivalent types. 
...
* for all elements $x:A$, there exists a unique element $y:B$ such that $\mathrm{hId}_{A, B}(e, x, y)$

$$\prod_{x:A} \exists!y:B.\mathrm{hId}_{A, B}(e, x, y)$$

* for all elements $y:B$, there exists a unique element $x:A$ such that $\mathrm{hId}_{A, B}(e, x, y)$

$$\prod_{y:B} \exists!x:A.\mathrm{hId}_{A, B}(e, x, y)$$
...
for all $a:A$, $b:A$, $p:\mathrm{Id}_A(a, b)$, $y:B(a)$, and $z:B(b)$. For constant families $x:A \vdash B$, $\overrightarrow{\mathrm{transport}}_{x:A.B}(a, b, p, y) \coloneqq y$ and $\overleftarrow{\mathrm{transport}}_{x:A.B}(a, b, p, z) \coloneqq z$, so the above two equivalences simplify down to
