...
In [[dependent type theory]], a **heterogeneous identity type** is a version of the [[identity type]] which is able to compare two elements from different types for equality; hence the name "heterogeneous identity type". There are multiple versions of the heterogeneous identity type:

* given types $A$ and $B$ and an [[equivalence of types]] $e:A \simeq B$, the "non-dependent" heterogeneous identity type between two elements $x:A$ and $y:B$ is a type $\mathrm{hId}_{A,B}(e, x, y)$ whose elements witness that $x$ and $y$ are "equal" over or modulo the equivalence $e$; and

* given a [[type]] $A$, a [[type family]] $x:A \vdash B(x)$, [[terms]] $a:A$, $b:A$, and an [[identification]] $p:a =_A b$, the "dependent" heterogeneous identity type between two elements $y:B(a)$ and $z:B(b)$ is a type whose elements witness that $y$ and $z$ are "equal" over or modulo the identification $p$. 

The dependent heterogeneous identity types are usually simply called **dependent identity types**. 
...
There are many different ways to define non-dependent heterogeneous identity types. The simplest definition states that given types $A$ and $B$, non-dependent heterogeneous identity types are an [[inductive family]] of [[types]] 

$$e:A \simeq B, x:A, y:B \vdash \mathrm{hId}_{A, B}(e, x, y)$$

generated by the family of elements called "heterogeneous reflexivity"

$$x:A \vdash \mathrm{hrefl}_{A}(x):\mathrm{hId}_{A, A}(\mathrm{id}_A, x, x)$$

where $\mathrm{id}_A$ is the [[identity function|identity]] [[equivalence of types|equivalence]] on $A$. 

#### Inference rules

The inference rules for forming non-dependent heterogeneous identity types and terms are as follows.  First the inference rule that defines the heterogeneous identity type itself, as a [[dependent type]], in some [[context]] $\Gamma$.

Formation rule for non-dependent heterogeneous identity types:
$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma \vdash B \; \mathrm{type} \\
      \Gamma \vdash e:A \simeq B \quad \Gamma \vdash x:A \quad \Gamma \vdash y:B
    \end{array}
  }{\Gamma \vdash \mathrm{hId}_{A, B}(e, a, b) \; \mathrm{type}}$$ 
...
$$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash x:A
    \end{array}
  }{\Gamma \vdash \mathrm{hrefl}_{A}(x):\mathrm{hId}_{A, A}(\mathrm{id}_A, x, x)}$$ 
...
$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma \vdash B \; \mathrm{type} \\
      \Gamma, e:A \simeq B, x:A, y:B, q:\mathrm{hId}_{A, B}(e, x, y) \vdash C(e, x, y, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{x:A} C(\mathrm{Id}_A, x, x, \mathrm{hrefl}_{A, A}(x)) \\
      \Gamma \vdash e:A \simeq B \quad \Gamma \vdash x:A \quad \Gamma \vdash y:B \quad \Gamma \vdash q:\mathrm{hId}_{A, B}(e, x, y, q)
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{A, B}}(t, e, x, y, q):C(e, x, y, q)}$$

The elimination rule then says that if:

1. for any equivalence $e:A \simeq B$, any elements $x:A$ and $y:B$, and any heterogeneous identification $q:\mathrm{hId}_{A, B}(e, x, y)$ we have a type $C(e, x, y, q)$, and
1. we have a specified dependent function 
$$t:\prod_{x:A} C(\mathrm{id}_A,x,x,\mathrm{hrefl}_{A, A}(x))$$

then we can construct a canonically defined term called the *eliminator*
$$\mathrm{ind}_{\mathrm{hId}_{A, B}}(t,e,x,y,q):C(e,x,y,q)$$ 
for *any* $e$, $x$, $y$, and $q$. 

Then, we have the [[computation rule]] or [[beta-reduction]] rule. This says that for all elements $x:A$, substituting the dependent function $t:\prod_{x:A} C(\mathrm{id}_A,x,x,\mathrm{hrefl}_{A,A}(x))$ into the eliminator along heterogeneous [[reflexive relation|reflexivity]] for $x$ yields an element equal to $t(x)$ itself.  Normally "equal" here means [[judgmental equality]] (a.k.a. definitional equality), but it is also possible for it to mean [[propositional equality]] (a.k.a. typal equality), so there are two possible computation rules
...
$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma \vdash B \; \mathrm{type} \\
      \Gamma, e:A \simeq B, x:A, y:B, q:\mathrm{hId}_{A, B}(e, x, y) \vdash C(e, x, y, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{x:A} C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A, B}(x)) \quad \Gamma \vdash x:A
    \end{array}
  }{\Gamma \vdash \mathrm{ind}_{\mathrm{hId}_{A, B}}(t, \mathrm{id}_A, x, x, \mathrm{hrefl}_{A, B}(x)) \equiv t(x):C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A, B}(x))}$$
...
$$\frac{
    \begin{array}{l}
      ...
      \Gamma \vdash t:\prod_{x:A} C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A, B}(x)) \quad \Gamma \vdash x:A
    \end{array}
  }{\Gamma \vdash \beta_{\mathrm{hId}_{A, B}}(t, x):\mathrm{Id}_{C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A, B}(x))}(\mathrm{ind}_{\mathrm{hId}_{A, B}}(t, \mathrm{id}_A, x, x, \mathrm{hrefl}_{A, B}(x), t(x))}$$

Finally, one might consider a [[uniqueness rule]] or [[eta-conversion]] rule.  But similar to the case for [[identity types]], a judgmental uniqueness rule for heterogeneous identity types implies that the type theory is an [[extensional type theory]], in which case there is not much need for non-dependent heterogeneous identity types, so such a rule is almost never written down.  And as for identity types and other inductive types, the propositional/typal uniqueness rule is provable from the other four inference rules, so we don't write it out explicitly.

#### Dependent universal property

The elimination, typal computation, and typal uniqueness rules for non-dependent heterogeneous identity types state that non-dependent heterogeneous identity types satisfy the **dependent universal property of non-dependent heterogeneous identity types**. With the [[uniqueness quantifier]], the non-dependent universal property of dependent heterogeneous identity types could be simplified to the following rule:

$$\frac{
    \begin{array}{l}
      \Gamma \vdash A \; \mathrm{type} \quad \Gamma \vdash B \; \mathrm{type} \\
      \Gamma, e:A \simeq B, x:A, y:B, q:\mathrm{hId}_{A,B}(e, x, y) \vdash C(e, x, y, q) \; \mathrm{type} \\
      \Gamma \vdash t:\prod_{x:A} C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A}(x)) \quad \Gamma \vdash x:A
    \end{array}
  }{\Gamma \vdash \mathrm{up}_{\mathrm{hId}_{A, B}}(t, x):
\begin{array}{c}
\exists!J:\left(\prod_{x:A} C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A}(x))\right) \\ 
\to \left(\prod_{e:A \simeq B} \prod_{a:A} \prod_{b:B} \prod_{s:\mathrm{hId}_{A, B}(e, a, b)} C(e, a, b, s)\right) \\
.\mathrm{Id}_{C(\mathrm{id}_A, x, x)}(J(t, \mathrm{id}_A, x, x, \mathrm{hrefl}_{A}(x)), t(x))
\end{array}
}$$

In natural language this states that given types $A$ and $B$, and given a type family $C(e, x, y, q)$ indexed by elements $e:A \simeq B$, $x:A$, $y:B$, and $q:\mathrm{hId}_{A, B}(e, x, y)$, for all elements $t:\prod_{x:A} C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A}(x))$ and $x:A$, there exists a unique function $J$ with domain 
$$\prod_{x:A} C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A}(x))$$
and codomain 
$$\prod_{e:A \simeq B} \prod_{a:A} \prod_{b:B} \prod_{s:\mathrm{hId}_{A, B}(e, a, b)} C(e, a, b, s)$$
such that $J(t, \mathrm{id}_A, x, x, \mathrm{hrefl}_{A}(x))$ is equal to $t(x)$ in the type $C(\mathrm{id}_A, x, x, \mathrm{hrefl}_{A}(x))$. 

### Dependent heterogeneous identification types

There are many different ways to define dependent heterogeneous identity types. The simplest definition states that given a [[type family]]  $x:A \vdash B(x)$, heterogeneous identity types are an [[inductive family]] of [[types]] 
...
The inference rules for forming dependent heterogeneous identity types and terms are as follows.  First the inference rule that defines the heterogeneous identity type itself, as a [[dependent type]], in some [[context]] $\Gamma$.
...
Finally, one might consider a [[uniqueness rule]] or [[eta-conversion]] rule.  But similar to the case for [[identity types]], a judgmental uniqueness rule for heterogeneous identity types implies that the type theory is an [[extensional type theory]], in which case there is not much need for dependent heterogeneous identity types, so such a rule is almost never written down.  And as for identity types and other inductive types, the propositional/typal uniqueness rule is provable from the other four inference rules, so we don't write it out explicitly.
...
To convert a dependent heterogeneous identity type $\mathrm{hId}_{x:A.B(x)}(a, b, p, y, z)$ to a non-dependent heterogeneous identity type, the equivalence used is [[transport]] $\mathrm{transport}_{x:A.B(x)}(a, b, p):B(a) \simeq B(b)$ across the given identification $p:a =_A b$:

$$\mathrm{hId}_{B(a), B(b)}(\mathrm{transport}_{x:A.B(x)}(a, b, p), y, z) \simeq \mathrm{hId}_{x:A.B(x)}(a, b, p, y, z)$$

while to convert a non-dependent heterogeneous identity type $\mathrm{hId}_{A, B}(e, x, y)$ to a dependent heterogeneous identity type, we first define a dependent type from the [[interval type]] $x:\mathbb{I} \vdash C(x)$ defined as 
...
$$\mathrm{hId}_{x:\mathbb{I}.C(x)}(\mathrm{left}, \mathrm{right}, \mathrm{segment}, x, y) \simeq \mathrm{hId}_{A, B}(e, y, z)$$

In addition, one could then show that there exist equivalences between identity types and non-dependent heterogeneous identity types

$$\mathrm{hId}_{A, B}(e, x, y) \simeq \mathrm{Id}_{B}(e(x), y)$$

and 

$$\mathrm{hId}_{A, B}(e, x, y) \simeq \mathrm{Id}_{A}(x, e^{-1}(y))$$

for all $e:A \simeq B$, $x:A$, and $y:B$. 

Similarly, there are equivalences between identity types and heterogeneous identity types
...
### Heterogeneous identity types and universes

Given a Tarski universe $(U, T)$, there is an equivalence 

$$\mathrm{hId}_{T(A), T(B)}(\mathrm{transport}_U(A, B, p), x, y) \simeq \mathrm{hId}_{X:U.T(X)}(A, B, p, x, y)$$
...
for the heterogeneous identity type and

$$\mathrm{hId}_{A, B}(\mathrm{idtoequiv}_U(A, B, p), x, y) \simeq \mathrm{hId}_{X:U.(X)}(A, B, p, x, y)$$

for the equivalence. Usually, for Russell universes, the dependent heterogeneous identity types are written as 

$$\mathrm{hId}_{A, B}(p, x, y) \coloneqq \mathrm{hId}_{X:U.(X)}(A, B, p, x, y)$$ 

for $p:A =_U B$, $x:A$ and $y:B$. 
...
$$\mathrm{hId}_{T(A), T(B)}(e, x, y) \simeq \mathrm{hId}_{X:U.T(X)}(A, B, \mathrm{ua}_U(A, B)^{-1}(e), x, y)$$
...
$$\mathrm{hId}_{A, B}(e, x, y) \simeq \mathrm{hId}_{A, B}(\mathrm{ua}_U(A, B)^{-1}(e), x, y)$$

for the equivalence. 

### One-to-one correspondence structure

Heterogeneous identity types are [[one-to-one correspondences]]. 

For non-dependent heterogeneous identity types, given an equivalence $e:A \simeq B$, 

* for all elements $y:B$, there exists a unique element $x:A$ such that $\mathrm{hId}_{A, B}(e, x, y)$; the witness is given by the right projection function of the equivalence $e:A \simeq B$

$$\pi_2(e):\prod_{y:B} \exists!x:A.\mathrm{hId}_{A, B}(e, x, y)$$

* for all elements $x:A$, there exists a unique element $y:B$ such that $\mathrm{hId}_{A, B}(e, x, y)$; the witness is given by the right projection function of the inverse equivalence $e^{-1}:B \simeq A$

$$\pi_2(e^{-1}):\prod_{x:A} \exists!y:B.\mathrm{hId}_{A, B}(e, x, y)$$

Similarly, for dependent heterogeneous identity types, given elements $a:A$, $b:A$, and $p:\mathrm{Id}_A(a, b)$, 
...
The [[dependent function application to identifications]] is defined in the usual way, by induction on the [[identity type]] of the index type. There are two ways to define it, using dependent and non-dependent heterogeneous identity types. For simplicity, only the definition using the dependent heterogeneous identity type is given, and the definition using the non-dependent heterogeneous identity type could be derived using the above equivalence between dependent and non-dependent heterogeneous identity types:
