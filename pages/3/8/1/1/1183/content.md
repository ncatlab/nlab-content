...
Finally, just like an ordinary [[principal U(1)-bundle]] has an [[associated bundle|associated]] [[vector bundle]] once we fix a [[representation]] of $U(1)$ to be the [[fiber]]s, any "[[circle n-bundle]]" has an associated "[[n-vector bundle]]" once we fix a "[[∞-representation]]" $\rho : \mathbf{B}^n U(1) \to n Vect$ on "[[n-vector spaces]]".  Just as for the ordinary $U(1)$, here we usually pick the canonical 1-dimensional such "representation".  Finally, we define bundles $V_\Sigma : Conf_\Sigma \to \mathcal{C}$ of "internal states" by [[transgression]] of these associated bundles.
