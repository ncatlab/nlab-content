...
It is straightforward to generalise from fields to other [[rings]], and even to [[rigs]] whose subrig generated by $1$ has [[cancellative monoid|cancellative]] addition. See [[characteristic of a rig]] for the characteristic of rigs whose subrig generated by $1$ is not necessarily cancellative, since the definition of a rig given in this article does not generalize to those rigs. 

See also [[characteristic zero]]. 

## Definition

### For rings and certain rigs

Let $K$ be a [[rig]] whose subrig generated by $1$ has [[cancellative monoid|cancellative]] addition (possibly a [[ring]], possibly a [[commutative ring]], possibly even a [[field]]).  Then there exists a unique [[homomorphism]] $\phi_K\colon \mathbb{N} \to K$ to $K$ from the [[initial object|initial]] rig, which is the rig $\mathbb{N}$ of [[natural numbers]]. The [[kernel]] of $\phi_K$ is an [[ideal]] of $\mathbb{N}$, which (by a well-known property of $\mathbb{N}$) is a [[principal ideal]] with a unique generator.  This generator is the __characteristic__ of $K$, denoted $\char K$.
...
Every rig whose subrig generated by $1$ has cancellative addition and positive characteristic is in fact a ring, since we have $\char K - 1 = -1$. In other words, any rig whose subrig generated by $1$ has cancellative addition, other than a ring, must have characteristic zero (although many rings also have that characteristic).
