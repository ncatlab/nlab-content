...
There is an enriched category version of end, as follows. Let $V$ be a symmetric monoidal category, let $C$ be a $V$-enriched category, and let $F: C^{op} \otimes C \to V$ be a $V$-enriched functor. In particular there is a covariant action of $C$ on $F$, with components 
...
A $V$-<i>dinatural transformation</i> 
...
A $V$-enriched <i>end</i> of $F$ is an object $\int_{c: C} F(c, c)$ of $V$ equipped with a $V$-dinatural transformation 
...
If $X$ is any $V$-enriched category and $F: C^{op} \otimes C \to X$ is a $V$-enriched functor, then the <i>end</i> of $F$ in $X$ is an object $\int_{c: C} F(c, c)$ of $X$ equipped with an $Ob(C)$-indexed family of arrows 
