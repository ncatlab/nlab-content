...
1) for all X,Y in Ob(C) the [[hom-set|Hom-set]]s Hom<sub>C</sub>(X,Y) are finite dimensional [[chain complex]]es of **Z**-graded modules

2) for all objects X<sub>1</sub>,...,X<sub>n</sub> in Ob(C) there is a family of linear composition maps (the higher compositions)
...
3) m<sub>1</sub> is the differential on the chain complex Hom<sub>C</sub>(X,Y)

4) m<sub>n</sub> satisfy the quadratic A<sub>?</sub>-associativity equation for all n&#8805;0.

m<sub>1</sub> and m<sub>2</sub> will be [[chain complex|chain map]]s but the compositions m<sub>i</sub> of higher order are not chain maps, nevertheless they are [[massey product|Massey product]]s.
 
The framework of dg-categories and dg-functors is too narrow for many problems, and it is preferable to consider the wider class of A<sub>?</sub>-categories and A<sub>?</sub>-functors. Many features of A<sub>?</sub>-categories and A<sub>?</sub>-functors come from the fact that they form a symmetric closed [[multicategory]], which is revealed in the language of [[monad|comonads]].

From a higher dimensional perspective A<sub>?</sub>-categories are weak &#969;-categories with all morphisms invertible. A<sub>?</sub>-categories can also be viewed as noncommutative formal dg-manifolds with a closed marked subscheme of objects.


## Examples and remarks ##

* Every [[dg-category]] may be regarded as a special case when there are no higher maps (trivial homotopies) of an  [[A-infinity category]].
...
* [[A-infinity-algebra|A]]<sub>[[A-infinity-algebra|?]]</sub>[[A-infinity-algebra|-algebras]] as A<sub>?</sub>-categories with one object.
