...
This definition immediately generalizes to [[(∞,1)-category theory]] where $\mathcal{C}$ is an [[(∞,1)-category]] and $X$ is a [[simplicial object in an (∞,1)-category]]. Then for $X$ to satisfy the Segal conditions means that it sends the cocones of def. \ref{SegalCones} to [[(∞,1)-limit]] cones in $\mathcal{C}$.
