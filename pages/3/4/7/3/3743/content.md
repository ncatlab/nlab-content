...
An example of a rewrting system is given by a [[group presentation]], $(X,R)$, where $X$ is a set of generators for a group $G$ and $R$ is a subset of pairs of elements in the free group on $X$.  As a definte example, take 
 
* $X = \{a,b\}$

* $R = \{(aaa,1),(bb,1),(abab,1)\}
(and the group being presented is the symmetric group, $S_3$ on three symbols).
We thing of $(aaa,1)$ as a 'rewrite rule':''replace $aaa$ by 1'', often written $aaa\to 1$.
