...
One considers the problem of reconstructing the Lie algebra $\mathfrak{g}$ from
...
Indeed, write each element $g \in \mathfrak{g}$ as
$\sigma(b) + k, b \in \mathfrak{g}/\mathfrak{k}$, $k \in \mathfrak{k}$ by setting $b := [g], k := -\sigma([g]) + g$. The elements $b \in \mathfrak{b}$ and
$k \in \mathfrak{k}$ in this decomposition are unique.
...
\end{equation}
 ...
 Now $[\sigma(b_1),\sigma(b_2)] \in [[b_1,b_2]]$ so it can be represented uniquely
