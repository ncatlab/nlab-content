...
Derived algebraic geometry is about spaces locally (in some sense) represented by higher algebras in derived sense:
$E_\infty$-rings, $A_\infinity$-rings, simplicial rings and variants.
...
1. to generalize the algebraic geometry allowing the notion of derived [[moduli space]]s (moduli stacks if you wish), which extend or replace the usual [[moduli space]]s. Locally these moduli spaces should correspond to the deformation theory problem which geometrically realizes the cotangent complex for the deformation problem. 
