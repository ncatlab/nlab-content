...
A pure subobject is a [[monomorphism]] $A \rightarrowtail B$ such that any sufficiently small system of equations involving constants in $A$ that admits a solution in $B$ also admits a solution in $A$. This generalises the classical notions of 'pure group' and 'pure submodule'.
...
* Given any morphisms $f' : A' \to B'$, $a : A' \to A$,  $b : B' \to B$ in $\mathcal{C}$, if both $A'$ and $B'$ are $\kappa$-[[compact object|compact]] and $f \circ a = b \circ f'$, then there exists a (not necessarily unique) morphism $\bar{a} : B' \to A$ in $\mathcal{C}$ such that $a = \bar{a} \circ f$.
