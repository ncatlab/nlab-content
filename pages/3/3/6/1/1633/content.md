...
Or rather, if one does not at the same time assume that the space is a [[Hausdorff topological space]], then one needs to require that these compact neighbourhoods exist in a controlled way, e.g. such that one may find them inside every prescribed open neighbourhood (def. \ref{LocalCompactnessViaCompactNeighbourhoodBase} below) and possibly such that they are topological closures of smaller open neighbourhoods (def. \ref{LocallyCompactSpace} below).
