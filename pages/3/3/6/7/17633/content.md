...
From any example, we may form a sub-example by restricting to a [[subset]] of $A$, as long as $0$ belongs.  If the original example has a compatible group structure, then so does the subset if it is a [[subgroup]].  If the original example is nontrivial, then so is the subset.
