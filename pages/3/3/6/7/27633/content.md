...
* There is a [[pushout in simplicial type theory|pushout]] $x \sqcup^z_{f, g} y:A$ for all elements $x:A$, $y:A$, $z:A$ and morphisms $f:\mathrm{hom}_A(z, x)$ and $g:\mathrm{hom}_A(z, y)$. 

Equivalently, type $A$ is **finitely cocomplete** if 

* $A$ has an [[initial object in simplicial type theory|initial object]] $0:A$

* There is a [[coproduct in simplicial type theory|coproduct]] $x \sqcup y:A$ for all elements $x:A$ and $y:A$

* There is a [[coequalizer in simplicial type theory|coequalizer]] $\mathrm{coeq}(f, g):A$ for all elements $x:A$ and $y:A$ and morphisms $f:\mathrm{hom}_A(x, y)$ and $g:\mathrm{hom}_A(x, y)$
