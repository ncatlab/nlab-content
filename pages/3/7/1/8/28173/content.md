...
Let $\mathcal{B}_t$ be a convenient open-[[neighbourhood base]] for each point $t$ of $[a, b]$ (say, the open balls with rational radius), and let $\mathcal{U}$ be the collection of [[open subsets]] of the $N(t)$ that belong to the corresponding $\mathcal{B}_t$; that is, an open set $U$ of $[a, b]$ belongs to $\mathcal{U}$ iff for some $t \in [a, b]$, both $U \subseteq N(t)$ and $U \in \mathcal{B}_t$.  Then $\mathcal{U}$ is an open cover of $[a, b]$, because $\mathcal{B}_t$ is a neighbourhood base, so that $t \in U \subseteq N(t)$ for some $U \in \mathcal{B}_t$, and hence $U \in \mathcal{U}$.  (This is to keep the argument both constructive and predicative; otherwise, we could use the [[axiom of choice]] to pick a single open sub-neighbourhood of each $N(t)$, or impredicatively let $\mathcal{U}$ be the collection of *all* open subsets of the $N(t)$.  If each $N(t)$ is open, then no choice is needed and this step can be skipped entirely.)  Let $x_0, \ldots, x_n$ be a partition as given by Theorem \ref{basic}.  Each subinterval $[x_i, x_{i+1}]$ is contained in some member of $\mathcal{U}$, which in turn is contained in $N_t$ for some point $t$; pick one and call it $t_i$.  (These choices are no problem constructively, since there are only finitely many to make.)  Using these points as tags, we have a freely tagged partition as desired.
=--

Using Theorem \ref{positive} instead, we can guarantee that each subinterval has positive length if we wish (assuming that $a = b$ or $a \lt b$ to be constructive).  It\'s common to start with a [[function]] $\delta\colon [a, b] \to {]0, \infty]}$ (a _gauge_) and let $N(t)$ be a [[ball]] around $t$ of radius $\delta(t)$; this allows us to skip the business with $\mathcal{B}_t$ and remain constructive.  (Conversely, we can get $\delta$ from $N$ by taking a [[supremum]], although constructively this means that $\delta$ is valued in the [[lower reals]].)
