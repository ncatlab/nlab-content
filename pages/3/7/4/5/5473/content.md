...
A simple but basic fact is the following. Suppose $J$ is a [[small category]], and suppose that the functor $T$  preserves colimits over $J$, that is, suppose that for every $F: J \to C$ the canonical map 
...
+-- {: .num_prop} 
###### Proposition 
Under these hypotheses, $U: C^T \to C$ [[creates colimits]] over $J$. 
=--

For a proof, see Proposition 4.3.2 of [[Handbook of Categorical Algebra]] (Volume 2). In fact, what is shown there is a tighter result: if $D:J\to C^T$ is any diagram such that $U \circ D$ has a colimit preserved by both $T$ and $T \circ T$, then $D$ has a colimit preserved by $U$.

Here preservation by just $T$ is not sufficient. A counterexample is given by the algebraic theory generated by a 1-ary operation $f$ and a 3-ary operation $g$ with the following laws:

- $f(f(x)) = x$.
- $g(x,x,y) = g(x,y,x) = g(y,x,x) = x$.
- $g(x,f(x),y) = g(x,y,f(x)) = g(y,x,f(x)) = y$.

The corresponding monad on $\mathrm{Set}$ preserves the coproduct $1 + 1$. However, in the category of algebras we have $1 + 1 = 1$.
...
+-- {: .num_cor}
######Corollary 
Assuming the [[axiom of choice]], if $T$ is a monad on [[Set]], then $Set^T$ is cocomplete. Similarly upon replacing $Set$ by a slice $Set/X \simeq Set^X$, or by [[Vect]]. 
=-- 
...
$$\array{
T U E & \stackrel{\overset{T U\pi_1}{\to}}{\underset{T U\pi_2}{\to}} & T U B & \stackrel{T p}{\longrightarrow} & T Q \\ 
...
$$\array{
U E & \stackrel{\overset{U\pi_1}{\longrightarrow}}{\underset{U\pi_2}{\longrightarrow}} & U B & \stackrel{q}{\longrightarrow} & Q}$$ 
...
$$\array{
U E & \stackrel{\overset{U\pi_1}{\longrightarrow}}{\underset{U\pi_2}{\longrightarrow}} & U B & \stackrel{q}{\to} & Q
}$$ 
...
T S c & & \stackrel{\; \; \; \; \; \; \; \; T \xi \; \; \; \; \; \; \; \; }{\longrightarrow} & & T c
}$$ 
...
B \otimes A \otimes M & & \stackrel{\; \; \; \; \; \; \; \; B \otimes \alpha_M \; \; \; \; \; \; \; \; }{\longrightarrow} & & B \otimes M,
}$$ 
