...
+-- {: .proof}
###### Proof
We drop the quadratic forms from the notation. The $k$-linear map $L \colon V \oplus V' \to Cl(V) \otimes Cl(V)$ satisfies $L(v,v')^2 = Q(v)\,1 + Q'(v')\,1$. Therefore it extends to an algebra homomorphism $Cl(V \oplus V') \to Cl(V) \otimes Cl(V')$. To construct the inverse note that the algebra inclusions $Cl(V) \to Cl(V \oplus V')$ and $Cl(V') \to Cl(V \oplus V')$ yield $Cl(V) \otimes Cl(V') \to Cl(V \oplus V')$, which is easily seen to be an algebra homomorphism again.  
=--
