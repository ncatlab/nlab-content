...
These definitions only depned on the [[homotopy type]] of $X$, by definition: a [[weak homotopy equivalence]] between topological spaces is a [[continuous map]] that induces an [[isomorphism]] on all homotopy groups.
...
One can check that this operation respects homotopy equivalence and hence equipts $\pi_n(X,x)$ with the structure of a [[monoid]].
...
This monoid is in fact a [[group]]:

* the [[constant function]] that maps all of $S^n$ to $x$ represents the __neutral element__ of $\pi_n(X,x)$;
