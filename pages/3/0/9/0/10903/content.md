...
The original axiomatics for [[linear type theory]] in ([Girard 87](#Girard87)) crucially contained a certain (co-)[[modality]] traditionally denoted "$!$". In ([Barber 97, p. 26](#Barber97)) it is noted that the canonical [[categorical semantics]] for this modality identifies it with the [[comonad]] that is induced from an [[adjunction]]
...
where $\mathcal{C}$ is the [[closed monoidal category|closed]] [[symmetric monoidal category]] wich interprets the given linear type theory, where $S$ is a [[cartesian monoidal category]], and where $\Sigma$ is a [[strong monoidal functor]]. In ([Ponto-Shulman 12](#PontoShulman12)) it is observed that this in turn is canonically induced if $\mathcal{C} \simeq \mathcal{C}_\ast$ is the linear type theory over the trivial context of a dependent linear type theory/[[indexed closed monoidal category]] with category of contexts being $S$.
...
and is further discussed for instance in 
