...
Let $\mathcal{M}$ be the category whose [[objects]] are [[injections]] ([[monomorphisms]] in the [[category of sets]]) and whose [[morphisms]] are [[commutative squares]]. $\mathcal{M}$ is a [[reflective subcategory]] of the [[arrow category]] $Set^\to$, where the reflector [[product-preserving functor|preserves products]]; as a result, $\mathcal{M}$ is [[complete category|complete]], [[cocomplete category|cocomplete]] [[cartesian closed category]]. (In fact, $\mathcal{M}$ is a Grothendieck [[quasitopos]], as discussed [here](free+cartesian+category#Subset).) 
