...
### Derivation with value in a bimodule
...
where on the left the dot denotes the product in $A$, while on the right it denotes the right (first term) and left (second term) [[action]] of $A$ on the bimodule $N$. 
...
An important special case, often considered exclusively, is the case where the bimodule is $N = A$, the algebra itself with its canonical left and right action on itself.
