...
The formalization of the notion _[[quantum mechanical system]]_ (see there for details) is effectively simply: a non-commutative _[[C-star algebra]]_ $A$. The way to interpret this as encoding a physical system is traditionally taken to be the following:

* a $\mathbb{C}$-[[linear map]] $\rho : A \to \mathbb{C}$ (which is "positive" and "normalized") is thought of as being a _[[state]]_ that the system can be in -- a physical configuration;
...
* a one-parameter [[group]] of [[automorphism]]s $\mathbb{R} \to Aut(A)$ ([[inner automorphisms]] for "localized" systems, see below) as being an _evolution_ of the system, for instance in [[time]] or more general in [[spacetime]].

(Often in the literature quantum mechanical systems are instead conceived of interms of [[Hilbert space|Hilbert]] [[spaces of states]]. The relation between these two descriptions is established by the [[GNS-construction]] which allows to pass from one to the other.)

Historically, these axioms took some time to be isolated. Especially the very first one had been subject to a lot of discussion: by the [[spectral theorem]] two different [[observable]]s $a_1, a_2 \in A$ have a compatible set of observable values if and only if these elements commute with each other in $A$. Generally, a set $\{a_i\}_i$ of observables has a jointly consistent set of observable values if and only if the sub-$C^\ast$-algebra $\langle a_i\rangle_i \subset A$ generated by them is commutative. Therefore for the phenomenological interpretation of the axioms it seems to make no sense to demand that a [[state]] $\rho : A \to \mathbb{C}$ be linear on non-commuting observables: if $a_1$ and $a_2$ do not commute, it is not a-priori clear that it makes sense to require that $\rho(a_1 + a_2) = \rho(a_1) + \rho(a_2)$. This might experimentally fail, and hold only for commuting $a_1, a_2$.
