...
An alternative, but equivalent, definition of a **standard Borel space** which is more useful for applications to  probability (as opposed to analysis) requires some additional  terminology.

Given a set $X$ the finite field (Boolean algebra) of $X$ generated by $n$ subsets of $X$, say $S_n$ for
$n=0,1,...,n-1$, is denoted $\mathbb{F}_n = \{ S_0, S_1, ... , S_{n-1}\}$. The set of atoms of $\mathbb{F}_n$ are the set of all nonempty intersection sets of 
