...
Even for situations (such as metric spaces) where sequences classically suffice, the [[constructivism|constructive]] definition of an infinite sequence in $X$ arguably should be a [[multi-valued function]] from $\mathbf{N}$ to $X$, that is a [[span]] $\mathbf{N} \leftarrow A \rightarrow X$ where the map $A \to \mathbf{N}$ is a [[surjection]].  (This is a special case of an alternative definition of [[net]] that uses only [[partial order|partially ordered]] directed sets.)  In some type-theoretic [[foundations]] of mathematics, you can get the same result by allowing the sequence to be an 'operation' (which is like a function but need not preserve equality).

For example, using this definition of generalised sequence, every [[real number]] (defined as a located Dedekind cut) may be represented as a generlised Cauchy sequence, even when this fails with the strict definition of sequence.  Many usual properties of [[metric space]]s and other [[sequential space]]s also depend on this.
