...
The _space_ of smooth forms on $X$ may also be defined as follows:  It is generated by the smooth functions on $X$ (which are the same as the smooth $0$-forms as defined above) and three operations:
...
*  addition makes the space of forms into an [[abelian monoid]],
...
Although not directly stated, it can be proved that addition makes the space of forms into an [[abelian group]]; in fact, it is a [[module]] of the [[commutative ring]] of smooth functions on $X$.  This is further a [[graded module]], graded by the [[natural numbers]], with the elements of grade $p$ being the $p$-forms defined earlier.  If $\omega$ is a $p$-form and $\eta$ is a $q$-form, we have:
...
There is a [[cohomology]] theory of differential forms.  If $\Omega^p(X)$ is the space of $p$-forms on $X$, then we have a [[chain complex]]
$$ \cdots \stackrel{\mathrm{d}}\to \Omega^2(X) \stackrel{\mathrm{d}}\to \Omega^1(X) \stackrel{\mathrm{d}}\to \Omega^0(X) \stackrel{\mathrm{d}}\to 0 ;$$
the [[chain cohomology]] of this complex is the __de Rham cohomology__ of $X$.

As differential forms are the cochains in de Rham cohomolgy, the theory of integration of forms allows us to interpret compact oriented submanifolds as chains on $X$, giving us a [[homology]] theory.  Combining these, we have __Stokes\'s theorem__
