...
There are typically two approaches that one can take to defining semi-simplicial types in [[dependent type theory]]: 

* One can define semi-simplicial types as an [[(infinity,1)-presheaf]] on the category $\Delta_+$ of [[finite]] [[inhabited]] [[ordinals]] and [[injections]] via synthetic $(\infty,1)$-category theory. 

* One can define semi-simplicial types as a family of families by iterating the [[categorical semantics of dependent types|correspondence between fibrations and families]] in what could be called an "indexed" presentation of semi-simplicial types based on "iterated dependencies". 
...
### Using synthetic $(\infty,1)$-category theory
...
With the [[op modality]] $(-)^\op$, the [[directed univalent]] [[type universe]] $\mathcal{S}$, and the $(\infty,1)$-subcategory $\Delta_+ \subseteq \mathcal{S}$ of [[finite]] [[inhabited]] [[ordinals]] and [[injections]], a **semi-simplicial type** is simply a [[function]] $X:\Delta_+^op \to \mathcal{S}$, which are provably [[presheaves]] by the directed structure identity principle on presheaf $(\infty,1)$-categories. 

### Using bridge types

Suppose that the [[dependent type theory]] has [[unary bridge types]] $(\mathrm{Br}_A(x))_{x:A}$. Then given a [[type universe]] $U$, the type of $U$-[[small type|small]] semi-simplicial types $U^{\Delta_{+}^\op}$ is a [[displayed coinductive type]] cogenerated by 
...
A semi-simplicial type is simply an element of $U^{\Delta_{+}^\op}$. See e.g. [Kolomatskaia 2022](#Kolomatskaia22), [Kolomatskaia & Shulman 2023](#KS23), [Narya docs](#NaryaDocs). 

### In other dependent type theories

There are other dependent type theories in which it is possible to define semi-simplicial types in a universe:
