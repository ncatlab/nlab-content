...
Suppose that the [[dependent type theory]] has [[unary bridge types]] $(\mathrm{Br}_A(x))_{x:A}$. Then given a [[type universe]] $U$, the type of $U$-[[small type|small]] indexed semi-simplicial types $U^{\Delta_{+}^\op}$ is a [[displayed coinductive type]] cogenerated by 

  * a function $Z^+:U^{\Delta_{+}^\op} \to U$ and 

  * a dependent function 
$$S^+:\prod_{X:U^{\Delta_{+}^\op}} Z^+(X) \to \mathrm{Br}_{U^{\Delta_{+}^\op}}(X)$$
