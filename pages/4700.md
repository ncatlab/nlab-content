...
1. generators $e^i_\alpha$ for $\alpha \in \{1, \cdots N\}$, $j \in \{1, \cdots, N\}$ of degree that of the respective basis element $e_\alpha$ as above,
...
There is a canonical [[graded object|bigrading]] on the graph complex, where $\mathcal{G}_{ij}$ is generated by those graphs which have $i$ [[vertices]] and $j$ [[edges]]; the differential has bidegree $(-1,-1)$; each $\mathcal{G}_{ij}$ is finite-dimensional, while the whole complex is infinte-dimensional. 
