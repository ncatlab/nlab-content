...
Let $C$ be a [[locally presentable (∞,1)-category]] with [[universal colimits]]. Assume moreover that finite limits commute with filtered colimits in $C$ (this holds for example if $C$ is an [[(∞,1)-topos]]). Let $S_0 \subset Mor(C)$ be a small set of morphisms, and $S$ the smallest strongly saturated class containing $S_0$ and stable under pullbacks. Then $S$ is strongly generated by a small set.
