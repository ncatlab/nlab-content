...
Given two [[rings]] $R$ and $S$, a $R$-$S$-bimodule is an [[abelian group]] $M$ with a [[bilinear function|bilinear]] [[left action|left $R$-action]] $\alpha_R:R \times M \to M$ and a bilinear [[right action|right $S$-action]] $\alpha_S:M \times S \to S$ such that for all $r \in R$, $m \in M$, and $s \in S$, $\alpha_R(r, \alpha_S(m, s)) = \alpha_S(\alpha_R(r, m), s)$. 

#### With a biaction

Equivalently, given two [[rings]] $R$ and $S$, a $R$-$S$-bimodule is an [[abelian group]] $M$ with a [[multilinear function|trilinear]] [[biaction|$R$-$S$-biaction]], a function $\alpha:R \times M \times S \to M$ such that 

* for all $m \in M$, $\alpha(1_R, m, 1_S) = m$

* for all $m \in M$, $r_1 \in R$, $r_2 \in R$, $s_1 \in S$, $s_2 \in S$, $\alpha(r_1, \alpha(r_2, m, s_1), s_2) = \alpha(r_1 \cdot_R r_2, m, s_1 \cdot_S s_2)$

* for all $r_1 \in R$, $r_2 \in R$, $m \in M$, $s \in S$, $\alpha(r_1 + r_2, m, s) = \alpha(r_1, m, s) + \alpha(r_2, m, s)$

* for all $r \in R$, $m_1 \in M$, $m_2 \in M$, $s \in S$, $\alpha(r, m_1 + m_2, s) = \alpha(r, m_1, s) + \alpha(r, m_2, s)$

* for all $r \in R$, $m \in M$, $s_1 \in S$, $s_2 \in S$, $\alpha(r, m, s_1 + s_2) = \alpha(r, m, s_1) + \alpha(r, m, s_2)$
...
$$\alpha(r, m, s) \coloneqq \alpha_R(r, \alpha_S(m, s)) = \alpha_S(\alpha_R(r, m), s)$$
...
$$\alpha_R(r, m) \coloneqq \alpha(r, m, 1_S)$$

for all $r \in R$ and $m \in M$. It is a left action because 

$$\alpha_R(1_R, m) = \alpha(1_R, m, 1_S) = m$$

$$\alpha_R(r_1, \alpha_L(r_2, m)) = \alpha(r_1, \alpha(r_2, m, 1_S), 1_S) = \alpha(r_1 \cdot_R r_2, m, 1_S \cdot_S 1_S) = \alpha(r_1 \cdot_R r_2, m, 1_S) = \alpha_R(r_1 \cdot_R r_2, m)$$

The [[right action|right $S$-action]] is defined from the $R$-$S$-biaction as 

$$\alpha_S(m, s) \coloneqq \alpha(1_R, m, s)$$

for all $s \in S$ and $m \in M$. It is a right action because 

$$\alpha_S(m, 1_S) = \alpha(1_R, m, 1_S) = m$$

$$\alpha_S(\alpha_S(m, s_1), s_2) = \alpha(1_R, \alpha(1_R, m, s_1), s_2) = \alpha(1_R \cdot_R 1_R, m, s_1 \cdot_S s_2) = \alpha(1_S, m, s_1 \cdot_S s_2) = \alpha_S(m, s_1 \cdot_S s_2)$$

The left $R$-action and right $S$-action satisfy the following identity: 

* for all $m \in M$, $r \in R$ and $s \in S$, $\alpha_R(r, \alpha_S(m, s)) = \alpha_S(\alpha_R(r, m), s)$. 

This is because when expanded out, the identity becomes:

$$\alpha(r, \alpha(1_R, m, s), 1_S) = \alpha(1_R, \alpha(r, m, 1_S), s)$$

$$\alpha(r \cdot_R 1_R, m, s \cdot_S 1_S) = \alpha(1_R \cdot_R r, m, 1_S \cdot_S s)$$

$$\alpha(r, m, s) = \alpha(r, m, s)$$
