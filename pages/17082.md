...
(1.) that the corresponding unique morphisms $\varnothing \longrightarrow B$ (from the [[initial object]]) has the left lifting property
in the sense of Def. \ref{LiftingPropertiesOfMorphisms}
...
against the given morphism $p$;

(2.) that the unique morphism $X \longrightarrow \ast$ (to the [[terminal object]]) has the right lifting property: 
...
Taking the [[Quillen negation]] of a class $C$ is a simple way to define a class of morphisms excluding [[isomorphisms]] from $C$, in a way which is useful in [[diagram chasing]] computations.

\begin{example}
In the category [[Set]] of [[sets]], the right Quillen negation $\{\emptyset \to \{*\}\}^{&solb;  r}$ of the simplest [[surjective|non-surjection]] $\emptyset\to \{*\}$ is the class of [[surjections]]. The left and right Quillen negation of $\{x_1,x_2\}\to \{*\},$ the simplest non-injection, are both precisely the class of injections, 
