...
In [[combinatory logic]], in the [[λ-calculus]], or more generally in [[type theory]], a _fixed-point [[combinator]]_ is a [[term]] $Y$ which, when [[function application|applied]] to a term $n$, yields a term $Y n$ that is a fixed-point of $n$:
$$ n (Y n) = Y n .$$
...
In the [[unityped λ-calculus]], a traditional construction (due to Curry) is
