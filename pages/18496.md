...
The _wave front set_ at $x$ is the [[intersection]] of these sets as $b$ ranges over [[bump functions]] whose [[support]] includes $x$:

$$
  \Sigma_x(u)
  \;\coloneqq\;
  \underset{ { b \in C^\infty_{cp}(\mathbb{R)^n)} \atop { x \in supp(b) } }{\cap}
  \Sigma(b u)
  \,.
$$
