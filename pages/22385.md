...
A subgroupoid $G$ of a setoid $A$ is a maximal subgroupoid if the dagger functor $f:G \to A$ is bijective-on-objects, and additionally if, for every other subgroupoid $H$ of $A$, there is a unique faithful injective-on-objects dagger functor $h:H \to G$ such that $h \circ f = g$. 
