...
In his book _Abelian Varieties_, David Mumford gives at least two proofs that every connected projective algebraic group over $k$ is automatically abelian.  One of them uses a 'rigidity lemma' which has an interesting category-theoretic interpretation.  We outline this here:
...
**Theorem 1.** The category of connected projective varieties over any field $k$ has finite products and obeys the rigidity lemma.

+-- {: .proof}
######Proof
The hard part, the rigidity lemma, is proved for complete varieties on page 43 of Mumford's _Abelian Varieties_, but projective varieties are complete.  (Do we need connectedness?  If not, we would seem to get a proof that even non-connected projective algebraic groups are abelian.)
=-- 
...
**Corollary 2.** Let $Var_*$ be the category of pointed connected projective varieties, and let $AbVar$ be the category of abelian varieties.  Then the forgetful functor $U : AbVar \to Var_*$ is full.
...
A consequence of Corollary 2 is that if $Alb : Var_* \to AbVar$ is the left adjoint to $U$, sending any connected pointed projective variety to its Albanese variety, the monad $T = U \circ Alb$ is an [[idempotent monad]].  For more on this see [[Albanese variety]] and the nCaf&eacute; discussion [Two miracles in algebraic geometry](https://golem.ph.utexas.edu/category/2016/08/the_magic_of_algebraic_geometr.html).
