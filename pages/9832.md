...
$$
  (C_c(X))^{\ast_{KK}}
  \simeq
  C_{c^{-1}\otimes W_3(\tau_X)}(X)
  \,,
$$

hence by the same manifold but with twist the difference between the third [[integral Stiefel-Whitney class]] and the inverse of the original twist.

=--

This is ([Brodzki-Mathai-Rosenberg-Szabo 06, section 7.3](#BrodzkiMathaiRosenbergSzabo06)) and generalized to the equivariant case ([Tu 06, theorem 3.1](#Tu06)) (wehere we use remark \ref{OppositeConvolutionAlgebraAndInverseTwist} in order to interpret the opposite tiwsted convolution algebra up to equivalence as inducing the inverse twist).
