...
**Lindstr&#246;m's theorems** are important results in classical [[model theory]] due to the Swedish logician Per Lindstr&#246;m that give abstract characterizations of standard (unsorted finitary) [[first-order logic]] ('abstract' in the sense of not referring to the syntactic make up of the formulas).

The _first_ (semantic) one states that the validity of the downward [[Löwenheim-Skolem theorem]] (a sentence that holds in some model already holds in an at most countable model) and the [[compactness theorem]] (a set of sentences has a model if every finite subset has a model) characterizes up to expressive power standard first-order logic among all logics satisfying certain regularity conditions[^fine]:
