...
As a result, in constructive mathematics not every [[Bézout domain]] that is a [[unique factorization domain]] is a [[principal ideal domain]], as $\mathbb{Z}$ is both a Bézout domain and a unique factorization domain, but is not a principal ideal domain. Since unique factorization domains are [[atomic domains]], using the example of $\mathbb{Z}$, in constructive mathematics Bézout atomic domains are also not the same as principal ideal domains. 

Some authors have tried to define a principal ideal domain as a [[Noetherian ring|Noetherian]] Bézout domain, but in constructive mathematics, this definition does not coincide with the definition of principal ideal domain as a [[integral domain]] whose [[ideals]] are all [[principal ideals]], since $\mathbb{Z}$ is [[Noetherian ring|Noetherian]] and a Bézout domain, but it is only a principal ideal domain if and only if [[excluded middle]] holds. Similarly, every Noetherian Bézout domain, such as $\mathbb{Z}$, satisfies the ascending chain condition on principal ideals, but $\mathbb{Z}$ is a principal ideal domain if and only if [[excluded middle]] holds. 

What is currently unknown is if the following conditions are still equivalent to each other on a Bézout domain in constructive mathematics: 
