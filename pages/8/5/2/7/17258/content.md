...
Hence, the category $\mathcal{K}$ has a rich mathematical structure and naturally admits not only finite products $S\times T$ that play an important role already in [Courr&#232;ge (1965)](#Cour65) but also more 'exotic' operations like exponentials $T^S$ and, in particular, power systems $\Omega^S$.

Here $\Omega$ is the truth-value object or [[subobject classifier]] which, in the case of a topos of group actions, is the set $\{\emptyset , \tilde{G}\}\cong\{0,1\}$ with trivial action $0\cdot \tilde{g}=0$ and
...
* Ruth M. Vaz, _Relatives, Molecules and Particles_ , MACT **7** no. 1 (2014). ([pdf](http://mathematicalanthropology.org/pdf/RMVaz_MACT%200614.pdf))

Kinship systems are studied from a mathematical perspective in

* Mauro W. Barbosa De Almeida, _On the Structure of Dravidian Relationship Systems_ , MACT **3** no.1 (2010). ([pdf](http://mathematicalanthropology.org/pdf/BarbosadeAlmeida.0810.pdf))
