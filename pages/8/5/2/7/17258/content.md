...
Since marriages become deterministic on the classes, elementary kinship systems are amenable to analysis by permutations of the marriage classes. This group-theoretic modeling of elementary structures has been pioneered by [[André Weil]] in 1949 and later been systemized by Phillipe Courr&#232;ge in 1965.
...
+-- {: .num_remark}
###### Remark
The idea is that $S$ represents the sets of marriage classes and the _conjugal_ function $\omega$ links a class $x$ to the class $\omega (x)$ of the spouses for $x$ whereas the _maternal_ function $\mu$ links the class $x$ to the class that contains the mothers of the individuals in $x$. One can then define a _paternal_ function $\pi :=\mu\circ\omega$ that links $x$ to the class of spouses of the mothers.
=--
