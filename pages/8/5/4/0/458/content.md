...
For example, there is a locale of all [[surjections]] from [[natural numbers]] (thought of as forming the [[discrete space]] $N$) to [[real numbers]] (forming the [[real line]] $R$).  This locale has no points, since there are no such surjections, but it contains many nontrivial open subspaces; these open subspaces are generated by a family parametrised by $n\colon N$ and $x\colon R$; the basic open associated to $n$ and $x$ may be described as $\{f\colon N \twoheadrightarrow R \;|\; f(n) = x\}$.
