...
Another advantage of locales is that they are better-behaved than topological spaces in [[constructivism|constructive mathematics]] or [[internalization|internal]] to an arbitrary topos.  For example, constructively the topological space $[0,1]$ need not be compact, but the _locale_ $[0,1]$ is always compact (in a suitable sense).  It follows that the locale $[0,1]$, and hence also the locale $R$ of real numbers, is not always spatial.  When it fails to be spatial, because there are "not enough real numbers," the locale is generally a better-behaved object than the topological space of real numbers.
