...
holds precisely if $p$ is a prime ideal. But by definition $V(p)$ consists of all the prime ideals $q$ such that $p \subset q$.

Now of $p$ is maximal, then by definition there is no larger proper ideal, and hence in this case $V(P) = \{p\}$.

Conversely, if $V(\{p\}) = p$ this means that there is no prime ideal larger than $p$ and containing $p$. Hence (...)
...
Let $R = \mathbb{Z}$ be the [[commutative ring]] of [[integers]].
