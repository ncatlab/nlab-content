...
Notice that rules for conjonctions and disjonctions are eerly similar, in particular if $A$ and $B$ are composed only of conjonctions, disjonctions and atoms, noting by $\overline{A}$ and $\overline{B}$ the same formulas but replacing each conjonctions by disjonctions and disjonctions by conjonctions simultaneously, then $A \vdash_{\text{SL}} B$ if and only if $\overline{B} \vdash_{\text{SL}} \overline{A}$. That is what we mean by saying that conjonctions and disjonctions are dual. In the same vein, top and bottom are duals.
...
This rule usually do not have a dual in intuitionistic logic, but we'll just create one written $A - B$, rules of substractions:
...
Giving the syntax of propositional substractive logic, to make it first order, it suffice to add the existential operator and its dual, the forall operator (where $x$ does not occur free in $C$):
