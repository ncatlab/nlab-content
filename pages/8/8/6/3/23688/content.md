[[!redirects Heyting dagger 2-posets]]
...
A **Heyting dagger 2-poset** is a [[dagger 2-poset]] $C$ such that 
...
* For each object $A \in Ob(C)$, $B \in Ob(C)$, $D \in Ob(C)$ with monic maps $i_{B,A} \in Hom(B,A)$, $i_{D,A} \in Hom(D,A)$, there is an object $B \Rightarrow D \in Ob(C)$ with monic map $i_{D,A} \in Hom(D,A)$, 
$$i_{(B \Rightarrow D) \cap D, B}:Hom((B \Rightarrow D) \cap D, B)$$
and 
$$i_{D, B \Rightarrow (D \cap B)}:Hom(D, B \Rightarrow (D \cap B))$$ 
...
* The [[isomorphism classes]] of monic maps into every object $A$ is a [[Heyting algebra]]. Since every monic map is a map, the [[category of maps]] is a [[Heyting category]]. As a result, Heyting dagger 2-posets are the same as [[division allegories]]. 

## Examples ##

The dagger 2-poset [[Rel]] of [[sets]] and [[relations]] is a Heyting dagger 2-poset. 
