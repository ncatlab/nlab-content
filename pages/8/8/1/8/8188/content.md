...
The sequence spaces $c_c$, $c_0$, $c_\infty$, $c_b$, and $K^N$ generalise to the spaces $C_c$, $C_0$, $C_\infty$, $C_b$, and $C$ of [[continuous maps]] on a [[locally compact topological space|locally compact]] [[Hausdorff space]].  In fact, $c_*(N)$ is simply $C_*(N,\tau)$, where $\tau$ is the [[discrete topology]].  (Note that one never uses the symbol '$C$' for '$C_\infty$' with capital letters.)
