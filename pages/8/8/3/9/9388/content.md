...
It is traditional to consider only _[[proper filter|proper]]_ uniform filters, which allows one to leave out (2); but in that case (5) and (6) must be modified to apply only when the generated filters are proper: (5) applies when each element of $F$ [[intersection|meets]] each element of $G$, and (6), if used at all, applies when $X$ is [[inhabited set|inhabited]].
...
$$ \lim_{n \to \infty} {\|a_n - b_n\|} = 0 .$$
The [[eventuality filters]] of the uniform sequences in $X \times X$ form a base for a uniform convergence structure.  The same holds even if $X$ is only a [[pseudometric space]] or an [[extended metric space]]; but if $X$ is only a [[quasimetric space]], then we get only a quasiuniform convergence structure.
