...
a **sieve on** an [[object]] $X$ in a [[category]] $\mathcal{C}$ is a collection of [[morphisms]] with [[codomain]] $X$ that are closed under [[composition|precomposition]] with composable morphisms in $\mathcal{C}$.
...
+-- {: .num_remark #SieveGeneratedByAFamily}
###### Remark

Sometimes the condition of a sieve being closed under the operation of precomposing with an arbitrary morphism $g: e \to d$ is called a "saturation condition".

Given any collection of morphisms $\mathcal{F}=\{d_i\to c\}_i$ targeted at $c$, the *saturation of $\mathcal{F}$* or *sieve generated by $\mathcal{F}$* is the smallest sieve on $c$ containing $\mathcal{F}$ (the intersection of all such sieves): it equals the sieve of morphisms $e\to c$ for which there exists $i$ with factorization $e\to d_i\to c$.
...
If the sieve is generated by (is the saturation of) a collection of morphisms $\{U_\alpha \to U\}$ (see Remark \ref{SieveGeneratedByAFamily}) then the same statement remains true with $U$ being the coproduct over just these $U_\alpha$.
