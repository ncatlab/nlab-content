...
* There are multiple inequivalent notions of a [[set]], because [[denial inequality]] does not have the same properties in constructive mathematics that it does in [[classical mathematics]]. These include objects such as sets with [[stable equality]], sets with [[decidable equality]], sets with [[tight apartness relations]], and finally [[classical sets]], the last of which are precisely the sets in constructive mathematics whose denial inequality has all the same properties as it does in classical mathematics. 
...
* There are multiple inequivalent definitions of [[real numbers]].
  ...
  * That every real number with a locator in the [[unit interval]] can be represented by a [[sequence]] of (positive) digits (i.e. [[radix notation]]) is equivalent to the lesser [[limited principle of omniscience]], so it is no longer true that every such real number has an infinite decimal representation. 
...
On constructive [[commutative algebra]]:
