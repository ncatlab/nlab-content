...
* the cycles of the permutation $\phi = \sigma^{-1}\alpha^{-1}$ give, for each face $f$ of $M$, an ordering of darts on the boundary of that face.  With the above convention for vertices, it is easy to check that $\phi$ computes the clockwise cycle of darts having $f$ to the right.
