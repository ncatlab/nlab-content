...
Note that the actual English word "structure" is used slightly differently in the two cases of category theory and model theory. In category theory, the "structure" is the operations _and_ properties with which we want to equip sets, considered abstractly. But in model theory, the operations are called the "language" and the properties are called the "theory", while a "structure" refers to a _particular_ set (or family of sets) that is equipped with the operations and a "model" refers to a structure which also satisfies the properties.  Thus the category theorist would refer to "the structure of a group" as consisting of a multiplication, a unit, etc., while the model theorist would say that each particular group (like $\mathbb{Z}$) is a model of a theory of groups.  In particular, this entails being a structure for the language of groups, but in general a structure for the language of groups need not satisfy any of the axioms of a group (like associativity or unitality).
