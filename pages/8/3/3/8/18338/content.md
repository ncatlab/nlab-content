...
$\mathrm{lim}_{\mathsf{J}}\colon\mathrm{Ob}( [\mathsf{C},\mathsf{J}] )\rightarrow\mathrm{Ob}(\mathsf{C})$

such that for each $F\in[\mathsf{C},\mathsf{J}]$ the object $\mathrm{lim}_{\mathsf{J}}(F)$ is a limit of $F$.

Now this assignation $\mathrm{lim}_{\mathsf{J}}$ is never uniquely determined except in degenerate situations, so it is _non-canonical_, but basic category teaches that this assignation is _functorial_:  $\mathrm{lim}_{\mathsf{J}}$ can be _extended_ to the class $\mathrm{Ob}(\mathsf{C})\cup\mathrm{Mor}(\mathsf{C})$ to become a full-fledged functor $[\mathsf{C},\mathsf{J}] \rightarrow\mathsf{C}$. 
...
+-- {: .query}
(here someone may include a look at unnatural isomorphism from the point of view of higher category theory)
=--
