...
The interval category in [[homotopy type theory]] is more commonly called the **interval preorder** or the **interval poset**, and consists of the booleans type $\mathbb{2}$ and a [[higher inductive type]] [[type family|family]] $\leq_\mathbb{2}$ on the [[booleans type]] $\mathbb{2}$ generated by 

* a term $p:0 \leq_\mathbb{2} 1$

* a dependent function 
$$\mathrm{refl}:\prod_{i:\mathbb{2}} i \leq_\mathbb{2} i$$

As a type, the interval category is [[equivalence of types|equivalent]] to the [[booleans type]]. Hence, we use the symbol $\mathbb{2}$ rather than the symbol $\mathbb{I}$, since $\mathbb{I}$ is reserved for the [[interval type]], which is [[contractible type|contractible]]. 
