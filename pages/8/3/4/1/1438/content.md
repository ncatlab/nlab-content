...
+-- {: .un_thm}
######Theorem (Ad&#225;mek) 
Let $C$ be a category with an [[initial object]] $0$ and [[colimits]] of chains $\omega \to C$ (where $\omega$ is the first infinite [[ordinal]]), and suppose $F: C \to C$ preserves colimits of $\omega$-chains. Then the colimit $\gamma$ of the chain 
...
This theorem applies in particular to any functor $F: Set \to Set$ which is a colimit of finitely representable functors $hom(n, -): X \mapsto X^n$, as in the following examples. 
