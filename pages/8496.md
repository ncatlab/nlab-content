...
Since the [[ground field]] has [[characteristic zero]], [[group averaging]] provide a projection

$$
  p \;\colon\; V_\bullet \to V_\bullet^G
$$

onto the $G$-invariants. 

Now a [[chain homology]]-class $[x] \in H_\bullet((V_\bullet,\partial))$ being $G$-invariant means that $g[x] \coloneqq [g(x)] = [x]$ for all $g \in G$, which implies that $[x] = [p(x)]$. 

This means that each invariant homology class has an invariant representative, hence that the map
