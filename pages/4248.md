Hodge numbers are the dimensions of certain cohomology spaces for a compact complex manifold. More precisely,
$$
h^{p,q}(X)=dim_\mathbb{C}H^q(X;\Omega^p),
$$
where $\Omega^p$ is the sheaf of holomorphic $p$-forms on $X$. If $dim_\mathbb{C}(X)=n$, then $h^{p,q}(X)=h^{n-p,n-q}(X)$; in particular, $h^{n,n}(X)=1$.
