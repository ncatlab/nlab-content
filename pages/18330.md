...
* A dagger precategory is a precategory $\mathcal{C}$ with a contravariant identity-on-objects prefunctor $\dagger:\mathcal{C} \to \mathcal{C}^\op$ such that for all objects $A:\mathcal{C}$ and $B:\mathcal{C}$ and morphisms $f:Hom(A,B)$, $(f^\dagger)^\dagger = f$. 
