...
* axioms $\top\vdash_{x:A,y:B}\underset{A\overset{f}{\leftarrow} C\overset{g}{\rightarrow B}}{\bigvee}\exists (z:\lceil C\rceil)(\lceil f\rceil (z:\lceil C\rceil)=x:\lceil A\rceil\wedge \lceil g\rceil (z:\lceil C\rceil)=y:\lceil B\rceil)$ where the disjunction ranges over all cones on the discrete diagram consisting of $A$ and $B$.
...
* Let $(\emptyset,\emptyset)$ be the empty site. Then the signature $\Sigma$ is empty and only the third condition takes hold, yielding $\mathbb{T}_\emptyset^{\emptyset}=\{\top\vdash\bot\}$ i.e. the empty topos $Sh(\emptyset,\emptyset)=\mathbf{1}$ classifies the **inconsistent theory** whose models are precisely the [[initial objects]].
