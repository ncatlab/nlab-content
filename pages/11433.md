...
The actual _constants of nature_, when they are "dimensionful", are instead elements of these abstract torsors. For example, the [Planck length](https://en.wikipedia.org/wiki/Planck_length) belongs to the torsor of length, a torsor over $\mathbb{R}_{\gt0}^\times$. We can describe it by its ratio to another element of the same torsor, such as the meter — the Planck length is approximately $1.616 \cdot 10^{-35} \mathrm{m}$ — but the number appearing in this statement is completely driven by our arbitrary choice of the meter.

An actual constant of nature corresponds to a specific, non-arbitrary real number only when it is a _dimensionless_ quotient of physical units. For instance the [[fine structure constant]] is the [[quotient]]
