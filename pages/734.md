...
A **weak limit** for a diagram in a [[category]] is a [[cone]] over that diagram which satisfies the existence, but not the uniqueness, property that defines a [[limit]].

For example, a **weak pullback** of a [[cospan]] $A\overset{f}{\to} C \overset{g}{\leftarrow} B$ consists of a commutative square
...
A weak limit is not to be confused with a [[homotopy limit]] or a [[2-limit]], which satisfy both existence and uniqueness up to higher [[homotopy|homotopies]] or [[equivalence|equivalences]].

Some homotopy limits induce the corresponding type of weak limit in the corresponding [[homotopy category]].  For example, suppose that
$$\array{ P & \overset{p}{\to} & A\\
  ^q \downarrow && \downarrow ^f\\
  B & \overset{g}{\to} & C}
$$
