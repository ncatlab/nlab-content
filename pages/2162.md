...
Given any group $G$ with a [[presentation]] $\langle X; R\rangle = F/N$ such that $F=\langle X\rangle$ is the [[free group]] on the set of letters $X$ and $N$ the [[normal subgroup|normal closure]] of the set of relations $R$, let $\bar{G}:=G/[G,G]$, let $\phi:F\to G$, $\bar\phi:F\to \bar{G}$ be the canonical projections; denote by the same letter their linearizations for group rings $\phi:\mathbb{Z}F\to \mathbb{Z}G$ and $\bar\phi:\mathbb{Z}F\to\mathbb{Z}\bar{G}$. The __Jacobi matrix__ of the presentation is the matrix 
