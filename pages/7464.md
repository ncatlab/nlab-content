...
Here is a proof of theorem \ref{IF} from theorem \ref{IT} written in synthetic differential geometry. Let $\epsilon$ be a nilpotent. Let $S_\epsilon$ denote the circle of radius $\epsilon$ centered at $\zeta$. Since $z\mapsto f(z)/(z-\zeta)$ is holomorphic on the region bounded by $C$ and $S_\epsilon$, by theorem \ref{IT}, 
$$\oint_C \frac{f(z)}{z - \zeta} \,\mathrm{d}z = \oint_{S_\epsilon} \frac{f(z)}{z - \zeta} \,\mathrm{d}z.$$
Parametrize $S_\epsilon$ by $(t\mapsto \zeta + \epsilon \exp(2\pi\mathrm{i}t))$ to transforms the above integral to
$$\int_0^1 \frac{f(\zeta + \epsilon \exp(2\pi\mathrm{i}t))}{\epsilon \exp(2\pi\mathrm{i}t))} \,\mathrm{d}(\zeta + \epsilon \exp(2\pi\mathrm{i}t))) = 2\pi\mathrm{i}\int_0^1 f(\zeta + \epsilon \exp(2\pi\mathrm{i}t)) \, \mathrm{d}t$$
...
$$2\pi\mathrm{i}\int_0^1 \left[f(\zeta) + \epsilon\exp(2\pi\mathrm{i}t) \frac{\partial f(\zeta)}{\partial z}\right] \, \mathrm{d}t = 2\pi\mathrm{i}\left[f(\zeta) + \epsilon\frac{\partial f(\zeta)}{\partial z} \int_0^1 \exp(2\pi\mathrm{i}t)  \, \mathrm{d}t\right] = 2\pi\mathrm{i}f(\zeta).$$
