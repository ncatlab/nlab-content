...
Here is a proof of [[Cauchy's integral formula]], theorem \ref{IF} from [[Cauchy's integral theorem]], theorem \ref{IT}, written in [[synthetic differential geometry]]: 

Let $\epsilon$ be a nilpotent. Let $S_\epsilon$ denote the circle of radius $\epsilon$ centered at $\zeta$. Since $z\mapsto f(z)/(z-\zeta)$ is holomorphic on the region bounded by $C$ and $S_\epsilon$, by [[Cauchy's integral theorem]], theorem \ref{IT}, 
