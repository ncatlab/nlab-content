...
If $T: Set \to Set$ is a monad on $Set$, with corresponding monadic functor $U: Set^T \to Set$, then the left adjoint $F: Set \to Set^T$ is comonadic provided that the unique algebra map $!: F(0) \to F(1)$ is a [[regular monomorphism]] and not an [[isomorphism]]. In particular, if $T$ is given by an [[algebraic theory]] with at least one constant and a model with more than one element, then the left adjoint is comonadic (if the initial algebra $F(0)$ is nonempty, then the unique algebra map $!: F(0) \to F(1)$ is a split monomorphism since any constant $c: 1 \to F(0)$ induces an algebra map $F(1) \to F(0)$ that retracts $!$; on the other hand there is more than one algebra map $F(1) \to M$ if $M$ has more than one element, so $F(1)$ isn't itself initial). 
