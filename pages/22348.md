...
Note that to give a morphism $T^\leq_\bullet \to M_\bullet$
is the same as to give a a tree of parameters $(a_s\,\,|\,\,\emptyset \not= s \in {}^{\lt\omega}\omega )$ satisfying item b,
in notation of the definition. 

Let $T^{antichains}_\bullet$ be the simplicial set of 
antichains of $T^\leq$, namely 
$T^{antichains}_\bullet(n^\leq)$ is the set of 
...
equivelently, a subset is small iff there is no infinitely branching subtree of $T^\leq$ such that it contains all 
tuples of sublings in that subtree. 
...
Finally, let $T^\leq_\bullet\cup T^{antichains}_\bullet$
denote, as notation suggests, be the situs defined by:
 
i. $(T^\leq_\bullet\cup T^{antichains}_\bullet)(n)=T^\leq_\bullet(n)\cup T^{antichains}_\bullet(n)$.

ii. a subset of $(T^\leq_\bullet\cup T^{antichains}_\bullet)(n)$
is large iff it can be represented as a union of large subset
of $T^\leq_\bullet(n)$ 
and a large subset of $T^{antichains}_\bullet(n)$.
...
ii. in s&#x12CB; there is no morphism $\tau:T^\leq_\bullet\to M_\bullet$ such that
...
$T^\leq_\bullet \to T^\leq_\bullet\cup T^{antichains}_\bullet
\rightthreetimes M_\bullet\to \top $
\end{proposition}
...
above the vertex a copy of ${}^{\lt n}\omega$. 
