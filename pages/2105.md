...
Given a variety $V$ and a [[cartesian monoidal category]] $C$ (such as $Set$), let $C^V$ be the category of algebras in $V$ in $C$, with homomorphisms as morphisms.  There is an obvious [[forgetful functor]] $U\colon C^V \to C$; $C$ is a [[cocomplete category|cocomplete]] $W$-[[W-pretopos|pretopos]] (or if $C$ is just a $W$-pretopos if $V$ is finitary), then $U$ has a [[left adjoint]], the [[free functor]] $F\colon C \to C^V$.  In fact, this adjunction is [[monadic adjunction|adjunction]]; we can identify the models of $V$ in $C$ with the [[module of a monad|modules/algebras]] of the [[monad]] $U \circ F$.
