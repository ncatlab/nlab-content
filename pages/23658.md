...
A __Cauchy structure__ is a $\mathbb{Q}_{+}$-[[premetric space]] $(S, \sim)$ with a function $rat:\mathbb{Q}_{+} \to S$ and a function $lim:C(S) \to S$, where $C(S)$ is the set of [[Cauchy approximation]]s in $S$, such that
...
A __Cauchy structure__ is a $\mathbb{Q}_{+}$-[[premetric space]] $S$ with 
