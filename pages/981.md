...
Denoting the generator $q(\tau_n)$ of $\Aut_\Lambda([n])$ also by $\tau_n$, we saw $\tau_n^{n+1} = \mathrm{id}_{[n]}$. The development above allows a (perhaps more standard, and equivalent) presentation of $\Lambda$ by [[presentation of a category by generators and relations|generators and relations]]. In addition to the cosimplicial identities between the coboundaries $\delta_i$ and codegeneracies $\sigma_j$ and $\tau^{n+1}_n = \mathrm{id}$ there are following identities:
$$\array{
...
We reiterate the development in Section \ref{structure} in summary form: 
