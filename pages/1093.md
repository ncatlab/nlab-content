...
1.  $f$ is an [[epimorphism]] precisely if it is so _locally_, in that: for all $U \in C$ and every element $y \in Y(U)$ there is a [[covering]] $\{p_i : U_i \to U\}_{i \in I}$ such that for all $i \in I$ the element $Y(p_i)(y)$ is in the image of $f(U_i) : X(U_i) \to Y(U_i)$.
