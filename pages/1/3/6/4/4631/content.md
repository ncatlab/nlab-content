...
For instance, a major source of augmented [[simplicial object]]s is given by [[colimit]]s or rather [[homotopy colimit]]s over simplicial diagrams in a [[model category]]: for $X\colon \Delta^{op} \to C$ a simplicial object in a category with colimits, its colimit [[cocone]] may be thought of as a morphism 
...
Denote by $\Delta_+$ (also denoted $\Delta_a$) the [[augmented simplex category]], which may be defined as the [[full subcategory]] of [[Cat]] on the [[free categories]] over finite and _possibly empty_ linear [[directed graphs]], which are
...
There is a canonical inclusion $\Delta \hookrightarrow \Delta_+$ of the ordinary [[simplex category]] and the [[restriction]] of an augmented simplicial set along this inclusion is a simplicial set. This gives a [[forgetful functor]]
...
It follows that $d^{-1}$ coequalizes every pair of composites of face maps $X_n \to X_0$, so that an augmented simplicial set $X_\bullet$ is equivalent to a morphism of ordinary simplicial sets
...
We say that $U(X_\bullet)$ is _augmented_ over $X_{-1}$.
...
While this numbering is very nice for augmented simplicial sets, it is not standard, and it can be easily misunderstood, so we do not use it in this article.
...
* For $C$ a [[site]], $\{U_i \to X\}$ a [[covering]] family, we have the [[Cech nerve]] [[simplicial presheaf]] $C(U) \in [C^{op}, sSet]$. This comes equipped with a canonical morphism of simplicial presheaves to the one represented by $X\colon C(U) \to X$. This is an augmented simplicial object in the category of presheaves $[C^{op}, Set]$. Moreover, for each object $V \in C$, its component
