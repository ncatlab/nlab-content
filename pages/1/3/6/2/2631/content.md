A _generating function_ is an element of $R[ [z] ]$, the ring of formal power series over the rig $R$.  A general element takes the form
\[f(z) = \sum_{n=0}^{\infty}f_n z^n.\]
If the power series has a radius of convergence $r \ge 0$ and if there's an analytical continuation, then we also say that the continuation is the generating function.

When $R = \mathbb{N},$ we can think of the coefficients on $z^n$ as counting the number of ways to put a particular structure on the finite set $n$.  (You get [structure types](structure+type) if you take this literally.)  
