...
Therefore, each point defines a [[completely prime filter]] on the frame of all opens, which is the definition of a [[point of a locale|point]] in general locale theory.  Conversely, given such a completely prime filter $P$, let $L$ be the set of all rational numbers $a$ such that, for every $G \in P$, $(b,c) \in G$ for some $c \gt b \gt a$; and let $U$ be the set of all $a$ such that, for every $G \in P$, $(c,b) \in G$ for some $c \lt b \lt a$.  Then ... (is this right? why is this a real number?).
