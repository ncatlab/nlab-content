...
Therefore, each point defines a [[completely prime filter]] on the frame of all opens, which is the definition of a [[point of a locale|point]] in general locale theory.  Conversely, given such a completely prime filter $P$, let $L$ be the set of all rational numbers $a$ such that the open interval $(a, \infty)$ (see below for the definition of this as a binary relation) is in $P$, and symmetrically let $U$ be the set of all $a$ such that the open interval $(-\infty, a) \in P$.
