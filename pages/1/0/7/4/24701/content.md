...
The univalent Tarski universe of a type family $(A; x:A \vdash B(x))$ is a higher inductive-inductive type $(\mathrm{Univ}_{x:A.B(x)}; x:\mathrm{Univ}_{x:A.B(x)} \vdash \mathrm{El}_{x:A.B(x)}(x))$ generated by 

* a function $\eta:A \to \mathrm{Univ}_{x:A.B(x)}$

* a dependent function $\delta:\prod_{x:A} B(x) \to \mathrm{El}_{x:A.B(x)}(\eta(x))$

* a dependent function $\upsilon:\prod_{x:\mathrm{Univ}_{x:A.B(x)}} \prod_{y:\mathrm{Univ}_{x:A.B(x)}} \mathrm{isEquiv}(\mathrm{transport}_{\mathrm{El}_{x:A.B(x)}}(x,y))$

The rules for univalent Tarski universes are as follows:

Formation rules for univalent Tarski universes:

$$\frac{\Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type}}{\Gamma \vdash \mathrm{Univ}_{x:A.B(x)} \; \mathrm{type}} \qquad \frac{\Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type}}{\Gamma, T:\mathrm{Univ}_{x:A.B(x)} \vdash \mathrm{El}_{x:A.B(x)}(T) \; \mathrm{type}}$$

Introduction rules for univalent Tarski universes

$$\frac{\Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type}}{\Gamma \vdash \mathrm{code}:A \to \mathrm{Univ}_{x:A.B(x)}} \qquad \frac{\Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \quad \Gamma \vdash a:A}{\Gamma \vdash \mathrm{def}(a):B(a) \to \mathrm{El}_{x:A.B(x)}(\mathrm{code}(a))}$$

$$\frac{\Gamma \vdash A \; \mathrm{type} \quad \Gamma, x:A \vdash B(x) \; \mathrm{type} \quad \Gamma \vdash S:\mathrm{Univ}_{x:A.B(x)} \quad \Gamma \vdash T:\mathrm{Univ}_{x:A.B(x)}}{\Gamma \vdash \mathrm{ua}(S, T):\mathrm{isEquiv}(\mathrm{transport}_{\mathrm{El}_{A, B}}(x,y))}$$
