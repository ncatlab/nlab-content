...
On the other hand, we don\'t need this as a separate notion, since we could work with $C$ directly instead of $X$ or $\Gamma$.
...
Given a [[real number|real]]-valued function $F$ defined on a real [[interval]] $[a,b]$, we can speak of tagged partitions of $[a,b]$ and consider the [[Riemann sum|Riemann sums]] of $F$ on these tagged partitions.  Let $X$ and $Y$ each be the [[real line]], let $\Gamma$ be the set of tagged partitions of $[a,b]$, let $g\colon \Gamma \to X$ map a partition to the maximum length of its parts (often called the norm or mesh of the partition), and let $f\colon \Gamma \to Y$ map a tagged partition to the Riemann sum of $F$ on it, and let $c$ be the real number $0$.  Then the [[Riemann integral]] of $F$ on $[a,b]$ is defined to be the limit of $f$ as $g$ approaches $c$.  That is, the Riemann integral of a function on an interval is the limit of the Riemann sum of the function on a tagged partition of the interval as the mesh of the partition approaches zero.  Or writing $\sum_P F$ for the Riemann sum of $F$ on the partition $P$ and $\|{P}\|$ for the partition\'s mesh, the integral is $\lim_{{\|{P}\|} \to 0} \sum_P f$.

Redoing the previous example in a more roundabout way, let $X$ and $Y$ still each be the real line, $\Gamma$ now be the [[disjoint union]] $\biguplus_{n=0}^\infty (\mathbb{R}^{n+1} \times \mathbb{R}^n)$ (essentially the set of odd-length [[tuples]] of real numbers, but with each tuple decomposed into a pair of tuples), let $C$ be the subset of $\Gamma$ consisting of those pairs of tuples that specify a tagged partition of $[a, b]$, let $g\colon \Gamma \to X$ and $f\colon \Gamma \to Y$ be the obvious extensions of $g$ and $f$ from the previous paragraph, and let $c$ be $0$ again.  Then we can write
$$ R\int_a^b F \coloneqq \lim_{\max_{i=1}^n (x_i - x_{i-1}) \to 0 \atop a = x_0 \leq t_1 \leq x_1 \leq \cdots \leq x_{n-1} \leq t_n \leq x_n = b} \sum_{i=1}^n F(t_i) (x_i - x_{i-1}) ,$$
an expression found in calculus textbooks but regarded with some suspicion, now with perfect rigour; the only things not made explicit on the right-hand side are the types of the dummy variables ($i$ and $n$ are natural numbers, and the $x$s and $t$s are real numbers) and the dots (which say that $x_{i-1} \leq t_i \leq x_i$ for all $i$ from $1$ to $n$).  But for studying the Riemann integral, it\'s probably better to work directly with the set of partitions.

For the [[Henstock–Kurzweil integral]], we can use the same $\Gamma$ and $Y$ (and $f$), but now $X$ is more complicated (with $g$ and even $c$ to match).  Specifically, $X$ is the set of possibly degenerate gauges on $[a,b]$, that is functions from $[a,b]$ to $[0,\infty]$ (the degeneracy is that $0$ is allowed as a value of the gauge) with the [[product topology]]; $c$ is the fully degenerate gauge whose only value is $0$; and $g$ takes a tagged partition (or pair of tuples) $(x,t)$ to the degenerate gauge where $g(x,t)(t_i) = \max(x_i - t_i, t_i - x_{i-1})$ but every other value of $g(x,t)$ is $0$.  Note that a neighbourhood of $c$ in $X$ is essentially the same as a (non-degenerate) gauge on $[a,b]$ (more precisely the gauges define a neighbourhood base).  We might write
$$ \int_a^b F \coloneqq \lim_{\bigl(\max(t_1 - x_0, x_1 - t_1), \ldots, \max(t_n - x_{n-1}, x_n - t_n)\bigr) \to (0, \ldots, 0) \atop a = x_0 \leq t_1 \leq x_1 \leq \cdots \leq x_{n-1} \leq t_n \leq x_n = b} \sum_{i=1}^n F(t_i) (x_i - x_{i-1}) ,$$
where we\'re suppressing all mention of the values of $g(x,t)$ at points other than the tags (since they\'re all already $0$).
