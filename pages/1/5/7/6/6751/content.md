...
$G$ a suitable ([[semisimple Lie group|semisimple]] [[compact topological group|compact]]) [[Lie group]].

$\mathfrak{g}$ the [[Lie algebra]]

$T \hookrightarrow G$ inclusion of [[maximal torus]]

$\mathfrak{t} \hookrightarrow \mathfrak{g}$ the corresponding [[Cartan subalgebra]]

for $\lambda \in \mathfrak{g}^*$ write 
...
$$
  \mathcal{O}_{\lambda} = \{ Ad_g^*(\lambda) \in \mathfrak{g}^* | g \in G \}
  \,.
$$

If $G_\lambda \hookrightarrow G$ is the [[stabilizer subgroup]] of $\lambda$ under the coadjoint action, then there is an equivalence
...
$$
  g G_\lambda \mapsto Ad_g^* \lambda
  \,.
$$

For generic values of $\lambda$ we have $G_\lambda \simeq T$. Say that such $\lambda$ are **regular**. (The value far from regularity is $\lambda = 0$ for which $G_\lambda = G$ instead.)

### The symplectic form

Write $\theta \in \Omega^1(G, &#223;mathfrak{g})$ for the [[Maurer-Cartan form]] on $G$

Write 

$$
  \Theta_\lambda := \langle \theta, \theta \rangle \im \Omega^1(G)
$$
...
This descends to a closed $G$-invariant 2-form on the quotient, to be denoted by the same symbol 
...
One finds that this is non-degenerate and hence defines a [[symplectic form]] on $G/G_\lambda$.
