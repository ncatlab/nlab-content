...
Over the years (as on the FOM mailing list roughly a dozen years ago, as of revision number 4 of this article), various people have "challenged" category theorists to write down a fully formal presentation of ETCS. We will do so here. 
...
* **Convention:** The notation $f \circ g$ denotes the unique $h$ for which $c(f, g, h)$, whenever it exists. 
...
$$(\forall_{g, h} (s(g) = s(h) \wedge t(g) = t(h) = s(f) \wedge f \circ g = f \circ h) \Rightarrow g = h) \vdash \exists_i (s(i) = t(f) \wedge t(i) = s(f) \wedge f \circ i = t(f))$$
