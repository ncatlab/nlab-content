...
that sends a function on $G^{\times k}$ to its differential at the identity. If $G$ is simply connected, this morphism is an isomorphism on cohomology. Hence the cosimplicial algebra $C^\infty(\mathbf{B}G)$ is weakly equivalent to the (cosimplicial version of) the Chevalley-Eilenberg algebra.

(for instance theorem 6.9.1 in Azcarr&#225;rraga, Izquierdo, _Lie groups, Lie alghebras, cohomology and some applications in physics_) .
