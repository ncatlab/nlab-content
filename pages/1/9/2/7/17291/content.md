...
is inhabited; this is equivalent to the claim that for pairwise distinct $S_1, \ldots, S_m, T_1, \ldots, T_n \in 2^X$ there exists $\phi \in Bool(X)$ such that $\langle \phi, S_i \rangle = 1$ and $\langle \phi, T_j \rangle = 0$. But this is clear because $\phi \mapsto \langle \phi, - \rangle$ is an isomorphism $Bool(X) \to CH(2^X, 2)$ by [[Stone duality]], and there always exists a [[continuous map]] $\phi: 2^X \to 2$ taking the points $S_i$ to $1$ and the points $T_j$ to $0$ (say by the [[Urysohn lemma]] applied to the compactum $2^X$). 
