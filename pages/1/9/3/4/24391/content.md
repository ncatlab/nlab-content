...
\begin{example}
 Let $A=\{\underline{u}, \underline{0}, \underline{1}\}$ denote the quotient space obtained as the coequalizer of a parallel pair of distinct maps, say $\frac{1}{3}: \one \rightarrow [0,1]$ and $\frac{2}{3}: \one \rightarrow [0,1]$,   in $\mathbf{SCvx}$, where $[0,1]$ has the natural super convex space structure. The quotient space $A$ has the (super) convex space structure specified by $r \underline{0} + (1-r) \underline{u} = \underline{u}$ for all $r \in (0,1)$, $r \underline{1} + (1-r) \underline{u} = \underline{u}$ for all $r \in (0,1)$, and $r \underline{0} + (1-r) \underline{1} = \underline{u}$ for all $r \in (0,1)$.
 
 Consider the category $Ho(\mathbf{SCvx})$ where the objects are objects of $\mathbf{SCvx}$ and the morphisms are homotopy equivalence classes of countably affine maps.  We say two countably affine maps $m,k: A \rightarrow B$ are homotopy equivalent if and only if there exists  $\mathbf{SCvx}$-isomorphisms $\phi: A \rightarrow A$ and $\psi: B \rightarrow B$ such that 
 the $\mathbf{SCvx}$-diagram
 \begin{centre}
 \begin{tikzpicture}
      \node   (A)  at  (0,0)   {$A$};
      \node   (A2) at  (0,1.3) {$A$};
      \node  (B)  at  (2,1.3)  {$B$};
      \node  (B2) at  (2,0)  {$B$};
      \draw[>->>,left] (A) to node {$\phi$} (A2);
      \draw[->,above] (A2) to node {$m$} (B);
      \draw[>->>,right] (B) to node {$\psi$} (B2);
      \draw[->,below] (A) to node {$k$} (B2);
 \end{tikzpicture}
 \end{centre}
 commutes.  In $\mathbf{SCvx}$ the only isomorphism $\mathbb{N} \rightarrow \mathbb{N}$ is the identity map, and hence each equivalence classes of morphisms $\mathbb{N} \rightarrow \mathbb{N}$ is a singleton set.  The isomorphisms $\Delta_{\mathbb{N}}\rightarrow \Delta_{\mathbb{N}}$ are of the form $\Delta_{\phi}: \Delta_{\mathbb{N}} \rightarrow \Delta_{\mathbb{N}}$ where $\phi$ is any permutation on $\mathbb{N}$, mapping $\delta_i \mapsto \delta_{\phi(i)}$.
 
 Up to homotopy equivalence, the only countably affine maps $m:A \rightarrow \mathbb{N}$ are of the form $m(\underline{u})=m(\underline{0})=i$  and $m(\underline{1})=i+k$ for all $i,k \in \mathbb{N}$.  (One can take an isomorphism on $A$ interchanging  $0$ and $1$, to obtain another element in each of the equivalence classes.)  Due to the condition 
that $r \underline{0} + (1-r) \underline{1} = \underline{u}$ for all $r \in (0,1)$ the only countably affine maps $m: A \rightarrow \Delta_{\mathbb{N}}$ are constant maps. 

 
 Endowing $A$ with the smallest $\sigma$-algebra such that the morphisms into $\mathbb{N}$, endowed with the discrete $\sigma$-algebra, are all measurable functions yields, up to an isomorphism, the measurable space $\mathbf{\Sigma}{A} = (A, \{ \{\underline{u}, \underline{0}\}, \{\underline{1}\}, A, \emptyset\}$ which is not a separated measurable space, but it is isomorphic to the separated measurable space $(\mathbf{2}, \mathcal{P}{\mathbf{2}})$.
Moreover there exists a countably affine  map $\epsilon_A: \mathcal{G}(\mathbf{\Sigma} A) \rightarrow A$ given by
\begin{equation} 
\begin{array}{ccl}
\mathcal{G}\big((\mathbf{2}, \mathcal{P}{\mathbf{2}})\big) & \longrightarrow & A \\
(1-r) \delta_0 + r \delta_1 & \mapsto & \left\{ \begin{array}{ll} \underline{u} & \forall \, r \in (0,1) \\
\underline{0} & iff \, r=0 \\
\underline{1} & iff \, r=1 \end{array} \right.
\end{array}.
\end{equation}
(Here we view $\mathcal{G}$  as the Giry (probability) monad into $\mathbf{SCvx}$ rather than as an endofunctor on $\mathbf{Meas}$.)
Thus, by probing $A$ with maps to the two objects $\mathbb{N}$ and $\Delta_{\mathbb{N}}$,  we have recovered the original space $[0,1] \cong \mathcal{G}((\mathbf{2}, \mathcal{P}{\mathbf{2}}))$ we started with prior to constructing $A$ as a coequalizer of two points  $\frac{1}{3},\frac{2}{3}: \mathbf{1} \rightarrow [0,1]$.
\end{example} 
