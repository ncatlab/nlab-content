...
### Equivalent localization
 {#EquivalenceLocalization}

We discuss a different set of morphisms, such that the model structure $[\Omega^{op}, sSet]_{gReedy}$ localized at it still coincides with the localization $[\Omega^{op}, sSet]_{cSegal}$ from def. \ref{ModelStructures}. This different localization 
makes more immediate the [[Quillen equivalence]] to the [[model structure on dendroidal sets]]
that we discuss below in in _[Relation to dendroidal sets](http://ncatlab.org/nlab/show/model+structure+for+dendroidal+complete+Segal+spaces#RelationToDendroidalSets)_.

Notice that, by the discussion there, the _[[model structure on dendroidal sets]]_, $dSet_{CM}$, is a [[cofibrantly generated model category]]. Accordingly, there is a set of generating acyclic cofibrations, which we will write
...
1. it contains a morphism $\eta \to J$, where $J$ is the [[codiscrete groupoid]] on two objects, regarded as a unary operad, regarded as a dendroidal set.

+-- {: .num_prop #LocalizationAtWeakOperadicEquivalences}
###### Proposition

The model structure $[\Omega^{op}, sSet]_{cSegal}$ coincides with the [[Bousfield localization of model categories|left Bousfield localization]] of $[\Omega^{op}, sSet]_{gReedy}$ at the set of [[pushout-product]] morphisms

$$
  \left\{
    (A \to B) \bar \cdot ( \partial \Delta[n] \to \Delta[n] ) 
  \right\}_{(A \to B) \in S, n \in \mathbb{N}}
  \,.
$$
...
$A \otimes_{BV}(J \to \eta)$ is a weak equivalence in 
$[\Omega^{op}, sSet]_{cSegal}$.

Moreover, every "$J$-anodyne extension" is a weak cSegal-equivalence,
meaning every morphism generated by [[pushout]], [[transfinite composition]]
and [[retracts]] from the [[pushout-products]] of $\{e\} \to J$ with 
tree boundary inclusions.

=--

([Cis-Moer, prop. 6.3](#CisinskiMoerdijk))

+-- {: .proof}
###### Proof

For the first statement, 
it is sufficient to show that the morphism is a weak equivalence regarded in the 
...
The [[category of elements]] $\Delta \times \Omega/A$ is a "regular skeletal category"
in the sense of _[[Cisinski model structure]]_ theory. By a lemma there, 
[[natural transformations]] between functors preserving [[colimits]] and [[monomorphisms]]
are componentwise weak equivalences is they are so on representables.

Now $J \otimes (-)$ does preserve colimits and monomorphisms, and on
representables the transformation $J \otimes (-) \to \eta \otimes (-)$
is a cSegal-equivalence by definition. 
...
+-- {: .proof}
###### Proof
of prop. \ref{LocalizationAtWeakOperadicEquivalences}
...
By prop. \ref{LocalizationAtWeakOperadicEquivalences} and the
basic nature of left Bousfield localization.
