...
In quantum computing research, there is currently much interest in [[non-abelian anyons]] on spaces that have topology of a network, or 1-D CW-complex. For [[anyons]] placed in a topological space $X$, the configuration space $C_n(X) is defined by:
$$
  C_n(X):=\left(X^n-\Delta_n \right)/S_n
$$
where $\Delta_n:= (x_1, \ldots, x_n)$ such that $x_i \neq x_j$ whenever $j \neq i$ is the [fat diagonal] of $X_n$ and $S_n$ is the [[permutation group]] that permutes particles in $X_n$. 
