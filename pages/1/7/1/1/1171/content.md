...
Note that the definition of 'convergence' varies in the literature; at the extreme end, one could define it as any relation whatsoever from $\mathcal{F}S$ (or even from the [[proper class|class]] of all nets on $S$, see [[preconvergence space]]) to $S$, but that is so little structure as to be not very useful.  An intermediate notion is that of [[filter space]], in which (3) is not required.  Here we follow the terminology of [Lowen-Colebunders](#LC).
