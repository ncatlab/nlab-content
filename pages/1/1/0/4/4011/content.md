...
+-- {: mynumdef #DefDual}
###### Definition
We define the **smooth dual** of $C^\infty(N,E)$ to be the vector space generated by the following linear functionals $C^\infty(N,E) \to \mathbb{R}$.  For $\alpha \in C^\infty(\mathbb{R}, N)$, $\phi \in E^*$, $k \in \mathbb{N}_0$, and $t \in \mathbb{R}$ define a functional by

$$
\big(g \colon N \to E\big) \mapsto (\phi \circ g \circ \alpha)^{(k)}(t).
$$

We write the smooth dual as $C^\infty(N,E)^{*\infty}$.
...
###### Proposition
With the weak smooth topology, $C^\infty(N,E)$ is a convenient vector space.
=--

+-- {: myproof}
###### Proof
_Exercise_ (not really)
=--
