...
A **valuation ring** is an [[integral domain]] $O$ which satisfies any of the following equivalent conditions: 
...
\begin{proof}
$1 \Rightarrow 2$. Let $M$ be a torsion-free $V$-module and let $\sum_{0 \leq i\leq n} v_i m^i = 0$ be a linear relation in $M$. One can assume that $v_0 \in V$ is the element with the minimal valuation. It follows that for every $i \gt 0$ , $v_i$ is divisible by $v_0$ in V. Now since $M$ is torsion-free, it means that we can divide the whole linear equation by $v_0$ and reduce to the case where $v_0 = 1$, that is $m^0 + \sum_{1 \leq i \leq n} v_i m^i = 0$.
Let $b_i \coloneqq m_i$ for $i \gt 0$ and define $a^i_j$ for $0 \leq i \leq n$ and $0 \lt j \leq n$ with $a^{j+1}_j \coloneqq 1$, $a^0_j \coloneqq - v_j$ for every $0 \lt j$ and $a^i_j  \coloneqq 0$ otherwise.
Then $m^i = \sum_j a^i_j b^j$ for every $0 \leq i$ and $\sum_i v_i a^i_j = 0$ for every $0 \lt j$.

$2 \Rightarrow 3$. We must show that $Tor_2^V(M, N) = 0$ for every $V$-modules $M, N$. Since $Tor_2$ commutes with [[filtered colimits]], it is enough to show this for every finitely generated $M$. Using the [[long exact sequence]] for Tor, one can further reduce to the case where $M$ is a cyclic module $V/I$.
By assumption, since all ideals $I \subset V$ are torsion-free, they are flat, that is $Tor_1^V(I, N) = 0$ for every $V$-module $N$. Tensoring the short exact sequence $0 \to I \to V \to V/I \to 0$ with $N$, one gets $Tor_2^V(V/I, N) = Tor_1^V(I, N) = 0$ for every $N$.
