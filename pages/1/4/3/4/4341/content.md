...
The notion of structures that are an archetypal model of the [[mathematical structure]], and usually refers to a *[[free construction|free]]* or *[[initial object|initial]]* form of such a kind of structure. 
...
* The [[walking morphism]], which represent [[morphisms]] in a [[category]].

* The [[walking isomorphism]], which represent [[isomorphisms]] in a [[category]]. 
...
Nevertheless, the first sits inside the second!  Specifically, for any category $C$ with products, the "underlying set" functor $C(1,-):C\to Set$ is product-preserving and hence carries monoid objects to monoid objects, and in the case of the walking pointed monoid we have $C_M(1,M) \cong \mathbb{N}$.
...
Now take $C$ to be the walking pointed monoid $C_M$ above.  Then its universal property tells us that functors $F:C_M\to Set$ are equivalent to pointed monoids $F(M)$ in $Set$; so we see that $C(1,M)$ is the initial pointed monoid in $Set$, i.e. $\mathbb{N}$.
...
The term *walking* is defined in:
...
The term *free-living* is defined in: 
