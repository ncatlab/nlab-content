...
**Proposition** A quantum channel given by $t: L(\mathcal{H}) \to L(\mathcal{H})$, together with the $d$-dimensional $C^{*}$-algebra, $C$, on which it acts, forms a category we call **Chan**($d$) where $C$ is the sole object and $t$ is the sole arrow.
...
Since $A$ and $B$ are summed over separate indices the trace-preserving property is maintained, i.e. $\sum_{k} C_{k}^{\dagger}C_{k}=1$ (not sure how to do a blackboard 1 on this site - \mathbb doesn't work since it's a package).  For a similar methodology see [Nayak and Sen](http://arxiv.org/abs/0605041).
...
and thus we have associativity.  Note that similar arguments may be made for the inverse process of the channel if it exists (it is not necessary for the channel here to be reversible). $\Box$