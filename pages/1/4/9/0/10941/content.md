...
Assuming the [[axiom of choice]], one of the standard characterizations of the [[projective modules]] $N$ (say left) over $R$ is that there is an [[epimorphism]] $F\to P$ from a [[free module]] over $R$  which is [[split epimorphism|split]] ("$P$ is a [[direct summand]] of a [[free module]]", [this prop.](projective%20module#ProjectiveIsPreciselyDirectSummandOfFreeModule)).
...
basis $(x_j)_j$ is a [[linear basis]] of $V$ then $x_i^*$ _defined by_ $x_i^*(x_j) = \delta_i^j$ is not necessarily a basis of $V^* = Hom_k(V,k)$; it is
