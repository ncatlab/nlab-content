...
A pair $(X,A)$ where $A$ is a closed subspace of $X$ is an **NDR-pair** or a (closed) Borsuk pair if there is a function $u:X\to I=[0,1]$ and a homotopy $H:X\times I\to I$ such that $H(x,0)=x$, for all $x\in X$, $H(a,t)=a$ for all $a\in A$, $H(x,1)\in A$ for all $x\in X$ such that $u(x)\lt 1$ and $u^{-1}(0)\subset A$. Every NDR-pair is a [[Hurewicz cofibration]]. 
