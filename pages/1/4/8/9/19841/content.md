...
The notion of a _blockchain_, originating with [(Nakomoto2008)](#NakomotoBitcoinAPeerToPeerElectronicCashSystem), allows for the creation of a linear log which can be added to, but is extremely difficult to otherwise modify, in a network with multiple nodes, without any central authority.

\section{Proof of work}

The key idea is that of _proof of work_. For the moment, let us think of a blockchain simply as a list $[ B_{1}, B_{2}, \ldots ]$ of 'blocks'. A _block_ $B$ is an _unencrypted block_ of content, which could be anything, together with a cryptographic hash of it with certain properties.

The key points are the following.

1. The cryptographic hash with the required properties is hard to create, where 'hard' means requiring significant computational resources (e.g. something like prime factorisation). Use of these computational resources to create the hash is referred to as 'proof of work'.
...
1. It is easy (requiring little resources) to check that a particular hash has the required properties. 
...
While in its way ingenious, the use of proof of work underlying the notion of a blockchain is rather crude. Enormous amounts of electricity, for example, are being consumed by large blockchain networks such as bitcoin during proof of work, whilst the outcome of that proof of work is typically useless. This is not sustainable. 
