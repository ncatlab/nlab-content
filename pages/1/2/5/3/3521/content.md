...
What is called the _d&#233;calage_ of a [[simplicial set]] is a model for its [[path space object]] --or rather: the union of all path space objects for all basepoints $x \in X_0$ -- similar to, but a little smaller than, the model $X^I \times_X X_0$, (as discussed at for instance at _[[factorization lemma]]_).
...
In terms of this the plain [[décalage]] is the 
the functor induced from the restriction $\sigma(-,[0]) : \Delta \to \Delta$, of [[ordinal sum]] with $0$, i.e.
...
This simplicial set $Dec_ X$ comes with 
...
See for instance ([Stevenson, around def. 2](#Stevenson11)) for a useful account.

In terms of the construction in remark  \ref{InTermsOfCones} we have

* the morphism $Dec_0 X \to const X_0$ is given by the augmentation map, regarding $\sigma(-,[0])^* : sSet \to asSet$.

* the morphism $Dec_0 X \to X$ is induced by the canonical inclusion $\Delta[n] \to \Delta[n] \star \Delta[0]$ of the base of the cone.
...
For each of these the statement that the projection $X_{/x} \to X$ is a [[Kan fibration]] if $X$ is a [[Kan complex]], and moreover that it is a a [[right fibration]] if $X$ is a [[quasi-category]], is ([Joyal, theorem 3.19](#Joyal)), reproduced also as ([[Higher Topos Theory|HTT, prop. 2.1.2.1]]).
...
A detailed account of many aspects is in
