...
+-- {: .num_remark #InTermsOfCones}
###### Remark

In terms of the [[ordinal sum]] operation $\sigma : \Delta_a \times \Delta_a \to \Delta_a$ on the [[asSet|augmented simplex category]], this is the precomposition with the resriction $\sigma(-,[0]) : \Delta \to \Delta$, i.e.
...
Since the [[left adjoint]] to $\sigma^*$ is the _[[join of simplicial sets]]_ operation $(-)\ast (-) : sSet \to sSet$ this means that equivalently

$$
  (Dec X)_n = Hom_{sSet}( \Delta[n] \ast \Delta[0], X)
  \,.
$$

Here $\Delta[n] \ast \Delta[0]$ is manifestly the simplicial [[cone]] over the $n$-[[simplex]]. So this exhibits the $n$-cells of $Dec X$ as being the cones of $n$-simplices in $X$.
...
This simplicial set $Dec X$ comes with 
...
See for instance ([Stevenson, around def. 2](#Stevenson)) for a useful account.

In terms of the construction in remark  \ref{InTermsOfCones} we have

* the morphism $Dec X \to const X_0$ is given by the augmentation map, regarding $\sigma(-,[0])^* : sSet \to asSet$.

* the morphism $Dec X \to X$ is induced by the canonical inclusion $\Delta[n] \to \Delta[n] \ast \Delta[0]$ of the base of the cone.
