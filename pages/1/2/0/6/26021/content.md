...
Informally, a global symmetry described by the [[action]] of a [[group]] $G$ may be "coupled" to a [[gauge potential]] $A$ (a [[connection on a bundle]]). Denoting the [[partition function]] coupled to such a connection as $Z(A)$, the partition function of the gauged theory is, modulo normalization factors, $Z=\sum_A Z(A).$ If the symmetry has a 't Hooft anomaly, then performing a [[gauge transformation]] on all connections results in a nontrivial phase multiplying each partition function $Z_g=\sum_A \phi(A,g) Z(g\cdot A)$ where each phase $\phi(A,g)$ depends on the particular background field and gauge transformation, so that generally $Z\neq Z_g$. This is problematic since the partition function is supposed to be gauge invariant.
