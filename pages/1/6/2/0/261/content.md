...
* The face map $\delta_i : X_n \rightarrow X_{n-1}$ is dual to the unique injection $d^i : [n-1] \rightarrow [n]$ in the category $\Delta$ whose image omits the element $i \in [n]$. Similarly, the degeneracy map $\sigma_i : X_n \rightarrow X_{n+1}$ is dual to the unique surjection $s^i : [n+1] \rightarrow [n]$ in $\Delta$ such that $i \in [n]$ has two elements in its preimage. The maps $d^i$ and $s^i$ satisfy certain obvious relations; the face are degeneracy maps satisfy relations dual to these (so we get a [[functor]] from $\Delta^{op}$ to $\Set$).


+--{.query}
[[Tim Porter|Tim]]  Can I make a plea in favour of adopting the standard conventional symbols for face and degeneracy maps. The usual one is $d$ is a face map and $s$ is a degeneracy map.  The corresponding maps in $\Delta$ which induce these might then be $\delta$ and $\sigma$. I find the dual use here VERY confusing and would expect that others will as well.
=--
#Examples#
