...
we ,ay call the **$G$-valued flat cotangent sheaf** of $X$.
...
With $X,A \in \mathbf{H}_{th}$ as above, write
...
for the [[morphism]] in $\mathbf{H}$ is the $(\underset{X}{\sum} \dashv X^*)$-adjunct of the the [[counit of a monad|counit]] of the $(\iota \dashv Et)$-coreflection of def. \ref{TheStructureSheafOfX}, where we notationally suppress the defining embedding $\iota \;\colon\; Sh_{\mathbf{H}}(X) \hookrightarrow \mathbf{H}_{/X}$.

This we call the **Liouville-Poincar&#233; $A$-cocycle** on $\underset{X}{\sum} \mathcal{O}_X(A)$.
...
Then for $X \in SmthMfd \hookrigtharrow \mathbf{H}$ a [[smooth manifold]], we have that 

$$
  \underset{X}{\sum} \mathcal{O}_X(Omega^1) \to X
$$
...
Then the Liouville-Poincar&#233; $\Omega^1$-cocycle according to \def{TheLiouvillePoincareCocycle} is a [[differential 1-form]] 

$$
  \theta
  \;\colon\;
  \underset{X}{\sum}\mathcal{O}_X(\Omega^1)
  \to 
  \Omega^1
$$

on the total space of the cotangent bundle. Then for
...
$$
  X \stackrel{\sigma}{\to} \mathcal{O}_X(\Omega^1) \stackrel{}{\to} \Omega^1
$$

hence by definition

$$
  \iota(X) \stackrel{\iota(\sigma)}{\to} \iota(Et(X \times \Omega^1)) \stackrel{}{\to} \Omega^1
$$

The $(\iota \dashv Et)$-[[adjunct]] of this is

$$
  X \stackrel{\sigma}{\to} Et(X \times \Omega^1) \to Et(X \times \Omega^1)
$$
