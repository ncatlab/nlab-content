...
For $X \in \mathbf{H}_{th}$ an object and $(Sh_{\mathbf{H}}(X), \mathcal{O}_X)$ its $\mathbf{H}$-[[structured (infinity,1)-topos]], according to def. \ref{TheStructureSheafOfX}, exhibited by the composite functor
...
$$
  \array{
    \mathcal{C} &\stackrel{\mathcal{O}_X}{\longrightarrow}& Sh_{\mathbf{H}}(X)
    \\
