...
A morphism $f \colon X \to Y$ in $\mathbf{H}_{th}$ is **formall &#233;tale** if it is $\mathbf{\Pi}_{inf}$-[[Pi-closed morphism|closed]], hence if its $\mathbf{\Pi}_{inf}$-unit naturality square
...
We formulate sheaves of [flat differential forms]() over a given base homotopy type abstractly in differential cohesion.

To motivate the following construction, notice that for $G \in Grp(\mathbf{H}_{th})$ a differential cohesive [[∞-group]] with [de Rham coefficient object](cohesive+%28infinity,1%29-topos+--+structures#deRhamCohomology) $\flat_{dR}\mathbf{B}G$ and for $X \in \mathbf{H}_{th}$ any differential homotopy type, the product projection 
...
The inclusion of def. \ref{EtaleSlice} is both [[reflective sub-(∞,1)-category|reflective]] as well as [[coreflective subcategory|coreflective]], hence it fits into an [[adjoint triple]] of the form

$$
  (\mathbf{H}_{th})_{/X}^{fet} 
  \stackrel{\overset{L}{\leftarrow}}{\stackrel{\hookrightarrow}{\underset{Et}{\leftarrow}}}
  (\mathbf{H}_{th})_{/X}
  \,.
$$
...
Using this &eacute;talification, we can now turn [de Rham coefficient objects](cohesive+%28infinity,1%29-topos+--+structures#deRhamCohomology) into genuine [[cotangent bundle]].

+-- {: .num_defn #CotangentBundle}
###### Definition
...
$$
  Et(X \times \flat_{dR}\mathbf{B}G)
  \in 
  (\mathbf{H}_{th})^{fet}_{/X}
$$ 

for its &eacute;talifiation, the coreflection by prop. \ref{EtalificationIsCoreflection}. The sections of this object we call the flat sections of the **$G$-valued [[cotangent bundle]]** of $X$.
...
$$
  \array{
    U &&\to&& Et(X \times \flat_{dR} \mathbf{B}G)
    \\
    & \searrow && \swarrow
    \\
    && X
...
where we are now simply including on the left the formally &eacute;tale map $(U \to X)$ along $(\mathbf{H}_{th})^{fet}_{/X} \hookrightarrow (\mathbf{H}_{th})_{/X}$.

In other words, the (flat) sections of the $G$-valued cotangent bundle $Et(X \times \flat_{dR}\mathbf{B}G) \to X$ are just the sections of $X \times \flat_{dR}\mathbf{B}G \to X$ itself, only that the _domain_ of the section is constrained to be a formally &eacute; patch of $X$.
...
Since the identity on $X$ is formally &eacute;tale, This we may regard as a section of the $G$-valued cotangent bundle, def. \ref{CotangentBundle}, 

$$
  \array{
    X &&\stackrel{(id, \mathbf{d}S)}{\to}&& Et(X \times \flat_{dR}\mathbf{B}G)
    \\
...
$$
  \array{
    ...
    \\
    X &\stackrel{\mathbf{d}S}{\to}& Et(X \times \flat_{dR}\mathbf{B}G)
  }
  \,.
$$
