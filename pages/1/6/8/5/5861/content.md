...
A morphism $f$ in $\mathbf{H}$ is formally etake in the sense of def. \ref{FormalRelativeSmoothnessByCanonicalMorphism} precisely if its image $i_!(f)$ in $\mathbf{H}_{th}$ is formally etale in the sense of def. \ref{FormallyEtaleInHTh}.
...
This is again given by the fact that $\athbf{\Pi}_{inf} = i_* i^*$ by definition and that $i_!$ is fully faithful, so that 
