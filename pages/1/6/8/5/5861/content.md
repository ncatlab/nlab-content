...
For $X, A \in \mathbf{H}_{th}$ and for $U \to X$ a formall etale morphism in $\mathbf{H}_{th}$, we have that 
...
This means that $\mathcal{O}_{X}(A)$ plays the role of the "sheaf of $A$-valued fucntions" over $X$. 
...
it preserves [[(∞,1)-limits]]. Therefore this is a [[structure sheaf]] which exhibits $Sh_{\mathbf{H}_{th}}(X)$ as a [[structured (∞,1)-topos]] over $\mathbf{H}_{th}$ regarded as a (large) [[geometry (for structured (∞,1)-toposes)]], with the formally &#233;tale morphisms being the "admissible morphhisms".
