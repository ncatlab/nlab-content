...
Let $\mathbf{H}$ be a [[(2,1)-topos]], hence a [[(2,1)-category]] whose [[objects]] may be thought of as [[groupoids]] equipped with some geometric structure ([[stacks]]). Then a **double groupoid** with that geometirc structure is a [[groupoid object in an (∞,1)-category|groupoid object in an (2,1)-category]] in $\mathbf{H}$, hence a [[simplicial object]]
...
The archetypical special case of this is that where $\mathbf{H} = $ [[Grpd]] is the [[(2,1)-category]] of bare (geometrically discrete) [[groupoids]]. 
...
(where now we are notationally suppressing the degeneracy maps/identity assigningf maps, for readability). In this form double groupoid are presented in traditional literature.


=--


More generally, one can consider double groupoids in an arbitrary [[(∞,1)-topos]] $\mathbf{H}$, to be a 3-coskeletal [[groupoid object in an (∞,1)-category]] consistibg degreewise of [[1-truncated]] objects. The [[realization]] map 

$$
 \underset{\lim}{\to} \colon \mathbf{H}^{\Delta^{op}} \to \mathbf{H}
$$
