...
* Let $p : D \to C$ be a functor into a [[free category]] $C = F\mathbb{G}$ generated by some graph $\mathbb{G}$.  Then $p$ is ULF if and only if $D$ is also isomorphic to a free category $D = F\mathbb{H}$ and $p$ is generated by a graph homomorphism $p = F\phi$, $\phi : \mathbb{H} \to \mathbb{G}$.  Indeed, it is immediate that any such graph homomorphism induces a ULF functor $F\phi : F\mathbb{H} \to F\mathbb{G}$. Conversely, if $p : D \to C$ is ULF, then $D$ is freely generated by the graph $\mathbb{H} = p^{-1}(\mathbb{G})$ spanned by the preimages of the generators of $C$. In particular, a category $D$ is free iff it admits a ULF functor to the monoid of natural numbers, considered as the one-object category freely generated by the terminal graph.
...
* A subset $I \subset P$ of a poset is said to be _convex_ if for all triples of elements $x,y,z \in P$ with $x \le y \le z$, if $x\in I$ and $y \in I$ then $z \in I$. Hence $I$ is convex just in case the inclusion $I \to P$ defines a (necessarily discrete) Conduch&#233; functor when considered as a functor between posetal categories.
...
Discrete Conduch&#233; functors (sometimes referred to as ULF functors or UFL functors) are considered in

* [[Ross Street]]. _Categorical structures_. In M. Hazewinkel, editor, Handbook of Algebra, volume 1, pages 529--577. North-Holland, 1996. ([doi](https://doi.org/10.1016/S1570-7954%2896%2980019-2))
...
* [[F. W. Lawvere]] and [[M. Menni]], _The Hopf algebra of Möbius intervals_, TAC **24** no.10 (2010) pp.221-265. ([url](http://www.tac.mta.ca/tac/volumes/24/10/24-10abs.html))
