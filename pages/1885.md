...
The **Moore space** $M(G, n)$, where $G$ is an abelian group and $n \geq 1$, is a space which has non-trivial (reduced) [[homology]] group $G$ only in dimension $n$. 
...
$0\rightarrow H \rightarrow F\rightarrow G\rightarrow 0$ where $F$ a free abelian group. Since $H=ker(F\rightarrow G)$, it is a free abelian group as well and we choose bases $\{f_\alpha\}_\alpha$ for $F$ and ${h_\beta}_\beta$ for $H$. We then construct a CW complex $X=M(G,n)$ by taking the n-th skeleton to be $X^n:=\vee_\alpha S^n_\alpha$ and for each $\beta$ an attach an $n+1$-cell as follows:
Write $h_\beta=\Sigma_\alpha d_{\alpha\beta}f_\alpha$ and let $\delta_{d_{\alpha\beta}}$ be $0$ if $d_{\alpha\beta}= 0$ and $d_{\alpha\beta}=1$ otherwise. Define an attaching map $S^n_\beta\rightarrow X^n$ by contracting $\ell_\beta:=(\Sigma_\alpha \delta_{d_{\alpha\beta}})-1$ $(n-1)-$spheres in $S^n$ thus defining a map $S^n_\beta\rightarrow \vee_{\ell_\beta} S^n_{\alpha\beta}$ and then map each $S^n_{\alpha\beta}$ to $S^n_{\alpha}$ by a degree $d_{\alpha\beta}$. The resulting CW-complex can be seen to have the desired properties via cellular homology.

## Properties

1) The homotopy type of $M(G,n)$ is determined by specifying $G$ and $n$. 

2) The construction above is not functorial in $G$ because of the choice of bases (see more below). However, it does give a functor to the homotopy category $M(-,n):Ab\rightarrow Ho(Top)$.  
...
###### Corollary
There is thus no functor $Ab\rightarrow Top$ that lifts $M(-,n):Ab\rightarrow Ho(Top)$ since if there was such, it would induce, for any group $G$ a functor $Ab^G\rightarrow Top^G$ and in particular a positive answer to the Steenrod conjecture.

Moreover, there can also not be an $(\infty,1)-functor$ $Ab\rightarrow Top$ that lifts $M(-,n)$ since this will similarly yield an $\infty$-functor $Ab\rightarrow Top^{hG}$ where $Top^{hg}$ is the $\infty$-category of $\infty$-actions of $G$ on spaces. Since there is a "rigidification" functor $Top^{hG}\rightarrow Top^G$ this would yield an (ordinary) functor $Ab\rightarrow Top^G$ which does not exist by our previous observation. 
