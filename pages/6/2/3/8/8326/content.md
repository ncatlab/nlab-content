...
To see which meaning this has we need to consider its action on a type which is not a [[mere proposition]] (for a mere proposition $P$ in the absolute context is either [[true]] or [[false]] and hence so is $\prod_W W^\ast$, there is nothing interesting happening in this case). For $V\in \mathbf{H}$ any type, then 
...
is the space of $V$-valued functions on the space of [[possible worlds]]. A modern mathematical incarnation of spaces of "possible worlds" (possible configurations of a system under consideration) are [[probability spaces]]. Now the space of ([[measurable function|measurable]]) functions on a probability space $W$ is to be interpreted as its space of _[[random variables]]_. 
...
In summery we get that the [[base change]] [[adjoint triple]] of a [[locally Cartesian closed category]]
...
induces an adjoint pair of (co-)monads and one coinciding pair of (co-)monads to a total of three operations which may be pronounced
