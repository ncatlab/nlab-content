...
An __invertible semigroup__ is a [[semigroup]] $(G,(-)\cdot(-) \colon G\times G\to G)$ such that for every [[element]] $a \in A$, left multiplication and right multiplication by $a$ are both [[bijections]]. 
...
An __invertible semigroup__ is a set $S$ with a binary operation $(-)\cdot(-) \colon S\times S\to S$ called __multiplication__ and a unary operation $(-)^{-1}:S\to S$ called __inverse__ satisfying the following laws:
...
Every invertible semigroup $G$ has a canonical structure of a [[pseudo-torsor]], or associative [[Malcev operations|Malcev algebra]], via the ternary operation $t \colon G^3\to G$ defined as $t(x,y,z) = x\cdot y^{-1}\cdot z$. If the invertible semigroup is [[inhabited set|inhabited]], then its associated pseudo-torsor is actually a [[heap]], or equivalently a [[torsor]]. 

## Properties

* Every invertible semigroup is either a [[group]] or the [[empty set|empty]] semigroup. 
