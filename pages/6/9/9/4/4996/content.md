...
If the $\infty$-cohesive site $C$ has the property that for all $U \in C$ $\Gamma(U)$ is not empty, then:
...
* $(\infty,1)$-[[regular monomorphism]]s.
...
The first statement is obvious. For the second we have to show that for $S$ a [[Kan complex]] and $U \in C$ with underlying set $\Gamma(U)$ the morphism
