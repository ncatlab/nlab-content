...
* There are vertices $s, t$ of $Q$ (called the *source* and *sink*, respectively) such that for every vertex $v$, the hom-sets $F(Q)(s, v)$ and $F(Q)(v, t)$ are [[inhabited set|inhabited]] (there exist directed paths in $Q$ from $s$ to $v$ and from $v$ to $t$). 
...
###### Definition 
**(after Power)** 
A pasting scheme consists of a progressive [[quiver]] $Q$ together with an subspace embedding $i: R(Q) \hookrightarrow \mathbb{R}^2$, such that $i$ is $C^1$ on each edge interior. 
=--
...
To complete the description of the computad, we need to define the source and target $\sigma(c), \tau(c) \in Mor(F(Q))$ of a 2-cell $c$. For each edge $e \in Q_1$ such that $i(e)$ lies in the boundary of $c$, and each interior point $x \in e$, we let $t(x)$ be the tangent vector $i'(x)$ and $n(x)$ be the inward-pointing normal (pointing into $c$); the $C^1$ condition ensures that these make sense. Then say that $e$ is a *negative edge* of $c$ if 
