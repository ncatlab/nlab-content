...
That this [[object classifier]] is the categorical semantics of a [[univalence|univalent]] [[type universe]] ([[type of types]]), hence that [[homotopy type theory]] with [[univalence]] has categorical semantics in [[(∞,1)-toposes]] had been conjectured in ([Awodey 10](#Awodey10)). 

This statement was proven base the canonical $(\infty,1)$-topos [[∞Grpd]] in ([Kapulkin-Lumsdaine-Voevodsky 12](#KapulkinLumsdaineVoevodsky12)) and more generally for [[(∞,1)-presheaf]] $(\infty,1)$-toposes over [[elegant Reedy categories]] in ([Shulman 13](#Shulman13)).

In these proofs the type universes are axiomatized "[&#224; la Russel](type+of+types#RusselStyle)", which requires the [[type-theoretic model categories]] which interpret the homotopy type theory syntax to provide type universes that behave strictly under pullback.

This is syntactically convenient, but somewhat awkward to implement in the categorical semantics. More flexibly one may consider syntactic [type universes weakly &#224; la Tarski](type+of+types#TarskiStyle). These are more complicated to work with syntactically, but should have interpretation indeed in ([[type-theoretic model categories]] presenting) any [[(∞,1)-topos]]. Discussion of in this general flexible sense is in ([Gepner-Kock 12](#GepnerKock12)).

While [[(∞,1)-sheaf]] [[(∞,1)-toposes]] are those currently understood, the type theory does not see or care about their [[locally presentable (∞,1)-category|local presentability]]. It is to be expected that there is a decent concept of [[elementary (∞,1)-topos]] such that [[homotopy type theory]] with [[univalence|univalent]] [[type universes]] and some supply of [[higher inductive types]] has categorical semantics precisely in [[elementary (∞,1)-toposes]] (as conjectured in [Awodey 10](#Awodey10)). But the fine-tuning of this statement is currently still under investigation.

Notice that this statement, once realized, makes (or would make) Univalent HoTT+HITs a direct [[homotopy theory|homotopy theoretic]] refinement of [[foundations of mathematics]] in [[topos theory]] as proposed by [[William Lawvere]], specifically of his [[elementary theory of the category of sets]].
