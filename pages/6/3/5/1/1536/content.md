...
### Other Σ-like constructions for categories
The Grothendieck construction generalizes the Σ-type construction for sets/types to categories. However, there are many other Σ-like constructions (as well as dualizations of the Grothendieck construction) for category-like objects. [Nuyts '20](#Nuyts20) discusses several of these, some of which are included here.

To begin with, from the inputs of the Grothendieck construction:

* A category $C$,
* A pseudofunctor $D : C \to Cat$

we can construct:

* The Grothendieck construction $\int_C D$,
  * Objects are pairs $(c, d)$ with $c \in Obj(C)$ and $d \in Obj(D(c))$,
  * Morphisms are pairs $(\gamma, \delta) : (c, d) \to (c', d')$ with $\gamma : c \to c'$ and $\delta : D(\gamma)(d) \to d'$,
* The Grothendieck construction $\int_C (Op \circ D)$, where $Op : Cat \to Cat$ is the functor (covariant as a 1-functor, but it flips the direction of 2-cells) that sends categories to their opposite. Here:
  * Objects are still pairs $(c, d)$ with $c \in Obj(C)$ and $d \in Obj(D(c))$,
  * Morphisms are pairs $(\gamma, \delta) : (c, d) \to (c', d')$ with $\gamma : c \to c'$ and $\delta : d' \to D(\gamma)(d)$.

In other words, when we consider the inputs $C$ and $D$, the orientation of morphisms in $C$ is unrelated to the orientation of morphisms in $D$; the latter can be flipped before building a Grothendieck construction, which couples the orientations.

The following construction does more right to the observation that these orientations are unrelated.
Let $C'$ be the free [[pro-arrow equipment | 2-category equipped with proarrows]] over $C$, so the arrows of $C'$ are those of $C$, whereas pro-arrows in $C'$ are freely generated by the [[companion pair|companions]] and [[conjoint pair|conjoints]]. Let $ProfCat$ be the pro-arrow equipment of categories, functors and profunctors. Then a pseudofunctor $D : C \to Cat$ automatically gives rise to an equipment functor $D : C' \to ProfCat$.

From the following inputs:

* a pro-arrow equipment $C$
* an equipment functor $D : C \to ProfCat$

we can build a Σ-like construction $\int_C D$ ([Nuyts '20](#Nuyts20)), which is an equipment whose

* objects are still pairs $(c, d)$ with $c \in Obj(C)$ and $d \in Obj(D(c))$,
* arrows $\gamma : (c, d) \to (c', d')$ are simply arrows $\gamma : c \to c'$ such that $D(\gamma)(d) = d'$,
* pro-arrows $(\bar \gamma, \delta) : (c, d) \nrightarrow (c', d')$ are pairs of a pro-arrow $\bar \gamma : c \nrightarrow c'$ and a heterogeneous arrow $\delta \in D(\bar \gamma)(d, d')$ over $\bar \gamma$.

Moreover, if $C$ and $D$ arose as the $C'$ and $D'$ above, then we find that the arrows of the (semi-opposite) Grothendieck construction are the pro-arrows of $\int_C D$ for which $\bar \gamma$ is actually a companion (conjoint) of an arrow in $C$.
...
Several generalizations of the Grothendieck construction are discussed in:
