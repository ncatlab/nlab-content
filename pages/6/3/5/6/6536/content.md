...
An $n$-form $\omega$ is (weakly) __positive__ ($\omega \geq 0$) if the following equivalent conditions hold:
...
*  the value of $\omega$ at any point $p$, when applied to any oriented collection of $n$ [[tangent vectors]] at $p$, is nonnegative.

Supposing $X$ is unoriented, an $n$-[[pseudoform]] $\omega$ is (weakly) __positive__ if the following equivalent conditions hold:
...
A (pseudo)-form is __strictly positive__ ($\omega \gt 0$) if it is also nondegenerate (nowhere zero), or equivalently if the conditions above hold with "positive" in place of "nonzero" (taking integrals over only [[inhabited subset|inhabited]] submanifolds).  A strictly positive form can be interpreted as a [[volume form|volume]] (pseudo)-form on $X$.
