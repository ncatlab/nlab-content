...
is essentially surjective. An **operation of arity** $x_1, \ldots, x_n \to s$ in $C$ is a morphism of the form $\Pi(n, x) \to \Phi(s)$ in $C$. 
=-- 

It is [[evil]], but nevertheless harmless and sometimes convenient, to suppose $\Pi$ is an isomorphism on objects, since we can define $C'$ to have the same objects as $Set/S$ and define hom-sets by $C'(x, y) = C(\Pi(x), \Pi(y)$, where the evident functor $C' \to C$ is an equivalence. 
