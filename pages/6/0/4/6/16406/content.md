...
#### Tangent vectors and Local diffeomorphism

traditionally, for $X$ and $Y$ smooth manifolds:

$f \colon X \longrightarrow Y$ is [[local diffeomorphism]] if

$$
  \array{
    T X &\stackrel{}{\longrightarrow}& X
    \\
    \downarrow^{\mathrlap{T f}} && \downarrow
    \\
...
now with $C^\infty(\mathbb{D}) = \mathbb{R}[\epsilon]/(\epsilon^2)$.
...
This condition generalizes to all of $FormalSmooth0Type$
