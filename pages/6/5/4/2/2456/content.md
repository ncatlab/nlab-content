...
One can also represent [[decidable subsets]] by functions into the [[booleans]], so this is equivalently represented by the statement that given a sequence of booleans $f:\mathbb{N} \to \mathrm{bool}$, if there exists a natural number $n \in \mathbb{N}$ such that $f(n) = 1$, then one can construct a [[minimum|minimal]] [[natural number]] $\min_f$ such that the $f(\min_P) = 1$, which is represented by the [[dependent sum type]] $\sum_{n:\mathbb{N}} f(n) = 1$. This is important for showing that the truncated versions of the [[limited principle of omniscience]] imply the untruncated versions of the limited principle of omniscience, since the [[existential quantifier]] $\exists n:\mathbb{N}.f(n) = 1$ and its [[negation]] are mutually exclusive propositions. 
