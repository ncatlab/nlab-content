...
There is a [[functor]] $dom:Arr(C)\to C$ given on objects by the domain (= source) map, and on morphisms it gives the lower arrow of the commutative square.  If $C$ has [[pushouts]], then this functor is in fact an [[Grothendieck opfibration|opfibered category]] in the sense of Grothendieck, whose pushforward functor $u_!$ amounts to the usual pushout of $f$ along $u$ in $C$.  The fiber over an object $c$ in $C$ is the [[undercategory]] $c\downarrow C$. This opfibered category is called the **domain opfibration** over $C$ (some say the *basic* opfibration). This notion is dual to the notion of [[codomain fibration]].
