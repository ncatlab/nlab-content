...
* [UFP13](#UFP13) (first edition) uses a hierarchy of cumulative Russell universes that is not indexed by the [[natural numbers]], but rather only has a base universe and an operation for the successor universe. Since there is no second-order induction for the indices of the universes, [[book HoTT]] admits non-standard models of the universe hierarchy. 

* [Rijke 2025](#Rijke25) similarly uses a hierarchy of non-cumulative Tarski universes that is not indexed by the [[natural numbers]], but rather has operations for the base universes (generated by lists of judgments), successor universes and the join of two universes. 

* Most type theories implemented in the computer, such as [[Lean]], [[Agda]], and [[Rocq]], use a hierarchy of universes indexed by a model of quantifier-free arithmetic, usually some version of [[Presburger arithmetic]] involving zero, successor, and the [[join]] of two operations. Strictly speaking, this is not the [[natural numbers]], since the arithmetic is not a second-order theory and so is not sufficient to uniquely determine the natural numbers up to isomorphism, but in practice, one usually only has access to a finite amount of universe levels on a computer, corresponding to a subset of the natural numbers. 

  * [[Agda]] uses a non-cumulative hierarchy of Russell universes indexed by a model of quantifier-free arithmetic with zero, successor, and the [[join]] of two levels. The strict inequality relation is defined as $a \lt b \coloneqq s(a) \vee b = b$

  * [[Rocq]] uses a hierarchy of Russell universes indexed by a model of quantifier-free arithmetic with zero, successor, and the [[join]] of two levels. For practical purposes, [[Rocq]] also has cumulativity, although there is some question (perhaps mainly semantic) of whether this is true internally or whether it uses casts that are simply hidden from the user.
