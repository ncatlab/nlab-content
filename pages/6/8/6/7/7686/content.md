Etale affine (resp. &#233;tale formal) $k$-groups are equivalent to finite (resp. all) Galois modules by
...
A $k$-formal group $Spf A$ is called  *local* (or *connected*) if the following two equivalent conditions hold:

1. $A$ is local

1. $G(k)$ is local for any field $K$.
