...
A PRO, an abbreviation for "product category", is similar to a [[PROP]] but more general, being merely a [[monoidal category]] but not necessarily [[symmetric monoidal category|symmetric monoidal]]. They may be used to describe theories for finitary algebraic and coalgebraic structures which make sense in any monoidal category, for example [[monoids]] and [[comonoids]]. 
...
$$
  x^{\otimes n} 
   \;=\; 
  \underset{n \; times}{\underbrace{x \otimes \ldots \otimes x}} 
$$

for some $n \geq 0$. Somewhat more precisely, a PRO is a strict monoidal category along with a [[strict monoidal functor|strict monoidal]] [[bijective-on-objects functor]]: 
