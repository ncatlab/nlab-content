...
Let $\mathrm{Set}$ be some [[category of sets]]. We shall stay neutral on whether $\mathrm{Set}$ is strict or not; e.g. in [[dependent type theory]], this means $\mathrm{Set}$ is a general [[precategory]]. Note that if $\mathrm{Set}$ is not a strict category, the notion of [[equality]] of [[sets]] in $\mathrm{Set}$, if it exists, is by definition not [[propositional equality]] of [[first order logic with equality]]; or in [[dependent type theory]], [[propositional equality]] in the sense of a [[h-proposition]] valued [[identity type]] or a [[unique identification]] of an [[identity type]]. 
