...
Equivalently, a locally cartesian category $C$ is a category with [[pullback]]s (and a terminal object, if required) such that each [[base change]] functor $f^*: C/y \to C/x$ has a right adjoint $\Pi_f$, called a [[dependent product]]. In particular, such pullbacks preserve all [[colimit]]s. Therefore, if a locally cartesian closed category [[finitely cocomplete category|has finite colimit]]s, it is automatically a [[coherent category]] and in fact a [[Heyting category]].
