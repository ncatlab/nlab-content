...
The converse holds on principal ideal domains. For this let us start with
a very useful lemma : it is enough to check flatness versus cyclic modules.

\begin{lemma}(Reduction to cyclic modules)
...
1. $Tor_1^R(M, R/I) = 0$ for every ideal $I \subset R$. Indeed, write $I$ as the union of the finitely generated ideals $I_\mathrm{fin} \subset I$, then $R/I = \varinjlim_{I_\mathrm{fin} \subset I} R/I_\mathrm{fin}$. Then
because $Tor_1$ commutes with filtered colimits
$$
  Tor_1(M, R/I) = Tor_1(M, \varinjlim_{I_fin \subset I} R/I_fin) = \varinjlim_{I_fin \subset I} Tor_1(M, R/I_fin) = 0
$$
...
$$
  Tor_1(M, X) = Tor_1\left(M, \bigcup_{X_fin \subset X} X_fin\right) = \varinjlim_{X_fin \subset X} Tor_1(M, X_fin) = 0
$$
for every module $X$.

\end{proof}

\begin{theorem}(Over a PID)
If $R$ is a [[PID]] and $M$ is a module, then
$$ 
  M\;\text{is flat}
  \;\;\;\;\;\;\;\;
  \Leftrightarrow
...
\begin{proof}
By the reduction lemma, it is enough to show that
$\mathrm{Tor}_1^R(M, R/I) = 0$ for every ideal $I \subset R$.
Since $R$ is a PID, every such $I$ is principal and generated by a regular element $r$.
Lastly, for any such $r$, $Tor_1(M, R/(r))$ is isomorphic to the $r$-torsion submodule of $M$, which is assume to be trivial.
\end{proof}
...
\begin{remark}
Notice that while $\mathbb{Z}[[X]]$ is flat, it is _not_ a free $\mathbb{Z}$-module.
\end{remark}
