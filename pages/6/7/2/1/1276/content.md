...
\begin{proof}
We need to show that $\mathrm{Tor}_1^R(M, X) = 0$ for every $R$-module $X$.
Since $\mathrm{Tor}_1^R(M, -)$ commutes with filtered colimits and $R$ is PID, one can reduce to the case where $X$ is finitely generated. Thanks to the Tor long exact sequences, one can further reduce to the case where $X$ is generated by only one element. That is, we only need to show that
