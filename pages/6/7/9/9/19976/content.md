

+-- {: .rightHandSide}
+-- {: .toc .clickDown tabindex="0"}
###Context###
#### Fields and quanta
+--{: .hide}
[[!include fields and quanta - table]]
=--
=--
=--



#Contents#
* table of contents
{:toc}

## Idea

In [[quantum chromodynamics]], in fact already in pure [[special unitary group|SU(3)]] [[Yang-Mills theory]], a _glueball_ is a [[bound state]] of (just) [[gluons]] (a kind of [[confinement]]).

Glueballs are hard to detect in [[experiment]] but their existence is confirmed by [[lattice QCD]], also by the [[AdS-QCD correspondence]] (see there). According to [Greensite 11, section 8.5](#Greensite11):

> The [[caloron]] idea is probably the most promising current version of [[monopole]] [[confinement]] in pure non-abelian gauge theories, but it is  basically  (in certain [[gauge fixing|gauges]]) a superposition of [[monopoles]] with spherically  symmetric abelian fields, and this leads to the same questions raised in connection with monopole Coulomb gases.

## Properties

### In holographic QCD

In [[holographic QCD]], glueballs are [[AdS/CFT duality|dual]] to massless [[scalar fields]] in the $\sim$[[AdS spacetime|AdS]]-[[bulk spacetime]] (for instance, but not necessarily, the [[dilaton]]). 

The following graphics shows predictions of glueball spectra from a simple [[AdS/QCD]]-model compared to [[lattice QCD]]-computations

\begin{imagefromfile}
  "file_name": "GlueballSpectrumFromAdSQCD.jpg",
  "width": 600,
  "caption": "From [Brower-Mathur-Tan 00](#BrowerMathurTan00)",
  "href": "https://arxiv.org/pdf/hep-th/0003115.pdf#page=22"
\end{imagefromfile}

>  Apparently the spin structure of [[type IIA supergravity]] does resemble the low mass glueball spin splitting. ([Brower-Mathur-Tan 00, p. 22](#BrowerMathurTan00))

## References

### General

The first article discussing glueballs is apparently

* [[Harald Fritzsch]], [[Peter Minkowski]], _Ψ-resonances, gluons and the Zweig rule_, Nuov. Cim. A, 1975,30: 393 ([doi:10.1007/BF02730295](https://doi.org/10.1007/BF02730295))

Review:

* Vincent Mathieu, _The Physics of Glueballs_, Zakopane, February 2009 ([pdf](https://itp.uni-frankfurt.de/~excitedqcd/Talks09/Mathieu_ZakopaneExQCD09.pdf), [[MathieuGlueballs09.pdf:file]])

* {#Greensite11} Jeff Greensite, _An Introduction to the Confinement Problem_, Lecture Notes in Physics, Volume 821, 2011 ([doi:10.1007/978-3-642-14382-3](https://link.springer.com/book/10.1007%2F978-3-642-14382-3))

See also

* Wikipedia, _[Glueball](https://en.wikipedia.org/wiki/Glueball)_

### In holographic QCD

Discussion of glueballs in [[holographic QCD]]/[[AdS/QCD]]:

* {#BrowerMathurTan00} Richard C. Brower, [[Samir Mathur]], Chung-I Tan, _Glueball Spectrum for QCD from AdS Supergravity Duality_, Nucl. Phys. B587:249-276, 2000 ([arXiv:hep-th/0003115](https://arxiv.org/abs/hep-th/0003115))

* Alex S. Miranda, C. A. Ballon Bayona, Henrique Boschi-Filho, Nelson R. F. Braga, _Glueballs at finite temperature from AdS/QCD_, Nucl. Phys. Proc. Suppl. 199:107-112, 2010 ([arXiv:0910.4319](https://arxiv.org/abs/0910.4319))

* Xue-Feng Li, Ailin Zhang, _Scalar glueball in a soft-wall model of AdS/QCD_,  Chinese Physics C, Volume 38, Number 1 ([arXiv:1309.7154](https://arxiv.org/abs/1309.7154))




[[!redirects glueballs]]
