...
The o-minimality condition places a sharp restriction on which subsets of $R$ can be defined in the language. Roughly, it means that the only definable subsets are those which are definable in terms of the predicates $\lt$ and $=$. 

The archetypal example of an o-minimal structure is that of [[semi-algebraic set]]s defined over $\mathbb{R}$ (which form a structure due to the [[Tarski-Seidenberg theorem]]). 

Quite remarkably, quite a lot can be said about the structure of definable sets in an o-minimal structure over $\mathbb{R}$, and this is a very active area of model theory. The notion of o-minimal structure has been proposed as a reasonable candidate for an axiomatic approach to Grothendieck's hoped-for "tame topology" (_topologie moder&#233;e). 
...
* Alexandre Grothendieck, _Esquisse d'un Programme_, section 5. 
