...
\begin{proposition}
\label{ClosurePropertiesOfAlmostFreeMaps}
 The [[class]] of almost free maps (Prop. \ref{AlmostFreeMaps}) is closed under
...
\end{proposition}
[Speirs (2015), Prop. A.4.1](#Speirs15)
...
For the second statement, use with Prop. \ref{sGrpIsCofibrantlyGenerated}, that the model category is cofibrantly generated by $I = \{F(i_n)\}_{n \in \mathbb{N}}$, which [means](cofibrantly+generated+model+category#CellsAndInjectives) that the cofibrations of $sGrp$ are the [[retracts]] of the [[relative cell complexes]] (i.e. of the [[transfinite compositions]] of [[pushouts]]) of the $F(i_n)$. Therefore it is now sufficient to see that 
$$
  Cell\big(
    \{ F(i_n) \}
  \big)
  ...
  \,.
$$
The first statement is [Goerss & Jardine (1999), V Prop. 1.9](#GoerssJardine99). The second statement follows by Prop. \ref{ClosurePropertiesOfAlmostFreeMaps} after observing that $F(i_n)$ itself is almost free.
\end{proof}

\begin{corollary}\label{AllCofibrationsOfSimplicialGroupsAreMonomorphisms}
  All cofibrations in [[sGrp]] are [[monomorphisms]], hence the [[underlying]] maps are [[cofibrations]] in [[classical model structure on simplicial sets|$sSet_{Qu}$]].
\end{corollary}
\begin{proof}
  Since the almost free maps (Def. \ref{AlmostFreeMaps}) are manifestly monomorphisms and since all cofibrations are retracts of almost free maps (Prop. \ref{CharacterizationOfCofibrations}) this follows form the fact that injections (cofibrations in $sSet_{Qu}$) are closed under retracts.
\end{proof}
