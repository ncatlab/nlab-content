...
### Explicit construction over a field 
...
#### An even more explicit construction 

Specializing still further, we discuss the detailed structure of the cofree coalgebra over a 1-dimensional vector space (over a field) $k$. 

Here, the cofree coalgebra $T(k)^\circ$ is the filtered colimit of finite-dimensional coalgebras of the form 

$$(k[x]/I)^\ast$$ 

where $k[x]/I$ is a (finite-dimensional) quotient of the polynomial algebra $k[x]$ modulo a non-zero ideal $I$. As a vector space this is embedded in $k[x]^\ast = \prod_n k \cdot x^n$, the space of formal power series in $x$. Thus $T(k)^\circ$ is the (set-theoretic) union of the coalgebras $(k[x]/I)^\ast$ along the canonical inclusions between them, i.e., every $\xi \in T(k)^\circ$ belongs to some $(k[x]/I)^\ast$, and it suffices to consider the structure of a typical such $\xi$ as a formal power series, and how the comultiplication behaves on it. 
...
for all $p(x) \in (g(x))$, where the pairing is the canonical one: 

$$\langle x^j, \sum_n a_n x^n \rangle = a_j$$ 

Taking the case $p(x) = x^j g(x)$, this leads to recurrence relations 
...
Now let $g^{rev}(x)$ be the polynomial obtained by reversing the order of the coefficients $b_0, \ldots, b_n$ of $g$, and consider the problem of multiplicatively inverting $g^{rev}(x)$ (which we may do since the constant coefficient of $g^{rev}$ is non-zero). We have 

$$1 = g^{rev}(x)\sum_n a_n x^n = (b_n + \ldots + b_0 x^n)(\sum_j a_j x^j)$$ 
...
Thus the power series of $g^{rev}(x)^{-1}$ yields a $\xi$ which vanishes on $I$, and similarly the power series of 
...
From all this, we take away the fact that an arbitrary element of $T(k)^\circ$ is of the form $\frac{p(x)}{h(x)}$ for some $h$ with non-zero constant coefficient, so that we now have a precise identification 

$$T(k)^\circ \cong k[x]_{(x)}$$ 

where the right side is the localization of $k[x]$ at the prime ideal $(x)$. 
...
The definition of $\delta(\frac{x^i}{g^{rev}(x)})$ can in principle be extracted from the multiplication table for residue classes $x^j \pmod (g(x))$, but the algebra works out more nicely if we assume if $k$ is algebraically closed. In that case we can take advantage of partial fraction decompositions of the form 

$$\frac{x^i}{g^{rev}(x)} = \sum_k \frac{A_k}{(1- r_k x)^{n^k}}$$ 
