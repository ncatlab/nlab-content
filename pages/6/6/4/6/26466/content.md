...
A **total preorder** or **preference relation** is a [[preorder]] whose [[posetal reflection]] is a [[total order]], or equivalently it is a [[preorder]] which is also a [[total relation]]: 
...
for rational numbers $a$, $b$, $c$, and $d$. This strict order is not [[connected relation|connected]] because $0 \neq \epsilon$, and thus the negation of the strict order, $a \leq b \coloneqq \neg(b \lt a)$, is not [[antisymmetric]]. However, $\leq$ is a total preorder, because the strict order $\lt$ is irreflexive, weakly linear, transitive, and asymmetric, which implies that $\leq$ is reflexive, transitive, and total. 
