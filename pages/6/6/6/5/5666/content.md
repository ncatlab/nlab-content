...
If $G$ is an [[affine scheme|affine]] [[commutative ring|commutative]] [[unitary]] [[group scheme]] over $k$ we construct (in [Proposition (codir)](#thmcodir)) a codirected diagram of Witt modules such that the codirected system $M(G)_n:= Hom(G, W_n(k))$ and ist limit $M(G):=lim_n Hom(G, W_n(k))$ become Dieudonn&#233; modules.
...
For a (affine commutative unipotent) group scheme the codirected systems of Witt modules endows the limit of the hom-spaces $hom(G,W_n(k))$ with the structure of a Dieudonn&#233; module:
