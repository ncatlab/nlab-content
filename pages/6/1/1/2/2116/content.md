...
On one hand, the analytic result that every non-negative real number has a [[square root]] and for every odd degree real polynomial there exists a root of the polynomial is constructively valid. However, the stronger statement that for every odd degree real polynomial one can construct a specified root of the polynomial is not provable in [[constructive mathematics]] without some kind of choice. This stronger statement is needed for the algebraic result of being able to factor real polynomial of odd degree into a real polynomial of degree 1 and a monic polynomial of even degree with coefficients in the reals. 

On the other hand, the first portion, that proves that $R[\mathrm{i}]$ is algebraically closed whenever $R$ is a [[real closed field]], can be provable or can be not provable in constructive mathematics, depending upon which definition of a real closed field one uses in the proof. 
