...
This notion also makes sense when $X$ is a [[category]], if $I$ is taken to be the 'directed interval' $\{ 0 \to 1 \}$, i.e. the [[ordinal]] $\mathbf{2}$.

Another, perhaps more common, meaning of 'cone' in category theory is that of a _cone over a [[diagram]]_; a cone over $F\colon J \to C$ is an [[object]] $c$ in $C$ equipped with a [[morphism]] from $c$ to each vertex of $F$, such that every *new* triangle arising in this way [[commutative triangle|commutes]].  The dual notion is _[[cocone]]_, where $c$ is equipped with a morphism *to* $c$ *from* each vertex of $F$ (but $c$ itself still belongs to $C$).
