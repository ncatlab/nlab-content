...
The monad is given on objects by $X \mapsto I^{\hom(X, I)}$; this lands in compact Hausdorff spaces. The [[image]] of the unit $u_X: X \to I^{\hom(X, I)}$ may be identified with the closure of the set-theoretic image; under the ultrafilter principle, this image $im(u_X)$ is compact Hausdorff and the natural map $X \to im(u_X)$ is universal among maps from $X$ to compact Hausdorff spaces, and thus gives the left adjoint to the forgetful functor $U: Top \to Comp$. As discussed at the article [[Tychonoff space]], the unit interval $I$ is coseparating for maps between Tychonoff spaces $X$, i.e., $I$ is a [[cogenerator]], and the map 
