...
For $n \in \mathbb{N}$, let $u \in \mathcal{D}'(\mathbb{R}^n \setminus \{0\})$ be a [[distribution]] on the [[complement]] of the origin, with non-[[positive number|positive]] [[degree of divergence]] at the origin
...
Then $u$ has a _unique_ [[extension of distribitions]] $\hat u \in \mathcal{D}'(\mathbb{R}^n)$ to the origin with the same degree of divergence
...
for [[smooth functions]] $r_\alpha \in C^\infty_{cp}(\mathbb{R}^)$.
...
Therefore lemma \ref{ExtensionUniqueNonPositiveDegreeOfDivergence} says that each $x^\alpha u$ for ${\vert \alpha\vert} = \rho + 1$ has a unique extension $\widehat{ x^\alpha u}$ to the origin. It follows via the decomposition in (eq:ForExtensionOfDistributionsTestFunctionDecomposition) that every extension $\hat u$ of $u$ is of the form
