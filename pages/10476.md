...
More generally, an analog of Elemendorf's theorem holds in situations where the orbit category for $G$ is replaced by an orbit category generated by any small category, $I$, where the $I$-orbits are $I$-diagrams in $Top$ whose strict colimit is equal to a point (see pt. 11 of p. 8 of [PHCTIntro](Parametrized+Higher+Category+Theory+and+Higher+Algebra#PHCTIntro)).
