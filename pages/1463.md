...
It can prove *some* traditional applications of [[dependent choice]]. For example, it can be used to [[linear order|linearly order]] any set. From there it can be used to prove that a countable union of nonempty finite sets is countable, which in turn can be used to establish the weak K&ouml;nig lemma; see [here](/nlab/show/compactness+theorem#totalorder). 
