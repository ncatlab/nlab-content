...
be the positive rational numbers. Let $S$ be a [[premetric space]], and let $x:I \to S$ be a [[net]] with index set $I$. A __modulus of convergence__ is a function $M \in I^{\mathbb{Q}_{+}}$ such that for all positive rational numbers $\epsilon$ and all indices $i \in I$ and $j \in I$, if $i \geq M(\epsilon)$ and $j \geq M(\epsilon)$, then $x_i \sim_{\epsilon} x_j$. 
