...
Given [[presentation]]s for the $G_i$, it is straightforward to find a presentation of $\star_i G_i$; if $G_i = \langle S_i|R_i\rangle = F_i/N_i$, where each $F_i=\langle S_i\rangle$ is the [[free group]] on the set $S_i$ and $N_i\subset F_i$ is the [[normal subgroup]] of $F_i$ generated by the subset $R_i\subset F_i$, then the free product
$$\star_i G_i := \langle \coprod_i S_i | \coprod_i R_i \rangle = (\star_i F_i)/\langle\cup_i N_i\rangle$$
is presented by the [[disjoint union]]s of the $S_i$ and the $R_i$.  As with anything satsifying a [[universal property]], the result (up to a unique coherent isomorphism) does not depend on the presentation.
