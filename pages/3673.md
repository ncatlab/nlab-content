...
1. $RLP\big( L^{-1} Cof \big) \,\subset\, L^{-1}(W)$

then the left-transferred model category structure on $\mathcal{D}$ exists (i.e. with cofibrations $L^{-1}(Cof)$ and weak equivalences $L^{-1}(W)$)
