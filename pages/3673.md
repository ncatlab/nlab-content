...
Necessary and sufficient conditions for the existence of the right transferred model structure (Def. \ref{RightTransferredModelStructure}) are:
...
Moreover, if every anodyne map is a weak equivalence so that the transferred model structure on $D$ exists (by Thm. \ref{NeccSuff}), then it is is cofibrantly generated, and for $I$ (resp. $J$) a set of generating cofibrations (resp. trivial cofibrations) in $C$, the image set $F(I)$ (resp. $F(J)$) forms a set of generating cofibrations (resp. trivial cofibrations) in $D$.
\end{proposition}
