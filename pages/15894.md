...
Yes. One could define a category as a type $C$ whose identity types are sets, and for every term $a:C$ and $b:C$, there is a set $Hom_C(a, b)$, and for every term $a:C$, there is a function $id_a:Hom_C(a, a)$ and for every term $a:C$, $b:C$, and $c:C$, and sets $Hom_C(a, b)$, $Hom_C(b, c)$, and $Hom_C(a, c)$, there is a function 
$$(-)\circ_{a,b,c}(-):Hom_C(b, c) \times Hom_C(a, b) \to Hom_C(a, c)$$ 
such that for every term $a:C$ and $b:C$ and term $f:Hom_C(a, b)$, $id_b \circ_{a,b,b} f = f$ and $f \circ_{a,a,b} id_a = f$, and for every term $a:C$, $b:C$, $c:C$, and $d:C$, and terms $f:Hom_C(a, b)$, $g:Hom_C(b, c)$, $h:Hom_C(c, d)$, 
$$h \circ_{a,c,d} (g \circ_{a,b,c} f) = (h \circ_{b,c,d} g) \circ_{a,b,d} f$$

Then one could define a functor between categories $C$ and $D$ as a function $F:C \to D$ with functions $f_{a,b}:Hom_C(a, b) \to Hom_D(F(a), F(b))$ such that $f_{a,a}(id_a) = id_{F(a)}$ and 
