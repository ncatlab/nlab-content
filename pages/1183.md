...
and called the brane _perturbation series_ . For instance the _Feynman perturbation series_ for $n = 1$, or the _string perturbation series_ for $n = 2$: we sum over all possible $S$-structured [[cobordism]]s $\Sigma$, each representing an interaction "channel" for branes of shape $\partial_{in} \Sigma$ to scatter into shape $\partial_{out} \Sigma$ the linear maps $S(\Sigma) : \mathcal{V}_{\partial_{in} \Sigma} \to \mathcal{V}_{\partial_{out} \Sigma}$ weighted by some [[measure]] $\mu(\Sigma)$, and all regarded as giving one single endomorphism on $Sym^\bullet(\mathcal{V})$ in the evident way.
