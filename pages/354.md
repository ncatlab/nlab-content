...
* Kan complexes are among the most convenient and popular models for [[infinity-groupoid|infinity-groupoids]]. The horn filling condition from this point of view is read as guaranteeing that
...
This is one way to see and motivate that a simplicial set that is a [[Kan complex]] but which does not necessarily have unique fillers makes models an [[nLab:infinity-groupoid|infinity-groupoid]].
...
The [[nerve]] $N(C)$ of a [[strict omega-category]] 
is a Kan complex if and only if $C$ is a [[strict omega-groupoid]].
...
That $\Pi(X)$ is indeed a Kan complex is intuitively clear. Technically it follows from the fact that the inclusions ${{\Lambda^n}_{Top}}_k \hookrightarrow \Delta^n_{Top}$ of topological horns into tolological simplices are [[retract]]s.
