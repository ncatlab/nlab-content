...
of $n$ different [[types]]/species of [[fields (physics)|physics]], such that the [[gauge fixing|gauge fixed]] [[free field|free]] [[Lagrangian density]] $\mathbf{L}'$ is the [[sum]] 

$$
  \mathbf{L}' \;=\; \mathbf{L}'_{sp_1} + \cdots + \mathbf{L}'_{sp_n}
$$

of [[free field theory|free]] [[Lagrangian densities]] on these separate field bundles
...
In this situation we say that a _[[Feynman diagram]]_ $FD$ is an _$Sp$-labeled finite multigraph_, hence:
...
We write $\mathcal{\Gamma}^{Sp}$ for the set of [[isomorphism classes]] of Feynman diagrams with labels in $Sp$.
...
$$
  \Delta_F 
  \;=\;
  \Delta_{F,1} \oplus \cdots \Delta{F,n}
  \;\in\;
...
In this case also [[Feynman amplitude]] $\Gamma\left( (V_r))_{r = 1}^v \right)$ (def. \ref{FeynmanPerturbationSeriesAwayFromCoincidingPoints})
for a [[finite multigraph]] $\Gamma \in \mathcal{G}_v$ (def. \ref{Graphs})
decomposes as a sum 
...
$$
  ...
  \right)
  \;=\;
  \underset{ E_\Gamma \ovrset{L}{\to}  Sp }{\Sum}
    \Gamma\left( 
