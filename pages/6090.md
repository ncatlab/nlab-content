...
(A priori could be a [[Laurent]] series in $\hbar$, but in fact it contain no negative powers in $\hbar$, this is prop. \ref{FeynmanDiagramLoopOrder} below.)
...
A [[finite multigraph]] $\Gamma$ (def. \ref{Graphs}) is called a _[[tree]]_ if for any two of its [[vertices]]
there is at most one [[path]] of [[edges]] connecting them. We write
...
+-- {: .num_prop #GraphPlanar}
###### Proposition
**([[planar graph]])**

A [[finite multigraph]] (def. \ref{Graphs}) is called a _[[planar graph]]_ if it
admits an [[embedding]] into the [[plane]], hence if it may be "drawn on a plane" without intersections, in the evident way.
...
is precisely that of [[Feynman amplitudes]] of those [[finite multigraphs]] (prop. \ref{FeynmanPerturbationSeriesAwayFromCoincidingPoints}) which are [[trees]] (def. \ref{GraphTree}); thus called the _[[tree level]]_-contribution:
...
Moreover, a [[finite multigraph]] $\Gamma$ (def. \ref{Graphs}) which is [[planar graph|planar]] (def. \ref{GraphPlanar}) and [[connected graph|connected]] (def. \ref{ConnectedGraphs}) has a [[Feynman amplitude]] (prop. \ref{FeynmanPerturbationSeriesAwayFromCoincidingPoints})
precisely of order 
...
often called the _[[loop order]]_ of the diagram.
...
corresponding to some $\Gamma \in \mathcal{G}$ contributes precisely at 
...
Finally, the [[effective action]], by definition (eq:ExpansionEffectiveAction), has the same contributions 
of Feynman amplitudes as in (eq:GeneralFeynmanDiagramhbarContribution) but multiplied by another power of $\hbar^1$, hence it contributes at order
...
The first claim is equivalent to saying
that the [[Feynman amplitude]] of every [[connected graph|connected]] [[finite multigraph]]
contributes powers in $\hbar$ of order $\geq -1$ and comtributes the power $= -1$ precisely if the graph is a tree.
...
This means that the connected multigraph $\Gamma$ in general contributes at order
...
and precisely if it is a tree it contributes at order $\hbar^{-1}$.
