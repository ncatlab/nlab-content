...
We consider here the [[axioms]] for a perturbative S-matrix relative to a fixed [[free field theory|free]] [[quantum field theory|quantum field]] [[vacuum]] as used in  [[causal perturbation theory]] (def. \ref{LagrangianFieldTheoryPerturbativeScattering} below).
...
+-- {: .num_defn #VacuumFree}
###### Definition
**([[free field theory|free]] [[relativistic field theory|relativistic]] [[quantum field theory|quantum field]] [[vacuum]])**
...
1. $\mathcal{G} \overset{fb}{\to} \Sigma$ a [[gauge parameter bundle]] for $(E,\mathbf{L})$ ([this def.](A+first+idea+of+quantum+field+theory#GaugeParameters)), with induced BRST-reduced Lagrangian field theory $(E \times_\Sigma \mathcal{G}[1], \mathbf{L} - \mathbf{L}_{BRST})$
...
1. $\Delta_H \in \Gamma'( E_{\text{BV-BRST}} \boxtimes E_{\text{BV-BRST}} )$ a [[Wightman propagator]] $\Delta_H = \tfrac{i}{2} \Delta + H$ compatible with the [[causal propagator]] $\Delta$ which corresponds to the [[[Green-hyperbolic differential equation|Green hyperbolic]] [[Euler-Lagrange equation|Euler-Lagrange]] [[equations of motion]] induced by the gauge-fixed [[Laranian density]] $\mathbf{L}'$.

Write $(PolyObs(E_{\text{BV-BRST}})_{mc}[ [\hbar] ], \star_H)$ for the corresponding [[Wick algebra]] of [[microcausal polynomial observables]] of the [[free field theory|free]] [[quantum field theory]] and write
...
for the induced [[Hadamard vacuum state]] ([this prop.](Wick+algebra#WickAlgebraCanonicalState)), hence the state whose [[2-point function]]
is the chose [[Wightman propagator]]: 
...
Def. \ref{LagrangianFieldTheoryPerturbativeScattering} is due to ([EBewpstein-Glaser 73 (1)](#EpsteinGlaser73)),
...
Beware the terminology in def. \ref{LagrangianFieldTheoryPerturbativeScattering}: Strictly speaking "an S-matrix" is one single observable
...
for a fixed ([[adiabatic switching|adiabatically switched]] [[local observable|local]]) [[interaction]] $S_{int}$, reflecting the [[scattering amplitudes]] with respect to that particular interaction.  Hence the function
...
axiomatized in def. \ref{LagrangianFieldTheoryPerturbativeScattering} is really a whole _scheme_ for constructing compatible S-matrices for all possible (adiabatically switched local) interactions at once.
...
This perspective on $\mathcal{S}$ becomes more manifest with the proof of the [[main theorem of perturbative renormalization]].
