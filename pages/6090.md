...
[[algebra of observables]], also a _[[state on a star-algebra|state]]_ 
  ...
  [[state on a star algebra|state]] on $\mathcal{A}_{int}$, it is no longer expected to be a _[[vacuum]]_ state in a
  suitable sense. Rather, just as the [[interacting field algebra of observables]] $\mathcal{A}_{int}$ is a [[deformation]] of the free field algbra of observables ([[Wick algebra]]), there ought to be a corresponding deformation of the free [[Hadamard vacuum state]] $\langle- \rangle$ into an "interacting vacuum state" $\langle - \rangle_{int}$.
  
  This is an open problem of [[perturbatibe quantum field theory]].
