...
* $e_i (e_j e_k) = - (e_j e_k) e_i$ for $i,j,k$ pairwise distinct.
...
Once we have given the octonions their natural inner product, an imaginary octonion squares to $-1$ iff it has norm $1$, and two imaginary octonions anticommute iff they are orthogonal.  Thus we can equivalently define a basic triple to be an orthonormal triple of imaginary octonions such that the third is orthogonal to the subalgebra generated by the first two.   (Given this, each is orthogonal to the subalgebra generated by the other two.)
