...
### As a commutative $\mathbb{Z}$-algebra

Let $(\mathbb{N}^+,1:\mathbb{N}^+,s:\mathbb{N}^+\to \mathbb{N}^+)$ be the set of positive integers, and let $\mathbb{Z}$ be the [[commutative ring]] of integers. Then there exist [[commutative algebra|commutative $\mathbb{Z}$-algebras]] $A$ with injections $inj,inv:\mathbb{N}^+\to\A$ satisfying the following axioms:

* $inj(1) = 1$
* $inj(s(n)) = inj(n) + 1$ for all $n:\mathbb{N}^+$
* $inj(n) \cdot inv(n) = 1$ for all $n:\mathbb{N}^+$. 

The first two axioms state that there is an embedding of the positive integers in the rational numbers, amd the third axiom states that every positive integer has a multiplicative inverse. The commutative $\mathbb{Z}$-algebra of rational numbers $\mathbb{Q}$ is the [[initial object|initial]] such commutative $\mathbb{Z}$-algebra, and general commutative $\mathbb{Z}$-algebras satisfying the axioms above are called commutative $\mathbb{Q}$-algebras. 

It can then be proven from the commutative algebra axioms that every rational number apart from zero has a multiplicative inverse, making $\mathbb{Q}$ a field.
