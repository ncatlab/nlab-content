...
This means in modern language, that if $G$ is the given group acting on a given space $X$, and if $S \hookrightarrow X$ is a given subspace (a configuration), then the "body" generated by this is the [[coset space]] $G/Stab_G(S)$ of $G$ by the [[stabilizer subgroup]] $Stab_G(X)$ of that configuration. See also there at _[Stabilizer of shapes -- Klein geometry](stabilizer%20group#KleinGeometry)_.
