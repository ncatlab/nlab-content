...
Putting your name in the text box next to the "Submit" button is optional.

This works best if your original comment was composed using the text filter (see the pulldown menu atop the blog comment edit box) called _Markdown with itex to MathML_ .
...
Slightly better is to copy the `XHTML+MathML` source (in _firefox_ : "View" followed by "Page Source") except that then you are pasting raw `XHTML` into the $n$Lab.  Whilst this will work, it makes it harder to others to edit afterwards.
