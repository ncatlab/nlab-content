...
* $v$ defines the homomorphism of rings $v|_K : K^*\to \mathbf{Z}$ where $K^*$ is the multiplicative group of $K$
...
The set $R_v = \{x\in K\,|\, v(x)\geq 0\}$ is called the __valuation ring__ of the valuation $v$, and it is an integral domain with quotient field $K$; its part $\mathfrak{p}_v = \{x\in K\,|\, v(x)\gt 0\}$ is a maximal ideal of $R_v$ and is called the __valuation ideal__ of $v$. 
...
A valuation is __non-archimedean__ if $C$ above can be taken $1$ and archmedean otherwise. A (multiplicative) valuation is __discrete__ if there is a neighborhood $U\ni 1\in \mathbf{R}_+$ such that the only $x\in K$ such that $|x|\in U$ is $1_K\in K$.  
