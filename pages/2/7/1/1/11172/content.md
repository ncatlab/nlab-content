...
Generalizing all of these examples (except the first), if $\alpha$ is a plump ordinal, then the successor $\alpha^+$, consisting of all lower sets of $\alpha$, is also a plump ordinal.  This is bigger than $\alpha + 1 = \alpha \cup \{\alpha\}$, in that if $0 \lt \alpha$ and $\alpha^+ = \alpha + 1$, then excluded middle holds.
