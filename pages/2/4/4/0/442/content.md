...
The tautological example of a Lawvere theory is the algebraic theory of _no_ operations.

Its syntactic category is the category $\mathcal{S}$ on objects $n \in \mathbb{N}$ with morphisms generated by the projections $\pi_i : n \to 1$. An algebra over this theory is just a bare [[set]]:
...
This sends each algebra to its _underlying set_ . For more see the secton [Adjunctions between categories of algebras](#AdjCatAlgs) below.
...
### Adjunctions between categories of $T$-algebras {#AdjCatAlgs}
...
Let $F_{T_i} \dashv U_{T_i}$ be the two adjunctions given by the previous lemma. For $A$ a $T_1$-algebra there is a $T_1$-[congruence](#congruence) $\Gamma$ such that
