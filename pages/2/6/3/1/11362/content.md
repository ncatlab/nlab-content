...
Consider the linear order $\mathbb{R}_{\geq 1}$ (this time not including $\infty$); let $\beta(x)$ be the smallest integer _strictly_ greater than $x$. 
...
$$\xi: \mathbb{R}_{\geq 1} \to \mathbb{N} \times \mathbb{R}_{\geq 1}$$ 

where $\xi(x) = (-\frac1{x - \beta(x)}, \beta(x))$. 
...
The initial $H$-algebra is the free monoid $\mathbb{N}_+^{\mathbb{N}}$, [[lexicographic order|lexicographically ordered]] according to the convention given in this [remark](nlab/show/lexicographic+order#antidictionary) (where initial segments of words $w$ are _greater_ than $w$). 
...
The $H$-algebra isomorphism $\mathbb{N}_+^{\mathbb{N}} \to \mathbf{Q}$ takes a finite word $a_0 a_1 \ldots a_n$ to the rational number to which the acf 
