...
### Rationals as initial $F$-algebra  

Let $\mathbf{Q} = (\mathbb{Q} \cap \mathbf{R}) + \{\infty\}$, the set of rational elements of $\mathbf{R}$. There is an $F$-algebra structure 

$$\gamma: 1 + \mathbf{Q} \times \mathbb{N}_+ \to \mathbf{Q}$$ 

where $\gamma(\ast) = \infty$, and $\gamma(q, n) = n + 1/q$. 

+-- {: .num_theorem #initial} 
###### Theorem 
$(\mathbf{Q}, \gamma)$ is the initial $F$-algebra. 
=-- 

Perhaps the most direct way of seeing this is to use the fact that the initial algebra is (as is well-known) the set of words $(\mathbb{N}_+)^\ast$ in the alphabet $\mathbb{N}_+$, and then to straight away invoke an algebra isomorphism $(\mathbb{N}_+)^\ast \to \mathbf{Q}$ which sends a word $a_0 a_1 \ldots a_n$ to the rational represented by the continued fraction $[a_0, a_1, \ldots, a_n]$ (and the empty word to $\infty$). 
...
John Baez asks a question about this in [TWF228](http://math.ucr.edu/home/baez/week228.html), and reports on an answer in [TWF229](http://math.ucr.edu/home/baez/week229.html). 
