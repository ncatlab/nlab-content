...
A closely related situation is considered in the context of [[groupoidification]], where the aim is to encode not only the operation of matrix multiplication geometrically in the above sense, but also the very notion of a function itself. In [[groupoidification]] functios with values in fields are essentially replaced by functions with values in (finite) [[groupoid]]s, and the notion of [[groupoid cardinality]] is used to interpret such a gadget as a function with values in rational numbers. 
...
In full generality the idea of _geometric function theory_ is to replace in the above toy example all finite sets with generalized higher spaces in the form of [[infinity-stack homotopically|infinity-stacks]] and to realizes a useful notion of higher "functions" $C(X)$, such that $C(X)$ is a monoidal [[infinity-category|infinity-category]] in a suitable sense and behaves as expected under [[homotopy theory|homotopy]] pullback and push-forward operations.
...
As discussed at [[span trace]] the [[homotopy limit|homotopy pullback]] of this is the [[loop space object]] $\Lambda X$ of $X$. 
...
The expression on the right is known as the higher _trace_ of the monoidal $\infty$-category $C(X)$, which is the higher _Hochschild homology_ of $C(X)$. (See next subsection for concrete realizations).


### Realization in terms of derived $\infty$-stacks on the algebraic site ###

Aspects of geometric function theory have a long history in the context of [[sheaf|sheaves]] and [[stack|stacks]] over algebraic [[site]]s. The _Fourier-Mukai transform_ is a classical example of a pull-tensor-push operation in a context where the object $C(X)$ of generalized functions on a space $X$ is taken to be $D(X)$, the derived category of coherent sheaves on $X$.
...
As any [[homotopy category]], the derived category $D(X)$ of coherent sheaves is naturally understood as the 1-categorical shadow of an [[(infinity,1)-category]]. In a suitable context of derived $\infty$-stacks there is for each such $\infty$-stack $X$ an $(\infty,1)$-category $QC(X)$ whose homotopy category is the ordinary derived category $D(X)$ of quasi-coherent sheaves on $X$.

The full context of geometric function theory with functions $C(X)$ taken to be given by $QC(X)$ was established in
