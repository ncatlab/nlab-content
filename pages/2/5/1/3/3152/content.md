...
If we assume the [[axiom of choice]], then every fibered category has a cleavage, and even a normal cleavege (but not neccesarily a splitting).  Given a cleavage of $p$ and an arrow $u:b'\to b$ in $B$, there is a functor $u^*\colon p^{-1}(b)\to p^{-1}(b')$ which to every object $e\in p^{-1}(b)$ assigns the domain of the specified arrow in the cleavage which is above $u$ and whose codomain is $e$.  This correspondence extends to a functor thanks to the universal property of the cartesian arrows.
