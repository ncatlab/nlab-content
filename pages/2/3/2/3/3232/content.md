...
More abstractly, the monads $s$ and $t$ in $K$ give rise to ordinary monads $s^*$ and $t_*$ on the hom-category $K(b,a)$.  The associativity isomorphism of $K$ then gives rise to an invertible [[distributive law]] between these, so that the composite $s^* t_* \cong t_* s^* \colon x \mapsto t x s$ is again a monad.  Then the category $Mod_K(s,t)$ of bimodules from $s$ to $t$ is the ordinary [[Eilenberg--Moore category]] $K(b,a)^{s^* t_*}$.
