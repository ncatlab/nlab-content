...
A __tagging__ of $\pi$ is a list $t_0, \ldots, t_{n-1}$ such that $c_i \leq t_i \leq c_{i+1}$ for $i \lt n$.  Tagged partitions are used to define the [[Riemann integral]], the [[Darboux integral]], and the [[gauge integral]].  A __free tagging__ (which following the [[red herring principle]] is not generally a tagging) is any list $t_0, \ldots, t_{n-1}$ of elements of $[a, b]$ whatsoever; these are used in the McShane integral (a Riemann-like approach to the [[Lebesgue integral]] on the real line).
...
(One might merely allow $A$ and $B$ to be equivalent, meaning that their [[symmetric difference]] is null, when $A \cap B$ is positive, but this does not give a good notion of tagged partition; really, we should insist that $\pi$ be _saturated_: if $A$ and $B$ are equivalent and $A \in \pi$, then $B \in \pi$.  Alternatively, especially if one views the partition an _[[indexed subset|indexed]]_ collection of subsets, then one would still require $A = B$ when $A \cap B$ is positive.)
...
In the last few examples, each part of a partition is an [[inhabited set]]; in such cases, a __tagged partitition__ is a partition in which each part has been equipped with one of its elements.  In these examples, the parts are all [[subsets]] of some ambient space; in such cases, a __freely tagged partition__ is a partition in which each part has been equipped with an element of the ambient space.
...
[[!redirects freely tagged partition]]
[[!redirects freely tagged partitions]]
[[!redirects free tagged partition]]
[[!redirects free tagged partitions]]
