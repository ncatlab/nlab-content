...
##Discussion## 
In fact each relation will give **two** simplicial complexes, one as above, the other obtained by reversing the roles of $X$ and $Y$ in the above. In other words, in the usual way define the _opposite relation_ by '$R^{op}\subseteq Y\times X$ is the relation in which $yR^{op}x$ if and only if $xRy$'.
...
Another obvious question is to compare $V(R)$ and $C(R)$, are they closely related. and that is discussed more in [[Dowker's Theorem]].  It looks very much a situation for a combinatorial duality result.
