...
For $0 \lt p \lt 1$, we can define the $L^p$ space using the same formula; however, it is no longer a Banach space (but still an [[F-space]]).  For $p = 0$, we can again take the limit, or equivalently use the essential infimum; every measurable function on $\Omega$ belongs to $L^0$.
