...
+-- {: .proof}
###### Proof 
The elementary proof we give follows [exercise 7H](http://katmat.math.uni-bremen.de/acc/acc.pdf#page=129) of ([AdamekHerrlichStrecker](#AdamekHerrlichStrecker)). It is however nonconstructive (because it contains if-then-else lines), so we will follow up with a second constructive proof. 
...
+-- {: .proof} 
###### Second Proof 
The following proof can be enacted in any [[topos]] with [[NNO]]. 

Let $\pi: H \to H/K$ be the canonical projection, considered as a [[surjective function]]. Embed the set $H/K$ into an [[abelian group]] $A$; for example, we could take the [[basis]] embedding $j: H/K \hookrightarrow A = (H/K) \cdot \mathbb{Z}/2$ into the [[free object|free]] $\mathbb{Z}/2$-[[vector space]] generated by $H/K$. The set $A^H$ of functions $f: H \to A$ is an abelian group under the pointwise addition inherited from $A$, and $H$ acts on $A^H$ via 

$$(h \cdot f)(h') \coloneqq f(h' h^{-1}).$$ 

For any $f \in A^H$, the map $d_f: H \to A^H$ defined by $d_f(h) = h f - f$ is an (inner) [[derivation]]. Taking $f = j \pi$, this derivation induces a homomorphism into the [[semi-direct product]] (the wreath product) 

$$\phi: H \to A^H \rtimes H$$ 

defined by $\phi(h) = (d_{j \pi}(h), h)$. There is a second homomorphism $\psi: H \to A^H \rtimes H$ defined by $\psi(h) = (0, h)$. The equalizer of $\phi, \psi$ is the original mono $i: K \to H$: 

$$\array{
d_{j \pi}(h) = 0 & iff & h \cdot j \pi = j \pi \\ 
 & iff & (\forall_{h': H})\; j \pi(h' h^{-1}) = j \pi(h') \\ 
 & iff & (\forall_{h': H})\; j(h' h^{-1} K) = j(h' K) \\ 
 & iff & (\forall_{h': H})\; h' h^{-1} K = h' K \\ 
 & iff & h^{-1} K = K \\ 
 & iff & h \in K.
}$$ 
