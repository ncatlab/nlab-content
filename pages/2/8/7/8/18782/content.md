...
3. $M_X$ is closed under gluing functions with [[disjoint union|disjoint]] [[Borel domains]]: for any partition $\mathbb{R} = \biguplus_{i\in\mathbb{N}} S_i$ by Borel $S_i$, and $\{\alpha_i \in M_X\}_{i\in\mathbb{N}}$, then the function $\beta(x) = \alpha_i(x)$ when $x \in S_i$ is in $M_X$.
