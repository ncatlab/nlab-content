...
On the other hand, we could generalize the notion of [[group]] to allow for an empty group.  This even remains a purely algebraic notion: we can define a group as an [[inhabited]] [[invertible magma|invertible]] [[semigroup]] $(S,\cdot,(-)^{-1})$. Then any [[invertible semigroup]] is a possibly-empty-heap, and every possibly-empty-heap arises in this way from its automorphism invertible semigroup (defined by either method (2) or (3)); the category of possibly-empty-heaps is equivalent to the category of inverse semigroups equipped with torsors over the point; etc.

This is even [[constructive mathematics|constructive]]; the theorems can be proved uniformly, rather than by treating the empty and inhabited cases separately. (This rather trivial method is obvious to a classical mathematician, but it\'s not constructively valid, since a invertible semigrouo/heap as defined here can\'t be constructively proved empty or inhabited; it can only be proved empty iff not inhabited.  Indeed, taking any group $G$ and any [[truth value]] $P$, the invertible subsemigroup $\{x \in G \;|\; P\}$ is empty or inhabited iff $P$ is false or true.)
