...
Because [[Set]] is a [[cartesian closed category]], currying $i_{a,b}$ in [[Set]] means that there is an [[evaluation map]] $(-)((-))\colon Hom(a,b) \times El(a) \to El(b)$ such that for every morphism $f \colon Hom(A,B)$ and $g \colon Hom(B,C)$ and every element $x:U(A)$, $(g \circ f)(x) = g(f(x))$ and $id_A(x) = x$. Similarly, since [[Set]] is well-pointed, 
