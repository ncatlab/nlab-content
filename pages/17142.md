...
**1. Well-defined stable homotopy groups**

We check that a single manifold with stable normal $\mathcal{B}$-structure (def. \ref{ManifoldWithBfStructure}) induces a well-defined element of the [[stable homotopy groups]] of the universal [[Thom spectra]] $M\mathcal{B}$.

Given an embedded manifold $X \overset{i}{\hookrightarrow}\mathbb{R}^{k_1}$ with normal $\mathcal{B}$-structure 
...
for its image under the [[Pontrjagin-Thom construction]] (def. \ref{PontrjaginThomConstruction}). Now given $k_2 \in \mathbb{N}$, consider the induced embedding $X \overset{i}{\hookrightarrow} \mathbb{R}^{k_1}\hookrightarrow \mathbb{R}^{k_1 + k_2}$ with normal $\mathcal{B}$-structure 

$$
  \array{
    && 
    B_{k_1-n}
      &\overset{j_{k_1-n, k_1+ k_2 -n}}{\longrightarrow}&  
    B_{k_1 + k_2-n}
    \\
    & {}^{\mathllap{(\hat g_i,\ast)}}\nearrow 
    &
    ...
    \\
    X 
      &\underset{(g_i,\ast)}{\longrightarrow}& 
    B O(k_1-n) 
      &\longrightarrow& 
    B O(k_1 + k_2-n)
  }
...
By prop. \ref{PullbackOfUniversalBfBundleUnderCoordinateRestriction} the classifying map $\hat g'_i$ for the second normal bundle sits in a diagram of the form

$$
  \array{
    (\nu_i \oplus \mathbb{R}^{k_2})
      &\longrightarrow&
    (E^{\mathcal{B}}_{k_1-n} \oplus \mathbb{R}^{k_2})
      &\longrightarrow&
    E^{\mathcal{B}}_{k_1 + k_2 - n}
    \\
    \downarrow &(pb)& \downarrow &(pb)& \downarrow
    \\
    X 
      &\underset{\hat g'_i}{\longrightarrow}& 
    B_{k_1-n}
      &\underset{j_{k_1-n, k_1 + k_2 - n}}{\longrightarrow}&
    B_{k_1 +k_2 - n}
  }
  \,.
$$

Hence the Pontrjagin-Thom construction now is the composite
...
$$
  ...
  Th(\mathbb{R}^{k_2}) \wedge Th(\nu)
    \overset{}{\longrightarrow}
  Th(\mathbb{R}^{k^2} \wedge Th(E^{\mathcal{B}}_{k_1-n}))
    \overset{}{\longrightarrow}
  Th(E^{\mathcal{B}}_{k_1 + k_2 - n})
  \,.
$$

The first part here is $S^{k_1}\wedge \alpha$, while the remainder is the structure map in the Thom spectrum. This is precisely the formula of the component maps in the sequential colimit that defines the stable homotopy groups. and hence both embeddings define the same stable homotopy group.
