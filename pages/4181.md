...
$$ L_a b = a \cdot b $$
and $[L_a, L_b]$ is the usual commutator of operators:
...
First, given a manifold with a torsion-free connection $\nabla$ on its tangent bundle, we can make the space of vector fields into a pre-Lie algebra by defining
$$  v \cdot w = \nabla_v w $$
...
Second, suppose $O$ is a linear [[operad]] whose space of unary operations is 1-dimensional, and let $A$ be the free $O$-algebra on one generator.  Then $A$ becomes a pre-Lie algebra in a manner described here:
...
Third, the Hochschild chain complex of any space, with grading shifted down by one, can be given the structure of a 'graded pre-Lie algebra', as discovered by Gerstenhaber, and described here:
