...
If one does with $f_k$, for $k \ge 0$, what Euler did with $f_0$ and $f_1$, one gets an infinite sequence of power series in $x$.  These functions of $x$ are those that appear in Cartwright's proof of the irrationality of $\pi$.  But in defining those functions, Cartwright used neither trigonometric expressions nor power series.  (The proof was published in an appendix to the third edition of Harold Jeffreys' book ''Scientific Inference''.  It is not in later editions.  It is outlined in Wikipedia's article on the proof of the irrationality of $\pi$.)
