...
\begin{remark}
${}_{A}\mathbf{Mod}^{\odot}$ is a [[reflective subcategory]] of ${}_{A}\mathbf{Mod}^{\bullet}$: the right adjoint of the inclusion functor
\[
    \iota \colon {}_{A}\mathbf{Mod}^{\odot} \longrightarrow {}_{A}\mathbf{Mod}^{\bullet}
\]
is given by the "cyclification" functor:
\[
    \mathsf{Cyc} \colon {}_{A}\mathbf{Mod}^{\bullet} \longrightarrow {}_{A}\mathbf{Mod}^{\odot}
\]
that acts on objects by assigning to any pointed module $(M,m)$ the cyclic pointed submodule generated by the orbit of $m$, i.e. $\mathsf{Cyc} \colon (M,m) \to (A \cdot m, m)$ and acts on morphisms by restriction to cyclic submodules.
\end{remark}
...
\[
   \mathsf{Ann}_{A} \colon (M,m) \mapsto \mathrm{Ann}_{A}(m),
\]
...
Moreover, $A \cdot [1_{A}] = A/I$; so, so the pointed module $(A/I, [1])$ is *cyclic*.
...
\begin{center}
  \begin{tikzcd}
    \mathbf{L}(A)
	\arrow[r, shift right=6pt, "\widehat{\mathsf{Quot}}_{A}"', "\bot"]
    & 
...
\end{remark}
\begin{remark}
	The subcategories $\mathbf{(N)Rep}^{\bullet}(\ldots)$ generated by cyclic objects are [[reflective subcategory|reflective subcategories]].
\end{remark}
