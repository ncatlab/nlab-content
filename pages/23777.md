...
\begin{remark}
We can easily define weaker versions of the relation $\leq$ that may also be of interest: for $f$ and $g$ positive linear functionals on a $C^\ast$ algebra $E$, we can define relations $\ll$ and $\lessim$ in the following manner:
...
* $f \leq g \to f \lesssim g \to f \ll g$.
