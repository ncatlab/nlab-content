...
where $f$ is continuous and $i$ is an open inclusion (or, up to [[equivalence of categories|equivalence]] of $2$-[[2-category|categories]], an embedding). The spans are locally (that is, for fixed $X$ and $Y$) ordered by inclusion.
...
whose opposite $i^op: X \to Ext(r)$ also belongs to $Reg$ (that is, $i$ is an embedding), and the equations
