...
An $R$-$S$-[[bimodule]] $B$ is **Artinian** if it satisfies the **descending chain condition** on its subbimodules: for any descending chain of $R$-$S$-subbimodules, there exists a natural number $m \in \mathbb{N}$ such that for all natural numbers $n \geq m$, the $R$-$S$-bimodule monomorphism $i_n:A_{n+1} \hookrightarrow A_{n}$ is an $R$-$S$-[[bimodule isomorphism]]. 
