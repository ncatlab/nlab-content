...
A _tree_ is a 1-dimensional [[CW-complex]] which is contractible, i.e., homotopy equivalent to a point. 
...
Let $\mathbf{F}$ denote the [[Lawvere theory]] freely generated by $n$-ary operations $\theta_n: x^n \to x$, one for each arity $n \geq 1$. A (finite) **planar forest** is a morphism $\phi: x^n \to x^m$ in $\mathbf{F}$; a (finite) **planar tree** is a morphism $\phi: x^n \to x$. 
