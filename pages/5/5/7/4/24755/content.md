...
Alternatively, given two [[parallel morphisms|parallel]] functions $s:B \to A$ and $t:B \to A$, the coequalizer of $s$ and $t$ could be defined as the W-suspension generated by the graph defined by the family of dependent sum types $\sum_{z:B} (s(z) =_A x) \times (t(z) =_A y)$
