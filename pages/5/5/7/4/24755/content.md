...
### Higher inductive types generated by graphs

A [[graph]] in dependent type theory consists of a type $A$ and a binary type family $R(x, y)$ indexed by $x:A$ and $y:A$. 

Given a graph $(A, R)$, one could construct a higher inductive type $\underset{x:A,y:A}{\mathcal{G}} R(x, y)$ which is generated by the graph in the following sense: there are point constructors 
$$\mathrm{points}:A \to \underset{x:A,y:A}{\mathcal{G}} R(x, y)$$ 
and for each $x:A$ and $y:A$, path constructors
$$\mathrm{paths}(x, y):R(x, y) \to \left(\mathrm{points}(x) =_{\underset{x:A,y:A}{\mathcal{G}} R(x, y)} \mathrm{points}(y)\right)$$

The recursion principle of $\underset{x:A,y:A}{\mathcal{G}} R(x, y)$ says that given a type $C$ together with 
...
there exists a function $c:\left(\underset{x:A, y:A}{\mathcal{G}} R(x, y)\right) \to C$ such that 
...
Similarly, the induction principle of $\underset{x:A,y:A}{\mathcal{G}} R(x, y)$ says that given a type family $C(z)$ indexed by $z:\underset{x:A, y:A}{\mathcal{G}} R(x, y)$ together with 
...
there exists a dependent function $c:\prod_{z:\underset{x:A, y:A}{\mathcal{G}} R(x, y)} C(z)$ such that 

* for all $x:A$, $c(\mathrm{points}(x)) \equiv c_\mathrm{points}(x)$, and
* for all $x:A$, $y:A$, and $r:R(x, y)$, $\mathrm{apd}_c(\mathrm{paths}(x,y,r)) \equiv c_\mathrm{paths}(x, y, r)$

The above higher inductive type could be constructed as the coequalizer of the left and middle dependent pair projections $\pi_{\sum}^A$ and $\pi_{\sum}^A \circ \pi_{\sum}^{\sum_{y:A} R(-, y)}$ of the [[dependent pair type]] $\sum_{x:A} \sum_{y:A} R(x, y)$, both of which have [[domain]] $\sum_{x:A} \sum_{y:A} R(x, y)$ and [[codomain]] $A$:

$$\underset{x:A,y:A}{\mathcal{G}} R(x, y) \coloneqq \mathrm{coeq}_{\sum_{x:A} \sum_{y:A} R(x, y),A}(\pi_{\sum}^A, \pi_{\sum}^A \circ \pi_{\sum}^{\sum_{y:A} R(-, y)})$$

Alternatively, given two [[parallel morphisms|parallel]] functions $s:B \to A$ and $t:B \to A$, the coequalizer of $s$ and $t$ could be defined as the higher inductive type generated by the graph defined by the family of dependent sum types $\sum_{z:B} (s(z) =_A x) \times (t(z) =_A y)$

$$\underset{x:A,y:A}{\mathcal{G}} \sum_{z:B} (s(z) =_A x) \times (t(z) =_A y)$$
