...
A __connection__ $\nabla:M\otimes_A\Omega^\bullet\to M\otimes_A\Omega^{\bullet+1}$ on a module $M$ over a [[semifree dga]] in the sense of entry [[connection for a differential graded algebra]] is determined by its value $\nabla_M|_M$ on $M\cong M\otimes_A A$. If $\rho^M:M\to M\otimes_A C$ is a right $C$-coaction then the formula 
...
As usually, we define **flat connection** as the connection whose curvature vanishes. 
