...
Let $f\colon [0,1] \to \mathbb{R}$ be a [[uniformly continuous function]] from the [[unit interval]] to the [[real line]], and suppose that $f(0) \lt 0$ while $f(1) \gt 0$.  Suppose further that, for any points $a,b$ in the unit interval with $a \lt b$, there exists a point $c_{a,b}$ such that $a \lt c_{a,b} \lt b$ and $f(c_{a,b}) \neq 0$ (in the sense of [[apartness relation|apartness]]).  Then there exists a point $c$ in the unit interval such that $f(c) = 0$.
=--
...
Since $f$ is uniformly continuous, there exists a positive number $\delta$ such that $|f(x) - f(y)| \lt \epsilon$ whenever $|x - y| \lt \delta$.  Since $\delta$ is positive, there exists a [[natural number]] $n$ such that $\delta \gt 1/n$.  We will approximate $c_\epsilon$ in $n$ steps.

We will define finite [[sequences]] $(a_0,\ldots,a_n)$, $(b_0,\ldots,b_n)$, and $(c_0,\ldots,c_n)$ such that $a_k \lt c_k \lt b_k$ at each stage.  To begin with, let $a_0 \coloneqq 0$ and $b_0 \coloneqq 1$.  At each stage, let $c_k$ be the any number (such as their arithmetic mean) strictly between $a_k$ and $b_k$.  By the [[comparison]] property of the [[linear order]] $\lt$, either $f(c_k) \gt -\epsilon$ or $f(c_k) \lt \epsilon$;
...
Either way, we have $a_{k+1} \lt b_{k+1}$, so we can continue.  Then some algebra shows that either $c_n$ is the desired result.
