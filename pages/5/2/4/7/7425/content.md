...
$$
  \pi_\bullet\left(K\left(n\right)\right)
  \simeq
  \mathbb{Z}/(p) [v_n, v_n^{-1}]
  \,,
$$

where $\mathbb{Z}/(p)$ is the [[localization]] of the integers at the given [[prime]].
...
1. $K(0)_\ast(pt.)=\mathbb{Q}$ and for $n\neq 0$, $K(n)_\ast(pt.)=\mathbb{Z}_p[v_n,v_n^{-1}]$ where $\vert v_n\vert=2p^n-2$.  
