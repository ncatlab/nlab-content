...
* with a [[metric]] $\rho:S \times S \to S$ such that $\rho(a, b) + \min(a, b) = \max(a, b)$. Every element in the image of $\rho$ is always not less than zero, because by definition of a pseudolattice, $\min(a, b) \leq \max(a, b)$. This can be shown to be a metric:
