...
Let $\mathfrak{g}$ be a [[Lie algebra]]. It is a **Lie bialgebra** if it is equipped with a skew-symmetric linear map (the *cobracket*)

$$    
\delta: {\mathfrak{g}}\to {\mathfrak{g}}\otimes {\mathfrak{g}},
$$

 such that the dual map $\delta^*: {\mathfrak{g}}^* \otimes {\mathfrak{g}}^* \to {\mathfrak{g}}$ makes $\mathfrak{g}^*$ a Lie algebra, and is a derivation
$$
    \delta([x,y]) = [x\otimes 1 + 1 \otimes x,\delta(y)] + [\delta(x),y\otimes 1 + 1 \otimes y].
$$
