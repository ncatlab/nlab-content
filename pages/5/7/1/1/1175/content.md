...
Write $CvN$ for the category of commutative von Neumann algebras.

Write $Meas$ for the category whose

* [[object]]s are triples $(X, \Omega, N)$, where $X$ is a [[set]], $\Omega$ is a [[sigma algebra]] and $N$ is a $\sigma$-ideal;

* [[morphism]]s are [[equivalence class]]es of [[measurable function]]s $f : X \to Y$, where two are regarded as equivalent if they differ on a set of measure 0.

By the [[Radon-Nokodym theorem]] we have that if two measures $\lambda, \mu$ 

Then we have an [[equivalence of categories]]

$$
  \{
    Fields of Hilbert spaces over X
  \}
  \simeq
  \{
    Hilbert modules over L^\infty(X)
  \}
$$

See [these comments by Dmitry Pavlov](http://mathoverflow.net/questions/49426/is-there-a-category-structure-one-can-place-on-measure-spaces-so-that-category-th/49542#49542) for more.
