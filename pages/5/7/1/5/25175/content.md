...
In [[cubical type theory]], [[cubical path types]] do not require [[dependent function types]] in the induction principle of cubical path types, unlike the inductively defined family of [[identity types]] in [[Martin-Löf type theory]]. Similarly, the [[unit type]] and [[dependent pair types]], as [[negative types]], also do not require [[dependent function types]], and the [[type of all propositions]] doesn't require function types either. For some positive inductive types, such as the [[empty type]] and the [[boolean domain]], one would have to define a limited set of dependent function types only with domain the empty type and the boolean type to express the induction principle of positive inductive types, but this is fine, because all dependent function types with domain the empty set are [[contractible types]] and the dependent function type for any type family over the boolean domain $x:\mathbb{2} \vdash A(x)$ is equivalent to the [[pair type]] $A(0) \times A(1)$. However, the general induction principle for the [[natural numbers type]] involves functions between types in an arbitary type family indexed by the natural numbers, and thus cannot be defined in [[coherent mathematics]]. Instead, in order to get numbers in the type theory, one would have to use [[circle types]], whose identity type is the [[integers]], since one could define [[transport]] and [[heterogeneous identity types]] in cubical type theory without the use of [[function types]]. 
