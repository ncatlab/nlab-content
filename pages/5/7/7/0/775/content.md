...
The square is _commutative_ if $y'x=x'y$. 

The class of commutative  squares in $C$ is written $\square C$. 

This has partial compositions $\circ_1$ and $\circ_2$ which are vertical and horizontal: 
...
The class of commutative squares in $C$  can also be described as $Cat(2 \times 2, C)$ where $2$ is the category with two objects $0,1$ and one arrow $0 \to 1$. 

If $D$ is a category, then $Cat(D, \square_1 C)$ can be regarded as the class of natural transformations of functors $D \to C$. Then the category structure $\square _2 C$ induces a category structure on $Cat(D,\square _1 C)$ giving the category $CAT(D,C)$ of functors and natural transformations. (This account is due to C. Ehresmann.) 
