...
A [[coalgebra]] is half of the structure of a [[bialgebra]]. Bialgebra with antipode is a [[Hopf algebra]], and morally a "Hopf algebra without counit" is essentially a [[quantum heap]]. 
So a cop should be "half a way toward quantum heap".
...
category and $C$ a monoid (=algebra) object in that category, i.e.
$C$ is equipped with a product $\mu : C \otimes C \rightarrow C$
and a unit morphism $\eta : 1 \rightarrow C$ satisfying the
standard axioms.
Then an _opposite monoid_ $C_{op}$ (lower index as it is on monoid side and upper index is for comonoid side duality here) is the same object $C$
...
satisfying the law (1). Notice the passage to the opposite monoid in the second tensor factor (which does not make sense for usual cops in nonsymmetric monoidal categories). 
...
__Proposition.__ A _character_ of a coheap monoid
...
* About the language: co in _cop_ mimics co in coalgebra;
furthermore, in the dialect of Kent, according to OED,
cop is _a small heap of hay or straw_. This way it is kind of coalgebra-like entity half way toward a (quantum) "heap" as in the idea section above.
