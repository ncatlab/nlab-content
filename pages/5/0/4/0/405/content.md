...
The ordered cube category, also called the restricted cube category (see the [paper](http://www.emis.de/journals/TAC/volumes/11/8/11-08.pdf) by Grandis and Mauri) has faces and degeneracies, but no other operations.
...
Among all [[geometric shapes for higher structures]] cubes are best suited for describing Gray-like [[tensor product|tensor products]] of higher structures: there is geometrically obvious way in which to combine the $n$-cube $[n]$ and the $m$-cube $[m]$ to the $(n+m)$-cube $[n] \otimes [m] := [n+m]$. The monoidal structure on $\Box$ induces the canonical monoidal structure on [[cubical set|cubical sets]] and then on [[strict omega-category|strict omega-categories]]: the [[Crans-Gray tensor product]].
