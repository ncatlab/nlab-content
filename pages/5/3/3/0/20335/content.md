...
A consistent system of diagonal maps $\Delta_x\colon x\to x\otimes x$ as $x$ varies through the objects of a monoidal category $(C,\otimes,I)$ should be [[natural transformation|natural]], so that $(f\otimes f)\circ \Delta_x = \Delta_y \circ f$, for any $f\colon x\to y$. Hence such a system is a natural transformation from the identity functor on $C$ to the composite $C \to C\times C \stackrel{\otimes}{\to} C$.
