...
In [[dependent type theory]], given types $A$ and $B$, an [[anafunction]] is a type family $x:A, y:B \vdash R(x, y) \; \mathrm{type}$ which comes with a witness 
$$p:\prod_{x:A} \mathrm{isContr}\left(\sum_{y:B} R(x, y) \right)$$
that for each element $x:A$, the [[dependent sum type]] $\sum_{y:B} R(x, y)$ is a [[contractible type]]. A *dependent anafunction* is an anafunction where we allow $B$ to also depend on $x:A$. 

## Definition 

In [[dependent type theory]], given a type $A$ and a type family $x:A \vdash B(x) \; \mathrm{type}$, a **dependent anafunction** is a type family $x:A, y:B(x) \vdash R(x, y) \; \mathrm{type}$ which comes with a witness 
$$p:\prod_{x:A} \mathrm{isContr}\left(\sum_{y:B(x)} R(x, y) \right)$$
that for each element $x:A$, the [[dependent sum type]] $\sum_{y:B(x)} R(x, y)$ is a [[contractible type]]. 
