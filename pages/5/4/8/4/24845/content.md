

+-- {: .rightHandSide}
+-- {: .toc .clickDown tabindex="0"}
### Context
#### Categorical algebra
+-- {: .hide}
[[!include categorical algebra -- contents]]
=--
=--
=--

#Contents#
* table of contents
{:toc}


## Idea

*Kleisli triple* is the term chosen in [Moggi (1991), Def. 1.2](#Moggi91) for what was previously called an *algebraic theory in extension form* in [Manes (1976), Ex. 3.12 (p. 32)](#Manes76) and what later authors refer to as *[[extension systems]]*. 

All of these terms refer to the equivalent [[axiom|axiomatization]] of [[monads]] (old terminology: "triples") via the ingredients of their [[Kleisli categories]] under the [Kleisli equivalence](Kleisli+category#KleisliEquivalence). 

This is the natural and common incarnation of [[monads in computer science]]; see there for more.

## References

* {#Moggi89} [[Eugenio Moggi]], Def. 1.2 in: *Computational lambda-calculus and monads*, in: *Proceedings of the Fourth Annual Symposium on Logic in Computer Science* (1989) 14-23 &lbrack;[doi:10.1109/LICS.1989.39155](https://doi.org/10.1109/LICS.1989.39155)&rbrack;

* {#Moggi91} [[Eugenio Moggi]], Def. 1.2 in: *Notions of computation and monads*, Information and Computation, **93** 1 (1991) &lbrack;<a href="https://doi.org/10.1016/0890-5401(91)90052-4">doi:10.1016/0890-5401(91)90052-4</a>, [pdf](http://www.disi.unige.it/person/MoggiE/ftp/ic91.pdf)&rbrack;

following

* {#Manes76} [[Ernest G. Manes]], Sec 3, Ex. 12 (p. 32) of: *Algebraic Theories*, Springer (1976) &lbrack;[doi:10.1007/978-1-4612-9860-1](https://doi.org/10.1007/978-1-4612-9860-1)&rbrack;



[[!redirects Kleisli triples]]