...
*  __[[intuitionistic set theory|Intuitionistic set theory]]__ usually refers to IZF (intuitionistic Zermelo-Frankel) in which [[excluded middle]] fails but [[power sets]] are included (making it [[impredicative mathematics|impredicative]]).  When adding excluded middle one indeed retrieves standard Zermelo-Frankel set theory. 
The system is technically convenient, but constructively not well motivated.
In contrast, _constructive_ set theory, Aczel's CZF, has [[function sets]] but not power sets (making it [[predicative mathematics| predicative]]).  CZF can be interpreted in intuitionistic type theory, which has been philosophically motivated.
