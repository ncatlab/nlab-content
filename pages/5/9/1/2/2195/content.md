...
+-- {: .num_remark #ConvolutionProductOnSchwartzSpace}
###### Remark
...
Often the factor of $2 \pi$ in the exponent in (eq:IntegralExpressionForFourierTransform) is omitted. In contrast to prop. \ref{FourierTransformIsInvolution} this makes the Fourier transform be its own inverse only up to a factor of $(2\pi)^n$.
...
With the [[Fourier inversion theorem]] for smooth functions (prop. \ref{FourierInversion}), def. \ref{FourierTransformOnTemperedDistributions} immediately implies that
