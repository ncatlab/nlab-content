...
* If $L$ is a meet-[[semilattice]], then $Filters(L)$ is a complete join-semilattice: $\bigvee_{i\in I} F_i = \left\uparrow\left\{ \bigwedge_{j\in J} A_j \mid J\subseteq I\text{ finite}, (A_j)_{j\in J}\in \prod_{j\in J}F_j \right\}\right. $. If $f:L_1\to L_2$ respects finite meets, then $f_\ast$ respects all joins. If $f$ is merely monotone, then $f_\ast$ respects all filtered joins.
