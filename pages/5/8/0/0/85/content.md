...
Before continuing our discussion, we comment a bit on a convention adopted in [(Day's thesis)](#Day70Thesis). To define [[promonoidal structures]], Day used functors of the form $P\colon\mathcal{A}^\mathrm{op}\times\mathcal{A}^\mathrm{op}\times\mathcal{A} \to \mathcal{V}$, whereas the nLab convention is that a [[profunctor]] $P\colon\mathcal{A}\times\mathcal{A}$⇸$\mathcal{A}$ is a functor $P\colon\mathcal{A}^\mathrm{op}\times\mathcal{A}\times\mathcal{A} \to \mathcal{V}$. Following modern usage and [(Corner 2016)](#Corner2016), instead of defining Day convolution for $\mathcal{V}$-[[enriched functors]], we do so for $\mathcal{V}$-presheaves.
