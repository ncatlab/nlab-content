...
If $R\hookrightarrow X\times X$ is an equivalence relation which is the kernel pair of $f:X \to Y$, then if $f = m p$ is the image factorization of $f$, one can show that $p$ is a [[coequalizer]] of $R$.  Therefore, equivalence relations have quotients in an exact category.  However the converse is not true: there are regular categories having all coequalizers which are not exact.
