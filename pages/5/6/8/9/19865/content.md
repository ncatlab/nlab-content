...
This definition of substitution avoids capture, but it does so at the cost of making substitution a partial function $RawTm \times RawTm \times Var \rightharpoonup RawTm$.  For example, suppose that $M$ is $\lambda(x : A.B). App^{x : A.B}(f, x)$, which is the [[η-expansion]] of some constant $f$, and that $N$ is $App^{z: C.(\Pi(x:A).B)}(x, z)$.  Then $M[N/f]$ isn't defined: on the one hand $x \neq f$,  but on the other $x$ is a free variable in $N$.  So neither of the two structural recursion cases involving $\lambda$ abstractions applies.
