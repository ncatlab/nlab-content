...
$$X^{[V]} = \{ x_1^{[V]}, x_2^{[V\cup \{x_1\}]}, \dots, x_n^{[V \cup \{x_1,\dots,x_{n-1}\}]}\}$$
...
* Suppose $\mathtt{C}$ is an operator of sort $s$, argument arity $m$ and binding arity $n$.  Suppose also $V, X\in P_{fin}(Var)$, with $V\cap X = \emptyset$, ${|X|} = n$, and $X = \{x_1,\dots, x_n\}$ totally ordered.  Given $j\in [m]$, write $X_j = \{x_k \mid j \lhd k \}$ for the variables bound in argument $j$, and suppose furthermore that for each $j\in [m]$ we have $M_j\in Raw(V\cup X_j, t_j)$, where $t:[n] \to Sort$ gives the argument sorts of $\mathtt{C}$.  Then we have an element $\mathtt{C}(X;\vec{M}) \in Raw(V,s)$.
