...
We let $T^*$ be the adjoint of an operator $T$. Note that for an only densely defined $T$, the domain of the adjoint may be strictly larger.
...
* Theorem: a selfadjoint operator $A$ generates a semigroup via $U(t) := \exp(itA)$
...
This shows that the operator $-i \frac{d}{dx}$ generates the semigroup of translations on the real line. Now we could, for example, use Stone's theorem to prove that $-i \frac{d}{dx}$ is selfadjoint by proving that the translation group is strongly continuous.
...
* Conjecture (**false!**): Let $A$ and $B$ be selfadjoint operators on a Hilbert space $\mathcal{H}$ and $D$ a dense subset such that both $A$ and $B$ restricted to $D$ are essentially selfadjoint. Suppose that for all $x \in D$ we have $ A B x - B A x = 0$. Then $A$ and $B$ commute.

We have already seen that on $\mathbb{R}^2$ we can define two essentially selfadjoint operators $-i \partial_x$ and $-i \partial_y$ that generate translations along the x-axis and the y-axis respectively. Both the generated translations and the operators commute (the latter if applied to differentiable functions, of course). 
...
We use the Riemann surface $M$ of $f(z) = \sqrt(z)$. We give a brief exposition of its construction: take two copies of $\mathbb{C}$, two "sheets", and call them I and II. Cut both sheets along $(0, \infty)$, label the edge of the first quadrant along the cut as $+$ and the edge of the fourth quadrant as $-$. Then *attach* the $+$ edge of I with the $-$ edge of II and the $-$ of I with the $+$ edge of II.

Let $\mathcal{H} = L^2(M)$ with respect to Lebesgue measure. As indicated above we define A = $-i \partial_x$ and B = $-i \partial_y$, this is with respect to the canonical chart on each sheet that projects it onto $\mathbb{C}$ and then identifies $\mathbb{C}$ with $\mathbb{R}^2$.
