...
**Double negation translation** is a method for converting [[propositions]] [[valid]] in [[classical logic]] into propositions valid in [[intuitionistic logic]] by applying [[double negation]]. In particular, if $P$ is a proposition, then $P$ is derivable in [[classical logic]] if and only if its [[double negation]] $\neg \neg P$ is derivable in [[constructive logic]].
