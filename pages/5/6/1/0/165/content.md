...
The fundamental groupoid $\pi_1(X)$ can be made into a [[topological groupoid]] (i.e. a [[internal groupoid|groupoid internal]] to [[Top]]) by noticing the set of objects is just the space $X$, and the set of arrows is just the quotient of the [[path space]] $X^I$ with the compact-open topology. The source, target, unit and composition maps arise from $ev_0,ev_1:X^I \to X$, $const:X \to X^I$ and concatenation of paths $X^I \times_X X^I \to X^I$. When $X$ is [[path-connected space|path-connected]], [[locally path-connected space|locally path-connected]] and [[semi-locally simply connected space|semi-locally simply connected]] the source fibre at an object of this topological groupoid is a universal [[covering space]], with projection the restriction of the target map. 
