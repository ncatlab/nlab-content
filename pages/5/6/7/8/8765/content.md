...
* $i \colon \Theta_0 \to \Theta$ for the [[wide subcategory|wide]] non-full subcategory on the morphism induced from morphisms of the underlying [[∞-graphs]]  (this means these morphisms in $\Theta_0$ send $n$-globes to single $n$-globes, not to pastings of them).
...
A globular theory $i_A \colon \Theta_0 \to \Theta_A$ is **homogeneous** if it contains a [[subcategory]] $\Theta^{cov}_A \to \Theta_A$ of $A$-covers, def. \ref{CoversAndImmersions} such that every every morphism in $\Theta_A$ factors uniquely as as an $A$-cover, def. \ref{CoversAndImmersions}, followed by an immersion, def. \ref{CoversAndImmersions}.
