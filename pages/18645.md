...
In this situaton every point $\varphi$ in the jet fiber defines a constant section of the shell:
...
1. restricted to the [[shell]] $\mathcal{E}$:

$$
  LocObs_\Sigma(E) \;\coloneqq\; \left(\Omega^{p+1,0}_{\Sigma,cp}(E)/(im(d))\right)\vert_{\mathcal{E}}
$$
...
[[infinitesimal neighbourhood]] of $\Sigma \times \{\varphi\}$.
...
$$
  ...
  \Omega^{0,0}_{\Sigma,cp}(E,\varphi) 
    \;\coloneqq\;
  C^\infty_{cp}(\Sigma)\left[ \left[ (\phi^a - \varphi^a ), (\phi^a_{,\mu} -\varphi^a_{,\mu}), \cdots, (\phi^a_{,\mu_1 \cdots \mu_k}- \varphi^a_{,\mu_1 \cdots \mu_l}) \right] \right]
  \,.
$$

A key simplification that occurs is that the further restriction of this ring to the [[shell]]
is now simply the further [[quotient ring]] by the ideal generated by the components $\frac{\partial_{EL}L}{\delta \phi^a}$ of
...
$$
  ...
  \begin{aligned}
    \Omega^{0,0}_{\Sigma,cp}(E,\varphi)\vert_{\mathcal{E}}
      & \coloneqq
    \Omega^{0,0}_{\Sigma,cp}(E,\varphi)/\left( \frac{\delta_{EL} L}{\delta \phi^a}  \right)_{a = 1}^s
    \\
    & =
  C^\infty_{cp}(\Sigma)\left[ \left[ (\phi^a - \varphi^a ), (\phi^a_{,\mu} -\varphi^a_{,\mu}), \cdots, (\phi^a_{,\mu_1 \cdots \mu_k}- \varphi^a_{,\mu_1 \cdots \mu_l}) \right] \right]/\left( \frac{\delta_{EL} L}{\delta \phi^a}  \right)_{a = 1}^s
  \end{aligned}
  \,.
$$
...
As in def. \ref{LocalObservablesOnInfinitesimalNeighbourhood}, consider the restriction of the [[evolutionary vector fields]] 
(def. \ref{EvolutionaryVectorField}) to the fiberwise [[infinitesimal neighbourhood]] of this background solution.

Now we regard this as a _[[graded module]]_ over $\Omega^{0,0}_{\Sigma,cp}(E,\varphi)$ (eq:FunctionsOnInfinitesimalNeighbourhoodOfBackgroundSolution) concentrated in degree $-1$:
...
for the evolutionary vector field generator that takes derivatives along $\partial_{\phi^a}$, but regarded now in degree -1.
...
$$
  LocObs_\Sigma(E) \simeq \Omega^{p+1}_{\Sigma,cp}(E)/(im(s_{BV} + d))
  \,.
$$
...
[[obstruction]] turns out to be neatly caputered by methods of [[homological algebra]]
...
To see this, notice two magic properties of ordinary [[differential geometry]]:
