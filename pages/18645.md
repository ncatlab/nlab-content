...
+-- {: .num_example #LocalBRSTComplex}
###### Example
**(local [[BRST complex]] and [[ghost fields]])**
...
jet coordinates $((x^\mu), (c^\alpha), (c^\alpha_\mu), \cdots)$ and using the 
corresponding coordinate expression for the gauge symmetries $R$ (eq:CoordinateExpressionForGaugeParameterized)
this [[BRST differential]] is given on the [[field (physics)|fields]] by
