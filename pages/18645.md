...
This vanishes for all $\Phi$ precisely if all components of $j^\infty_\Sigma(\Phi_{(-)})^\ast (  \delta_{EL}   \mathbf{L} )$
...
from compactly supported sections of the [[dual vector bundle]] of the [[field bundle]] (def. \ref{CompactlySourceCausalSupport}) to linear observables (def.  \ref{LinearObservables}) on spatially compactly supported on-shell field histories (eq:LinearObservablesOnSpatiallyCompactlySupportedOnShellFieldHistories) is a [[monomorphism]]. We call the subspace $LinObs(E_{scp},\mathbf{L})^{reg}$ of linear observables defined thereby that of _regular linear observables_.
...
from regular linear observables (eq:RegularLinearObservables) to on-shell [[field histories]] with spatially compact support (def. \ref{CompactlySourceCausalSupport}).

For every [[Cauchy surface]] $\Sigma_p \hookrightarrow \Sigma$ (def. \ref{CauchySurface}) this is an inverse to the
...
In the situation of theorem \ref{PPeierlsBracket}, assume that the [[field bundle]] $E \overset{fb}{\to} \Sigma$ is a [[trivial vector bundle]] 
with field coordinates $(\phi^a)$ (example \ref{TrivialVectorBundleAsAFieldBundle}). Then the corresponding 
point-evaluation field observables $\mathbf{\Psi}^a(x)$ (example \ref{PointEvaluationObservables}) are linear observables (example \ref{LinearPointEvaluationObservables}) but far from being regular (eq:RegularLinearObservables) 
