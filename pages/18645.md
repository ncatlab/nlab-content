...
where $a = f(0)$ and $b = \frac{\partial f}{\partial x}(0)$ is the first [[derivative]] of $f$ at the origin.

Therefore this algebra behaves like the algebra of smooth function on an [[infinitesimal neighbourhood]] $\mathbb{D}^1$
of $0 \in \mathbb{R}$ which is so tiny that it [[generalized element|elements]] $\epsilon \in \mathbb{D} \hookrightarrow \mathbb{R}$
...
is the [[formal smooth set]] $N_Y X$ spring whose plots are those
