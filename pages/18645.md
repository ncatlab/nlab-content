...
([Bär 14, def. 3.2, cor. 3.10](Green+hyperbolic+differential+equation#Baer14}))
...
We are headed towards the [[on-shell]] version of prop. \ref{LinearObservablesAreTheCompactlySupportedDistributions},
for [[equations of motion]] which are [[Green hyperbolic differential equation|Green hyperbolic]] (def. \ref{GreenHyperbolicDifferentialOperator}), namely a characterization of the on-shell linear observables
as [[continuous linear functionals]] on spaces of [[on-shell]] [[field histories]]. Therefore we first need
to establish [[topological vector space]] structure on these spaces:
...
Let $E \overset{fb}{\to} \Sigma$ be a [[smooth vector bundle]] (def. \ref{VectorBundle}) over [[Minkowski spacetime]] (def. \ref{MinkowskiSpacetime}). On its [[real vector space]] $\Gamma_\Sigma(E)$ [[space of sections|of smooth sections]] consider the [[seminorms]] indexed by a [[compact subset]] $K \subset \Sigma$ and a [[natural number]] $N \in \mathbb{N}$ and given by
...
([Bär 14, 2.1, 2.2](space+of+sections#Baer14))
...
+-- {: .num_prop #DistributionalDerivatives}
###### Proposition
...
$$
  P u \coloneqq u(P^\ast(-)) \;\colon\; \Gamma_{\Sigma,cp}(E) \to \mathbb{C}
  \,.
$$
...
The key theorem characterizing linear on-shell observables is now the following:
