...
$$
  \mathcal{F}_{loc}
    \simeq
  \Omega^{p+1}_{\Sigma,cp}(\mathcal{E})/im(d)
  \,.
$$

> add statement about polynomial (power series) local observables
...
$$
  ...
  \;\colon\;
  \Gamma_{\Sigma, poly}^{ev}(V_\Sigma E)[-1]
    \longrightarrow
  \Omega^{p+1,0}_{\Sigma, poly}(E)
  \,.
$$
...
just [[formal power functions]] in the field derivative coordinates on the jet bundle
...
By inspection we find that the [[cochain cohomology]] of the polynomial [[BV-complex]] $PolyLocObs_{BV}$ (def. \ref{BVComplexOfOrdinaryLagrangianDensity}) has the following interpretation:
...
is the ideal of functions that vanish [[on-shell]]. Since there is trivial differential to degree +1,
...
$$
  \left(
    \frac{\delta_{EL} L}{\delta \phi^a} \in \Omega^{0,0}_{\Sigma,poly}(E)
  \right)_{a = 1}^s
$$
...
In view of this, prop. \ref{NonTrivialImplicitInfinitesimalGaugeSymmetriesPbstructExistenceOfCauchySurfaces} says that
