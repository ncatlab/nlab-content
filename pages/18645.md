...
and then their quantization applies degree-wise.
...
If $\hat \epsilon$ is (the prolongation of, prop. \ref{EvolutionaryVectorFieldProlongation}) an implicit 
infinitesimal gauge transformation, then so is
...
Let $\Sigma \times \{\varph\} \hookrightarrow \mathcal{E}$ be a constant section of the shell (eq:ConstantSectionOfTrivialShellBundle). 


Evaluation of [[evolutionary vector fields]] (def. \ref{EvolutionaryVectorField}) in the [[Euler-Lagrange variational derivative]] $\delta_{EL} \mathbf{L} \in \Omega^{p,0}_\Sigma(E) \wedge \delta \Omega^{0,0}_\Sigma(E)$ (prop. \ref{EulerLagrangeOperatorForTivialVectorBundleOverMinkowskiSpacetime}) yields a [[linear map]]
over $\Omega^{0,0}_\Sigma(E,\varphi)$ (eq:ObservablesOnInfinitesimalNeighbourhoodOfZeroInShellInFieldFiber)
...
$$
  Sym_{\Omega^{0,0}_\Sigma(E,\varphi)}\left(
    \Gamma_\Sigma^{ev}(V_\Sigma E,\varphi)[-1]
  \right)
$$

which is generated over $\Omega^{0,0}_\Sigma(E,\varphi)$ from the [[evolutionary vector fields]] in degree -1, and let
...
and independent of the spacetime coordinates. The corresponding BV-complex we denote $LocObs_{BV}$,

There is a canonical homomorphism of [[dgc-algebras]] from the BV-complex to the 
local observables on the infintiesimal neighbourhood of the background solution $\varphi$ (eq:ObservablesOnInfinitesimalNeighbourhoodOfZeroInShellInFieldFiber)
...
###### Example
**([[cochain cohomology]] of [[BV-complex]])**
...
By inspection we find that the [[cochain cohomology]] of the [[BV-complex]] $LocObs_{BV}$ (def. \ref{BVComplexOfOrdinaryLagrangianDensity}) has the following interpretation:

In degree 0 the [[image]] of the [[BV-differential]] coming from degree -1

$$
  im( \Gamma_\Sigma^{ev}(V_\Sigma(E,\varphi)) \overset{\delta_{BV}}{\to} \Omega^{0,0}_\Sigma(E,\varphi))
$$
...
$$
  \begin{aligned}
    H^0(LocObs_{BV}(\varphi))
      & \simeq
    \Omega^{0,0}_{\Sigma}(E,\varphi) / ( \delta_{EL}\mathbf{L} )
    \\
    & = LocObs(\varphi)
  \end{aligned}
$$

of functions [[on-shell]].

In degree -1 the [[kernel]] of the [[BV-differential]] going to degree 0

$$
  ker( \Gamma_\Sigma^{ev}(V_\Sigma(E,\varphi)) \overset{\delta_{BV}}{\to} \Omega^{0,0}_\Sigma(E,\varphi))
$$
...
$$
  im(
    ...
    \Gamma_{\Sigma}^{ev}(V_\Sigma E,\varph)
  )
$$
...
Then the BV-complex $LocObs_{BV}(\varphi)$ of local observables (def. \ref{BVComplexOfOrdinaryLagrangianDensity}) is a [[homological resolution]] of the the algebra of functions on the [[infinitesimal neighbourhood]] of $\varphi$ in the [[shell]] (example \ref{ShellForSpacetimeIndependentLagrangians})
...
gauge symmetries is equivalent to the vanishing of the cochain cohomology of the BV-complex in degree -1 (eq:NegativeOneCohomologyBV).
...
is a [[homological resolution]] of $LocObs$, hence has vanishing cohomology in all negative degrees,
...
1. the ring $\Omega^{0,0}_{\Sigma,\varphi}(E)$ is the tensor product of $C^\infty(\Sigma)$ with a [[Noetherian ring]];

1. the elements $\frac{\delta_{EL} L }{\delta \phi^a}$ are contained in its [[Jacobson radical]].

The first condition is the case since $\Omega^{0,0}_{\Sigma,poly}(E)$ is by definition a [[formal power series ring]] over a [[field]]
