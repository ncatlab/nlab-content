...
Thanks also to [[Klaus Fredenhagen]] and [[Kasia Rejzner]] for helpful replies.
...
Hence there are three aspects of the concept of "[[field (physics)|field]]" in [[physics]], which are closely related, but crucially different:
...
that field history. For instance concepts like "average [[field strength]] in spacetime region $\mathcal{O}$" should be
observables.
...
Similarly, it is useful to consider also smooth functions on the entire (i.e. "[[off-shell]]") [[space of field histories]],
...
Every off-shell observables unduces an on-shell observable by restriction, this yields a smooth function
...
###### Proposition
**([[linear map|linear]] [[off-shell]] [[observables]] are the [[compactly supported distributions]])**
...
$$
  LinearOffShellObservables \overset{\simeq}{\longrightarrow} CompactlySupportedDistributions
  \,.
$$

saying that all [[compactly supported distributions]] arise from linear off-shell observables this way, and uniquely so.
...
against some "[[generalized function|generalized smooth functions]]" (a limit of actual smooth functions):

+-- {: .num_remark #LinearObservablesAsGeneralizedFunctions}
###### Remark
**([[linear map|linear]] [[off-shell]] [[observables]] as [[integration]] against [[generalized functions]])**
...
simply by restricting it (eq:OffShellObservablesRestrictToOnShellObservables),
different off-shell observables may restrict to the _same_ on-shell observales this
way. It is therefore useful to find a condition on off-shell observables
...
Write, as before, $\Gamma_{\Sigma}(E)$ for the [[space of sections|space of smooth sections]], and write
...
for the [[linear subspaces]] on those smooth sections whose [[support]] is, in this order,

1.  inside a [[compact subset]] of $\Sigma$,

1.  inside the [[closed future cone]]/[[closed past cone]], respectively, of a [[compact subset]],

1.  inside the [[closed causal cone]] of a [[compact subset]] (def. \ref{CausalPastAndFuture}).

=--

([Khavkine 14, def. 2.1](Green+hyperbolic+partial+differential+equation#Khavkine14))
...
is called a _[[Green hyperbolic differential operator]]_ if for every [[section]] of the [[dual vector bundle]] (def. \ref{DualVectorBundle})
with compactly source future/past support (def. \ref{CompactlySourceCausalSupport})

$$
  \alpha^\ast \in \Gamma_{\Sigma,\pm cp}(\tilde E^\ast)
$$
...
has a unique solution  $\Phi_{\pm} \in \Gamma_{\Sigma, \pm cp}(E)$; and such that this unique solution has [[support]] inside the [[closed future cone]]/[[closed past cone]] (def. \ref{CausalPastAndFuture}) of the [[support]] of $\alpha^\ast$, respectively.

=--

([Khavkine 14, def. 2.2](Green+hyperbolic+partial+differential+equation#Khavkine14))
...
###### Proposition
**([[exact sequence]] of [[Green hyperbolic differential equation]])**
...
is an [[exact sequence]], in that that the [[image]] of each map coincides with the [[kernel]] of the subsequent one.

In particular this means that there is a [[linear isomorphism]] between the space $ker_{scp}(P)$ of spatially compactly supported solutions to the differential equation and the [[quotient space]] of the [[compact support|compactly supported]] dual sections by the [[image]] of $P$:
...
([Khavkine 14, prop. 2.1](Green+hyperbolic+partial+differential+equation#Khavkine14))
...
[[superpositions]] of [[plane waves]] via _[[Fourier transform]]_. We briefly survey the relevant definitions
and facts from [[Fourier analysis]] [below](#FourierAnalysis).
...
Throughout, let $n \in \mathbb{N}$ and consider the [[Cartesian space]] $\mathbb{R}^n$ of [[dimension]] $n$ (def. \ref{CartesianSpacesAndSmoothFunctionsBetweenThem}).
...
+-- {: .num_defn #CausalPropagatorAndPeierlsBracket}
###### Definition
...
$$
  ...
  \right) d^p \vec k
    \\
    & =
   -i (2\pi)^{-p} \int \delta\left( 2\pi k_\mu 2\pi k^\mu + m^2 \right) sgn( k_0 ) e^{ - i k_\mu (x-y)^\mu } d^{p+1} k
    \,.
  \end{aligned}
$$
...
* _[UV-Divergences and Microlocal analysis](#UVDivergencesAkaMicrolocalAnalysis)_
...
**[[UV divergences]] and [[microlocal analysis]]**
  {#UVDivergencesAkaMicrolocalAnalysis}
...
This is known as _[[Lars Hörmander|Hörmander's]] condition_ on the _[[wave front set]]_ of the distributions ([Hörmander90](Fourier+transform#Hoermander90)), and the study of distributions with attention to this condition is part of what is called _[[microlocal analysis]]_.
