...
One caveat is that the built-in products in Haskell are "lifted", the are not exactly categorical products. For example, if we define 
``` 
loop = loop 
```
then the element `loop :: ((),())` is different from `(loop,loop) :: ((),())`. 
This means that 
```
