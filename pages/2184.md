...
An **oriented graph** is an undirected graph equipped with an _orientation_, in the sense of a choice of direction for each edge.
...
Any orientation of an undirected graph of course induces a corresponding directed graph $E^+ \rightrightarrows V$, but the natural reading of oriented graphs is rather as undirected graphs equipped with extra [[structure]].
(Note that various [[graph invariants]] &ndash; such as the [[flow polynomial]], or [[W. T. Tutte|Tutte]]'s original definition of the [[Tutte polynomial]] &ndash; can be defined for an arbitrary orientation of a graph, but are independent of the choice of orientation.)
