...
There is an equivalent definition of pseudograph in the above sense (i.e., of an undirected graph allowing loops and multiple edges) which sometimes appears in the study of [[ribbon graphs]] and [[combinatorial maps]], and which replaces the set of edges by a set of **half-edges**.  In this formulation, a graph is defined as a pair of sets $V$ and $H$ together with a function $s : H \to V$ and a [[fixed point]] free [[involution]] $i : H \to H$.  An equivalent but more symmetrical formulation (which appears in Chapter 2.1 of [Serre 1977](#Serre1977)) uses a pair of source and target functions $(s,t) : H \to V\times V$ such that $s\circ i = t$ (and hence $s = t \circ i$).  In either formulation, observe that this definition of an undirected graph is analogous to the definition of a [[dagger category]].

The edges of the graph $(V,H,s,i)$ are just the orbits of $i$, which (since $i$ is a fixed point free involution) must all be of length 2, corresponding to pairs of half-edges.  Lifting the condition that $i$ has no fixed points allows to represent graphs with "dangling" edges, while lifting the condition that it is an involution (i.e., letting $i$ be an arbitrary permutation) allows for the representation of [[hypergraphs]].
