...
* For every vertex $x$, there is exactly one edge over $(x,x)$.  If we do not make this requirement, then we have a __pseudograph__.  (In a pseudograph, such an edge is called a __loop__.)
...
*  The graph be __undirected__: the [[fibre]] over $(x,y)$ should be isomorphic (same [[cardinality]]) as the fibre over $(y,x)$.
