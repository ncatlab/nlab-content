...
When it comes to parsing the notation &ldquo;$f : X \to Y$&rdquo;, everyone takes the part &ldquo;$X \to Y$&rdquo; to specify the _type_ of the function, that is, the pair $(obj_1 f, obj_2 f)$, but &ldquo;$f$&rdquo; is used equivocally to denote both the triple and the subset $obj_{12} f$ that forms one part of it.  One way to resolve the ambiguity is to formalize a distinction between a function and its _graph_, letting $graph (f) := obj_{12}f$.
