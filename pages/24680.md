...
An dagger functor between two presetoids is **full** if every morphism function $f_{Hom}(a, b):\mathrm{Hom}_A(a, b) \to \mathrm{Hom}_B(f_{Ob}(a), f_{Ob}(b))$ is a [[surjection]], and an extensional function is **faithful** if every edge function is an [[injection]]. A dagger functor is **[[embedding-on-objects]]** if the object function $f_V:V_A \to V_B$ is an [[embedding]], and **[[equivalent-on-objects]]** if the vertex function is a [[equivalence in homotopy type theory|equivalence of types]]. 
...
$$\mathrm{idtocorehoms}(a, b):(a =_A b) \simeq (a \cong_A b)$$
