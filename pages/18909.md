...
First we have to allow a more general "logic" than untruncated propositions-as-types, such as the topos logic of [[subobjects]].   Second, such a more general logic will no longer satisfy the [[function comprehension]] (unique choice) principle *with respect to setoids* (though it may with respect to the "natural" equality of objects of the topos); thus we have to generalize the notion of "function" to be not just an operation (a term in the underlying type theory) that preserves equality, but a total functional relation.  This leads to the [[tripos]]-to-topos construction.
