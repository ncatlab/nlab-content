...
is monadic. To see this, we check the conditions in Thm. \ref{BeckMonadicityTheoremAlternative}:
...
1. By the assumption that $\mathcal{C}$ is [[bicomplete category|bicomplete]], the left and right [[Kan extensions]] of presheaves along $F$ exist and exhibit $F^\ast$ as the middle part of a ("[[base change]]") [[adjoint triple]] :
   ...
   This shows that $F^\ast$ has a left adjoint. In addition, it *is* a left adjoint and [[left adjoints preserve colimits|as such preserves colimits]].
