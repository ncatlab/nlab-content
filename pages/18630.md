...
1. equipped with the [[presymplectic form]] 
   ...
   which is the [[transgression of variantional differential forms|transgression]]
...
are compatible with these isomorphisms, so that all phase spaces are in fact isomorphism as 
...
be a [[submanifold]] [[manifold with boundary|with boundary]] with two [[boundary]] components
...
$$
  \array{
    ...
    \\
    & {}^{\mathllap{in^\ast}}\swarrow && \searrow^{\mathrlap{out^\ast}}
    \\
...
$$
  in^\ast\left( \omega_{in}\right)
  \;=\;
  out^\ast\left( \omega_{out} \right)
  \phantom{AAAA}
  \text{on space of on-shell field histories}
  \,.
$$
...
$\Sigma_{p+1} \hookrightarrow \Sigma$ a cobordism between them, yield a [[Lagrangian correspondence]].
