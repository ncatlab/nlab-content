...
Yaron: You are right if  only _full_ reflective subcategories are allowed (as in this entry, actually…) – an object in a reflective subcategory is assured to be isomorphic to its image under the reflector only if the reflective subcategory is full.  The inclusion $\mathbf{Rng}\to \mathbf{Rng}$' (where $\mathbf{Rng}$' is not-necessarily unital rings) does have a left adjoint (as any forgetful functor between varieties of algebras, by the [[adjoint lifting theorem]]), but this inclusion is not full (an arrow in $\mathbf{Rng}$' need not preserve the identity).  

Because reflective subcategories in nLab are defined to be full, I vote to remove this example.
=--
