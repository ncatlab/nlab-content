...
|--------------------|--|------------------|---|
| $\phantom{A}$[[nLab:topology]]$\phantom{A}$ | $\phantom{A}$$\phantom{NC}TopSpaces_{H,cpt}$$\phantom{A}$ | $\phantom{A}$$\overset{[[Gelfand-Kolmogorov]]}{\hookrightarrow} Alg^{op}_{\mathbb{R}}$$\phantom{A}$ | $\phantom{A}$[[nLab:commutative algebra]]$\phantom{A}$ |
