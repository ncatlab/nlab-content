...
* Page titles should contain only ASCII characters.
  ...
  * {#tricks} Tricks: To produce '$\omega$-[[omega-category|category]]', try `$\omega$-[[omega-category|category]]`. To produce '[[omega-category|∞-category]]', try `[[omega-category|∞-category]]`.  (The former has proper math-mode formatting, while the latter has proper link scope.)  Unfortunately, [[omega-category|$\omega$-category]] does not work.  You can also use [[redirects]] to make `[[∞-category]]` produce '[[∞-category]]', but only when the page already exists.
...
* Page titles should be singular nouns.
  ...
  * Tricks: To produce '[[category|categories]]', try `[[category|categories]]`. To produce '[[faithful functor|faithful]] [[endofunctor]]', try `[[faithful functor|faithful]] [[endofunctor]]`.  Again, you can use redirects to make `[[categories]]` produce '[[categories]]', if the page already exists.
