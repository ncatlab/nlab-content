...
* Page titles should contain only ASCII characters.
  ...
  * {#tricks} Tricks: To produce '$\omega$-[[omega-category|category]]', try `$\omega$-[[omega-category|category]]`. To produce '[[omega-category|ω-category]]', try `[[omega-category|ω-category]]`.  (The former has proper math-mode formatting, while the latter has proper link scope.)  Unfortunately, [[omega-category|$\omega$-category]] does not work.  You can also use [[redirects]] to make `[[ω-category]]` produce '[[ω-category]]', but only when the page already exists.
...
1. complete the diagogue by hitting "upload";

1. find in the rendered entry a new link labeled with the text that you entered in the box "Description" in the file upload dialogue;

1. due to a bug, if you want that link to persist, you need to make any further edit to the entry (say add a whitespace) and "submit" again.
