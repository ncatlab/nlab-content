...
The circumference of a circle as discussed in geometry. 
...
$$C = \int_{0}^{\tau} \vert r \cos(\theta) \hat{i} + r \sin(\theta) \hat{j} \vert d \theta = \int_{0}^{\tau} r((\cos(\theta))^2 + (\sin(\theta))^2) d \theta = r \int_{0}^{\tau} d \theta = \tau r$$
=--
