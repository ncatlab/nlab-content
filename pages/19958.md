...
1. for any $*\in  \Omega_2^\prime$, $\mathbb{E}$ includes the identity, $x*(y+z)=x*y+x*z$, (so each $*$ distributes over the group operation, $+$);

1. for any $\omega\in\Omega_1^\prime$ and $*\in  \Omega_2^\prime$, $\mathbb{E}$ includes the identities:

$$\omega(x+y) = \omega(x)+\omega(y)$ \quad and \quad $\omega(x)*y=\omega(x*y)$$

1. for $*\in  \Omega_2^\prime$, $x_1 + (x_2*x_3) = (x_2*x_3)+x_1$ for all $x_1$, $x_2$, $x_3$;

1.  for any $*, \overline{*} \in \Omega_2^\prime$, there is a word, $W$ such that 
