...
$$ 
sending a probability measure $P$ to $P(U)$, where $U$ ranges over all the measurable sets of $X$.
The unit of the monad sends a point $x \in X$ to the Dirac measure at $x$, $\delta_x$, while the counit of the monad is defined by the natural transformation 
$$
  \mu_{X}: G(G(X)) \to G(X)
$$
...
An alternative choice, convenient for analysis purposes, and introduced by Giry, is obtained by restricting the category of [[measurable spaces]] to the (full) subcategory which 
are those measurable spaces generated by  Polish (topological) spaces, $Pol$, which are  separable [[metric spaces]] for which a complete metric exists.  
...
The results of Doberkat can be generalized to the Giry monad $G$ on all measurable spaces by using the factorization of the Giry monad through the category of convex spaces $\mathbf{Cvx}$, by viewing the Giry monad itself as a functor into that category.  The right adjunct of that functor assigns to each convex space the measurable space, defined on the underlying set, with the initial $\sigma$-algebra generated by all the affine maps into the two convex spaces, $\mathbf{2}$ and $I=[0,1]$.  The counit of this adjunction, evaluated at  the convex space $A$, is given by noting that every weakly averaging affine map $\mathbf{Cvx}(A,I) \stackrel{P}{\longrightarrow} I$, corresponds to an evaluation map $ev_a$ for a unique point $a \in A$.  Consequently, viewing a probability measure $P$ on $A$ as an affine functional, restricted to the affine maps, one obtains a map $\epsilon_A:G(\Sigma A) \rightarrow A$.  For example, taking $A=I$ and $P$ the Lebesque measure restricted to $I$, it follows that $\epsilon_{I}(P)=\frac{1}{2}$, and one has $m(\frac{1}{2}) = \int m dx$ for every affine map $m \in \mathbf{Cvx}(I,I)$. (This example corresponds to the example above discussing Doberkat's article.)

 This factorization through the category of convex spaces, can then be used to give an alternative characterization of  the Giry-algebras in terms of a full subcategory of the category of convex measurable spaces.  A convex measurable space is a pair, $(A, \mathcal{A})$, where $A$ is a convex space and $\mathcal{A}$ is a $\sigma$-algebra on the underlying set $|A|$, so that $(|A|, \mathcal{A})$ is a measurable space.  A morphism of such spaces  $m:(A, \mathcal{A}) \rightarrow (B, \mathcal{B})$ is an affine map (with respect to the convex structure) and measurable with respect to the $\sigma$-algebras.  ([Sturtz 18](#Sturtz18))
...
The factorization of the Giry monad, defined on the category of measurable spaces, through the category of convex spaces, is given in

* {#Sturtz18} [[Kirk Sturtz]], _The Factorization of the Giry monad_, Advances in Mathematics, 340, pp. 76-105,  15 Dec. 2018

This characterizes the Eilenberg-Moore category of Giry-algebras for the general case.  That  article generalizes the earlier work 

* K. Sturtz, _Categorical Probability Theory_, [arXiv:1406.6030](http://arxiv.org/abs/1406.6030)

which views probability measures via double dualization, restricted to weakly averaging affine maps which preserves limits.  (The recent article shows the additional assumption of preserving limits is unnecessary.)   
