...
1. The principal ultrafilter at $x$ converges to $x$;
1. If $F \to x$ and $G \to x$, then $F \cap G \to x$;
1. If $F \to x$ and $A \in F$, then $x \in A$.
...
Every Cauchy space is a convergence space; $F \to x$ if the intersection of $F$ with the principal ultrafilter at $x$ is Cauchy.  Note that any convergent proper filter must be Cauchy.  Conversely, if every Cauchy filter is convergent, then the Cauchy space is called __complete__.
