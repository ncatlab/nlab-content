...
Let $E$ be any locally small category, and suppose the full subcategory $Tiny(E)$ of atomic objects in $E$ is essentially small. The inclusion $i \colon Tiny(E) \hookrightarrow E$ induces a restricted Yoneda embedding 

$$E \to Set^{Tiny(E)^{op}}$$ 

sending an object $e$ to $E(i-, e)$. We say that $E$ is **[[atomic category|atomic]]** if every object $e$ is a canonical colimit of atomic objects below it, in the precise sense that $e$ is a coend 

$$\int^{a \in Tiny(E)} E(i a, e) \cdot i a.$$ 

(Even this piece of terminology may not be quite universal among category theorists; Bunge for example apparently defines a category to be _atomic_ if it has a generating set of atomic objects. More on this below.) 

If $E$ is a preorder, the coend amounts to the supremum 
