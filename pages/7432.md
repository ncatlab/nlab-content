...
"Atomic" and "Atomistic" differ for the simple example of the [[divisor lattice]] for some number $n$. The atoms in this lattice are prime numbers while it may also contain [[semi-atom|semi-atoms]] which are powers of primes. This lattice is atomic because any object not equal to $1$ (the [[bottom]]) is divisible by a prime. However it is not atomistic but instead uniquely semi-atomistic (every object is the product of a unique set of semi-atoms, regarding $1$ as a semi-atom), which is one way of stating the [[fundamental theorem of arithmetic]], also known as the *unique factorization theorem*.
