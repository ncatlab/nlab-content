...
$$
  S 
    \;\colon\; 
  LocObs(E,\mathbf{L}_{kin})[ [\hbar] ] 
    \longrightarrow 
  PolyObs(E,\mathbf{L}_{kin})_{mc}
$$

which assigns to each [[local observable]] $\mathbf{L}_{int} \in LocObs(E,\mathbf{L})$, regarded as an [[adiabatic switching|adiabatically switched]] [[interaction]] [[Lagrangian density]], the corresponding perturbative [[scattering matrix]] $S(\mathbf{L}_{int}) \in PolyObs(E,\mathbf{L}_{kin})$ (in the form of an element of the [[Wick algebra]] $(PolyObs(E,\mathbf{L})_{mc}, \star_{H})$) satisfying some conditions (notably [[causal additivity]]).
