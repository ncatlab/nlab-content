...
We generalize the above discussion to [[twisted cohomology|twisted]] cohomology.

The argument is essentially the same as before, only that now the base spaces starts out be the product space $B \times B G$ of the previous base space $B$ with the [[classifying space]] for the twists, and that before passing to homotopy classes we of fiberwise maps we form their right base change $(B \times p_{B G})_\ast$ ([[dependent product]]) of this classifying space. To see that the revious argument generalizes to this case one needs to observe that this right base change is right Quillen and one needs to use that by the [[Beck-Chevalley relation]] it si compatible with pullback along $\{b\} \to B$.
