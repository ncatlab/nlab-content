...
For $n \in \mathbb{N}$ the vector space $C^\infty_c(\mathbb{R}^n)$ of [[compact support|compactly supported]] [[smooth functions]] ([[bump functions]]) on [[Euclidean space]] $\mathbb{R}^n$ is (algebraically and topologically) [[isomorphism|isomorphic]], via the [[Fourier-Laplace transform]] ([this prop.](Fourier+transform+of+distributions#FourierTransformOfCompactlySupportedDistributions)), to the space of [[entire functions]] $F$ on $\mathbb{C}^n$ which satisfy the following estimate: there is a [[positive number|positive]] [[real number]] $B$ such that for every [[integer]] $N \gt 0$ there is a [[real number]] $C_N$ such that:
...
Similarly the stronger estimate
...
characterizes the [[compactly supported functions]] ([[bump functions]]).
