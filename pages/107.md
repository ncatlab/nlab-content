...
that "[[semi-strict infinity-category|semi-strict infinity-groupoids]]" in which only the [[inverses]] would be weak would model all [[homotopy types]] (hence satiusfy the [[homotopy hypothesis]])

In ([Simpson 98](#Simpson98)) it was pointed out that this seems to be wrong for the [[homotopy type]] of the [[sphere|3-sphere]].

The issue however is quite subtle, as very much highlighted by Voevodsky [here](http://ncatlab.org/nlab/show/homotopy+type+theory#VoevodskyIASTalk2014))

(For instance  Corollary 5.2 in ([Simpson 98](#Simpson98)) leaves open the possibility that the [[geometric realization]] in  ([Kapranov-Voevodsky 91](#KapranovVoevodsky91)) is essentially surjective, and asserts just that the homotopy groups of the resulting space could not be read off from the groupoid in the obvious way. 
