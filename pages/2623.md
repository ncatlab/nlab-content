...
Then the time-ordered product $T_{n+1}$ of arity $n+1$, regarded as a [[generalized function]] on $\Sigma^{n+1}$ ([this remark](S-matrix#NotationForTimeOrderedProductsAsGeneralizedFunctions)) is uniquely fixed on the [[complement]]
...
The statement appears in ([Popineau-Stora 82](#PopineauStora82), with (unpublished) details in ([Stora 93](#Stora93)), following personal communicatin by [[Henri Epstein]] (according to [Dütsch 18, footnote 57](#Duetsch18)). Following this, statement and detailed proof appeared in ([Brunetti-Fredenhagen 99](#BrunettiFredenhagen99)).


+-- {: .proof}
###### Proof

We will construct an [[open cover]] of $\Sigma^{n+1} \setminus \Sigma$ by subsets $C_I \subset \Sigma^{n+1}$ which are [[disjoint unions]]
...
$$
  \underset{I \in P_{neq}(\{1, \cdots n+1\})}{\cup}
  \mathcal{C}_I
  \;=\;
...
and any such may be deformed a little such as not to intersect any of the other points in a given tuple of points being 
an element in the complement of the diagonal. )

Hence the condition of [[causal factorization]] on $T_{n+1}$ implies that 
[[restriction of distributions|restricted]] to any $\mathcal{C}_{I}$ these have to be given (in the condensed [[generalized functioon]]-notation from [this remakr](S-matrix#NotationForTimeOrderedProductsAsGeneralizedFunctions)) on 
any tuple $\mathbf{X} = (x_1, \cdots, x_{n+1}) \in \mathcal{C}_I$ with corresponding induced tuples
