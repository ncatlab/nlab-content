...
By the proof of prop. \ref{SpaceOfPointExtensions} the extension of the time-ordered products after the $p_\rho$-projections is unique, so that we may also write the actual S-matric as a limit:
...
Hence it is sufficient to find $\mathcal{Z}_\Lambda$ and $K_{n+1,\Lambda}$ such that for all $k \in \mathbb{k}$ we have
...
Further proceeding as in [that proof](#AnyTwoSMatrixRenormalizationSchemesDifferByAUniqueVertexRedefinition), by [[induction]] over the arity of the time-ordered products, assume that for $n \in \mathbb{N}$ that $\{Z_{k}\}_{k \leq n}$ has
...
$$
  ...
  \;=\;
  Z_{n+1,\Lambda} + (T_\Lambda \circ Z_{\leq n, \Lambda})_{n+1}
$$

we find the unique solution of (eq:CountertermsAndCorrectionTerm) for $Z_{n+1,\Lambda}$, in terms of the $K_{n+1,\Lambda}$
still to be chosen, to be:
...
To prove this, we need to show that 1) the resulting $Z_{n+1}$ is local and 2) the limit of $K_{n+1,\Lambda}$ vanishes as $\Lambda \to \infty$.
...
By definitionm $p_{\rho_{n+1}} - id$ is the identity on test functions (adiabatic switchings) that vanish at the diagonal. This means that $Z_{n+1,\Lambda}$ is indeed local.

Finally we need to show that $\underset{\Lambda \to \infty}{\lim} K_{n+1,\Lambda} = 0$.

Due to the presence of the projector, the time ordered product $T_{n+1}$ is evaluated only
