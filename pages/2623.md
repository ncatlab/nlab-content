...
says that indeed every re-normalization $\mathcal{S} \mapsto \mathcal{S}'$ is induced by such an [[interaction vertex redefinition]] in that 
there exists a _unique_ suh redefinition $\mathbf{Z}$ so that for every local interaction $g S_{int} +  j A$ we have
...
$\mathcal{S}'$ equal those computed with the [[renormalization scheme|("re"-)normalization scheme]] $\mathcal{S}$ but applied
...
which [[action|acts]] [[transitive action|transitively]] and [[faithful action|faithfully]] on the 
