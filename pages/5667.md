...
### Triangulated and spectrum-enriched structure
...
Furthermore $KK$ seems to naturally refines to something like an [[enriched category]] over the [[monoidal category of symmetric spectra]] ($\sim$ [[stable (∞,1)-category]]) 

This was claimed verbatim in ([Joachim-Stolz 09](#JoachimStolz09) [Mitchener 07](#Mitchener07)), but according to ([DEKM 11, p. 3](#DEKM11)) the construction there is not correct. But ([DEKM 11, p. 3](#DEKM11)) discusses something at least similar. (...)
...
1. $\mathbb{K}$ lifts to a [[lax monoidal functor]]
   ...
   $$
     C^\ast Alg \to Ho(\mathbb{K}(\mathbb{C}) Mod)
   $$

   to the [[homotopy category]] of [[module spectra]], and this in turn extends to a lax monoidal functor

   $$
     KK \to Ho(\mathbb{K}(\mathbb{C}) Mod)
     \,.
   $$
...
This artcle in turn considers a variant of the construction in ([Bunke-Joachim-Stolz 03](#BunkeJoachimStolz03)) which gives operator K-theory spectra that are functorial for general $\ast$-homomorphisms.
