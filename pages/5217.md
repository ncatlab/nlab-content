...
Classically: 
\begin{definition}
An __ordered field__ is a [[field]] $K$ equipped with a [[strict linear order]] $\lt$ such that:

*  $1 \gt 0$,
*  If $a, b \gt 0$, then so are $a + b$ and $a b$.
\end{definition}

One often sees the definition using a [[total order]] $\leq$ instead of the strict linear order $\lt$. This makes no difference in [[classical mathematics]], but the definition of the strict linear order is the one that generalizes to [[constructive mathematics]]. 
...
The strict linear order is defined as
$$a \lt b \coloneqq \mathrm{isPositive}(b - a)$$

### In constructive mathematics. 

Due to the fact that a set having a linear order no longer implies that the set has a total order, and a set having a total order no longer implies that the set has a linear order, the notion of ordered field bifurcates into multiple inequivalent notions. 
...
Thus, some authors in constructive mathematics, such as [insert references here], have defined an ordered field to additionally have a lattice structure and be a Heyting field. 

However, other fields with anl strict linear order, such as the [[MacNeille real numbers]] and the [[surreal numbers]], do not necessarily have a lattice structure, nor are Heyting, so other authors prefer the more traditional definition given above. 
