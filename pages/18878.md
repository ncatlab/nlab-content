...
1. (field independence)
   ...
   $$
     ...
     \mathcal{Z}(g S_{int} + j A)
     \;=\;
     \mathcal{Z}'_{g S_{int} + j A}( \frac{\delta}{\delta \mathbf{\Phi}^a(x)} (g S_{int} + j A) )
   $$
...
of [[local observables]] into [[polynomial observables]] we may think of each $Z_k$ as a [[generalized function]], just as for [[time-ordered products]] in [this remark](S-matrix#NotationForTimeOrderedProductsAsGeneralizedFunctions).
...
But by the axiom "field independence" this means that the support of these generalized functions in the [[integrand]] here must be on the [[diagonal]], where $x_1 = \cdots = x_k$.
