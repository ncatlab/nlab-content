...
* Composition of morphisms in $tE^\otimes_k$ is defined in the obvious way. We let $E^\otimes_k$ denote the nerve of the topological category $tE^\otimes_k$. 

  Corollary T.1.1.5.12 implies that $E^\otimes_k$ is an 
...
and $X : Assoc \to \mathcal{X}$ be a monoid object in $\mathcal{X}$.  Say that $X$ is _grouplike_ if the composite
