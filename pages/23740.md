...
\begin{theorem}
Given a commutative ring $R$, if for all elements $a \in R$ and $b \in R$, if the sum $a + b$ is cancellative, then either $a$ is cancellative or $b$ is cancellative, then the addition and multiplication operations on a local ring RR are strongly extensional with respect to the canonical apartness relation $\#$ defined by $x \# y$ iff $x - y$ is cancellative.

In this way such a commutative ring $R$ becomes an internal ring object in the category $\mathrm{Apart}$, consisting of sets with apartness relations and maps (strongly extensional functions) between them.
\end{theorem}
...
For addition, if $(x + y) \# (x' + y')$, then $x + y - (x' + y') = (x - x') + (y - y')$ is cancellative, so $x - x'$ or $y - y'$ is cancellative since for all elements $x \in R$ and $y \in R$, if the sum $x + y$ is cancellative, then either $x$ is cancellative or $y$ is cancellative, whence $(x, y) # (x', y')$. Thus addition is strongly extensional. 

For multiplication, if $x y # x' y'$, then $x y - x' y'$ is cancellative. Write $x y - x' y' = (x - x')y + x'(y - y')$. Since for all elements $x \in R$ and $y \in R$, if the sum $x + y$ is cancellative, then either $x$ is cancellative or $y$ is cancellative, either $(x - x')y$ is a cancellative or $x'(y - y')$ is cancellative. From this we easily conclude $x - x'$ is cancellative or $y - y'$ is, whence $(x, y) # (x', y')$. So multiplication is also strongly extensional. 
\end{proof}
