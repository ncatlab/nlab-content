...
An archetypal example of a flabby sheaf is the sheaf of all set-theoretic (not necessarily [[continuous map|continuous]]) [[sections]] of a [[bundle]] $E\to X$: Since every sheaf over a topological space is the sheaf of sections of an [[étale space]] (see [there](etale+space#RelationToSheaves)), every sheaf can be embedded into a flabby sheaf $C^0(X,F)$ defined by
...
Beware that this construction assumes the [[law of excluded middle]] and that all stalks $F_x$ are [[inhabited set|inhabited]]. In the absence of either, the refined construction
