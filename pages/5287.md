...
on [[regular polynomial observables]] is, up to a factor of $i \hbar$, the difference between the free component $\{-S',-\}$ of the gauge fixed global BV differential its [[conjugation]] with the [[time-ordered product|time-ordering]] [[isomorphism]] (eq:RecallIsomorphsimTimeOrdering)
...
It is useful to reformulate this as follows:

+-- {: .num_defn #AntibracketTimeOrdered}
###### Definition
**([[time-ordered product|time-ordered]] [[antibracket]])

The _time-ordered global [[antibracket]]_ on [[regular polynomial observables]] 

$$
  PolyObs(E)_{reg}[ [\hbar] ]
   \otimes
  PolyObs(E)_{reg}[ [\hbar] ]
    \overser{\{-,-\}_{\mathcal{T}}}{\longrightarrow}
  PolyObs(E)_{reg}[ [\hbar] ]
$$
...
$$
  \{-,-\}_{\mathcal{T}}
  \;\coloneqq\;
  \mathcal{T}\left\{ \mathcal{T}^{-1}(-), \mathcal{T}^{-1}(-)\right\}
$$
...
$$
  \array{
    ...
    PolyObs(E)_{reg}[ [\hbar] ]
    \\
    {}^{\mathllap{\mathcal{Z}}}_\simeq\downarrow && \downarrow^{\mathrlap{\mathcal{T}}}_\simeq
    \\
    PolyObs(E)_{reg}[ [\hbar] ]
      \otimes
    PolyObs(E)_{reg}[ [\hbar] ]
