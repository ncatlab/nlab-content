...
A special case of these relations is the following important property of [[on-shell]] [[time-ordered products]]
...
$$
  ...
  \;=\;
  \underset{\Sigma}{\int}
    A^a(x) \cdot \mathbf{\Phi}^\ddgger_a(x)
  \, dvol_\Sigma(x)
  \;\in\;
...
([Rejzner 16, remark 7.7](#Rejzner16))

+-- {: .proof}
###### Proof

By def. \ref{AntibracketTimeOrdered} the term on the left is $\mathcal{T}$...

By prop. \ref{ComponentsBVOperator} the term on the right is $i \hbar \mathcal{T}^{-1}\left(\Delta_{BV}(A)\right)$. 
